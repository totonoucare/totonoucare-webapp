...
/**
 * わたしのトリセツ Plus v1: 不調ループ見える化のための軽量集計。
 *
 * 注意:
...
export function buildKartePlusPreviewText({ intake = {}, rankedDirections = [], loopSignals = [] } = {}) {
  ...
  if (directionText && signalText) {
    return `今回の回答からは、${signalText}が重なったときに不調が出やすい流れが見えています。戻しやすい方向の手がかりは${directionText}です。全文では、無料トリセツの体質結果と統合して、繰り返す不調の流れ・重ねすぎ注意カード・身体ライン・相談メモまで整理します。`;
  }

  if (directionText) {
    return `今回の回答からは、戻しやすい方向の手がかりとして${directionText}が見えています。全文では、無料トリセツの体質結果と統合して、繰り返す不調の流れ・重ねすぎ注意カード・身体ライン・相談メモまで整理します。`;
  }

  return "回答をもとに、無料トリセツの体質結果と統合して、繰り返す不調の流れ・避けたい組み合わせ・身体ライン・相談メモまで整理します。";
}
