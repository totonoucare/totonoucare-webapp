...
/**
 * 体質トリセツ v1: 不調ループ見える化のための軽量集計。
 *
 * 注意:
 * - これは診断ロジックではなく、トリセツ編集用の整理ロジック。
...
function buildNgCombinationCards({ intake = {}, rankedDirections = [] } = {}) {
  ...
  if (cards.length < 3 && rankedDirections?.[0]) {
    push({
      ...
      reason: "追加チェックでは、合うケアだけでなく邪魔になりやすい条件も見ます。",
...
export function buildKarte詳細版LoopProfile({ diagnosis = {}, intake = {} } = {}) {
  const scores = {
    shizumeru: 0,
    yurumeru: 0,
    meguraseru: 0,
  ...
  const previewText = buildKarte詳細版PreviewText({ intake, rankedDirections, loopSignals });
...
export function buildKarte詳細版PreviewText({ intake = {}, rankedDirections = [], loopSignals = [] } = {}) {
  ...
  if (directionText && signalText) {
    return `今回の回答からは、${signalText}が重なったときに不調が出やすい流れが見えています。戻しやすい方向の手がかりは${directionText}です。全文では、体質トリセツの体質結果と統合して、繰り返す不調の流れ・重ねすぎ注意カード・身体ライン・相談メモまで整理します。`;
  }

  if (directionText) {
    return `今回の回答からは、戻しやすい方向の手がかりとして${directionText}が見えています。全文では、体質トリセツの体質結果と統合して、繰り返す不調の流れ・重ねすぎ注意カード・身体ライン・相談メモまで整理します。`;
  }

  return "回答をもとに、体質トリセツの体質結果と統合して、繰り返す不調の流れ・避けたい組み合わせ・身体ライン・相談メモまで整理します。";
}
