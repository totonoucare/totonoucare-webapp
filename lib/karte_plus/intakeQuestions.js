 ...
 * - 体質トリセツで得た「体質・天気相性・経絡ライン・主訴」に、
 *   時間帯・悪化条件・楽になる条件・生活負荷・回復阻害・ケア嗜好を足し、
 *   詳細レポートを「不調ループ見える化レポート」に変えるための追加問診。
 */
...
export const KARTE_PLUS_QUESTIONS = [
  ...
  },
  {
    ...
    helper: "詳細レポートでは「足すケア」だけでなく、減らす負担も整理します。",
    options: [
  ...
  },
  {
    ...
    group: "fit",
    type: "single",
    title: "トリセツの最後に、どんな形のまとめがあると使いやすいですか？",
    options: [
      ...
      { label: "避けたい習慣・飲食の組み合わせ", value: "ng_combo" },
      { label: "まずは読み物として理解できればよい", value: "reading_first" },
    ],
  },
];

export function getKarte詳細版QuestionMap() {
  const map = new Map();
  for (const q of KARTE_PLUS_QUESTIONS) map.set(q.key, q);
  return map;
}

export function getKarte詳細版QuestionByKey(key) {
  return KARTE_PLUS_QUESTIONS.find((q) => q.key === key) || null;
}

export function getKarte詳細版TotalQuestions() {
