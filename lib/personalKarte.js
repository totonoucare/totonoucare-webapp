  ...
  const sections = [
    {
      ...
      badge: "全体像",
      title: "あなたの未病パターン全体像",
      teaser: "あなたの崩れやすさを、毎日の体調管理に使える形で整理します",
      preview: `${core.title}は、刺激を受けたときの反応と回復の余力に特徴があります。無料結果では見えにくい“崩れ方のクセ”まで整理します。`,
      body: [
        `今回の体質軸は「${core.title}」。刺激や予定の変化を受けたときに、どんな方向へ崩れやすいかを見るための目印です。`,
        `${core.tcmHint} さらに、重だるさ・消耗・巡りに関わるサインが重なることで、今いちばん気になっている「${symptom.label}」として表に出やすくなります。`,
        `このカルテで見るのは、単なる不調名ではありません。「何に揺さぶられやすいか」「どんな小さなサインが先に出るか」「どこで止めると翌日に残りにくいか」を、あなた用の取扱説明書として整理します。`,
      ],
      bullets: [
        `体質軸：${core.title}（${core.short}）`,
        `崩れ方の傾向：${core.yinYangText} × ${core.driveText}`,
        `主訴：${symptom.label}`,
    ...
    },
    {
      id: "route",
      badge: "崩れ方",
      title: "不調が強くなる前に出る小さなサイン",
      teaser: "今のつらさの手前にある、気づきやすいサインを整理します",
      preview: `${symptom.label}が強くなる前に、重さ・消耗・こわばりなどの小さなサインが出やすいかを見ていきます。`,
      body: [
        `今回の結果では、「${primarySub.short}」と「${secondarySub.short}」に関わるサインが目立ちます。難しい言葉で覚える必要はなく、日常では「重さ」「消耗」「戻りにくさ」がどう重なるかを見ていきます。`,
        mechanism.loop,
        `表に出やすい小さなサインは、${mechanism.earlySigns.join("、")}です。${symptom.label}が強くなってから頑張るより、この段階で短く戻す方が未病ケアとしては効率的です。`,
      ],
      bullets: [
        `表に出やすいサイン：${mechanism.primarySurface}`,
        `奥で重なりやすいサイン：${mechanism.secondarySurface}`,
        `主訴として見えているサイン：${symptom.label}`,
      ],
      steps: [
        `見る順番：つらさ → その前の小さなサイン → 戻し方`,
    ...
    },
    {
      ...
      teaser: "どんな天気の日に、体が先に揺れやすいかを整理します",
      preview: `あなたの場合、注意したい天気は「${weather.ranked.map((item) => item.label).join("・")}」。点数が上がる日の見方を整理します。`,
      body: [
        `あなたが影響を受けやすい天気として、今回は「${weather.ranked.map((item) => item.label).join("・")}」が上位に出ています。特に${topWeather.day}は、${topWeather.note}`,
        selfWeatherText === "未指定"
          ? `まだ「この天気で必ず崩れる」という実感がはっきりしない場合でも、まずは${topWeather.day}${secondWeather ? `や${secondWeather.day}` : ""}に、${symptom.bodySign}が強まるかを観察してみてください。`
          : `質問では「${selfWeatherText}」も選ばれています。実感しているきっかけと、今回上位に出た天気を合わせて見ると、「どの日に先回りするか」が決めやすくなります。`,
        `未病レーダーの点数が高い日は、点数そのものより“主因”を見てください。${topWeather.day}${secondWeather ? `や${secondWeather.day}` : ""}が絡む日は、予定量・睡眠・食事のどれか1つを軽くする合図として使えます。`,
      ],
      bullets: [
        `注意したい天気：${weather.ranked.map((item) => `${item.rank}位 ${item.label}`).join(" / ")}`,
        selfWeatherText === "未指定" ? null : `実感しているきっかけ：${selfWeatherText}`,
    ...
    },
    {
      ...
      badge: "身体ライン",
      title: "負担が出やすい身体ライン",
      teaser: "つらさが出る場所と、動きで見えた負担ポイントを整理します",
      preview: movementPrimary
        ? `動作チェックでは「${movementPrimary.movementLabel}」で負担が出やすく、${movementPrimary.lineTitle}を見ておくと変化を拾いやすくなります。`
        : `主訴から見た観察ゾーンと、動きで見える負担ラインを分けて整理します。`,
      body: [
        `今いちばん気になる不調は「${symptom.label}」です。それとは別に、動きのチェックでは、体のどこに負担が出やすいかを見ています。`,
        lineComparison,
        movementSecondary
    ...
    },
    {
      ...
      badge: "季節",
      title: "季節ごとの守り方",
      teaser: "一年の中で、少し早めに守りたい時期を整理します",
    ...
    },
    {
      ...
      body: [
        ...
        `体質メモとしては「${core.title}」。重だるさ・消耗・戻りにくさが重なりやすいことを伝えられると、相談がスムーズです。`,
      ...
      bullets: [
        ...
        movementPrimary ? `動きで気になる場所：${movementPrimary.lineTitle}（${movementPrimary.bodyArea}）` : "動きのチェック：強い偏りなし",
