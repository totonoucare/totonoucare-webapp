...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。関連して見たい場所は「${symptomLine.lineTitle}」、動作チェックで負担が見えた場所は「${movementPrimary.lineTitle}」です。両方を見ると、痛い場所だけでは拾えない変化に気づきやすくなります。`
    : movementPrimary
      ? `動作チェックでは「${movementPrimary.lineTitle}」に負担のサインが見えています。強い日と軽い日の違いを、この場所で比べてみてください。`
      : symptomLine
        ? `今お困りの不調は「${symptom.label}」です。関連して「${symptomLine.lineTitle}」を見ておくと、日々の変化を拾いやすくなります。`
  ...
  const sections = [
    ...
    },
    {
      ...
      bullets: [
        symptomLine ? `今お困りの場所：${symptomLine.lineTitle}（${symptomLine.bodyArea}）` : `今お困りの不調：${symptom.label}`,
  ...
  return {
    ...
    heroLead: `${core.title}の体質軸をもとに、${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}、${topWeather.label}に関わる六淫（ろくいん）、動作チェックで見えた経絡ラインを合わせて、天気で出やすい体調サインを整理します。`,
    quickTakeaways,
    mapFlow,
    forecastUsage,
    sections: cleanSections,
    meta: {
      version: "deterministic-v6-compact-ui-copy",
