...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。不調が出ている場所だけを見ると${symptomLineText}に目が向きやすい一方、動作負担チェックでは「${movementPrimary.movementLabel}」の動きから${movementPrimaryText}にも負担のサインが見えています。これは別の不調を足す話ではなく、気血水の偏り・冷え・姿勢・使い方のくせが、どの経絡（けいらく）の通り道に表れやすいかを見る手がかりです。`
    : movementPrimary
      ? `動作負担チェックでは「${movementPrimary.movementLabel}」の動きから、${movementPrimaryText}に負担のサインが見えています。これは不調名を決めるためではなく、今お困りの不調の背景で負担が出やすい経絡（けいらく）ラインを探る手がかりです。`
      : symptomLine
        ? `今お困りの不調は「${symptom.label}」です。背景として、${symptomLineText}の重さ・張り・動き出しにくさを見ると、体調の変化を拾いやすくなります。`
  ...
  const sections = [
    ...
    },
    {
      ...
      body: [
        `未病レーダーの動作負担チェックは、M-testの考え方を参考に、体を動かしたときの張り・重さ・動きにくさから、負担が表れやすい経絡（けいらく）ラインを見ています。経絡名まで見ることで、「どこが痛いか」だけでは拾いにくい背景の負担を、身体の通り道として整理しやすくなります。`,
        `${lineComparison} 予報ページのツボ提案には、体質や天気だけでなく、この経絡ラインの情報も一部反映されます。動作チェックで出たラインを、日々のツボケアと軽いセルフチェックにつなげて使います。`,
      ],
      bullets: [
  ...
  return {
    ...
    meta: {
      version: "deterministic-v10-meridian-names",
