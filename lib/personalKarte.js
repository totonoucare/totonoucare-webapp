...
function buildForecastUsage({ weather, symptom, movementPrimary }) {
  const top = weather.top || weatherItem("pressure_down", 0.45, 1);
  return {
    title: "カルテで傾向を知り、今日のケアは予報ページで確認",
    body: `カルテでは、${top.day}に${symptom.bodySign}が出やすい理由を整理します。実際に今日使うツボ・食養生・過ごし方は、その日の気象データで変わるため、予報ページで確認してください。`,
    bullets: [
      `予報点数が高い日は「主因」が${top.label}かを見る`,
      `ツボ提案の一部には、動作チェックで見えた経絡ラインも反映されます`,
...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。あわせて見たい場所は「${symptomLine.lineTitle}」、動作チェックで負担が見えた場所は「${movementPrimary.lineTitle}」です。動作チェックの動きは、強く伸ばすためではなく、軽く動かして変化を見る目印です。予報ページのツボケアと組み合わせると、ケア前後で動きやすさが少し戻るかを確認しやすくなります。`
    : movementPrimary
      ? `動作チェックでは「${movementPrimary.lineTitle}」に負担のサインが見えています。この動きは、日々のセルフチェックと、予報ページで出るツボ提案を理解する手がかりになります。強く行う必要はなく、軽く動かして変化を見る使い方が向きます。`
      : symptomLine
        ? `今お困りの不調は「${symptom.label}」です。あわせて「${symptomLine.lineTitle}」の重さ・張り・動き出しにくさを見ると、変化を拾いやすくなります。`
  ...
  const sections = [
    {
      ...
      teaser: "今お困りの不調を手がかりに、これからも使える体調傾向として整理します",
      preview: `${core.title}は、${core.yinYangText}と${core.driveText}を軸にした体質の見取り図です。天気で揺れやすい日の見方まで整理します。`,
      body: [
        `今回まず見るのは「${symptom.label}」ですが、このカルテでは、不調名だけでなく、その手前にある体質・天気・動きのサインまで見ます。${core.title}という体質軸をもとに、内側の偏り、天気の負担、体に出るサインをひとつながりで見ます。`,
        `${core.tcmHint} この特徴があると、${topWeather.day}のような天気の変化を受けたときに、${relatedSigns.slice(0, 3).join("、")}などの小さなサインが先に出やすくなります。`,
        `今の不調が落ち着いたあとも、同じ体質から別の軽いサインとして出ることがあります。だからこそ「腰だけ」「頭だけ」「むくみだけ」と切り分けず、自分の崩れ方のクセとして見返せるようにしておきます。`,
      ],
      bullets: [
    ...
    },
    {
      ...
      body: [
        ...
        `${mechanism.loop} ここを知っておくと、不調が強くなってから整えるのではなく、東洋医学の名前がついた前触れとして、早めに拾いやすくなります。`,
      ],
      bullets: [
    ...
    },
    {
      id: "weather-rokuin",
      badge: "天気",
      title: "体質から見る、注意したい天気",
      teaser: "湿気・冷え・気圧など、揺れやすい天気を先に見ます",
      preview: `今回の注意天気は、${weather.ranked.map((item) => item.label).join("・")}。特に${topWeather.label}の日は、早めに体調サインを拾いたい日です。`,
      body: [
        `体質から見た注意天気の上位は、${weather.ranked.map((item) => `${item.rank}位 ${item.label}`).join("、")}です。これは「天気が悪いから不調になる」というより、${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}として出やすい内側のくせが、特定の天気で表に出やすいという見方です。`,
        `特に${topWeather.day}は、${topWeather.note} ${secondWeather ? `${secondWeather.day}も重なる日は、` : ""}${mechanism.earlySigns.slice(0, 3).join("・")}のような軽いサインを早めに見ておくと、${symptom.label}が強くなる前に調整しやすくなります。`,
        selfWeatherText === "未指定"
          ? `本人の自覚としては、まだ天気のきっかけが絞り込まれていません。まずは注意天気の日に、眠気・重さ・張り・乾きなどが増えるかを見ていきます。`
    ...
    },
    {
      ...
      preview: `${symptom.label}だけでなく、${relatedSigns.slice(0, 3).join("・")}も同じ体質傾向の中で見ていきます。`,
      body: [
        ...
        `大切なのは、不調名を固定しすぎないことです。${topWeather.day}に${symptom.label}が出ない日でも、眠気・重さ・張り・乾きなど別の形で出ているなら、同じ体質サインの手前にいる可能性があります。`,
    ...
    },
    {
      id: "meridian-line",
      badge: "経絡",
      title: "負担がかかる経絡（けいらく）ラインから見るサイン",
      teaser: "痛い場所だけでなく、動作チェックで見えた負担の通り道も見ます",
      ...
      body: [
        `未病レーダーの動作チェックは、M-testの考え方を参考に、体を動かしたときの張り・重さ・動きにくさから、負担が表れやすい経絡（けいらく）ラインを見ています。ここで見る動きは、強く伸ばすための運動ではなく、軽く動かして変化を見るセルフチェックです。予報ページで提案されるツボケアにも、この経絡ラインの情報がつながります。`,
        lineComparison,
        movementSecondary
          ? `2番目に気になる動きは「${movementSecondary.movementLabel}」です。今お困りの場所と動作チェックのラインが違う場合でも、内側の偏りがどの通り道に出ているかを見る手がかりになります。`
    ...
    },
    {
      ...
      preview: `カルテで傾向を知り、予報ページで今日のツボ・食養生を確認します。`,
      body: [
        `このカルテは、あなたの体調傾向をまとめたものです。一方で、毎日の予報ページは、その日の気圧・気温・湿度によって、どの負担が強まりそうかを見る場所です。`,
        `予報点数が高い日は、点数だけでなく「主因」を見てください。湿気・冷え込み・気圧低下・乾燥・暑さのどれが強いかで、合うツボや食養生も変わります。`,
        `ツボや食養生は、このカルテに固定で書くより、その日の主因に合わせて予報ページで見る方が実用的です。カルテで自分の傾向を知り、予報で今日の一手を選ぶ、という使い方がおすすめです。`,
      ],
    ...
    },
    {
      ...
      bullets: [
        ...
        `伝える条件：${topWeather.day} / ${symptom.watch}`,
  ...
  return {
    ...
    heroLead: `${core.title}の体質軸をもとに、${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}、${topWeather.label}の日に出やすいサイン、動作チェックで見えた経絡ラインを合わせて、体調が揺れる前触れと先回りの判断基準を整理します。`,
    quickTakeaways,
    mapFlow,
    forecastUsage,
    sections: cleanSections,
    meta: {
      version: "deterministic-v8-integrated-preview",
