...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。あわせて見たい場所は「${symptomLine.lineTitle}」、動作チェックで負担が見えた場所は「${movementPrimary.lineTitle}」です。痛みの場所だけでは見えない負担を、動きから拾うための目印になります。`
    : movementPrimary
      ? `動作チェックでは「${movementPrimary.lineTitle}」に負担のサインが見えています。この動きは、日々のセルフチェックとして使えます。強く行う必要はなく、軽く動かして変化を見る使い方が向きます。`
      : symptomLine
  ...
  const sections = [
    ...
    },
    {
      id: "meridian-care",
      badge: "5｜経絡",
      title: "動作負担チェックでわかる、不調に関わる経絡（けいらく）ラインとケア",
      teaser: "お困りの不調だけでなく、動きで見えた負担ラインと予報ページのツボ提案をつなげます。",
      preview: movementPrimary
        ? `動作負担チェックでは「${movementPrimary.movementLabel}」が手がかりです。${movementPrimary.lineTitle}は、日々のセルフチェックとツボケアの参考になります。`
        : "動作負担チェックから、負担が表れやすい経絡ラインとケアの見方を整理します。",
      body: [
        ...
        `${lineComparison} 予報ページのツボ提案には、体質や天気だけでなく、この経絡ラインの情報も一部反映されます。動きで変化を見て、ツボケアで整える、という使い方ができます。`,
      ],
      bullets: [
    ...
    },
    {
      ...
      teaser: `${season.headline}を中心に、年間の中で先に守りたい時期を整理します。`,
      preview: `${season.headline}は、体質と注意天気が重なりやすい時期です。季節ごとの守り方を先に決めておきます。`,
      body: [
