...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。あわせて見たい場所は「${symptomLine.lineTitle}」、動作チェックで負担が見えた場所は「${movementPrimary.lineTitle}」です。動作チェックの動きは、強く伸ばすためではなく、ケア前後で動きやすさが少し戻るかを見る目印になります。`
    : movementPrimary
      ? `動作チェックでは「${movementPrimary.lineTitle}」に負担のサインが見えています。この動きは、日々のセルフチェックと、予報ページで出るツボ提案を理解する手がかりになります。`
      : symptomLine
  ...
  const sections = [
    ...
    },
    {
      ...
      body: [
        `体質から見た注意天気の上位は、${weather.ranked.map((item) => `${item.rank}位 ${item.label}`).join("、")}です。これは「天気が悪いから不調になる」というより、もともとの体質の弱いところが、特定の天気で表に出やすいという見方です。`,
        `特に${topWeather.day}は、${topWeather.note} ${secondWeather ? `${secondWeather.day}も重なる日は、` : ""}${relatedSigns.slice(0, 3).join("・")}のような軽いサインを早めに見ておくと、${symptom.label}が強くなる前に調整しやすくなります。`,
        selfWeatherText === "未指定"
    ...
    },
    {
      ...
      body: [
        `未病レーダーの動作チェックは、M-testの考え方を参考に、体を動かしたときの張り・重さ・動きにくさから、負担が表れやすい経絡（けいらく）ラインを見ています。ここで見る動きは、毎日その動作を練習するためではなく、予報ページのツボ提案や体調観察につなげるための手がかりです。`,
        lineComparison,
        movementSecondary
    ...
    },
    {
      ...
      steps: ["予報点数だけでなく主因を見る", "その日のツボ・食養生を確認する"],
    },
    {
      id: "advance-care",
      badge: "先回り",
  ...
  return {
    ...
    meta: {
      version: "deterministic-v7-weather-signal-copy",
