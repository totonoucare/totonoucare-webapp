...
export const PERSONAL_KARTE_DISPLAY_NAME = "未病カルテ Plus";
...
function buildKarteTakeaways({ core, symptom, weather, primaryTerm, secondaryTerm, relatedSigns, loopProfile }) {
  ...
  return [
    ...
    {
      label: "優先方針",
      value: firstDirection ? firstDirection.label : `${joinTerm(primaryTerm)}を整える`,
      note: firstDirection?.hint || `${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}を生活のサインに翻訳します。`,
    },
    {
      label: "まず減らす負担",
      value: core.strategy.boundary,
      note: `${core.title}は、足すケアの前に負担を弱めるほど翌日に残しにくいタイプです。`,
    },
  ];
}
...
function buildKarteMapFlow({ core, primaryTerm, secondaryTerm, weather, symptom, relatedSigns, loopProfile }) {
  const top = weather.top || weatherItem("pressure_down", 0.45, 1);
  const directionText = loopProfile?.rankedDirections?.slice(0, 2).map((item) => item.label).join(" → ") || "整え方の優先順位";
  return [
    {
      label: "体質軸",
      title: core.title,
    ...
    },
    {
      label: "抜ける方向",
      title: directionText,
      description: "7方針の優先順位から、足すケアと減らす負担を決めます。",
    },
  ];
}
...
export function buildPersonalKarte(event = {}) {
  ...
  const sections = [
    {
      ...
      badge: "1｜全体像",
      title: "あなたの不調ループ全体像",
      teaser: `${core.title}の体質軸に、${topWeather.label}や生活負荷が重なったときの流れを整理します。`,
      preview: `${loopSignalText}が重なったとき、${symptom.label}として表に出やすい流れを見ます。優先方針は${directionText}です。`,
      body: [
        `${core.title}は、${core.yinYangText}と${core.driveText}を軸にした体質タイプです。これは性格診断ではなく、天気・疲労・予定の変化を受けたときに、体がどんな守り方をしやすいかを見るための目印です。`,
        `今回見たいのは「${symptom.label}」だけではなく、${loopSignalText}が重なったときに、前触れから不調へ進みやすい流れです。Plusでは、この流れを${directionText}の優先順位でほどいていきます。`,
      ],
      bullets: [
        `ループ入口：${loopSignalText}`,
        `優先方針：${directionText}`,
      ],
      steps: [],
    },
    ...
    },
    {
      id: "daily-loop",
      badge: "4｜生活",
      title: "生活の中で固定化しやすい不調の流れ",
      teaser: "時間帯・睡眠・食事・姿勢や予定量から、前触れが強くなる流れを見ます。",
      ...
      body: [
        ...
        `前触れは、痛みやつらさより少し手前にあります。${loopSignalText}のような条件が重なった日に、朝・食後・午後・寝る前のどこで変化するかを見ると、未病の段階で拾いやすくなります。`,
      ],
      bullets: [
    ...
    },
    {
      id: "care-priority",
      badge: "5｜優先順位",
      title: "7つの方針で見る、まず整える順番",
      teaser: `今回の優先方針は${directionText}。足すケアだけでなく、減らす負担も整理します。`,
      preview: `まずは${directionText}を目安に、やりすぎるケアではなく、戻しやすい方向を選びます。`,
      body: [
        `未病レーダーの7方針は、ケアを増やすための分類ではなく、「今のループをどこからほどくか」を決めるための目印です。今回の優先順位は${directionText}です。`,
        `楽になりやすい方向としては、${recoveryText}が手がかりになります。反対に、${coreStrategy.ng.join("、")}のようなやり方は、短期的には楽でも翌日に残ることがあるため、強く足すより先に負担を弱めます。`,
      ],
      bullets: [
        `優先方針：${directionText}`,
        `戻しやすい方向：${recoveryText}`,
      ],
      steps: [
        "まず減らす負担を1つ決める",
        "次に、予報ページで今日の方針とツボ・食養生を確認する",
      ],
    },
    {
      id: "seven-day-plan",
      badge: "6｜7日間",
      title: "7日間で試す、軽い整え方プラン",
      teaser: "続けやすさを優先して、1週間の中で観察とケアを小さく試します。",
      preview: "1週間で体質を変えるのではなく、前触れを見つけ、崩れやすい条件を1つ減らす練習をします。",
      body: [
        `7日間プランは、体質を一気に変えるためのものではありません。${core.title}の反応を観察しながら、${symptom.label}が強くなる前に気づく練習として使います。`,
        `前半は「見る」、中盤は「減らす」、後半は「合うケアを1つ残す」が目安です。完璧に行うより、${coreStrategy.boundary}を守れる日を増やす方が続きます。`,
      ],
      bullets: [
        "1〜2日目：前触れを観察",
        "3〜7日目：減らす負担と足すケアを1つずつ試す",
      ],
      steps: [
        "毎日、天気・前触れ・睡眠/食事のどれか1つだけ記録する",
        "7日目に、楽だったケアと続かなかったケアを分ける",
      ],
    },
    {
      id: "season-strategy",
      ...
      preview: "季節ごとに、天気・気血水・今お困りの不調が重なりやすい時期があります。あなたの結果に合わせて注意時期と守り方を整理します。",
      body: [
  ...
  return {
    ...
    subtitle: `${core.title}｜${symptom.label}の不調ループを見える化する`,
    coreCode: core.code,
    ...
    heroLead: `${core.title}の体質軸をもとに、${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}、${topWeather.label}の日、動作負担チェックで見えた経絡ラインを合わせて、${symptom.label}がくり返されやすい流れと抜け方の優先順位を整理します。`,
    quickTakeaways,
    mapFlow,
    forecastUsage,
    ...
    beautyColumn: {
      badge: "まとめ",
      title: "相談前シート・実践チェックリスト",
      teaser: "自分で見返すときも、専門家に相談するときも使いやすいように、要点を短く整理します。",
      preview: `${symptom.label}、${topWeather.label}の日、${directionText}の優先方針を、相談前に伝えやすい形でまとめます。`,
      body: [
        `相談時は、不調名だけでなく「いつ出るか」「何で強くなるか」「何で楽になるか」を伝えると共有しやすくなります。まずは、${symptom.label}、${topWeather.day}、${loopSignalText}をメモします。`,
        `セルフケアでは、${directionText}を優先し、${coreStrategy.boundary}を守ります。予報ページで今日・明日の点数、主因、ツボ、食養生を見て、その日の作戦に落とし込みます。`,
        `医療的な不安がある場合や、強い痛み・しびれ・発熱・転倒後から続く症状などがある場合は、セルフケアだけで判断せず医療機関に相談してください。`,
      ],
      points: [
        `不調：${symptom.label}`,
        `注意天気：${topWeather.label}`,
        `優先方針：${directionText.replace(/[「」]/g, "")}`,
      ],
    },
