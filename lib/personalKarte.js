...
import { buildKarte詳細版LoopProfile } from "@/lib/karte_plus/loopAnalysis";
import {
...
export const PERSONAL_KARTE_DISPLAY_NAME = "体質トリセツ";
...
const WEATHER_EXACT_CONTEXT = {
  pressure_down: {
    label: "気圧低下",
    day: "気圧が下がる日",
    note: "気圧が下がる変化に揺さぶられた時、体質に応じた張り・重さ・切り替えの疲れが表に出やすい日です。",
    season: "春・梅雨前・台風前後・季節の変わり目",
  },
  pressure_up: {
    label: "気圧上昇",
    day: "気圧が上がる日",
    note: "気圧が上がる変化に揺さぶられた時、体質に応じた張り・重さ・切り替えの疲れが表に出やすい日です。",
    season: "春・秋・天気が急に回復する日",
  },
  temp_shift: {
    label: "寒暖差",
    day: "寒暖差が大きい日",
    note: "体温調節と環境への切り替えに力を使い、疲れ・張り・重さなどが表に出やすい日です。",
    season: "春・秋・朝晩と日中の差が大きい時期",
  },
  cold: {
    label: "低温",
    day: "気温が低い日",
...
const SELF_WEATHER_CONTEXT = {
  ...
  temp_swing: { label: "寒暖差", exact: ["temp_shift"] },
...
const SEASON_LINES = {
  ...
  qi_deficiency: {
    ...
    rainy: "梅雨は湿気と寒暖差で体が重くなりやすい時期です。発散より補給を優先し、温かい汁物や早めの就寝で土台を守ります。",
  ...
  fluid_damp: {
    ...
    spring: "春は寒暖差で胃腸のリズムが乱れると、重さとして残りやすい時期です。甘いものと冷たい飲み物を続けすぎないようにします。",
...
function safeKarte詳細版Intake(event) {
  return (
    event?.karte_plus_intake ||
    event?.karte詳細版Intake ||
    event?.plus_intake ||
    event?.plusIntake ||
    event?.answers?.karte_plus_intake ||
    event?.answers?.karte詳細版Intake ||
    {}
  );
}
...
function buildForecastConstitution({ answers, computed, coreCode, subCodes }) {
...
function getPressureResponseDirection(affinityProfile) {
  const score = Number(affinityProfile?.normalized_constitution?.axes?.yin_yang_score || 0);
  if (score > 0.18) return "accel";
  if (score < -0.18) return "brake";
  return "balanced";
}

function pressureResponseNote(key, responseDirection) {
  const physical = key === "pressure_up" ? "気圧が上がる変化" : "気圧が下がる変化";
  if (responseDirection === "accel") {
    return `${physical}に揺さぶられた時、張り・力み・切り替えにくさとして表れやすい傾向です。`;
  }
  if (responseDirection === "brake") {
    return `${physical}に揺さぶられた時、重だるさ・動き出しにくさとして表れやすい傾向です。`;
  }
  return `${physical}に揺さぶられた時、環境変化へ適応する疲れとして表れやすい傾向です。`;
}

function weatherItem(key, value = null, rank = null, responseDirection = "balanced") {
  const base = WEATHER_EXACT_CONTEXT[key] || WEATHER_EXACT_CONTEXT.pressure_down;
  const note = key === "pressure_down" || key === "pressure_up"
    ? pressureResponseNote(key, responseDirection)
    : base.note;
  return { key, value, rank, ...base, note, response_direction: key.startsWith("pressure_") ? responseDirection : null };
}
...
function buildWeatherProfile({ answers, computed, coreCode, subCodes }) {
  ...
  const affinityProfile = buildConstitutionWeatherAffinityV2({
    constitution: buildForecastConstitution({ answers, computed, coreCode, subCodes }),
  });
  const pressureResponseDirection = getPressureResponseDirection(affinityProfile);

  const ranked = rankConstitutionWeatherAffinityV2(affinityProfile.weights)
    .slice(0, 3)
    .map((item, index) => weatherItem(item.key, item.value, index + 1, pressureResponseDirection));
  ...
  return {
    affinityProfile,
    ranked,
    top: ranked[0] || weatherItem("pressure_down", 0.45, 1, pressureResponseDirection),
...
export function buildPersonalKarte(event = {}) {
  ...
  const plusIntake = safeKarte詳細版Intake(event);
  const loopProfile = buildKarte詳細版LoopProfile({
    diagnosis: {
  ...
  const sections = [
    {
      ...
      body: [
        ...
        `今お困りの不調は「${symptom.label}」。ただし、急に強く出るというより、${relatedSigns.slice(0, 4).join("、")}のような小さな前触れをはさんで表に出ることがあります。詳細版では、この前触れを早めに拾える形にします。`,
      ],
      bullets: [
        `体質：${core.title}`,
    ...
    },
    {
      ...
      preview: `追加チェックから、崩れやすい条件の重なりと、戻りにくくなる分岐点を整理します。`,
      body: [
        ...
        `大事なのは「何を足すか」より先に、「何が重なると戻りにくくなるか」を見つけることです。詳細版では、体質トリセツの体質結果に、時間帯・睡眠・食事・作業負荷・楽になる条件を重ねて、あなたの不調ループを1本の流れとして整理します。`,
      ],
      bullets: [
        `入口：${loopSignalText}`,
    ...
    },
    {
      ...
      body: [
        ...
        `前触れは日によって顔を変えます。${topWeather.label}の日、睡眠不足の日、食後、画面作業が続いたあとなど、追加チェックで選んだ条件と合わせて見ると、予報ページを見るべきタイミングがわかりやすくなります。`,
      ],
      bullets: [
    ...
    },
    {
      ...
      body: [
        `この章は、追加チェックの回答から作る、重ねすぎ注意のカードです。目的は、悪い生活を責めることではなく、${symptom.label}が出やすい日に、負担が重なりすぎる組み合わせを先に見つけることです。`,
        `特に${mainNgTitle}は、今のループではケアを足すより先に軽くしたい候補です。ゼロにする必要はありません。まずは天気が揺れそうな前日だけ、量・時間帯・温度・姿勢のどれか1つを変えるくらいで十分です。`,
      ],
      bullets: [
        `まず外す候補：${mainNgTitle}`,
        loopProfile.mealLoadLabels?.length ? `飲食の負担：${loopProfile.mealLoadLabels.slice(0, 3).join(" / ")}` : "飲食の負担：追加チェックで反映",
      ],
      steps: [
    ...
    },
    {
      ...
      preview: `予報ページでは毎日1つずつ出るM-test系のラインケアを、詳細版ではあなた用の候補カードとしてまとめます。`,
      body: [
        `動作負担チェックでは、痛い場所だけでなく、動きにくさ・張り・重さが出やすい通り道を見ます。今回の主なラインは、${bodyLineSummary}です。`,
        `この章のツボ候補は、日々の予報ページの代わりではありません。予報ページはその日の天気で1つに絞る場所、詳細版はあなたに関係しやすいラインと候補を保存しておく場所です。実際に使う日は、今日・明日の予報と合わせて確認します。`,
      ],
      bullets: [
    ...
    },
    {
      ...
      teaser: "詳細版は保存版、予報ページは当日の作戦表。役割を分けて使います。",
      preview: `詳細版でループを理解し、予報ページではその日の点数・主因・ツボ・食養生を確認します。`,
      body: [
        `詳細版では、${symptom.label}がくり返されやすい背景を整理します。一方で、今日・明日のケアは、実際の気圧・湿度・気温の動きで変わります。だから、具体的なツボや食養生は予報ページで見るのが自然です。`,
        `使い方はシンプルです。前触れがある日、${topWeather.label}の日、NG組み合わせが重なりそうな日は、予報ページで点数・主因・ラインケアのツボ・食養生を確認します。詳細版は、その提案がなぜ自分に出やすいのかを理解するための地図です。`,
      ],
      bullets: [
        "詳細版：不調ループの地図",
        "予報ページ：今日・明日の作戦表",
    ...
    },
    {
      ...
      teaser: "追加チェックを踏まえて、相談時に伝えやすい短いメモにします。",
  ...
  return {
    ...
    heroLead: `${core.title}の体質軸をもとに、${joinTerm(primaryTerm)}・${joinTerm(secondaryTerm)}、${topWeather.label}の日、動作負担チェックで見えた身体ライン、追加チェックで見えたNG組み合わせを合わせて、${symptom.label}がくり返されやすい流れを整理します。`,
    quickTakeaways,
    mapFlow,
    forecastUsage,
    karte詳細版Loop: loopProfile,
    ...
    beautyColumn: {
      ...
      body: [
        `詳細版の中心は、読むことだけではありません。${mainNgTitle}のようなNG組み合わせを見つけ、警戒日前に1つ外すことで、予報ページの提案を実行しやすくします。`,
