...
export function buildPersonalKarte(event = {}) {
  ...
  const lineComparison = movementPrimary && symptomLine && movementPrimary.lineTitle !== symptomLine.lineTitle
    ? `今お困りの不調は「${symptom.label}」です。動作負担チェックでは「${movementPrimary.movementLabel}」の動きから、${movementPrimary.lineTitle}に負担のサインが見えています。これは不調名を言い換えたものではなく、気血水の偏り・姿勢・冷え・使い方のくせなどが表に出る経絡（けいらく）ラインの手がかりです。${symptom.label}がある日でも、背景では別のラインが動きを邪魔していることがあります。`
    : movementPrimary
      ? `動作負担チェックでは「${movementPrimary.movementLabel}」の動きから、${movementPrimary.lineTitle}に負担のサインが見えています。これは今お困りの不調の背景にある経絡（けいらく）ラインを探る手がかりです。強く伸ばすためではなく、軽く動かして変化を見るセルフチェックとして使います。`
      : symptomLine
        ? `今お困りの不調は「${symptom.label}」です。背景として、${symptomLine.lineTitle}の重さ・張り・動き出しにくさを見ると、体調の変化を拾いやすくなります。`
  ...
  const sections = [
    ...
    },
    {
      ...
      teaser: "動きで見えた経絡ラインを、不調の背景にある負担のサインとして読み、セルフチェックと予報ページのツボケアにつなげます。",
      preview: movementPrimary
        ? `動作負担チェックでは「${movementPrimary.movementLabel}」から、${movementPrimary.lineTitle}にサインが出ています。購入後は、このラインが不調の背景でどう関わるか、予報ページのツボケアと合わせて整理します。`
        : "動作負担チェックから、不調の背景にある経絡ラインとケアの見方を整理します。",
      body: [
        `未病レーダーの動作負担チェックは、M-testの考え方を参考に、体を動かしたときの張り・重さ・動きにくさから、負担が表れやすい経絡（けいらく）ラインを見ています。ここで選んだ動きは、強く伸ばすためではなく、軽く動かして変化を見るセルフチェックとして使います。`,
        `${lineComparison} 予報ページのツボ提案には、体質や天気だけでなく、この経絡ラインの情報も一部反映されます。負担のない範囲で同じ動きを軽く確認し、出てきたラインをツボケアで整える、という使い方ができます。`,
      ],
      bullets: [
        movementPrimary ? `動作チェック：${movementPrimary.movementLabel} → ${movementPrimary.lineTitle}` : "動作チェック：強い偏りなし",
        movementSecondary ? `補助ライン：${movementSecondary.movementLabel} → ${movementSecondary.lineTitle}` : "ツボケアは予報ページで確認",
      ],
      steps: [
        "予報ページで出たツボケアの前後に、同じ動きを軽く確認する",
        "負担のない範囲なら、動作チェックの動きを小さなセルフケアとして使う",
      ],
    },
    {
      id: "alert-day-care",
  ...
  return {
    ...
    meta: {
      version: "deterministic-v9-weather-care-preview",
