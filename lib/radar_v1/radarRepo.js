...
import { runSupabaseOperationWithFastRetry } from "@/lib/radar_v1/upstreamResilience";
...
export async function listPrimaryRadarLocationsPageForForecastCron({
  limit = 25,
  cursor = null,
  userId = null,
} = {}) {
  ...
  const fetchLimit = Math.min(1000, safeLimit * 3 + 1);
  ...
  if (error) {
    throw new Error(`listPrimaryRadarLocationsPageForForecastCron failed: ${error.message}`);
  }
  ...
  const items = unique.slice(0, safeLimit);
  const hasMore = !userId && (unique.length > safeLimit || (data || []).length === fetchLimit);

  return {
    items,
    hasMore,
    nextCursor: hasMore && items.length ? String(items.at(-1)?.user_id || "") || null : null,
  };
}

// Backward-compatible helper for one-page callers.
export async function listPrimaryRadarLocationsForForecastCron(options = {}) {
  const page = await listPrimaryRadarLocationsPageForForecastCron(options);
  return page.items;
}
...
export async function getForecastBundle({ userId, targetDate }) {
  ...
  const { data: forecasts, error: forecastError } =
    await runSupabaseOperationWithFastRetry(
      () =>
        supabase
          ...
          .limit(1),
      { label: "getForecastBundle.forecast" }
    );
  ...
  const { data: carePlan, error: carePlanError } =
    await runSupabaseOperationWithFastRetry(
      () =>
        supabase
          ...
          .maybeSingle(),
      { label: "getForecastBundle.carePlan" }
    );
