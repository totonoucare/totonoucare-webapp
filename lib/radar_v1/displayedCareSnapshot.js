...
import { displayedCareUsageNote } from "@/lib/records/displayedCareProvenance";
...
export function buildDisplayedCareSnapshot({
  ...
  const exactItems = buildDisplayedCareItems({
    ...
    domain: item.domain,
    kind: item.kind,
    label: ["ingredient_consumed","drink_consumed"].includes(item.meta?.record_semantics) ? `${item.meta.consumed_name}の提案` : compactText(item.label, 120),
    detail: compactText(["ingredient_consumed","drink_consumed"].includes(item.meta?.record_semantics) ? item.meta.selection_reason : item.detail, 180),
  }));
  ...
  const food = foodPlan?.title || foodItems.length ? {
    ...
    items: foodItems,
    proposals: safeArray(foodPlan?.action_cards).flatMap(card=>safeArray(card.item_details).map(item=>({
      name:compactText(item.consumed_name||item.label,60),slot:compactText(item.consumption_slot,20),
      reason:compactText(item.public_reason,180),meal_example:compactText(item.meal_example,100),
    }))).slice(0,8),
  ...
  const tsubo = points.length || tsuboSet?.line_care ? {
    ...
    line_care: tsuboSet?.line_care ? {
      ...
      meridian_code: compactText(tsuboSet.line_care.meridian_code, 30),
      selection_source: compactText(tsuboSet.line_care.selection_source, 30),
      selection_reason: compactText(tsuboSet.line_care.selection_reason, 240),
  ...
  return {
    ...
    usage_note: displayedCareUsageNote(source),
  };
}
