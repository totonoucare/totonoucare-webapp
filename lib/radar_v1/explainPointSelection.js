...
const TOMORROW_WEATHER_TONE = {
  ...
  damp: "湿気を含んだ服を着たように、胃腸や足もとに重さが残りやすい見込み",
...
const SYMPTOM_BRIDGE = {
  ...
  neck_shoulder: "首だけに原因を求めず、肩甲骨・胸・手元から力みを逃がしたい日です",
  ...
  digestion: "胃腸の重さを、お腹まわりからやさしく整えておきたい日です",
  fatigue: "だるさを無理に乗り切ろうとせず、足もとから動きやすさを支えたい日です",
  swelling: "足もとやお腹に残る水分の重さを、強く押さずに流したい日です",
};

const POINT_ROLE_COPY = {
  CV12: "お腹の中央から、胃腸の重さをやさしく整えたい時に使いやすい",
  ...
  LI4: "手元から、首肩・頭まわりの力みをゆるめたい時に使いやすい",
  ...
  ST36: "動き始めのだるさと胃腸まわりを、足から支えたい時に使いやすい",
...
const POINT_FAMILY_CONTEXT = {
  ST36: {
    base: "動き始めのだるさと胃腸まわりを足から支える",
    weather: {
      damp: "湿気で残りやすい胃腸・すねまわりの重さ",
      cold: "低温で重くなりやすい動き始め",
      heat: "高温で消耗しやすい胃腸まわり",
      pressure_down: "低気圧で重くなりやすい動き始め",
      pressure_up: "前のめりになった体を足もとから支える力",
      dry: "カサつき感で消耗しやすい体の土台",
    },
  },
  CV12: {
    base: "お腹の中央から胃腸の重さをやさしく整える",
    weather: {
...
function getBodyExact(point, tcmContext, riskContext = null) {
  const physicalExact = getPrimaryExact(point, tcmContext, riskContext);
  return (
    point?.care_body_trigger ||
    getLegacyCareTriggerKey(physicalExact, riskContext || tcmContext?.meta)
  );
}
...
function buildAbdomenRoleSummary(point) {
  if (point?.code === "CV12") {
    return "明日に胃腸の重さを残しすぎないよう、今夜のうちにお腹をやさしく整えるツボです";
  }
  if (point?.code === "CV6") {
...
function buildTcmRoleSummary(point, tcmContext, riskContext = null) {
  ...
  const bodyExact = getBodyExact(point, tcmContext, riskContext);
  ...
  if (point?.care_trigger_role === "secondary") {
    ...
    return buildSecondaryPointRoleSummary(point, bodyExact, symptomFocus);
  }
  ...
  if (weatherLabel) {
    if (exact === "pressure_down" || exact === "pressure_up") {
      const response = getPressureResponseCopy(readPressureResponseDirection(riskContext || point));
      return `${weatherLabel}の変化を受けた時に出やすい${response.short}を、${pointRole}ツボです`;
    }
...
function buildTcmSelectionReason(point, tcmContext, riskContext = null) {
  const exact = getPrimaryExact(point, tcmContext, riskContext);
  const bodyExact = getBodyExact(point, tcmContext, riskContext);
  const weatherTone = getWeatherTone(bodyExact);
  const weatherLabel = exactTriggerLabel(exact);
  const symptomFocus = getTcmSymptomFocus(tcmContext, riskContext);
  ...
  if (tcmContext?.need_abdomen && point?.code && point.code === tcmContext.abdomen_choice) {
    if (point.code === "CV12") {
      return `明日は、湿気や食後の重さが胃腸まわりに残りやすい見込みです。中脘はお腹の中央をやさしく整えたい時に使いやすいツボなので、今夜のうちに食後の重さを持ち越しにくくする候補にしています。`;
    }
    if (point.code === "CV6") {
  ...
  if (point?.care_trigger_role === "secondary" && weatherLabel) {
    const phrase = buildPointContextTarget(point, bodyExact, symptomFocus)
...
function buildMtestSelectionReason(riskContext, tsuboMeta) {
  ...
  const bodyExact = getLegacyCareTriggerKey(exact, riskContext);
  const weatherTone = getWeatherTone(bodyExact);
