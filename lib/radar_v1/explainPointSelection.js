...
function buildMtestRoleSummary(mtestContext) {
  if (mtestContext?.mode === "mother") {
    return "負担が出やすいラインを、やさしく支えるツボです";
  }
  return "張りや詰まりが出やすいラインを、ゆるめるツボです";
}
...
function buildMtestSelectionReason(riskContext, tsuboMeta) {
  ...
  if (mode === "mother") {
    return `この日は${lineLabel}をやさしく支えたい流れです。${triggerLabel}で負担が出やすい点も見て選んでいます。`;
  }

  return `この日は${lineLabel}の張りや詰まりをゆるめたい流れです。${triggerLabel}でこわばりが出やすい点も見て選んでいます。`;
}
