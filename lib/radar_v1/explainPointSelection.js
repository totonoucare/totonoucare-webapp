...
function buildTcmSelectionReason(point, tcmContext) {
  ...
  if (point?.care_trigger_role === "secondary" && careTriggerLabel) {
    ...
    if (phrase) {
      return `主因に加えて${careTriggerLabel}も少し響きそうなため、${phrase}方向を補助的に入れる目的で選んでいます。`;
    }
    return `主因に加えて${careTriggerLabel}も少し響きそうなため、補助的に偏りを逃がす目的で選んでいます。`;
  }
