...
export const RADAR_FORECAST_MODEL_VERSION = "radar_forecast_v2_2026-09-15_load_anchors_temperature_attention";

const UNIVERSAL_SHARE = 0.5;
const PERSONAL_SHARE = 0.5;
const PUBLIC_NEUTRAL_AFFINITY = 0.55;
const PUBLIC_REFERENCE_PROFILE = Object.freeze({
...
const ACUTE_SCORE_WEIGHTS = [10.2, 2.4, 1.2];
const PERSONAL_ABSOLUTE_SCORE_MAX = 10.4;
...
function buildForecastV2({ weatherStress, constitution, publicNeutral }) {
  ...
  const grouped = attachBodyResponseToGroups(addPersonalLoads(
    buildGroupedLoads({ eventLoads, weather, affinities, weatherStress }),
    reserveScalar,
    weatherStress
  ), manifestation.reaction_direction);
  ...
  const primary = triggerFactors[0] || emptyTriggerFactor(
    weatherStress,
    manifestation.reaction_direction
  );
  ...
  const selectedPeak = primary?.peak_start
    ? {
        start: primary.peak_start,
        end: primary.peak_end,
        direction: primary.attention_direction || null,
      }
    : null;
  ...
  return {
    ...
    personal_main_event_key: primary.event_key || null,
    pressure_direction: weatherStress?.pressure_direction || null,
    moisture_state: weatherStress?.moisture_state || weatherStress?.meta?.moisture?.state || null,
    pressure_response_direction: manifestation.reaction_direction,
    reaction_direction: manifestation.reaction_direction,
    reference_profile: publicNeutral ? PUBLIC_REFERENCE_PROFILE : null,
    environmental_cautions: Array.isArray(weatherStress?.environmental_cautions)
      ? weatherStress.environmental_cautions
      : [],
    ...
    meta: {
      ...
      forecast_model: {
        ...
        pressure_contract: {
          physical_event: "pressure_shift",
          physical_direction_field: "pressure_direction",
          body_response_field: "pressure_response_direction",
          legacy_exact_field: "personal_main_trigger_exact",
        },
        comfort_direction_policy: "away_from_band_primary_toward_band_transition_only",
        moisture_contract: {
          environment_state_field: "moisture_state",
          physical_direction_field: "moisture_direction",
          state_precedes_direction_for_affinity: true,
        },
        score_policy: "calibrated_load_anchors_with_independent_pressure_concurrence",
...
function normalizeConstitution(constitution = {}) {
  ...
  const directMaterials = computed?.material_scores || constitution?.material_scores || diagnosis?.material_scores || {};
  const materialScores = {
    qi_deficiency: finite(directMaterials.qi_deficiency ?? split?.qi?.deficiency),
    qi_stagnation: finite(directMaterials.qi_stagnation ?? split?.qi?.stagnation),
    blood_deficiency: finite(directMaterials.blood_deficiency ?? split?.blood?.deficiency),
    blood_stasis: finite(directMaterials.blood_stasis ?? split?.blood?.stasis),
    fluid_deficiency: finite(directMaterials.fluid_deficiency ?? split?.fluid?.deficiency),
    fluid_damp: finite(directMaterials.fluid_damp ?? split?.fluid?.damp),
  };
  const scoreScale = computed?.score_scale || constitution?.score_scale || split?.scale || null;
  ...
  const yinYang = finiteOrNull(axes?.reaction_score ?? axes?.yin_yang_score) ?? (
    coreCode ? (String(coreCode).startsWith("brake") ? -0.6 : 0.6) : 0
  );
  const drive = finiteOrNull(axes?.reserve_score ?? axes?.drive_score) ?? coreDriveFallback(coreCode);
  ...
  return {
    ...
    axes: {
      reaction_score: clamp(yinYang, -1, 1),
      reserve_score: clamp(drive, -1, 1),
      yin_yang_score: clamp(yinYang, -1, 1),
      drive_score: clamp(drive, -1, 1),
      obstruction_score: clamp(obstruction, 0, 1),
      cold_score: clamp01(finite(axes?.cold_score)),
      heat_score: clamp01(finite(axes?.heat_score)),
      thermal_direction: clamp(finite(axes?.thermal_direction), -1, 1),
      thermal_mixed: Boolean(axes?.thermal_mixed),
    ...
    normalized_material_scores: Object.fromEntries(
      MATERIAL_KEYS.map((key) => [key, round3(
        scoreScale === "0_100"
          ? clamp01(materialScores[key] / 100)
          : saturatingMaterialScore(materialScores[key])
      )])
    ),
  };
}
...
function buildAffinities(profile, weatherStress) {
  ...
  const hasIndependentThermalScores = profile.axes.cold_score > 0 || profile.axes.heat_score > 0;
  if (hasIndependentThermalScores) {
    add("cold", "cold_score", profile.axes.cold_score * 0.18);
    add("heat", "heat_score", profile.axes.heat_score * 0.18);
  } else {
    if (thermo === "cold") add("cold", "thermo_answer", 0.18);
    if (thermo === "heat") add("heat", "thermo_answer", 0.18);
    if (thermo === "mixed") {
      add("cold", "thermo_answer_mixed", 0.12);
      add("heat", "thermo_answer_mixed", 0.12);
    }
  }
...
function buildGroupedLoads({ eventLoads, weather, affinities, weatherStress }) {
  ...
  const moistureAbsoluteChoice = pickChoice(["damp", "dry"], eventLoads);
  const moistureDirection = weatherStress?.moisture_direction || "steady";
  const moisturePresentationDirection = resolvePresentationDirection({
    direction: moistureDirection,
    dominantDirection: weatherStress?.meta?.moisture?.dominant_direction,
    fallbackDirection: moistureAbsoluteChoice === "dry" ? "down" : "up",
  });
  const moistureShiftExact = resolveMoistureShiftExact({
    weatherStress,
    fallbackExact: moistureAbsoluteChoice,
    presentationDirection: moisturePresentationDirection,
  });
  const moistureShiftStrength = clamp01(weatherStress?.moisture_shift_strength);
  const moistureShiftAffinity = affinities?.[moistureShiftExact] || 0;
  const moistureShiftLoad = buildSingleEventLoad(
    moistureShiftStrength,
    moistureShiftAffinity
  );
  const moistureUsesShift =
    moistureShiftLoad > Number(eventLoads?.[moistureAbsoluteChoice] || 0);
  const moistureChoice = moistureUsesShift ? moistureShiftExact : moistureAbsoluteChoice;
  ...
  return {
    ...
    moisture: buildGroup({
      group: "moisture",
      eventKey: moistureUsesShift ? "moisture_shift" : moistureChoice,
      exact: moistureChoice,
      direction: moistureUsesShift
        ? moistureDirection
        : moistureChoice === "dry" ? "down" : "up",
      eventLoads,
      weather,
      affinities,
      effectiveLoad: moistureUsesShift ? moistureShiftLoad : null,
      weatherStrength: moistureUsesShift ? moistureShiftStrength : null,
      affinityWeight: moistureUsesShift ? moistureShiftAffinity : null,
      loadSource: moistureUsesShift ? "change" : "absolute",
    }),
  };
}
...
function buildGroup({
  ...
  return {
    ...
    load_source: loadSource || (
      ["pressure_shift", "temperature_shift", "moisture_shift"].includes(eventKey)
        ? "change"
        : "absolute"
    ),
    effective_load: round3(effectiveLoad ?? eventLoads[eventKey] ?? 0),
    weather_strength: round3(weatherStrength ?? weather[eventKey] ?? 0),
    affinity_weight: round3(affinityWeight ?? affinities[eventKey] ?? 0),
  };
}
...
function addPersonalLoads(grouped, reserveScalar, weatherStress) {
  return Object.fromEntries(Object.entries(grouped).map(([key, group]) => {
    ...
    const peak = getGroupPeak(weatherStress, group);
    return [key, {
      ...group,
      ...
      peak_direction: peak?.direction || null,
      attention_direction: peak?.direction || null,
    }];
  }));
}

function attachBodyResponseToGroups(grouped, responseDirection) {
  return Object.fromEntries(Object.entries(grouped || {}).map(([key, group]) => [key, {
    ...group,
    response_direction: group?.group === "pressure" ? responseDirection : null,
    body_response_key: group?.group === "pressure"
      ? `pressure_${responseDirection || "balanced"}`
      : group?.exact || null,
  }]));
}

function buildTriggerFactors(rankedGroups, weatherStress, pressureResponseDirection) {
  const top = rankedGroups[0];
  if (!top || top.effective_load <= 0.05) return [];
  const factors = [decorateFactor(top, "primary", weatherStress, pressureResponseDirection)];
  ...
  if (
    ...
    factors.push(decorateFactor(second, "secondary", weatherStress, pressureResponseDirection));
  }
  return factors;
}

function decorateFactor(group, role, weatherStress, pressureResponseDirection) {
  ...
  return {
    ...
    physical_direction: group.direction,
    response_direction: group.group === "pressure" ? pressureResponseDirection : null,
    body_response_key: group.group === "pressure"
      ? `pressure_${pressureResponseDirection || "balanced"}`
      : group.exact,
    main_trigger: compat.main_trigger,
    trigger_dir: compat.trigger_dir,
    ...
    load_source: group.load_source || null,
    peak_direction: group.peak_direction || null,
    attention_direction: group.attention_direction || group.peak_direction || null,
    peak_start: group.peak_start || null,
    peak_end: group.peak_end || null,
    peakStart: group.peak_start || null,
    peakEnd: group.peak_end || null,
  };
}
...
function computeScore({ rankedGroups, eventLoads, weather, affinities, reserveScalar, weatherStress }) {
  ...
  const moistureAffinityKey = resolveMoistureShiftExact({
    weatherStress,
    fallbackExact: moisturePresentationDirection === "down" ? "dry" : "damp",
    presentationDirection: moisturePresentationDirection,
  });
  const moistureShiftLoad = buildSingleEventLoad(
  ...
  const personalAbsoluteScore = PERSONAL_ABSOLUTE_SCORE_MAX
    * Math.pow(personalAbsoluteLoad, 1.2)
    * appliedReserve;
  ...
  // Pressure is independent of the combined thermal/moisture burden.
  const pressureScore = (eventLoads.pressure_shift || 0) * ACUTE_SCORE_WEIGHTS[0] * appliedReserve;
  const concurrenceBonus = Math.min(pressureScore, absoluteScore) * 0.22;
  const rawScore = Math.max(acuteScore, absoluteScore) + concurrenceBonus;
  const calibrated = rawScore <= 7 ? rawScore : 7 + 3 * (1 - Math.exp(-(rawScore - 7) / 3));
  const final = clamp(Math.max(calibrated, universalExtremeFloor + criticalExtremeBonus), 0, 10);
  ...
  return {
    ...
    score_components: {
      ...
      raw_score_before_upper_curve: round2(rawScore),
      upper_curve: "7 + 3 * (1 - exp(-(raw - 7) / 3)) above 7; independent extreme floor",
      final_policy: "max_acute_or_absolute_plus_independent_pressure_concurrence",
    },
    bonuses: {
...
function combineThermalEnvironmentLoad(temperatureLoad, moistureLoad) {
  ...
  return clamp01(Math.max(temperature, moisture * 0.75) + Math.min(temperature, moisture) * 0.2);
}

function buildUniversalExtremeFloor(rawAbsoluteLoad) {
  return 9.7 * Math.pow(clamp01(rawAbsoluteLoad), 3);
}
...
function buildReserveScalar(profile) {
  return 1 - profile.axes.reserve_score * 0.1;
}

function buildManifestationProfile(profile) {
  const yy = profile.axes.reaction_score;
  const reactionDirection = yy >= 0 ? "accel" : "brake";
  ...
  return {
    ...
    reserve_score: round3(profile.axes.reserve_score),
...
function resolvePresentationDirection({
  direction,
  dominantDirection = null,
  fallbackDirection = "up",
}) {
  if (direction === "up" || direction === "down") return direction;
  if (dominantDirection === "up" || dominantDirection === "down") return dominantDirection;
  return fallbackDirection;
}

function resolveMoistureShiftExact({
  weatherStress,
  fallbackExact = "damp",
  presentationDirection = null,
}) {
  const moisture = weatherStress?.meta?.moisture || {};
  const state = weatherStress?.moisture_state || moisture?.state || "neutral";

  // The absolute water environment selects the body affinity whenever the
  // target day remains clearly on one side of the comfort band. Change
  // direction is retained for peak timing and physical explanation, but must
  // not turn "damp but falling" into dryness (or vice versa).
  if (state === "damp") return "damp";
  if (state === "dry") return "dry";

  const direction = resolvePresentationDirection({
    direction: weatherStress?.moisture_direction || moisture?.direction,
    dominantDirection: moisture?.dominant_direction,
    fallbackDirection: presentationDirection || (fallbackExact === "dry" ? "down" : "up"),
  });
  const departure = Number(moisture?.comfort_departure_strength || 0);
  const relief = Number(moisture?.comfort_relief_strength || 0);
  const rawShift = Number(moisture?.raw_shift_strength || 0);
  const departureDrivesLoad =
    departure > 0 &&
    departure >= rawShift * MOISTURE_RELIEF_TRANSITION_SHARE;

  if (departureDrivesLoad) return direction === "down" ? "dry" : "damp";
  if (relief > departure) {
    // 中間帯へ戻る動きは、行き先を反対側の負荷と誤表示せず、
    // 戻る前にいた水分環境側へ含める。
    return direction === "down" ? "damp" : "dry";
  }
  return fallbackExact === "dry" ? "dry" : "damp";
}

function peakStrength(peak) {
  const strength = Number(peak?.strength);
  return Number.isFinite(strength) ? strength : null;
}

function chooseDirectionalPeak({
  peaks,
  direction,
  upKey,
  downKey,
  ...
  if (direction === "up") {
    return { ...(peaks?.[upKey] || peaks?.[fallbackKey] || {}), direction: "up" };
  }
  if (direction === "down") {
    return { ...(peaks?.[downKey] || peaks?.[fallbackKey] || {}), direction: "down" };
  }

  const upPeak = peaks?.[upKey] || null;
  const downPeak = peaks?.[downKey] || null;
  const upStrength = peakStrength(upPeak);
  const downStrength = peakStrength(downPeak);
  if (upStrength != null && downStrength != null && upStrength !== downStrength) {
    return upStrength > downStrength
      ? { ...upPeak, direction: "up" }
      : { ...downPeak, direction: "down" };
  }

  const resolvedFallbackDirection = fallbackDirection === "down" ? "down" : "up";
  const fallbackPeak =
    peaks?.[resolvedFallbackDirection === "up" ? upKey : downKey] ||
    peaks?.[fallbackKey] ||
    null;
  return fallbackPeak
    ? { ...fallbackPeak, direction: resolvedFallbackDirection }
    : { direction: direction === "mixed" ? "mixed" : resolvedFallbackDirection };
}

function getGroupPeak(weatherStress, group = {}) {
  const peaks = weatherStress?.channel_peaks || {};

  if (group.group === "pressure") {
    const fallbackDirection = group.exact === "pressure_up" ? "up" : "down";
    return chooseDirectionalPeak({
      peaks,
      direction: group.direction,
      upKey: "pressure_up",
      downKey: "pressure_down",
      fallbackKey: "pressure_shift",
      fallbackDirection,
    });
  }

  if (group.group === "temperature" && group.event_key === "temperature_shift") {
    // The attention window follows scored discomfort, including the small
    // transition allowance, rather than the larger raw up/down temperature delta.
    if (peaks.temp_shift?.direction) return peaks.temp_shift;
    const fallbackDirection = resolvePresentationDirection({
      direction: group.direction,
      dominantDirection: weatherStress?.meta?.temperature?.dominant_direction,
      fallbackDirection: "up",
    });
    return chooseDirectionalPeak({
      peaks,
      ...
      upKey: "temp_up",
      downKey: "temp_down",
      fallbackKey: "temp_shift",
      fallbackDirection,
    });
  }

  if (group.group === "moisture" && group.event_key === "moisture_shift") {
    const fallbackDirection = resolvePresentationDirection({
      direction: group.direction,
      dominantDirection: weatherStress?.meta?.moisture?.dominant_direction,
      fallbackDirection: group.exact === "dry" ? "down" : "up",
    });
    return chooseDirectionalPeak({
      peaks,
      direction: group.direction,
      upKey: "moisture_up",
      downKey: "moisture_down",
      fallbackKey: "moisture_shift",
      fallbackDirection,
    });
  }

  return peaks?.[group.exact] || null;
}

function emptyTriggerFactor() {
  return { exact: "none", direction: "none", physical_direction: "none", response_direction: null, body_response_key: null };
}
