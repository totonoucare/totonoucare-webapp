...
export const RADAR_FORECAST_MODEL_VERSION = "radar_forecast_v2_2026-07-21";

const UNIVERSAL_SHARE = 0.62;
const PERSONAL_SHARE = 0.38;
const SECONDARY_MIN_LOAD = 0.24;
const SECONDARY_MIN_RATIO = 0.45;

const GROUP_SCORE_WEIGHTS = [4.9, 1.55, 0.65];
...
export function personalizeForecastV2({ weatherStress, constitution }) {
  ...
  const affinities = buildAffinities(profile, weatherStress);
  const eventLoads = buildEventLoads(weather, affinities);
  const grouped = buildGroupedLoads({ eventLoads, weather, affinities, weatherStress });
  ...
  const scoreTrace = computeScore({
    rankedGroups,
    reserveScalar: buildReserveScalar(profile),
  });
  ...
  return {
    ...
    meta: {
      ...
      forecast_model: {
        ...
        group_overlap_policy: "one_contribution_per_weather_group",
...
function normalizeConstitution(constitution = {}) {
  ...
  const yinYang = finiteOrNull(axes?.yin_yang_score) ?? (String(coreCode || "").startsWith("brake") ? -0.6 : 0.6);
...
function buildAffinities(profile, weatherStress) {
  ...
  const common = 0.64 + sensitivity * 0.09;
  const affinities = {
    pressure_shift: common,
    temperature_shift: common,
    cold: common,
    heat: common,
    damp: common,
    dry: common,
  ...
  if (vectors.has("pressure_shift")) affinities.pressure_shift += 0.1;
  if (vectors.has("temp_swing")) affinities.temperature_shift += 0.1;
  if (vectors.has("humidity_up")) affinities.damp += 0.1;
  if (vectors.has("dryness_up")) affinities.dry += 0.1;
  if (vectors.has("wind_strong")) affinities.pressure_shift += 0.04;

  const thermo = profile.axes.thermo_answer;
  if (thermo === "cold") affinities.cold += 0.12;
  if (thermo === "heat") affinities.heat += 0.12;

  // Direction agreement is deliberately modest. The opposite direction never
  // receives a penalty, so both accel and brake types can respond to both.
  const yy = profile.axes.yin_yang_score;
  const pressureDirection = weatherStress?.pressure_direction;
  if (pressureDirection === "up") affinities.pressure_shift += Math.max(0, yy) * 0.06;
  else if (pressureDirection === "down") affinities.pressure_shift += Math.max(0, -yy) * 0.06;
  else if (pressureDirection === "mixed") affinities.pressure_shift += Math.abs(yy) * 0.03;

  return Object.fromEntries(Object.entries(affinities).map(([key, value]) => [key, round3(clamp(value, 0.5, 0.95))]));
}
...
function computeScore({ rankedGroups, reserveScalar }) {
  const contributions = rankedGroups.map((entry, index) => ({
    ...entry,
    rank: index + 1,
    base_weight: GROUP_SCORE_WEIGHTS[index] || 0,
    contribution: round2(entry.effective_load * (GROUP_SCORE_WEIGHTS[index] || 0)),
  }));
  const base = contributions.reduce((sum, entry) => sum + entry.effective_load * entry.base_weight, 0);
  const appliedReserve = clamp(reserveScalar, 0.94, 1.08);
  const afterReserve = base * appliedReserve;
  const top = rankedGroups[0] || {};
  const strongEventBonus = top.weather_strength >= 0.9 ? 0.35 : 0;
  const multiGroupCount = rankedGroups.filter((entry) => entry.effective_load >= 0.5).length;
  const multiGroupBonus = multiGroupCount >= 2 ? 0.25 : 0;
  const final = clamp(afterReserve + strongEventBonus + multiGroupBonus, 0, 10);
  return {
    final_score_0_10: round1(final),
    base_score_before_reserve: round2(base),
    reserve_scalar_applied: round3(appliedReserve),
    score_after_reserve: round2(afterReserve),
    weather_group_contributions: contributions,
    bonuses: {
      strong_weather_event: strongEventBonus,
      multi_group_overlap: multiGroupBonus,
      multi_group_count: multiGroupCount,
    },
  };
}

function buildReserveScalar(profile) {
  return 1 - profile.axes.drive_score * 0.08;
}
