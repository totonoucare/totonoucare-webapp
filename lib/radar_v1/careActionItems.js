...
function actionIdentityParts(action = {}) {
  ...
  if (domain === "eat") {
    if (["ingredient_consumed","drink_consumed"].includes(meta.record_semantics) && meta.consumed_id) {
      return {domain,kind:meta.record_semantics,identity:compact(meta.consumed_id,80)};
    }
    ...
    if (/_item$/.test(storedKind) && cardKey) {
      return {
        domain,
        kind: `${cardKey}_item`,
        identity: `${cardKey}|${meta.record_semantics === "ingredients_or_eating_pattern" ? "adoption|" : ""}${label}`,
      };
    }
...
export function buildDisplayedCareItems({
    ...
    foodCards.slice(0, 6).forEach((card, cardIndex) => {
      ...
      if (cardItems.length && cardKey !== "caution") {
        cardItems.forEach((foodItem, itemIndex) => {
          const itemDetail = safeArray(card.item_details)[itemIndex] || {};
          if (itemDetail.recordable === false) return;
          const consumed = ["ingredient_consumed","drink_consumed"].includes(itemDetail.record_semantics);
          const adopted = itemDetail.record_semantics === "ingredients_or_eating_pattern" && itemDetail.record_label;
          const recordLabel = compact((adopted || consumed) ? itemDetail.record_label : foodItem, 120);
          const itemKey = buildCareActionKey({
            domain: "eat",
            kind: consumed ? itemDetail.record_semantics : `${cardKey}_item`,
            identity: consumed ? itemDetail.consumed_id : `${cardKey}|${adopted ? "adoption|" : ""}${recordLabel}`,
          });
          items.push({
            item_key: itemKey,
            canonical_key: itemKey,
            ...
            label: recordLabel,
            detail: consumed ? `${itemDetail.consumed_name}を実際に${itemDetail.record_semantics === "drink_consumed" ? "飲んだ" : "食べた"}記録です。量や料理の完食は記録していません。` : adopted ? "提案の食材や食べ方を取り入れた記録です。料理例の調理・完食や、挙げた食材すべての摂取を意味しません。" : compact(card?.body || cardLabel, 240),
            meta: {
              card_key: cardKey,
              card_label: cardLabel,
              ...(consumed ? {
                record_semantics:itemDetail.record_semantics,consumed_id:itemDetail.consumed_id,consumed_name:itemDetail.consumed_name,
                consumption_slot:itemDetail.consumption_slot,selection_basis:itemDetail.selection_basis,
                selection_reason:itemDetail.public_reason,meal_example:itemDetail.meal_example||null,
              } : {}),
              ...(adopted ? {
                record_semantics: "ingredients_or_eating_pattern",
                suggested_ingredients: safeArray(itemDetail.focus_ingredients),
                meal_example: foodItem,
              } : {}),
  ...
  if (lineCare?.label || lineCare?.action) {
    ...
    items.push({
      ...
      meta: {
        rule_id: ruleId,
        line_group_id: compact(lineCare.line_id, 80) || null,
        meridian_code: compact(lineCare.meridian_code, 30) || null,
        selection_source: compact(lineCare.selection_source, 30) || null,
        selection_reason: compact(lineCare.selection_reason, 240) || null,
        selection_basis: lineCare.selection_basis || null,
        order: 0,
      },
      source_mode: mode,
    });
