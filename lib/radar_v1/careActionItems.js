    ...
    foodCards.slice(0, 6).forEach((card, cardIndex) => {
      ...
      if (cardItems.length && cardKey !== "caution") {
        cardItems.forEach((foodItem, itemIndex) => {
          ...
          const recordLabel = compact(adopted ? itemDetail.record_label : foodItem, 120);
          const itemKey = buildCareActionKey({
            domain: "eat",
            kind: `${cardKey}_item`,
            identity: `${cardKey}|${adopted ? "adoption|" : ""}${recordLabel}`,
          });
          items.push({
            item_key: itemKey,
            canonical_key: itemKey,
            ...
            label: recordLabel,
            detail: adopted ? "提案の食材や食べ方を取り入れた記録です。料理例の調理・完食や、挙げた食材すべての摂取を意味しません。" : compact(card?.body || cardLabel, 240),
            meta: {
              card_key: cardKey,
