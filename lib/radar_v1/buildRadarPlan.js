...
function buildFoodHowTo(riskContext, timing) {
  ...
  if (main === "temp" && dir === "down") {
    ...
    return secondaryHint ? `${base} ${secondaryHint}方向に寄せるとより合います。` : base;
  }
  ...
  if (main === "pressure" && dir === "down") {
    ...
    return secondaryHint ? `${base} ${secondaryHint}方向も合わせると安心です。` : base;
  }
  ...
  return secondaryHint ? `${base} ${secondaryHint}方向も少し添えると合います。` : base;
}
...
function buildFoodReason(riskContext) {
  ...
  const base = "明日のぶれを小さくしやすい方向に寄せるためです。";
