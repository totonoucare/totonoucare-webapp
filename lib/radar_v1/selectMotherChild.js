 ...
 *
 * Agreed MVP logic:
 * - pressure + brake response -> mother
 * - pressure + accel response -> child
 * - temp/down     -> mother
 * - temp/up       -> child
 * - humidity/up   -> child
 * - fallback      -> mother
...
export function selectMotherChild({ weatherStress, core_code = "", sub_labels = [] }) {
  ...
  if (main === "pressure" && pressureResponseDirection === "accel") {
    baseMode = "child";
    baseReason = "pressure_accel_release";
  } else if (main === "pressure") {
    baseMode = "mother";
    baseReason = pressureResponseDirection === "brake"
      ? "pressure_brake_support"
      : "pressure_balanced_support";
