...
const SYMPTOM_HINTS = {
  ...
  digestion: {
    organ_focus: { spleen: 1.4, liver: 0.2 },
    meridian_code: { cv: 0.8, st: 0.8, sp: 0.8, pc: 0.5 },
    point_region: { abdomen: 1.0, limb: 0.5 },
  },
  sleep: {
...
export async function pickTcmPoints({ differentiation, riskContext = null }) {
  ...
  const secondaryContext = buildSecondarySelectionContext({ context, differentiation, riskContext });
  ...
  if (differentiation.need_abdomen) {
    ...
    if (abdomenPick?.point) {
      abdomenPick.point = annotateCareTrigger(abdomenPick.point, context);
      chosen.push(abdomenPick.point);
    }
  }

  const primaryLimbContext = differentiation.need_abdomen && secondaryContext.hasSecondary
    ? secondaryContext
    : context;
  ...
  const primaryLimbPick = pickBestLimbPoint({
    ...
    context: primaryLimbContext,
    chosen,
  });

  if (primaryLimbPick?.point) {
    chosen.push(annotateCareTrigger(primaryLimbPick.point, primaryLimbContext));
  }
  ...
  const secondaryLimbContext = {
    ...(secondaryContext.hasSecondary ? secondaryContext : context),
  ...
  const secondaryLimbPick = pickBestLimbPoint({
    ...
    context: secondaryLimbContext,
    chosen,
  });

  if (!differentiation.need_abdomen && secondaryLimbPick?.point) {
    chosen.push(annotateCareTrigger(secondaryLimbPick.point, secondaryLimbContext));
  }
  ...
  return {
    ...
    meta: {
      ...
      exact_trigger: context.exactTrigger || null,
      body_trigger: context.bodyTrigger || null,
      pressure_response_direction: context.pressureResponseDirection || null,
      secondary_exact_trigger: secondaryContext.hasSecondary ? secondaryContext.exactTrigger : null,
      coarse_trigger: context.coarseTrigger || null,
      exact_trigger_source: context.exactTriggerSource || null,
      secondary_exact_trigger_source: secondaryContext.hasSecondary ? secondaryContext.exactTriggerSource : null,
      selection_debug: {
...
function buildSelectionContext({ differentiation, riskContext }) {
  const exactInfo = resolveExactTrigger({ differentiation, riskContext });
  const exactTrigger = exactInfo.exactTrigger;
  const pressureResponseDirection = readPressureResponseDirection(riskContext || differentiation?.meta);
  const bodyTrigger = getLegacyCareTriggerKey(exactTrigger, pressureResponseDirection);
  ...
  return {
    ...
    exactTriggerSource: exactInfo.source,
    hasSecondary: false,
    coarseTrigger,
...
function buildSecondarySelectionContext({ context, differentiation, riskContext }) {
  const secondaryExact = resolveSecondaryExactTrigger({ riskContext });
  if (!isKnownExactTrigger(secondaryExact) || secondaryExact === context.exactTrigger) {
    return { ...context, hasSecondary: false };
  }

  return {
    ...context,
    exactTrigger: secondaryExact,
    bodyTrigger: getLegacyCareTriggerKey(secondaryExact, context.pressureResponseDirection),
    exactTriggerSource: "riskContext.summary.secondary_trigger_exact",
    coarseTrigger: exactTriggerToMainTrigger(secondaryExact) || null,
    triggerDir: exactTriggerToDir(secondaryExact) || context.triggerDir,
    wantedActions: compactUnique(differentiation?.secondary_actions?.length ? differentiation.secondary_actions : context.wantedActions),
    wantedActionsSecondary: compactUnique(differentiation?.primary_actions),
    wantedLabels: compactUnique([
      differentiation?.secondary_label,
      differentiation?.primary_label,
    ]),
...
function resolveSecondaryExactTrigger({ riskContext }) {
  const candidates = [
  ...
  return candidates.find(isKnownExactTrigger) || null;
}

function annotateCareTrigger(point, context) {
  if (!point) return point;
  return {
    ...point,
    care_trigger_exact: context?.exactTrigger || null,
    care_body_trigger: context?.bodyTrigger || null,
    pressure_response_direction: context?.pressureResponseDirection || null,
    care_trigger_role: context?.hasSecondary ? "secondary" : "primary",
  };
}

function resolveExactTrigger({ differentiation, riskContext }) {
  const candidates = [
    [differentiation?.meta?.exact_trigger, "differentiation.meta.exact_trigger"],
    [differentiation?.meta?.personal_main_trigger_exact, "differentiation.meta.personal_main_trigger_exact"],
    [riskContext?.summary?.main_trigger_exact, "riskContext.summary.main_trigger_exact"],
    [riskContext?.weather_context?.personal_main_trigger_exact, "riskContext.weather_context.personal_main_trigger_exact"],
    [riskContext?.summary?.personal_main_trigger_exact, "riskContext.summary.personal_main_trigger_exact"],
    [differentiation?.meta?.weather_main_trigger_exact, "differentiation.meta.weather_main_trigger_exact"],
    [riskContext?.summary?.weather_main_trigger_exact, "riskContext.summary.weather_main_trigger_exact"],
    [riskContext?.weather_context?.weather_main_trigger_exact, "riskContext.weather_context.weather_main_trigger_exact"],
  ];

  const found = candidates.find(([value]) => isKnownExactTrigger(value));
  return {
    exactTrigger: found?.[0] || null,
    source: found?.[1] || null,
  };
}

function isKnownExactTrigger(value) {
  return [
    "pressure_down",
    "pressure_up",
    "cold",
...
function scorePoint(point, { slot, context, chosenCodes, chosenMeridians, chosenActionSet, chosenOrganSet }) {
  ...
  const keywordTags = safeArray(point.tags_keyword);
  ...
  const bodyTrigger = context.bodyTrigger || exactTrigger;
  ...
  // Trigger fit.
  // v7.38: Supabase側に pressure_down / pressure_up / cold / heat / damp / dry などの
  // exact trigger tag を追加できるようにした。既存の pressure/temp/humidity も引き続き有効。
  if (exactTrigger && triggerTags.includes(exactTrigger)) {
    score += 2.2;
  }
  if (coarseTrigger && triggerTags.includes(coarseTrigger)) {
    score += 1.3;
  }
  if (exactTrigger && keywordTags.includes(exactTrigger)) {
    score += 0.7;
  }

  score += sumActionBonuses(actions, ACTION_BONUS_BY_TRIGGER[bodyTrigger]);
  score += sumLabelBonuses(subTags, LABEL_BONUS_BY_TRIGGER[bodyTrigger]);
  score += sumOrganBonuses(organs, ORGAN_BONUS_BY_TRIGGER[bodyTrigger]);

  // Symptom bridge fit.
  if (symptomFocus && symptomTags.includes(symptomFocus)) {
    score += slot === "abdomen" ? 2.0 : 2.8;
  }
  if (symptomFocus && keywordTags.includes(symptomFocus)) {
    score += 0.5;
  }
  ...
  if (slot === "abdomen") {
    ...
    if (symptomFocus === "digestion" && point.code === "CV12") {
      score += 2.4;
    }
  }
...
function exactTriggerToDir(exactTrigger) {
  switch (exactTrigger) {
    case "pressure_down":
    case "cold":
    case "dry":
