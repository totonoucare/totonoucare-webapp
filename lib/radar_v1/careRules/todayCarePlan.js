...
export function buildTodayCarePlanCore({
  ...
  const food = buildTodayFoodContext(triggerKey, signal, symptomFocus, secondaryKey);

  return {
    id: `today-care-${forecast?.target_date || "live"}`,
    night_tsubo_set: {
      title: "今日これから使えるツボ",
      lead: "今見ている不調とこのあとの天気の山場に合わせて、短時間で触れやすい場所を中心にしています。",
      points,
    },
    night_tsubo_reason: points[0]?.explanation?.selection_reason || "今日これからの不調と天気の山場に合わせて、短時間で触れやすい場所を中心にしています。",
