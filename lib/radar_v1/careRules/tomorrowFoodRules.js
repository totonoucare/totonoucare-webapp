...
const TRIGGER_FOOD_RULES = {
  ...
  heat: {
    title: "明日は熱をこもらせない食べ方",
    recommendation:
      "辛味・濃い味・お酒を増やさず、常温の水分や軽い汁気を入れて、明日に熱っぽさを持ち越しにくくします。",
    how_to:
  ...
  default: {
    title: "明日に向けた温かさの準備",
    recommendation:
      "今夜〜明朝は、温かい飲み物や汁物を一つ足し、食べすぎ・お酒・甘いものを同じ時間に重ねないようにします。",
    how_to:
...
const SYMPTOM_FOOD_ITEMS = {
  ...
  digestion: {
    add: {
      default: ["味噌汁", "軽いスープ", "温かいお茶"],
      damp: ["具だくさん味噌汁", "温かいお茶", "小さめごはん"],
      pressure_down: ["軽いスープ", "温かいお茶", "小さめ主食"],
      cold: ["雑炊", "味噌汁", "生姜入りスープ"],
      heat: ["常温の水分", "野菜スープ", "梅入りおにぎり"],
      dry: ["温かいお茶", "汁物", "やわらかい主食"],
    },
    caution: {
      default: ["冷たい飲み物＋早食い", "食べすぎ＋座りっぱなし", "甘いもの＋カフェイン"],
      damp: ["冷たい飲み物＋甘いもの", "パン・麺＋甘いカフェラテ", "揚げ物＋食べすぎ"],
      pressure_down: ["脂っこいもの＋食べすぎ", "カフェイン＋空腹", "お酒＋夜更かし"],
      cold: ["冷たい飲み物＋空腹", "冷たい乳製品＋朝", "生もの中心＋薄着"],
      heat: ["冷たい飲み物を続ける", "辛味＋冷たいもの", "甘いドリンク＋暑さ"],
    },
    examples: {
      default: ["味噌汁＋小さめごはん", "軽いスープ", "温かいお茶"],
      damp: ["具だくさん味噌汁", "雑炊", "温かいお茶"],
      cold: ["雑炊", "卵スープ", "味噌汁＋ごはん"],
    },
  },
  neck_shoulder: {
    add: {
...
const SYMPTOM_FOOD_RULES = {
  fatigue: {
    title: {
      default: "明日はだるさを残さない食べ方",
      damp: "明日の朝に重だるさを残さない食べ方",
      cold: "明日の動き出しに温かさを足す食べ方",
      pressure_down: "明日は食べすぎ・空腹の差を小さくする食べ方",
    },
    how_to: {
  ...
  sleep: {
    title: {
      default: "今夜は眠りを邪魔しない食べ方",
      cold: "寝る前に足首とお腹を冷やさない食べ方",
      heat: "今夜は辛味・お酒・濃い味を残さない食べ方",
      pressure_down: "首肩の重さを残さない夜の準備",
    },
    how_to: {
  ...
  digestion: {
    title: {
      default: "明日は胃腸を重くしない食べ方",
      damp: "明日は胃腸に湿気の重さを残さない食べ方",
      pressure_down: "明日は胃腸の動きを重くしない食べ方",
      cold: "今夜からお腹を冷やさない食べ方",
      heat: "明日は冷やしすぎで胃腸を疲れさせない食べ方",
    },
    how_to: {
      default: "今夜〜明日の朝に温かい汁物かお茶を足して、量や刺激で押し切らない形にします。",
      damp: "今夜の冷たさ・甘さ・揚げ物を重ねすぎず、明日は温かい汁物か軽めの主食を足します。",
      pressure_down: "小さめ主食や軽いスープを足して、脂っこさや食べすぎで胃腸の重さを増やさないようにします。",
      cold: "夕食か明日の朝に味噌汁・雑炊・温かい飲み物のどれかを足して、お腹まわりを冷やしすぎない形にします。",
      heat: "冷たい飲み物を続けすぎず、常温の水分や軽い汁物を一つ挟みます。",
    },
    avoid: {
      default: "早食い・食べすぎ・冷たい飲み物を夜から朝に重ねる。",
      damp: "冷たい飲み物、甘いもの、パン・麺中心を夜から朝に重ねる。",
      pressure_down: "脂っこいもの・お酒・食べすぎを重ねて、明日の胃腸の重さを増やす。",
      cold: "冷たい飲み物や生もの中心で、お腹まわりを内側から冷やす。",
      heat: "暑いからと冷たいものだけで済ませ、胃腸に重さを残す。",
    },
    reason: "胃腸の調子が気になる日は、前夜の冷たさ・食べすぎ・急いで食べる流れを軽くするほど、翌朝の重さを残しにくいためです。",
    lifestyle_tip: "今夜は食後すぐ座りっぱなしにせず、数分だけ立つか歩く時間を作ります。",
  },
  neck_shoulder: {
    title: {
      default: "明日は首肩にこわばりを残さない食べ方",
      pressure_down: "明日は首肩のこもりを増やさない食べ方",
      cold: "今夜から首元と胃腸を冷やさない食べ方",
      dry: "明日は首肩の乾き・こわばりに汁気を足す食べ方",
    },
    how_to: {
      default: "今夜〜明日の朝に温かい飲み物か汁物を足して、甘いもの・カフェインだけで済ませないようにします。",
    ...
    avoid: {
      ...
      pressure_down: "お酒や脂っこいものを重ねて、そのまま首肩を固める。",
    },
  ...
  low_back_pain: {
    title: {
      default: "明日は腰腹を冷やさない食べ方",
      damp: "明日は腰まわりに重だるさを残さない食べ方",
      cold: "今夜から腰腹を冷やさない食べ方",
      pressure_down: "明日は腰まわりに重さを残さない食べ方",
    },
    how_to: {
  ...
  swelling: {
    title: {
      default: "明日はむくみを増やさない食べ方",
      damp: "今夜は甘さ・冷たさ・塩気を重ねない食べ方",
      heat: "明日は水分を小分けに入れる食べ方",
      cold: "足元を冷やさず、むくみを残さない食べ方",
    },
    how_to: {
  ...
  headache: {
    title: {
      default: "明日は頭の重さを増やさない食べ方",
      pressure_down: "今夜はお酒と脂っこさを重ねない食べ方",
      damp: "明日は頭に重さを残さない食べ方",
      cold: "今夜から首元と胃腸を冷やさない食べ方",
      dry: "明日は目と喉の乾きを残さない食べ方",
    },
    how_to: {
      default: "今夜〜明日の朝に温かい飲み物か軽い汁物を足して、空腹のままカフェインや甘いもので済ませないようにします。",
  ...
  dizziness: {
    title: {
      default: "明日は空腹と急な動き出しを避ける食べ方",
      pressure_down: "明日は空腹と食べすぎの差を小さくする食べ方",
      heat: "明日は水分を小分けにする食べ方",
      cold: "明日は足元まで冷やさない食べ方",
    },
    how_to: {
  ...
  mood: {
    title: {
      default: "明日は気分を急に上げ下げしない食べ方",
      pressure_down: "甘さとカフェインだけに寄せない食べ方",
      heat: "今夜は辛味・濃い味・カフェインを足しすぎない食べ方",
      cold: "温かい食事で体を縮こませない食べ方",
    },
    how_to: {
      default: "今夜〜明日の朝に温かい飲み物・汁物・軽い主食のどれかを足して、甘いものやカフェインだけで持ち上げない形にします。",
      pressure_down: "軽い主食や汁物を足し、甘いもの・カフェインだけで済ませないようにします。",
...
function getLeadTail({ triggerKey, symptomFocus, intensity }) {
  const soft = intensity === "low";
  const strong = intensity === "high";
  const end = soft ? "少し残ることがあります。" : strong ? "強く残りやすい日です。" : "残りやすい日です。";
  const overlapEnd = soft ? "少し重なることがあります。" : strong ? "強く重なりやすい日です。" : "重なりやすい日です。";

  if (symptomFocus === "fatigue") return `今夜の冷たさ・甘さ・食べすぎが、明日のだるさに${end}`;
  if (symptomFocus === "sleep") return `夕方以降の食べすぎや刺激が、今夜の眠りに${end}`;
  if (symptomFocus === "digestion") return `今夜の冷たさ・食べすぎ・早食いが、明日の胃腸の重さに${end}`;
  if (symptomFocus === "neck_shoulder") return `今夜の食後の重さや画面姿勢が、明日の首肩のこわばりに${end}`;
  if (symptomFocus === "low_back_pain") return `今夜の冷たさや食後の重さが、明日の腰まわりに${end}`;
  if (symptomFocus === "swelling") return `今夜の塩気・冷たさ・甘さが、明日のむくみに${end}`;
  if (symptomFocus === "headache") return `空腹・画面刺激・食後の重さが、明日の頭の重さに${overlapEnd}`;
  if (symptomFocus === "dizziness") return `空腹や冷たい飲み物の一気飲みが、明日のふらつきに${overlapEnd}`;
  if (symptomFocus === "mood") return `甘いもの・カフェイン・食事抜きが、明日の気分の揺れに${overlapEnd}`;

  if (triggerKey === "damp") return `今夜の冷たさ・甘さ・塩気が、明朝の重さに${end}`;
  if (triggerKey === "cold") return `今夜の冷たさが、明朝のこわばりに${end}`;
  if (triggerKey === "heat") return `夜の辛味・濃い味・お酒が、明日のほてりやだるさに${end}`;
  if (triggerKey === "dry") return `水分不足や乾いた間食が、明日の喉・目の乾きに${end}`;
  if (triggerKey === "pressure_down") return `空腹・食べすぎ・お酒が、明日の頭や首肩の重さに${overlapEnd}`;
  if (triggerKey === "pressure_up") return `辛味・濃い味・カフェインが、明日のこわばりや張りつめに${end}`;
  return `今夜の食べ方が、明日の体感に${end}`;
}

function buildRecommendation({ signalContext, triggerKey, symptomFocus }) {
  const lead = signalContext?.lead || "明日の天気の影響を少し見ておきます。";
  const tail = getLeadTail({
    triggerKey,
    symptomFocus,
    intensity: signalContext?.intensity || "low",
  });
  return `${lead}${tail}`;
}
...
export function buildTomorrowFoodContext(riskContext) {
  ...
  const recommendation = buildRecommendation({ signalContext, triggerKey, symptomFocus });
