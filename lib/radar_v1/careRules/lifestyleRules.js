...
function withTodaySymptomFocus(plan, primaryKey, symptomFocus) {
  const focus = getTodayLifestyleFocus(primaryKey, symptomFocus);
  if (!focus) return plan;

  const steps = Array.isArray(plan.steps) ? plan.steps : [];
  const mergedSteps = Array.from(new Set([steps[0], focus.step, ...steps.slice(1)].filter(Boolean))).slice(0, 4);

  return {
    ...plan,
    lead: focus.lead ? `${plan.lead} ${focus.lead}` : plan.lead,
    steps: mergedSteps,
  };
}
...
function getTodayLifestylePlan(primaryKey, secondaryKey, signal, symptomFocus) {
  ...
  return withTodaySymptomFocus(plan, key, symptomFocus);
}
