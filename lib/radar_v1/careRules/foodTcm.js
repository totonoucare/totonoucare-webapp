...
export const FOOD_TCM_VERSION = "food_tcm_v69_2026-09-21";
...
const WORDS = {
 補気:'疲れが気になるとき、食事から元気を補う',健脾:'消化・吸収の働きを支える',
 ...
 理気:'巡りを整え、張りつめた感じをゆるめる',和胃:'食事のあとにお腹が重くなりやすいときに整える',養胃:'乾きが気になるとき、胃腸を養う',
...
export function tcmFoodDetail(food,{targetDate,slot='today'}={}) {
 ...
 const dampSupport=matched?.key==='健脾'&&food.context.labels.includes('fluid_damp')&&food.context.symptom!=='digestion';
 const why=matched?(dampSupport
  ? `${food.name}は、重だるさをためやすい体質に合わせた候補です。食養生では、消化・吸収を支えることも重だるさへのケアに取り入れます。`
  : `${food.name}を${reason}選びました。食養生では、${WORDS[matched.key]||'体を養う'}食材として取り入れます。`)
...
export function tcmEveningCaution(options={}) {
 ...
 if(c.triggers.includes('damp')||c.labels.includes('fluid_damp'))return {id:'night-heavy',items:['夜遅い揚げ物・クリームたっぷりの菓子'],reason:c.triggers.includes('damp')?'湿気への備えとして、油や甘さの重なる食べ方を控えめに。':'重だるさをためやすい体質に合わせ、油や甘さの重なる食べ方を控えめに。',alternative:'小腹が空いたら、油や甘さを足しすぎないものを少量。',basis:'湿・滋膩'};
 if(c.thermal==='warm'||focus==='digestion')return {id:'night-cold',items:['氷入りの飲み物・アイスを続けて摂ること'],reason:focus==='digestion'?'胃腸の不調が気になるときは、冷たいものを続けて摂ることを控えめに。':'冷えへの備えとして、冷たいものを続けて摂ることを控えめに。',alternative:'飲み物は常温〜温かめに。食べるなら少量ずつ。',basis:'寒涼・胃腸負担'};
