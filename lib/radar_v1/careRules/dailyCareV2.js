...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_19_2026-08-08_ux_polish";
...
const RESPONSE_MEAL_CATALOG = [
  ...
  mealCandidate("relax-mushroom-risotto", "鶏肉ときのこの和風リゾット＋青菜", "だしの香りとやわらかさを使い、食べきりやすい一皿にします。", ["yurumeru", "uruosu"], ["moist", "aroma", "digestion", "support"], "和洋", "chicken", "rice"),
  ...
  mealCandidate("relax-mild-mapotofu", "生姜とねぎを使った辛くない麻婆豆腐＋青菜＋ごはん", "温かい豆腐料理に生姜とねぎの香りを使い、花椒や唐辛子は強くしません。", ["yurumeru", "meguraseru"], ["aroma", "warm", "support"], "中華", "tofu", "rice"),
...
const TOMORROW_BREAKFAST_CATALOG = [
  ...
  mealCandidate("am-move-salmon", "鮭としそ・白ごまの小さな混ぜごはん", "しそとごまの香りで、朝の主食と魚を一緒に取ります。", ["meguraseru", "sasaeru"], ["aroma", "support", "quick"], "和食", "fish", "breakfast", { prep: "鮭としそを用意し、ごはんを一食分よけておく" }),
  ...
  mealCandidate("am-light-ume", "梅おにぎり＋生姜入り鶏スープ＋茶碗蒸し", "油を増やさず、朝も主食とたんぱく質を小さくそろえます。", ["nagasu", "sasaeru"], ["light", "warm", "support"], "和食", "egg", "breakfast", { effort: "quick", prep: "梅おにぎり・鶏スープ・茶碗蒸しを一か所へまとめる" }),
...
const MERIDIAN_LINE_CARE = {
  lung_li: {
    ...
    tomorrow_action: "寝る前に両手を鎖骨の下へ置き、息を長く吐きながら肩先へ3回なでる",
  ...
  heart_si: {
    ...
    tomorrow_action: "布団に入る前、片腕ずつ肩の後ろから小指側へゆっくりなで、腕の重さを床へ預ける",
  ...
  kidney_bl: {
    ...
    tomorrow_action: "今夜は腰へ手を当てて温め、横になる前にふくらはぎの後ろを足首へ向けてゆっくりなでる",
  ...
  liver_gb: {
    ...
    tomorrow_action: "寝る前に脇腹へ手を当て、息を吐くたびに体側から外ももへ手のひらを3回滑らせる",
  ...
  spleen_st: {
    ...
    tomorrow_action: "食後すぐを避け、お腹へ手を置いて3呼吸したあと、太もも前からすねを軽くなでる",
  ...
  pc_sj: {
    ...
    tomorrow_action: "入浴後か寝る前、手首から肩の外側へ片腕ずつゆっくりなで、最後に腕を下へ預ける",
...
function buildLifestyleForecastInsight({ theme, symptomFocus, primary }) {
  ...
  const reaction = theme?.reaction_direction === "accel"
    ...
    : theme?.reaction_direction === "brake"
      ? "ブレーキ寄りの反応"
      : "体質の反応";
  const symptom = SYMPTOM_CONTEXT_LABELS[symptomFocus] || "体調の揺らぎ";
  const actionLead = theme?.mode === "tomorrow" ? "今夜からの暮らしでは" : "暮らしでは";
  const action = !primary
    ? "無理に一手を足さず、食べるケアやほぐすケアを優先します。"
    : primary.care_kind === "environment"
      ? "道具や配置を整え、負担が一か所へ集まりにくくします。"
      : "身体の使い方を少し変え、負担が一か所へ集まりにくくします。";
  return `${timing}${weather}の予報に${reaction}が重なり、${symptom}が出やすい見込みです。${actionLead}、${action}`;
}
...
export function enhanceLifestylePlan({
  ...
  if (!primary) {
    ...
    return {
      ...
      recommendation: buildLifestyleForecastInsight({ theme, symptomFocus, primary: null }),
      selection_basis: {
        primary_kind: "none",
  ...
  const enhanced = {
    ...
    title: timingLabel,
    lead: theme?.summary || "身体の使い方か、環境調整から今の一手を選びます。",
    recommendation: buildLifestyleForecastInsight({ theme, symptomFocus, primary }),
    primary_action: {
      id: primary.id,
      label: primary.text,
...
// 料理名を正解として押しつけず、今日の食事ケアで中心になる食材を先に見せる。
...
// 食材の性質は、専門用語をそのまま出さず「どちらへ整える食材か」まで
...
const MEAL_FOOD_NATURE_RULES = [
  {
    ...
    policies: ["shizumeru", "uruosu"],
    text: (names) => `${names.join("・")}は、こもった熱を冷ましながら水分を補う方向の食材です。`,
  },
  {
    pattern: /洋梨|梨|りんご/,
    ingredients: ["洋梨", "りんご"],
    policies: ["uruosu", "shizumeru"],
    text: (names) => `${names.join("・")}は、口や喉の乾きをやわらげる方向の食材です。`,
  },
  {
    pattern: /豆腐/,
    ingredients: ["豆腐"],
    policies: ["shizumeru", "uruosu"],
    text: (names) => `${names.join("・")}は、熱をこもらせにくく、乾いた食事へ水分を足しやすい食材です。`,
  },
  {
  ...
  },
  {
    ...
    ingredients: ["大根", "海藻"],
    policies: ["nagasu", "sasaeru"],
    text: (names) => `${names.join("・")}は、食後の重さや水分の偏りをためにくくする方向の食材です。`,
  },
  {
    pattern: /きのこ/,
    ingredients: ["きのこ"],
    policies: ["nagasu", "sasaeru", "yurumeru"],
    text: (names) => `${names.join("・")}は、胃腸の働きを支え、余分な重さをためにくくする方向の食材です。`,
  },
  {
    pattern: /青菜|青梗菜|白菜|キャベツ/,
    ingredients: ["青菜", "白菜", "キャベツ"],
    policies: ["shizumeru", "nagasu", "sasaeru"],
    text: (names) => `${names.join("・")}は、熱や重さを増やさず、食事を軽く整える方向の食材です。`,
  },
  {
    pattern: /白身魚|鯛|たら|鮭|さば|かつお|まぐろ|しらす|海老|あさり/,
    ingredients: ["白身魚", "鯛", "たら", "鮭", "さば", "かつお", "まぐろ", "しらす", "海老", "あさり"],
    policies: ["shizumeru", "nagasu", "sasaeru"],
    text: (names) => `${names.join("・")}は、熱や油の重さを増やしにくい主菜として使います。`,
  },
  {
    pattern: /豚肉|豚しゃぶ|牛肉|鴨/,
    ingredients: ["豚肉", "牛肉", "鴨"],
    policies: ["sasaeru", "uruosu", "nukumeru"],
    text: (names) => `${names.join("・")}は、消耗した日の食事を支える方向に使います。`,
  },
  {
    pattern: /ごま|くるみ|アボカド/,
    ingredients: ["ごま", "くるみ", "アボカド"],
    policies: ["uruosu", "sasaeru"],
    text: (names) => `${names.join("・")}は、乾きをやわらげる方向に使います。量は少なめにします。`,
  },
  {
    pattern: /ヨーグルト|チーズ|リコッタ/,
    ingredients: ["ヨーグルト", "チーズ"],
    policies: ["uruosu", "shizumeru", "yurumeru"],
    text: (names) => `${names.join("・")}は、乾きをやわらげる方向に使います。湿気で重さが出やすい日は少量にします。`,
  },
  {
    ...
    policies: ["uruosu", "sasaeru"],
    text: (names) => names.length
      ? `${names.join("・")}と汁気を残す調理で、乾いた食事が続かないようにします。`
      : "汁気を残す調理で、乾いた食事が続かないようにします。",
  },
  {
  ...
  },
  {
    ...
    policies: ["sasaeru", "nukumeru"],
    text: (names) => names.length
      ? `${names.join("・")}は、消耗した日の食事を支える方向の食材です。`
      : "主食と主菜をそろえ、消耗した日の食事を支えます。",
  },
];
...
function buildMealFoodNatureReason(idea) {
  const label = String(idea?.label || "");
  ...
  const matching = MEAL_FOOD_NATURE_RULES
    ...
    .sort((a, b) => Number(b.matchedNames.length > 0) - Number(a.matchedNames.length > 0)
      ...
      || a.index - b.index);
  if (matching[0]) {
    return matching[0].text(matching[0].matchedNames);
  }
...
function foodTagScore(idea, { theme, symptomFocus, subLabels }) {
  ...
  if (trigger === "cold" && tags.has("warm")) score += 1.15;
  if (trigger === "cold" && !tags.has("warm")) score -= 0.85;
  if (trigger === "cold" && tags.has("cool")) score -= 2.4;
  if (trigger === "cold" && idea?.format === "snack" && !tags.has("warm")) score -= 0.8;
  if (trigger === "heat" && (tags.has("calm") || tags.has("cool"))) score += 0.55;
  if (trigger === "heat" && tags.has("warm") && !tags.has("calm")) score -= 0.45;
...
export function enhanceFoodContext({
  ...
  const todayCards = [
    {
      ...
      body: null,
      items: [primary.label],
      item_details: [buildMealItemDetail(primary)],
      primary: true,
      prominent: true,
    },
    drinkItems.length ? {
    ...
    buyChoice || eatOutChoice ? {
      ...
      item_details: [
        buyChoice ? buildMealItemDetail(buyChoice, { label: `コンビニ・スーパー｜${buyChoice.label}` }) : null,
        eatOutChoice ? buildMealItemDetail(eatOutChoice, { label: `外食｜${eatOutChoice.label}` }) : null,
      ].filter(Boolean),
    } : null,
    alternatives.length ? {
      key: "alternative",
      ...
      item_details: alternatives.map((item) => buildMealItemDetail(item)),
    } : null,
  ].filter(Boolean);
  ...
  const tomorrowCards = [
    ...
    nightChoice ? {
      ...
      item_details: [{
        ...buildMealItemDetail(nightChoice, { label: nightChoice.label.replace(/^小腹が空いたら、/, "") }),
