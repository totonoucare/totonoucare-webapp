...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_9_2026-07-31_human_lifestyle_care";
...
// アプリが把握していない部屋・持ち物・行動を、暮らすケア側で作り話にしない。
// 天気ストレスのある日に広く使えて、余裕が落ちた後にも実行しやすい段取りだけを残す。
const ENVIRONMENT_EXPERIMENT_CANDIDATES = [
  {
    id: "env-stop-line",
    care_kind: "environment",
    kind_label: "段取りをラクに",
    scene_family: "task_boundary",
    scene_label: "止め時",
    scene: "今日、家事や仕事を始める前に",
    headline: "先に「ここまで」を一つ決めておく",
    text: "始める前に「ここまでで終わり」を一つ決めておく。そこまで来たら、続きは明日へ回してOK",
    reason: "しんどくなってから、止め時を考えなくて済みます。",
    felt_sense: "「あと少し」を一回やめられたら成功。",
    reset: "区切りを作れない仕事は、終える時刻だけ先に決めよう。",
    ...
    policies: ["sasaeru", "shizumeru", "yurumeru", "nagasu", "meguraseru", "nukumeru", "uruosu"],
    item_role: "rest_support",
    effort: "low",
    score_adjustment: -1.2,
  },
  {
    id: "env-breathing-room",
    care_kind: "environment",
    kind_label: "段取りをラクに",
    scene_family: "schedule_buffer",
    scene_label: "予定のすき間",
    scene: "外せない予定がある日は",
    headline: "予定の前後どちらかを10分だけ空ける",
    text: "予定はそのままでOK。直前か直後の用事を一つ後ろへ送り、10分だけ空けておく",
    reason: "天気ストレスがある日に、用事の切り替えまで詰め込まずに済みます。",
    felt_sense: "終わった直後、次へ急がずに済んだらOK。",
    reset: "動かせる用事がない日は、終わったあと最初に座る場所だけ決めておこう。",
    symptoms: ["fatigue", "sleep", "digestion", "neck_shoulder", "low_back_pain", "swelling", "headache", "dizziness", "mood"],
    triggers: ["pressure_down", "pressure_up", "temp_shift", "damp", "heat", "cold", "dry"],
    policies: ["sasaeru", "shizumeru", "yurumeru", "nagasu", "meguraseru", "nukumeru", "uruosu"],
...
export function enhanceLifestylePlan({
  ...
  if (!primary) {
    return {
      ...
      trap: "",
      no_suggestion: true,
      no_suggestion_text: "今日は、暮らし方を変える提案はお休みです。食べる・ほぐすに、今の調子へ合う一手があります。",
      selection_basis: {
        primary_kind: "none",
