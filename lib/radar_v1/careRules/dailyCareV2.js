...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_32_2026-09-19_individual_scene_care";
...
function selectMeridianLineCare({ meridianCode, theme } = {}) {
  const options = safeArray(MERIDIAN_LINE_CARE[meridianCode]);
...
export function buildMeridianLineCare({ theme, riskContext = null } = {}) {
  ...
  const meridianCode = MERIDIAN_LINE_CARE[primary] ? primary : MERIDIAN_LINE_CARE[secondary] ? secondary : null;
  const selected = selectMeridianLineCare({ meridianCode, theme });
  if (!selected) return null;
  return {
    ...selected,
    meridian_code: meridianCode,
    intensity: theme?.stimulus || "やさしく短く",
    guidance_note: buildMeridianGuidanceNote(theme, selected),
    timing_label: theme?.mode === "tomorrow" ? "今夜〜明朝の一手" : "今日の一手",
    target_date: theme?.target_date || null,
...
export function enhanceTsuboSet({ baseTsuboSet = null, theme, riskContext = null } = {}) {
  ...
  return {
    ...
    lead: lineCare
      ? [lineCare.reason, lineCare.guidance_note].filter(Boolean).join(" ")
