...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_17_2026-08-07_food_nature_first_reasons";
...
const RESPONSE_MEAL_CATALOG = [
  ...
  mealCandidate("relax-mild-mapotofu", "辛くしない麻婆豆腐＋青菜＋ごはん", "花椒や唐辛子を強くせず、生姜とねぎの香りで食べやすくします。", ["yurumeru", "meguraseru"], ["aroma", "warm", "support"], "中華", "tofu", "rice"),
...
function buildMealFoodNatureReason(idea) {
  ...
  if (tags.has("moist")) return "食養生では、汁気を残す調理で乾いた食事が続かないようにします。";
  if (tags.has("warm")) return "食養生では、温かい料理でお腹を冷やさず、内側から温める方向へ整えます。";
  if (tags.has("aroma")) return "食養生では、だしや香味のある食材で気分を切り替え、張りつめた感じをゆるめる方向へ整えます。";
  if (tags.has("light")) return "食養生では、油を控えた調理で食後の重さをためにくくします。";
  return "食養生では、温度・汁気・量を整え、今日の崩れ方を増やしにくい一食として選びます。";
}

function buildMealReasons(idea) {
  const nutrition = MEAL_NUTRITION_REASONS[idea?.protein] || MEAL_NUTRITION_REASONS.other;
  return [
    { label: "食養生", text: buildMealFoodNatureReason(idea) },
...
function parseDrinkChoice(value) {
  ...
  return {
    label,
    reasons: [
      { label: "食養生", text: "天気と体調に合わせ、温める・冷ます・うるおす方向の偏りが小さい候補として選びます。" },
