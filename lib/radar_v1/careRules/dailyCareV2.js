...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_31_2026-09-18_clear_actions";
  ...
  pc_sj: [
    {
      ...
      line_id: "line-pc-sj",
      title: "前腕の外側をなでる",
      action: "反対の手で、手首の外側から肘へ向かってゆっくり3回なでます。左右を順番に行います。",
...
function lifestyleDisplayCopy(item, theme) {
  ...
  return {
    short_action: item.headline || ({
      "tool-arm-support": "ひじの下に折ったタオルを置き、腕の重さを預ける",
      "tool-screen-height": "顔を深く下げずに読める高さへ、画面を近づける",
      "tool-work-height": "よく使う物を、ひじを軽く曲げたまま届く位置へ寄せる",
      "tool-foot-support": "足裏全体を、床や安定した台に預ける",
      "tool-leg-rest": "横になり、膝下からふくらはぎを薄いクッションに預ける",
      "tool-carry-distribution": "片手に集めた荷物を、左右へ分けて持つ",
    })[item.id] || item.text,
    continuity_note: setupDone
...
function selectLifestyleCareCandidates({
  ...
  const pendingEnvironment = environmentPool.filter((item) =>
    item._score >= (environmentPool[0]?._score || 0) - LIFESTYLE_CANDIDATE_NEAR_TIE_DELTA &&
    (!PERSISTENT_SETUP_IDS.has(item.id) || !completedRuleIds(theme).has(item.id)) &&
    !completedRuleIds(theme, 2).has(item.id));
  const primaryPool = theme?.reserve_small && pendingEnvironment.length
    ? pendingEnvironment
    : bodyFirst && bodyPool.length > 1
      ? bodyPool
      : relevantPool;
  const orderedPrimary = rotateNearTies(preferUnrepeatedLifestyle(primaryPool, theme), {
    targetDate,
  ...
  const alternatives = scored
    ...
    .sort((a, b) => {
      ...
      const sameSetting = (item) => item.scene_family === primary.scene_family || safeArray(item.care_needs).some((key) => usedNeeds.has(key));
      if (sameSetting(a) !== sameSetting(b)) return Number(sameSetting(b)) - Number(sameSetting(a));
  ...
  const alternativePeers = topAlternative ? alternatives.filter((item) =>
    item.care_kind === topAlternative.care_kind &&
    safeArray(item.care_needs).some((key) => !usedNeeds.has(key)) === safeArray(topAlternative.care_needs).some((key) => !usedNeeds.has(key)) &&
    item._score >= topAlternative._score - LIFESTYLE_CANDIDATE_NEAR_TIE_DELTA) : [];
  const rotatedAlternatives = rotateNearTies(preferUnrepeatedLifestyle(alternativePeers, theme), {
    targetDate, contextKey: `${contextKey}|alternative`,
  });
  for (const item of [...rotatedAlternatives, ...alternatives]) {
