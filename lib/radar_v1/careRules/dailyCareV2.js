...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_23_2026-08-10_persona_ux_polish";
...
const ENVIRONMENT_ADJUSTMENT_CANDIDATES = [
  ...
  },
  {
    ...
    symptom_copy: {
      sleep: {
        scene: "眠る前に部屋の光が強いなら",
        text: "天井灯を消し、手元灯か間接照明だけにする。画面は明るさを一段下げ、見終えたら伏せる",
        reason: "眠る前に目へ入る光を減らし、休む側へ切り替える時間を作れます。",
        felt_sense: "目の奥や眉間へ力を入れずに過ごせれば、その明るさでOK。",
        reset: "暗すぎて落ち着かないなら、手元灯だけ少し明るくしよう。",
      },
    },
    triggers: ["pressure_down", "pressure_up", "heat", "dry", "cold", "damp", "temp_shift"],
  ...
  },
  {
    ...
    symptom_copy: {
      digestion: {
        scene: "胃腸が重い日に、座って過ごすなら",
        text: "椅子の奥へ座って背もたれへ背中を預ける。お腹が折れ曲がらない範囲で、腰の隙間へ薄いタオルを入れる",
        reason: "背もたれへ重さを分けると、お腹を折りたたんだ前かがみが続きにくくなります。",
        felt_sense: "お腹を圧迫せず、背中を預けて呼吸できればその厚さでOK。",
        reset: "腰が押されるなら、タオルを一段薄くしよう。",
      },
    },
    symptom_affinity: { low_back_pain: 1, fatigue: 0.9, digestion: 0.7 },
...
const RESPONSE_MEAL_CATALOG = [
  ...
  mealCandidate("calm-pork-oroshi", "豚しゃぶと焼きなすのみぞれだれ＋麦ごはん", "油を控えた豚肉に大根おろしを合わせ、重さを残しにくくします。", ["shizumeru", "nagasu"], ["calm", "cool", "light", "aroma", "support"], "和食", "pork", "set", { triggers: ["heat", "damp"], avoid_triggers: ["cold"] }),
  ...
  mealCandidate("relax-basil-pasta", "鶏肉とズッキーニのバジルトマトパスタ", "辛味を足さず、バジルとトマトの香りで気分を切り替えます。", ["yurumeru", "meguraseru"], ["aroma", "calm", "cool", "support"], "イタリアン", "chicken", "noodle", { triggers: ["heat", "damp"], avoid_triggers: ["cold"] }),
  ...
  mealCandidate("light-ume-pork", "豚しゃぶと焼きなすのみょうが梅だれ＋麦ごはん少なめ", "油を控え、梅とみょうがで食後の重さを残しにくくします。", ["nagasu", "sasaeru"], ["light", "aroma", "cool", "support"], "和食", "pork", "set", { symptoms: ["digestion", "swelling"], sub_labels: ["fluid_damp"], triggers: ["heat", "damp"], avoid_triggers: ["cold"] }),
...
const TOMORROW_BREAKFAST_CATALOG = [
  ...
  mealCandidate("am-move-salmon", "温かい鮭としそ・白ごまの小さな混ぜごはん＋根菜のみそ汁", "しそとごまの香りで、朝の主食と魚を一緒に取ります。", ["meguraseru", "sasaeru"], ["aroma", "warm", "moist", "support", "quick"], "和食", "fish", "breakfast", { prep: "鮭としそ、ごはん、根菜のみそ汁を一食分まとめる" }),
  ...
  mealCandidate("am-light-ume", "梅おにぎり＋鶏と冬瓜の薄味スープ＋茶碗蒸し", "油を増やさず、朝も主食とたんぱく質を小さくそろえます。", ["nagasu", "sasaeru"], ["light", "moist", "support", "digestion"], "和食", "egg", "breakfast", { effort: "quick", prep: "梅おにぎり・鶏と冬瓜のスープ・茶碗蒸しを一か所へまとめる", triggers: ["heat", "damp"], avoid_triggers: ["cold"] }),
  ...
  mealCandidate("am-moist-tomato", "温かい海老とトマトの小さなリゾット", "トマトの汁気と海老を使い、一皿で朝食をまとめます。", ["uruosu", "meguraseru"], ["moist", "warm", "aroma", "support"], "イタリアン", "seafood", "breakfast", { prep: "冷凍海老とトマト缶、ごはん半量を用意する" }),
...
const MERIDIAN_LINE_CARE = {
  lung_li: {
    ...
    tomorrow_action: "寝る前に、鎖骨の下を内側から肩先へゆっくり3往復する",
...
const LIFESTYLE_FORECAST_BY_SYMPTOM = {
  fatigue: "同じ姿勢や荷重が続く時に、回復へ回す余力を使いやすい",
  sleep: "夜まで光・音・身体の支え不足が重なると、眠る側へ切り替わりにくい",
  digestion: "前かがみや座り姿勢が続く時に、お腹まわりの重さを残しやすい",
  neck_shoulder: "画面や手作業が続く時に、首肩へ腕と頭の重さが集まりやすい",
  low_back_pain: "座りっぱなしや立ち上がりの時に、腰のこわばりを残しやすい",
  swelling: "座面や足元の支えが少ない時に、脚の重さを残しやすい",
  headache: "光や画面、腕を固定する作業が続く時に、首から頭へ力が集まりやすい",
  dizziness: "頭を何度も動かす場面で、ふらつきを増やしやすい",
  mood: "光・音・画面の刺激が重なる時に、頭と身体の緊張を残しやすい",
};

function buildLifestyleForecastInsight({ theme, symptomFocus, primary }) {
  const timing = theme?.mode === "tomorrow" ? "明日は" : "今日は";
  const careTiming = theme?.mode === "tomorrow" ? "今夜は" : "今日は";
  ...
  const forecast = LIFESTYLE_FORECAST_BY_SYMPTOM[symptomFocus]
    || `${SYMPTOM_CONTEXT_LABELS[symptomFocus] || "体調の揺らぎ"}が出やすい`;
  const reason = String(primary?.reason || "負担を一か所へ集めないように整えます。").replace(/。$/, "");
  return `${timing}${weather}の予報に${reaction}が重なり、${forecast}見込みです。${careTiming}${reason}。`;
}
...
function mealReasonContextScore(kind, { theme = null, symptomFocus = null } = {}) {
  const primary = normalizeDailyCareTrigger(theme?.trigger_key);
  const secondary = normalizeDailyCareTrigger(theme?.secondary_trigger_key);
  const reaction = theme?.reaction_direction || "";
  const weights = {
    heat: { cooling: 8, aroma: 3, light_veg: 2, light_protein: 1.5, moistening: 1.5 },
    damp: { drain: 7, spleen: 6, light_veg: 3, light_protein: 2, aroma: 1.5 },
    cold: { warming: 9, moist_cooking: 3, support: 2.5, support_protein: 1.5 },
    dry: { moistening: 8, moist_cooking: 7, moist_fat: 4, dairy_moist: 2 },
    pressure_down: { spleen: 4, drain: 3.5, support: 2.5 },
    pressure_up: { aroma: 4, cooling: 2.5, light_protein: 1.5 },
    temp_shift: { warming: 3, moist_cooking: 2, support: 1.5 },
  };
  let score = Number(weights[primary]?.[kind] || 0) + Number(weights[secondary]?.[kind] || 0) * 0.45;
  const symptomWeights = {
    digestion: { spleen: 7, drain: 5, light_veg: 3, light_protein: 2.5, warming: 2 },
    swelling: { drain: 7, spleen: 4, light_veg: 2.5 },
    fatigue: { support: 5, support_protein: 4, warming: 2 },
    dizziness: { support: 4, support_protein: 3, moist_cooking: 2 },
    sleep: { aroma: 3, moist_cooking: 2, support: 1.5 },
    headache: { aroma: 4, cooling: 3, light_protein: 2 },
    neck_shoulder: { aroma: 4, moist_cooking: 1.5 },
    mood: { aroma: 5, cooling: 1.5 },
    low_back_pain: { warming: 5, support: 2.5, moist_cooking: 1.5 },
  };
  score += Number(symptomWeights[symptomFocus]?.[kind] || 0);
  if (reaction === "accel" && ["aroma", "cooling"].includes(kind)) score += 1.5;
  if (reaction === "brake" && ["support", "spleen", "warming"].includes(kind)) score += 1.5;
  return score;
}
...
function buildMealFoodNatureReason(idea, context = {}) {
  ...
  if (matching[0]) {
    ...
    const triggers = new Set([
      ...
      normalizeDailyCareTrigger(context?.theme?.secondary_trigger_key),
    ]);
    const aroma = matching.find((rule) => rule.kind === "aroma" && rule.matchedNames.length);
    if (matching[0].kind === "cooling" && aroma && triggers.has("heat") && triggers.has("damp")) {
      return `${primaryText}${aroma.matchedNames.join("・")}の香りや酸味を添え、蒸し暑い日にも食べやすくします。`;
    }
    return primaryText;
  }
...
function buildMealItemDetail(idea, { label = null, reasonContext = null } = {}) {
  return {
...
function buildFoodForecastInsight({ theme, symptomFocus, primaryPolicy }) {
  ...
  const symptomForecast = symptomFocus === "digestion"
    ? "胃腸の調子が崩れやすい"
    : `${symptom}が出やすい`;
  ...
  return `${timing}${weather}の予報に${reaction}が重なり、${symptomForecast}見込みです。${actionLead}${action}`;
}
...
export function enhanceFoodContext({
  ...
  const mealReasonContext = { theme, symptomFocus };
...
function buildMeridianWeatherFinish(theme) {
  const trigger = normalizeDailyCareTrigger(theme?.trigger_key);
  if (trigger === "cold") return "最後に手のひらを当て、冷えない程度に10秒休む";
  if (trigger === "heat") return "強くこすらず、息を長く吐いて終える";
  if (["damp", "pressure_down"].includes(trigger)) return "左右を一度ずつ行い、足裏を床へ置いて終える";
  if (trigger === "dry") return "最後に手を止め、そのまま呼吸を2回待つ";
  if (["pressure_up", "temp_shift"].includes(trigger)) return "呼吸を止めず、痛くない範囲で終える";
  return "痛くない範囲で一度行えば十分";
}
...
export function buildMeridianLineCare({ theme, riskContext = null } = {}) {
  ...
  const baseAction = theme?.mode === "tomorrow" && selected.tomorrow_action
    ...
    : selected.action;
  const action = `${baseAction}。${buildMeridianWeatherFinish(theme)}`;
  return {
    ...selected,
