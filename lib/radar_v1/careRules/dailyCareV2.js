...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_22_2026-08-09_ux_care_polish";
...
const RESPONSE_MEAL_CATALOG = [
  ...
  mealCandidate("calm-pork-oroshi", "豚しゃぶと焼きなすのみぞれだれ＋麦ごはん", "油を控えた豚肉に大根おろしを合わせ、重さを残しにくくします。", ["shizumeru", "nagasu"], ["calm", "light", "aroma", "support"], "和食", "pork", "set"),
  mealCandidate("calm-chicken-citrus", "蒸し鶏とトマト・きゅうりの柑橘だれ＋ごはん少なめ", "熱い物や辛い物へ寄せず、さっぱりした主菜で一食を整えます。", ["shizumeru", "sasaeru"], ["calm", "cool", "light", "support"], "和食", "chicken", "plate"),
  ...
  mealCandidate("relax-mushroom-risotto", "温かい鶏肉ときのこの和風リゾット＋青菜", "だしの香りとやわらかさを使い、食べきりやすい一皿にします。", ["yurumeru", "uruosu"], ["moist", "warm", "aroma", "digestion", "support"], "和洋", "chicken", "rice"),
  mealCandidate("relax-basil-pasta", "鶏肉とズッキーニのバジルトマトパスタ", "辛味を足さず、バジルとトマトの香りで気分を切り替えます。", ["yurumeru", "meguraseru"], ["aroma", "calm", "support"], "イタリアン", "chicken", "noodle"),
  ...
  mealCandidate("light-ume-pork", "豚しゃぶと焼きなすのみょうが梅だれ＋麦ごはん少なめ", "油を控え、梅とみょうがで食後の重さを残しにくくします。", ["nagasu", "sasaeru"], ["light", "aroma", "support"], "和食", "pork", "set", { symptoms: ["digestion", "swelling"], sub_labels: ["fluid_damp"] }),
...
const BUY_MEAL_CATALOG = [
  mealCandidate("buy-calm-salmon", "鮭おにぎり＋蒸し鶏と海藻のサラダ＋麦茶", "濃い味やカフェインへ寄せず、主食とたんぱく質を小さくそろえます。", ["shizumeru", "sasaeru"], ["calm", "support", "cool"], "和食", "fish", "buy", { effort: "buy" }),
  ...
  mealCandidate("buy-calm-soba", "とろろそば＋ほうれん草のごまあえ＋ルイボスティー", "濃い味を重ねず、麺だけで終えない組み合わせです。", ["shizumeru", "yurumeru"], ["calm", "moist", "aroma"], "和食", "other", "buy", { effort: "buy" }),
  ...
  mealCandidate("buy-relax-rice", "鶏五目おにぎり＋茶碗蒸し＋ほうじ茶", "おにぎり・茶碗蒸し・飲み物を、一口ずつ取りやすい組み合わせにします。", ["yurumeru", "sasaeru"], ["calm", "support", "warm"], "和食", "chicken", "buy", { effort: "buy" }),
  ...
  mealCandidate("buy-light-salmon", "焼き鮭おにぎり＋きのこと鶏肉の和風スープ＋ほうじ茶", "主食を小さくし、油の少ない温かい具を合わせます。", ["nagasu", "sasaeru"], ["light", "warm", "support"], "和食", "fish", "buy", { effort: "buy" }),
...
const TOMORROW_BREAKFAST_CATALOG = [
  ...
  mealCandidate("am-relax-shiso", "鶏そぼろとしその小さな混ぜごはん＋ほうじ茶", "しその香りを足し、甘いパンだけで終えません。", ["yurumeru", "sasaeru"], ["aroma", "support", "quick"], "和食", "chicken", "breakfast", { prep: "鶏そぼろか市販の鶏そぼろおにぎりを用意しておく" }),
  ...
  mealCandidate("am-move-salmon", "温かい鮭としそ・白ごまの小さな混ぜごはん＋根菜のみそ汁", "温かいごはんと汁物に、鮭・しそ・ごまを合わせます。", ["meguraseru", "sasaeru"], ["aroma", "support", "quick", "warm", "moist"], "和食", "fish", "breakfast", { prep: "鮭としそ、ごはん、根菜のみそ汁を一食分まとめる" }),
  ...
  mealCandidate("am-light-ume", "梅おにぎり＋鶏と冬瓜の薄味スープ＋茶碗蒸し", "梅の酸味と冬瓜の汁気を使い、油を増やさず朝食を小さくそろえます。", ["nagasu", "sasaeru"], ["light", "moist", "support", "digestion"], "和食", "egg", "breakfast", { effort: "quick", prep: "梅おにぎり・鶏と冬瓜のスープ・茶碗蒸しを一か所へまとめる", triggers: ["heat", "damp"] }),
  ...
  mealCandidate("am-moist-chowder", "温かい鮭とじゃがいもの豆乳チャウダー＋小さめのパン", "魚と温かい汁気を合わせ、乾いたパンだけにしません。", ["uruosu", "sasaeru"], ["moist", "warm", "support"], "洋食", "fish", "breakfast", { prep: "チャウダーを朝の一食分よけておく" }),
  ...
  mealCandidate("am-moist-tomato", "温かい海老とトマトの小さなリゾット", "温かいトマトの汁気と海老を使い、一皿で朝食をまとめます。", ["uruosu", "meguraseru"], ["moist", "warm", "aroma", "support"], "イタリアン", "seafood", "breakfast", { prep: "冷凍海老とトマト缶、ごはん半量を用意する" }),
...
const MERIDIAN_LINE_CARE = {
  lung_li: {
    ...
    tomorrow_action: "寝る前に、鎖骨の下を内側から肩先へゆっくり3往復し、最後に長く息を吐く",
  ...
  heart_si: {
    ...
    tomorrow_action: "画面を置いたあと、肩甲骨の外側から腕の小指側を痛くない範囲でゆっくりなでる",
  ...
  kidney_bl: {
    ...
    tomorrow_action: "入浴後か寝る前に腰へ手を当て、ふくらはぎの後ろを手のひらでゆっくりなでる",
  ...
  liver_gb: {
    ...
    tomorrow_action: "布団へ入る前に脇腹へ手を当て、息を吐きながら外ももまでゆっくりなでる",
  ...
  spleen_st: {
    ...
    tomorrow_action: "寝る前にお腹へ手を置いて呼吸し、太もも前からすねを手のひらで軽くなでる",
  ...
  pc_sj: {
    ...
    tomorrow_action: "画面や作業を終えたあと、手首から肩の外側へ反対の手でゆっくり3往復なでる",
...
function buildLifestyleForecastInsight({ theme, symptomFocus, primary }) {
  ...
  const reaction = theme?.reaction_direction === "accel"
    ...
    : theme?.reaction_direction === "brake"
      ? "ブレーキ寄りの反応"
      : "体質の反応";
  const symptom = SYMPTOM_CONTEXT_LABELS[symptomFocus] || "体調";
  const symptomOutlook = symptomFocus === "digestion"
    ? "胃腸の調子が崩れやすい"
    : `${symptom}が出やすい`;
  const careDirection = primary?.care_kind === "environment"
    ? "環境調整では、負担が集まりやすい場所を道具や配置へ預けます。"
    : "身体の使い方では、負担を一か所へ集めず、身体全体へ分けます。";
  return `${timing}${weather}の予報に${reaction}が重なり、${symptomOutlook}見込みです。${careDirection}`;
}
...
export function enhanceLifestylePlan({
  ...
  const enhanced = {
    ...
    title: timingLabel,
    lead: buildLifestyleForecastInsight({ theme, symptomFocus, primary }),
    forecast_insight: buildLifestyleForecastInsight({ theme, symptomFocus, primary }),
    primary_action: {
      id: primary.id,
      label: primary.text,
...
const MEAL_FOOD_NATURE_RULES = [
  {
    ...
    policies: ["shizumeru", "uruosu"],
    text: (names) => `食養生では、${names.join("・")}はこもった熱を冷ましながら、水分を補う方向の食材です。`,
  },
  {
    kind: "moisten",
    pattern: /洋梨|梨|りんご/,
    ingredients: ["洋梨", "りんご"],
    policies: ["uruosu", "shizumeru"],
    text: (names) => `食養生では、${names.join("・")}は口や喉の乾きをやわらげる方向の食材です。`,
  },
  {
    kind: "cool_hydrate",
    pattern: /豆腐/,
    ingredients: ["豆腐"],
    policies: ["shizumeru", "uruosu"],
    text: (names) => `食養生では、${names.join("・")}は熱をこもらせにくく、乾いた食事へ水分を足しやすい食材です。`,
  },
  {
    kind: "aroma",
    ...
    text: (names) => names.length
      ? `食養生では、${names.join("・")}の香りで気分を切り替え、張りつめた感じをゆるめる方向へ整えます。`
      : "食養生では、香味野菜や香草の香りで気分を切り替え、張りつめた感じをゆるめる方向へ整えます。",
  },
  {
    kind: "drain",
    ...
    ingredients: ["大根", "海藻"],
    policies: ["nagasu", "sasaeru"],
    text: (names) => `食養生では、${names.join("・")}は食後の重さや水分の偏りをためにくくする方向の食材です。`,
  },
  {
    kind: "drain",
    pattern: /きのこ/,
    ...
    text: (names) => `食養生では、${names.join("・")}は胃腸の働きを支え、余分な重さをためにくくする方向の食材です。`,
  },
  {
    kind: "light",
    ...
    policies: ["shizumeru", "nagasu", "sasaeru"],
    text: (names) => `食養生では、${names.join("・")}は熱や重さを増やさず、食事を軽く整える方向の食材です。`,
  },
  {
    kind: "light_protein",
    ...
    policies: ["shizumeru", "nagasu", "sasaeru"],
    text: (names) => `食養生では、${names.join("・")}を熱や油の重さを増やしにくい主菜として使います。`,
  },
  {
    kind: "support",
    ...
    text: (names) => `食養生では、${names.join("・")}を消耗した日の食事を支える方向に使います。`,
  },
  {
    kind: "moisten",
    pattern: /ごま|くるみ|アボカド/,
    ...
    policies: ["uruosu", "sasaeru"],
    text: (names) => `食養生では、${names.join("・")}を乾きをやわらげる方向に使います。量は少なめにします。`,
  },
  {
    kind: "moisten",
    ...
    text: (names) => `食養生では、${names.join("・")}を乾きをやわらげる方向に使います。湿気で重さが出やすい日は少量にします。`,
  },
  {
    kind: "moist_cooking",
    ...
    policies: ["uruosu", "sasaeru"],
    text: (names) => names.length
      ? `食養生では、${names.join("・")}と汁気を残す調理で乾いた食事が続かないようにします。`
      : "食養生では、汁気を残す調理で乾いた食事が続かないようにします。",
  },
  {
    kind: "warm",
    ...
    text: (names) => names.length
      ? `食養生では、${names.join("・")}でお腹を冷やさず、内側から温める方向へ整えます。`
      : "食養生では、温かい料理でお腹を冷やさず、内側から温める方向へ整えます。",
  },
  {
    kind: "support",
    ...
    policies: ["sasaeru", "nukumeru"],
    text: (names) => names.length
      ? `食養生では、${names.join("・")}は消耗した日の食事を支える方向の食材です。`
      : "食養生では、主食と主菜をそろえ、消耗した日の食事を支える方向に使います。",
  },
];

function mealReasonContextScore(kind, { theme = null, symptomFocus = null, subLabels = [] } = {}) {
  ...
  const secondary = theme?.secondary_trigger_key
    ...
    : null;
  const triggers = new Set([primary, secondary].filter(Boolean));
  const labels = new Set(safeArray(subLabels).map((key) => key === "dampness" ? "fluid_damp" : key));
  const reaction = theme?.reaction_direction || "balanced";
  let score = 0;

  if (triggers.has("heat")) {
    if (kind === "cool_hydrate") score += 7;
    if (["light", "light_protein", "moist_cooking"].includes(kind)) score += 2;
    if (kind === "warm") score -= 7;
  }
  if (triggers.has("damp")) {
    if (kind === "drain") score += 6;
    if (["light", "light_protein", "aroma"].includes(kind)) score += 2.5;
    if (kind === "moisten") score -= 1;
  }
  if (triggers.has("dry")) {
    if (["moisten", "moist_cooking"].includes(kind)) score += 6;
  }
  if (triggers.has("cold")) {
    if (kind === "warm") score += 7;
    if (kind === "moist_cooking") score += 2;
    if (kind === "cool_hydrate") score -= 7;
  }
  if (triggers.has("pressure_down")) {
    if (kind === "drain") score += 4;
    if (["light", "light_protein", "support"].includes(kind)) score += 1.5;
  }
  if (triggers.has("pressure_up") || reaction === "accel") {
    if (kind === "aroma") score += 4;
    if (kind === "cool_hydrate") score += 1.5;
  }
  if (reaction === "brake") {
    if (["drain", "light", "support"].includes(kind)) score += 1.5;
  }

  if (symptomFocus === "digestion") {
    if (kind === "drain") score += 4;
    if (["light", "moist_cooking", "aroma"].includes(kind)) score += 2;
  }
  if (symptomFocus === "swelling" && kind === "drain") score += 4;
  if (["neck_shoulder", "headache", "mood", "sleep"].includes(symptomFocus) && kind === "aroma") score += 2.5;
  if (["fatigue", "dizziness"].includes(symptomFocus) && kind === "support") score += 3;
  if (symptomFocus === "low_back_pain" && triggers.has("cold") && kind === "warm") score += 3;

  if (labels.has("fluid_damp") && kind === "drain") score += 3;
  if (labels.has("fluid_deficiency") && ["moisten", "moist_cooking", "cool_hydrate"].includes(kind)) score += 3;
  if (labels.has("qi_stagnation") && kind === "aroma") score += 2.5;
  if (labels.has("qi_deficiency") && kind === "support") score += 2.5;
  return score;
}

function cleanMealReasonText(value) {
  return String(value || "").replace(/^食養生では、/, "");
}
...
function buildMealFoodNatureReason(idea, context = {}) {
  ...
  const matching = MEAL_FOOD_NATURE_RULES
    .map((rule, index) => ({
      ...
      contextScore: mealReasonContextScore(rule.kind, context),
    ...
    .sort((a, b) => Number(b.matchedNames.length > 0) - Number(a.matchedNames.length > 0)
      || b.contextScore - a.contextScore
      || b.policyScore - a.policyScore
      || a.index - b.index);
  if (matching[0]) {
    const primaryText = cleanMealReasonText(matching[0].text(matching[0].matchedNames));
    const triggers = new Set([
      normalizeDailyCareTrigger(context?.theme?.trigger_key),
      context?.theme?.secondary_trigger_key ? normalizeDailyCareTrigger(context.theme.secondary_trigger_key) : null,
    ].filter(Boolean));
    if (triggers.has("heat") && triggers.has("damp") && matching[0].kind === "cool_hydrate") {
      const aroma = matching.find((rule) => rule.kind === "aroma" && rule.matchedNames.length > 0);
      if (aroma) {
        const aromaNames = aroma.matchedNames.join("・");
        return `${primaryText}${aromaNames}の香りや酸味を添え、蒸し暑い日にも食べやすくします。`;
      }
    }
    return primaryText;
  }
...
function buildFoodForecastInsight({ theme, symptomFocus, primaryPolicy }) {
  ...
  return `${timing}${weather}の予報に${reaction}が重なり、${symptomOutlook}見込みです。${actionLead}${action}`;
}
...
export function enhanceFoodContext({
  ...
  const drinkChoices = safeArray(existingDrinkCard?.items).slice(0, 2).map((item, index) => {
    ...
    return detail?.label && safeArray(detail?.reasons).length ? detail : parseDrinkChoice(item);
  });
  ...
  const mealReasonContext = { theme, symptomFocus, subLabels, mode: normalizedMode };
...
export function buildMeridianLineCare({ theme, riskContext = null } = {}) {
  ...
  const action = theme?.mode === "tomorrow" && selected.tomorrow_action
...
export function enhanceTsuboSet({ baseTsuboSet = null, theme, riskContext = null } = {}) {
  ...
  return {
    ...
    lead: lineCare
      ? `${lineCare.reason} ${theme?.mode === "tomorrow" ? "今夜は" : "今日は"}${lineCare.intensity}で十分です。`
