...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_33_2026-09-19_daily_line_care";
...
export function enhanceFoodContext({
  ...
  const enhanced = {
    ...
    title: primary.label,
    practical_tip: ({
      shizumeru: "いつもの食事で、辛味や濃い味を控えめに。だしや穏やかな酸味を使えます。",
      yurumeru: "だしや香りを添え、落ち着いて食べやすい一食に。汁物を添えるだけでも取り入れられます。",
      meguraseru: "しそやねぎなど、身近な香味野菜をいつもの一品に少し添えてみましょう。",
      nukumeru: "温かい汁物や煮物を一品。手元の食材で、温かく食べられる形を選びましょう。",
      uruosu: "スープや煮物など、汁気のある一品を取り入れてみましょう。",
      nagasu: "蒸す・煮る料理を選び、油や濃い味を重ねすぎない一食にしましょう。",
      sasaeru: "ごはんや麺に、卵・豆腐・魚などを一品添えましょう。食べられる量に合わせて大丈夫です。",
    })[theme?.policies?.[0]?.key] || "手元にある食材で取り入れられる一品を選びましょう。",
    ...
    how_to: primary.note,
    reason: `${SYMPTOM_CONTEXT_LABELS[symptomFocus] || "普段気になる不調"}と体質に合わせ、${safeArray(theme?.policies).map((item) => item.guide).filter(Boolean).join("・") || "食事の負担を軽くする"}ための食べ方を選びました。`,
