...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_34_2026-09-20_food_rebuilt";
...
export function enhanceFoodContext({baseFood=null,theme={},targetDate=null,symptomFocus=null,subLabels=[],mode="today"}={}) {
  ...
  const rawDrink=safeArray(food.action_cards).find(c=>c.key==="drink");
  const drinkRows=safeArray(rawDrink?.items).slice(0,2).map((item,index)=>{
    const detail=safeArray(rawDrink?.item_details)[index];
    if(detail?.record_semantics==="drink_consumed") return detail;
    return detail?.label&&safeArray(detail.reasons).length?normalizeDrinkChoiceCopy(detail):parseDrinkChoice(item);
  });
  const drinkCard=drinkRows.length?{...rawDrink,label:normalizedMode==="tomorrow"?"明日の朝に合わせる飲み物":"今日、食事と合わせる飲み物",items:drinkRows.map(r=>r.label),item_details:drinkRows}:null;
