...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_16_2026-08-07_everyday_food_visible_reasons";
...
function buildMealReasons(idea) {
  ...
  let therapy = "食べきれる量に調整しやすい料理です。";
  if (tags.has("aroma")) therapy = "食養生では、だし・薬味・香味野菜の香りを、気分と食欲を切り替える「理気」の方向に使います。";
  else if (tags.has("moist")) therapy = "汁物・蒸し煮・あんかけなど、食事からも水分を取りやすい調理です。";
  else if (tags.has("warm")) therapy = "温かい料理にして、冷たい食事が続かないようにします。";
  else if (tags.has("light")) therapy = "油と量を増やしすぎず、食後の重さを残しにくい組み合わせです。";
  else if (tags.has("calm")) therapy = "辛味や濃い味を強くせず、刺激を重ねにくい組み合わせです。";
  return [
    { label: "栄養面", text: nutrition },
    { label: "食養生", text: therapy },
  ];
}
...
function parseDrinkChoice(value) {
  ...
  return {
    label,
    reasons: [{ label: "選んだ理由", text: reason }],
  };
}
...
export function enhanceFoodContext({
  ...
  const drinkChoices = safeArray(existingDrinkCard?.items).slice(0, 2).map(parseDrinkChoice);
  ...
  const todayCards = [
    {
      ...
      body: null,
      items: [primary.label],
      item_details: [{ label: primary.label, reasons: buildMealReasons(primary) }],
      primary: true,
      prominent: true,
    },
    drinkItems.length ? {
    ...
    buyChoice || eatOutChoice ? {
      ...
      item_details: [
        buyChoice ? { label: `コンビニ・スーパー｜${buyChoice.label}`, reasons: buildMealReasons(buyChoice) } : null,
        eatOutChoice ? { label: `外食｜${eatOutChoice.label}`, reasons: buildMealReasons(eatOutChoice) } : null,
      ].filter(Boolean),
    } : null,
    alternatives.length ? {
      key: "alternative",
      ...
      item_details: alternatives.map((item) => ({ label: item.label, reasons: buildMealReasons(item) })),
    } : null,
  ].filter(Boolean);
  ...
  const tomorrowCards = [
    ...
    nightChoice ? {
      ...
      item_details: [{
        label: nightChoice.label.replace(/^小腹が空いたら、/, ""),
        reasons: buildMealReasons(nightChoice),
      }],
      prominent: false,
    } : null,
    commonCautionCard,
