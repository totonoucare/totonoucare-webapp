...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_10_2026-08-02_broad_scene_timing_care";
...
const PUBLIC_ACTION_COPY_BY_ID = {
  ...
  "tension-phone-thumb-line": {
    ...
    text: "片手で持ち続けず、下側を反対の手でも支える。端へ指を伸ばす時は、届く方の手へ持ち替える",
...
export function enhanceLifestylePlan({
  ...
  const timingLabel = theme?.mode === "tomorrow"
    ? (primary?.care_kind === "environment" ? "今夜の準備" : "明日の一手")
    : "今日の一手";
  ...
  if (!primary) {
    ...
    return {
      ...plan,
      version: DAILY_CARE_LOGIC_VERSION,
      timing_label: theme?.mode === "tomorrow" ? "明日の一手" : "今日の一手",
      title: theme?.mode === "tomorrow" ? "明日の暮らしの一手" : "今日の暮らしの一手",
