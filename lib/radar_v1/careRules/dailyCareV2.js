...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_7_2026-07-30_lifestyle_care_lanes";
...
function bestLifestyleCandidateScore(candidates, { theme, symptomFocus }) {
  const scores = safeArray(candidates)
    .filter((item) => item?.id && item?.text)
    .filter((item) => !symptomFocus || !safeArray(item?.excluded_symptoms).includes(symptomFocus))
    .map((item) => candidateScore(item, { theme, symptomFocus }));
...
function selectLifestyleCareCandidates({
  ...
  const laneScores = {
    body: bestLifestyleCandidateScore(BODY_MECHANICS_LIFESTYLE_CANDIDATES, { theme, symptomFocus }),
    environment: bestLifestyleCandidateScore(contextualEnvironment, { theme, symptomFocus }),
  };
  ...
  // 差が小さい日は日付で主役を交代し、どちらか一方だけのアプリにしない。
