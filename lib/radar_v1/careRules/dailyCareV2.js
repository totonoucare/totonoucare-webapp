...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_15_2026-08-06_response_meals_macro_shop";
...
const RESPONSE_MEAL_CATALOG = [
  ...
  mealCandidate("calm-lemon-fish", "白身魚のレモンハーブ蒸し＋クスクスと温野菜", "辛味や濃い味を重ねず、柑橘とハーブで食べやすくします。", ["shizumeru", "uruosu"], ["calm", "moist", "aroma", "support"], "地中海", "fish", "plate"),
  ...
  mealCandidate("relax-acquapazza", "白身魚とあさりのアクアパッツァ＋パン少量", "魚介のだしとトマトの汁気で、濃い味に頼らず一皿をまとめます。", ["yurumeru", "uruosu"], ["aroma", "moist", "calm", "support"], "イタリアン", "fish", "plate"),
  mealCandidate("relax-mushroom-risotto", "鶏肉ときのこの和風リゾット＋青菜", "だしの香りとやわらかさを使い、食事を小さくまとめます。", ["yurumeru", "uruosu"], ["moist", "aroma", "digestion", "support"], "和洋", "chicken", "rice"),
  ...
  mealCandidate("relax-chicken-tagine", "鶏肉と根菜のレモンタジン風煮込み＋クスクス", "穏やかなスパイスとレモンの香りで、油を増やさず一皿を整えます。", ["yurumeru", "meguraseru"], ["aroma", "warm", "moist", "support"], "北アフリカ", "chicken", "plate"),
...
const BUY_MEAL_CATALOG = [
  ...
  mealCandidate("buy-relax-basil", "バジルチキンのラップサンド＋ミネストローネ", "ハーブの香りと温かい野菜スープで一食をまとめます。", ["yurumeru", "meguraseru"], ["aroma", "moist", "support"], "洋食", "chicken", "buy", { effort: "buy" }),
  mealCandidate("buy-relax-rice", "鶏五目おにぎり＋茶碗蒸し＋ほうじ茶", "一口ずつ食べやすい組み合わせで、食事を小さく整えます。", ["yurumeru", "sasaeru"], ["calm", "support", "warm"], "和食", "chicken", "buy", { effort: "buy" }),
...
const EAT_OUT_MEAL_CATALOG = [
  ...
  mealCandidate("out-relax-tagine", "鶏肉と野菜のタジン風煮込み＋クスクス", "油を増やさず、香りのある煮込みをゆっくり食べます。", ["yurumeru", "meguraseru"], ["aroma", "moist", "support"], "北アフリカ", "chicken", "out", { effort: "out" }),
  mealCandidate("out-relax-sushi", "白身魚と赤身中心の握り＋茶碗蒸し", "揚げ物やマヨネーズ系を重ねず、温かい一品を添えます。", ["yurumeru", "shizumeru"], ["calm", "support"], "和食", "fish", "out", { effort: "out" }),
...
const POLICY_FOOD_INSIGHTS = {
  shizumeru: "今日は食事量を減らすことより、辛味・濃い味・カフェインを重ねない方が合いやすい日です。",
  yurumeru: "今日は栄養を足し続けるより、香りと汁気のある一皿で食事を小さく区切る方が合いやすい日です。",
  meguraseru: "今日は強い刺激ではなく、柑橘や香味野菜を使うと食事の重さを残しにくい日です。",
  nagasu: "今日は食事量不足より、油・濃い甘味・冷たい物を重ねて食後の重さを残すことに注意したい日です。",
  uruosu: "今日は量を増やすより、蒸し煮やあんなど料理の汁気を残す方が合いやすい日です。",
  nukumeru: "今日は辛さを強くするより、温かい煮込みや焼き物で冷たい食事の連続を切る方が合いやすい日です。",
  sasaeru: "今日は軽く済ませすぎず、主食とたんぱく質を小さくそろえる方が合いやすい日です。",
};
...
export function buildFoodCareProfile({ theme, symptomFocus = null, subLabels = [] } = {}) {
  ...
  const rawInsight = symptomFocus === "digestion" && primaryPolicy === "nagasu"
    ? "今日は食事量不足より、油・濃い甘味・冷たい物が重なって食後の重さを残しやすい組み合わせです。"
    : symptomFocus === "fatigue" && primaryPolicy === "sasaeru"
      ? "今日は食べすぎより、食事を軽く済ませすぎて余力を減らす方に注意したい日です。"
      : symptomFocus === "sleep" && ["shizumeru", "yurumeru"].includes(primaryPolicy)
        ? "今日は栄養を足すことより、夜まで辛味・濃い味・カフェインを重ねない方が合いやすい日です。"
        : POLICY_FOOD_INSIGHTS[primaryPolicy] || POLICY_FOOD_INSIGHTS.sasaeru;
  const insight = theme?.mode === "tomorrow" ? rawInsight.replace(/^今日は/, "明日は") : rawInsight;
  return {
...
function selectFoodIdeas({ catalog, catalogKey, theme, targetDate, symptomFocus, subLabels, limit = 3 }) {
  const scored = safeArray(catalog)
...
export function enhanceFoodContext({
  ...
  const todayCards = [
    ...
    buyChoice ? {
      ...
      label: "買って済ませるなら",
      body: buyChoice.note,
      items: [buyChoice.label],
    } : null,
    eatOutChoice ? {
      key: "eat_out",
      label: "外で選ぶなら",
      body: eatOutChoice.note,
      items: [eatOutChoice.label],
    } : null,
    alternatives.length ? {
      key: "alternative",
      label: "別の気分なら",
      body: "身体への狙いは保ったまま、料理文化と主菜を変えた候補です。",
  ...
  const enhanced = {
    ...
    detail_title: normalizedMode === "tomorrow" ? "今夜の準備・朝の別案" : "買う・外食・別の気分",
