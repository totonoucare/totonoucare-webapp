...
export const DAILY_CARE_LOGIC_VERSION = "daily_care_v2_11_2026-08-04_grounded_lifestyle_needs";
...
const TOOL_LAYOUT_CANDIDATES = [
  ...
  },
  {
    ...
    constitution_affinity: { brake: 0.55, reserve_small: 0.7, blood_stasis: 0.6 },
...
const BODY_MECHANICS_LIFESTYLE_CANDIDATES = BODY_MECHANICS_INTERNAL_CANDIDATES.map((candidate) => {
  ...
  return {
    ...
    // 身体操作から商品を逆算して主役を決めない。道具が主役になる候補は
    // TOOL_LAYOUT_CANDIDATESへ分け、身体操作そのものにはショップ導線を出さない。
    shop_eligible: false,
...
const LIFESTYLE_CANDIDATE_NEAR_TIE_DELTA = 4;
...
function activeConstitutionSignals(theme) {
  ...
  if (coreCode.startsWith("accel_")) signals.accel = 1;
  if (coreCode.startsWith("brake_")) signals.brake = 1;
  ...
  if (yy > 0) signals.accel = Math.max(Number(signals.accel || 0), yy);
  if (yy < 0) signals.brake = Math.max(Number(signals.brake || 0), Math.abs(yy));
...
function lifestyleCandidateScore(candidate, { theme, symptomFocus }) {
  ...
  const breakdown = { symptom: 0, weather: 0, constitution: 0, timing: 0, feasibility: 0 };
  ...
  if (candidateTriggers.includes(primaryTrigger)) {
    ...
    const points = Math.min(25, 25 * affinity);
  ...
  if (secondaryTrigger && secondaryTrigger !== primaryTrigger && candidateTriggers.includes(secondaryTrigger)) {
    ...
    const points = Math.min(14, 14 * affinity);
    breakdown.weather += points;
    selectedBecause.push({
      axis: "weather",
  ...
  breakdown.weather = Math.min(25, breakdown.weather);
  ...
  if (theme?.reserve_small) {
    breakdown.feasibility = candidate?.effort === "low" ? 5 : candidate?.effort === "medium" ? 2 : 0;
  } else {
...
function selectLifestyleCareCandidates({
  ...
  const primaryPool = symptomPool.length ? symptomPool : scored;
  const orderedPrimary = rotateNearTies(primaryPool, {
    targetDate,
    contextKey: `${contextKey}|primary`,
    offset: theme?.mode === "tomorrow" ? 1 : 0,
  });
  ...
  if (!primary) {
    return {
      ...
      laneScores: { body: bestLaneScore(scored, "body"), tool_layout: bestLaneScore(scored, "tool_layout") },
  ...
  return {
    ...
    laneScores: {
      ...
      tool_layout: bestLaneScore(scored, "tool_layout"),
    },
...
export function enhanceLifestylePlan({
  ...
  if (!primary) {
    ...
    return {
      ...
      lead: theme?.summary || "身体の使い方か、道具と配置から今の一手を選びます。",
      primary_action: null,
      alternatives: [],
      steps: [],
      step_ids: [],
      ...
      selection_basis: {
        ...
        tool_layout_score: Number(selection.laneScores?.tool_layout || 0),
        environment_score: 0,
  ...
  const enhanced = {
    ...
    // から持ち越さない。暮らすは身体の使い方と道具・配置だけに限定する。
    trap: "",
    selection_basis: {
      ...
      lane_score_gap: Number(
        Math.abs(Number(selection.laneScores?.body || 0) - Number(selection.laneScores?.tool_layout || 0)).toFixed(3)
      ),
      near_tie_delta: LIFESTYLE_CANDIDATE_NEAR_TIE_DELTA,
