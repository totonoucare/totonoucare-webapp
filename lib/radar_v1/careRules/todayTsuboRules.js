...
const POINT_ROLE_SUMMARIES = {
  ...
  PC6: "前腕から胸・みぞおちまわりの力みをゆるめたい時に使いやすいツボです。",
  ...
  SP5: "内くるぶしの前下にあり、足元の重さやむくみ感を見たい時に使いやすいツボです。",
};

const POINT_USE_PHRASES = {
  LI4: "手で触れやすく、頭・首肩まわりの力みを一度ゆるめたい時に使いやすい",
  LR3: "足先から力みを逃がし、頭や気分のこもりを残しにくくしたい時に使いやすい",
  GB20: "首のつけ根から後頭部にかけて、こわばりや重さを見たい時に使いやすい",
  PC6: "前腕から胸・みぞおちまわりの力みをゆるめたい時に使いやすい",
  ST36: "すねの外側から、だるさや下半身の重さを見たい時に使いやすい",
  SP6: "足元の冷え・だるさ・重さをまとめて見たい時に使いやすい",
  KI3: "足首まわりから腰腹の冷えや重さを見たい時に使いやすい",
  CV6: "下腹部を冷やさず、腰腹まわりの力を抜きたい時に使いやすい",
  LU7: "手首から首肩や呼吸まわりのこわばりをゆるめたい時に使いやすい",
  SP5: "足元の重さやむくみ感を見たい時に使いやすい",
};

function getSignalTiming(signal) {
  if (Number(signal) >= 2) return "山場の前に先に";
  if (Number(signal) === 1) return "山場の前に一度";
  return "気づいた時に";
}
...
function buildSelectionReason(point, { symptomFocus, triggerKey, signal } = {}) {
  ...
  return `今日は${triggerSign}、${symptomSign}時間があります。${point.name_ja}は${pointUse}ため、${timing}短く触るツボとして使いやすいです。`;
}
