...
const POINT_ROLE_SUMMARIES = {
  ...
  CV12: "お腹の中央にあり、胃腸に水分を含んだような重さがある日に、やさしく触れたいツボです。",
...
const TSUBO_TRIGGER_TONE = {
  ...
  cold: "低温で体がきゅっと縮こまりやすい日",
  heat: "高温で熱やそわつきが上に残りやすい日",
...
const TSUBO_SYMPTOM_TONE = {
  ...
  neck_shoulder: "首だけに原因を求めず、肩甲骨まわりや呼吸からも力みを逃がしたい状態です",
  ...
  fatigue: "だるさを我慢して動き続けず、足元から身体を支えたい状態です",
  swelling: "足元にたまりやすい重さを、強く押さずに流したい状態です",
};

function getSignalTiming(signal) {
  if (Number(signal) >= 2) return "気象負担が強まる時間の前に、休憩を取って";
  if (Number(signal) === 1) return "作業の合間に一度、";
  return "気になった時に、";
}

function buildSelectionReason(point, { symptomFocus, triggerKey, signal, primaryMeridian, secondaryMeridian } = {}) {
  const triggerTone = triggerKey === "none" ? "目立った気象負担はありません" : Number(signal) === 0 ? "気象による負担は小さめです" : TSUBO_TRIGGER_TONE[triggerKey] || "普段気になる不調に合わせて整えたい日";
  ...
  const profileLine = safeArray(POINTS_BY_MERIDIAN_LINE[primaryMeridian]).includes(point.code)
    ...
    : safeArray(POINTS_BY_MERIDIAN_LINE[secondaryMeridian]).includes(point.code)
      ...
      : "";
  return `今日は、${triggerTone}。${symptomTone}。${point.name_ja}は${pointUse}ツボです。${profileLine}${timing}強く押し込まず、やさしく短時間触れてください。`;
}
  ...
  return {
    ...
    explanation: {
      ...
      match_tags: [
        ...
        triggerKey && triggerKey !== "none" ? `天気の影響：${triggerLabel}` : null,
