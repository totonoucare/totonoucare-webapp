...
const POINT_ROLE_SUMMARIES = {
  ...
  LR3: "足にあり、頭や気分の重さが気になる日に使いやすいツボです。",
  ...
  CV12: "お腹の中央にあり、食後の重さ・胃もたれ・お腹の力みをやさしく見たい時に使いやすいツボです。",
  SP4: "足の内側にあり、胃の重さや食後のお腹の力みが気になる時に使いやすいツボです。",
...
function buildSelectionReason(point, { symptomFocus, triggerKey, signal } = {}) {
  const symptomSign = SYMPTOM_SIGN_PHRASES[symptomFocus] || "今見ている不調が出やすい";
  ...
  return `今日は${triggerSign}、${symptomSign}日です。${point.name_ja}は${pointUse}場所です。${timing}のがおすすめです。`;
}
