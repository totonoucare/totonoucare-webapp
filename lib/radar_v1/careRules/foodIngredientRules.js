...
function drinkScore(drink, { triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today", reactionDirection = null }) {
  ...
  const weatherKeys = new Set([triggerKey, secondaryKey].filter(Boolean));
  const isActuallyHot = weatherKeys.has("heat");
  const isHotAndDamp = isActuallyHot && weatherKeys.has("damp");
  const needs = uniq([
  ...
  if (isHotAndDamp && flags.weakDigestion) {
    if (drink.name === "とうもろこし茶") score += 5;
    if (drink.name === "麦茶") score += 2;
    if ((drink.nature === "平" || drink.nature === "neutral") && tags.includes("support_spleen")) score += 2;
    if (["温", "熱", "温寄り"].includes(drink.nature) || tags.includes("warm_hot")) score -= 4;
  }
  if (isActuallyHot && (["温", "熱", "温寄り"].includes(drink.nature) || tags.includes("warm_hot"))) score -= 2;
  if (isActuallyHot && flags.accel && symptomFocus === "headache" && drink.caffeine) score -= 2;
...
function buildDrinkFoodNatureReason(drink, context) {
  ...
  const direction = nature === "涼"
    ? `${drink.name}は体にこもった熱を冷ます方向の飲み物です。`
    : nature === "熱"
      ? `${drink.name}は内側をしっかり温める方向の飲み物です。`
      : nature === "温" || nature === "温寄り"
        ? `${drink.name}はお腹や体を冷やさず、温める方向の飲み物です。`
        : `${drink.name}は温める・冷ますの偏りが少ない飲み物です。`;
...
function buildDrinkComponentReason(drink, context) {
  ...
  if (flags.weakDigestion && flags.keys.includes("heat") && flags.keys.includes("damp")
    && !["温", "熱", "温寄り"].includes(drink?.nature)) {
    component += "蒸し暑い日は冷やしすぎず、常温で少量ずつ飲みます。";
  }
  return component;
}
...
function buildDrinkActionCard({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  ...
  const body = mode === "tomorrow"
    ? "明日の天気と体調に合う一杯を、飲む時間や温度まで含めて選びます。"
