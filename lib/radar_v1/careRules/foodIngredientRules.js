...
const RAW_DRINK_ITEMS = [
  ...
  },
  {
    ...
    note: "カフェインを含まず、暑い日の水分補給に取り入れやすい",
  },
  {
    name: "ほうじ茶",
    nature: "温寄り",
  ...
  },
  {
    ...
    caffeine: false,
    caffeineLevel: "none",
    tags: ["move_qi", "drain_damp", "aromatic", "antioxidant", "polyphenol", "low_stimulus"],
...
// Food nature, serving temperature and caffeine are separate axes.
export const DRINK_AUDIT = {
  "水": {nature:"平",tags:["fluids","neutral","base","light"],review:"common",note:"日中の水分補給に。喉の渇きに合わせて飲みます。"},
  "白湯": {nature:"平",tags:["fluids","neutral","base","easy"],servedWarm:true,review:"temperature",note:"温かい水として、お腹を冷やさず飲みたいときに。"},
  "麦茶": {nature:"涼",review:"variable",note:"暑い日の水分補給に。冷えが気になるときは常温で。"},
  "ほうじ茶": {nature:null,tags:["light","aromatic","fluids"],review:"processing",note:"香ばしいお茶を食事に添える候補です。カフェインは含まれます。"},
  "緑茶": {nature:"涼",review:"common",note:"暑さが気になる日の一杯に。カフェインを含むため朝〜昼に。"},
  "コーヒー": {nature:null,tags:["aromatic","wake","light"],review:"variable",note:"香りを楽しむ朝〜昼の一杯に。空腹時や夜は控えめに。"},
  "デカフェコーヒー": {nature:null,tags:["aromatic","light","low_stimulus"],review:"variable",note:"コーヒーの香りを楽しみつつカフェインを減らしたいときに。少量のカフェインを含む製品もあります。"},
  "ルイボスティー": {nature:null,flavors:[],tags:["fluids","light","low_stimulus"],review:"modern",note:"カフェインを控えたいときの一杯に。"},
  "はとむぎ茶": {nature:"涼",tags:["drain_damp","support_spleen","light","cool","fluids"],inferredTags:["drain_damp","support_spleen"],review:"infusion",note:"重だるさをためやすい体質に合わせる、甘くない一杯に。"},
  "とうもろこし茶": {nature:"平",tags:["light","neutral","fluids"],review:"infusion",note:"香ばしい穀物のお茶を、甘い飲み物の代わりに。"},
  "小豆茶": {nature:"平",tags:["drain_damp","light","neutral","fluids"],inferredTags:["drain_damp"],review:"infusion",note:"水分をため込みやすい体質に合わせる一杯に。砂糖を加えず取り入れます。"},
  "黒豆茶": {nature:"平",tags:["blood","kidney","support","aromatic","neutral","fluids"],inferredTags:["blood","kidney","support"],review:"infusion",note:"疲れや消耗が気になるときの、体を養う食養生に添える一杯です。"},
  "生姜湯": {nature:"温",tags:["warm","digest","support_spleen"],review:"preparation",note:"冷えが気になるときに、薄めの生姜湯を少量。甘くしすぎないようにします。"},
  "葛湯": {nature:null,tags:["easy","fluids"],servedWarm:true,review:"processing",note:"温かいとろみのある一杯。小腹が空いたときは甘さ控えめで少量に。"},
};
// Inferred associations are design weights, not measured potency or clinical efficacy.
export const DRINK_ITEMS = RAW_DRINK_ITEMS.map((d,index)=>({...d,...DRINK_AUDIT[d.name],
 inference_basis:DRINK_AUDIT[d.name].inferredTags?.length?'食材の食養生分類から飲料の選び方へ応用した仮説（専門家確認前）':null,
 id:`drink-${index+1}`}));

const DRINK_TRIGGER_NEEDS = {
  damp: ["drain_damp", "support_spleen", "light"],
  cold: ["warm", "support_spleen", "easy"],
  heat: ["clear_heat", "fluids", "cool"],
  dry: ["moisten", "fluids", "neutral"],
  pressure_down: ["wake", "light", "support_spleen", "drain_damp"],
  pressure_up: ["calm", "light", "fluids"],
...
const DRINK_SYMPTOM_NEEDS = {
  ...
  headache: ["fluids", "calm", "light"],
  dizziness: ["qi", "support_spleen", "fluids"],
  mood: ["move_qi", "calm", "support_spleen"],
};
...
const DRINK_WEATHER_BODY = {
  damp: "湿気で重だるさが出やすい日は、重さをためにくい食性の一杯を選びます。温度は胃腸の状態に合わせます。",
  cold: "冷えの日は、お腹や体を温める食性の一杯を選びます。少量ずつ飲み、内側まで冷やさないようにします。",
  heat: "暑い日は、熱を冷ます食性か、温冷の偏りが少ない一杯を選びます。胃腸が弱い時は常温で飲みます。",
  dry: "のど・目・肌がカサつく日は、うるおいを補う食性の一杯を選びます。日中の水分補給へ無理なく足します。",
  pressure_down: "低気圧で重さや眠気が出やすい日は、重さをためにくく胃腸を支える食性の一杯を選びます。",
  pressure_up: "気圧変化で張りや高ぶりが出やすい日は、熱と刺激を重ねにくい食性の一杯を選びます。",
  default: "今日の崩れ方に合わせ、温める・冷ます・うるおす方向を見て一杯を選びます。",
};

function drinkContextFlags({ triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today", reactionDirection = null }) {
  ...
  return {
    ...
    mode,
    reactionDirection,
    accel: reactionDirection === "accel",
    brake: reactionDirection === "brake",
    hasCold: keys.includes("cold") || sub.cold,
    hasHeat: keys.includes("heat") || sub.heat,
    hasDamp: keys.includes("damp") || sub.fluidDamp,
...
function drinkScore(drink, { triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today", reactionDirection = null }) {
  const flags = drinkContextFlags({ triggerKey, secondaryKey, symptomFocus, subLabels, mode, reactionDirection });
  const needs = uniq([
  ...
  if (flags.hasHeat && drink.name === "コーヒー") score -= 1; // 暑い日や高ぶりやすい日は、カフェインの分だけ少し控えめ
  ...
  if (flags.accel && tags.includes("calm")) score += 2;
  if (flags.accel && drink.caffeine) score -= 1;
  if (flags.brake && (tags.includes("support_spleen") || tags.includes("wake"))) score += 1;

  // 暑さと湿気が重なり、胃腸が弱りやすい時は、冷ますだけの一杯より
  // 常温で取り入れやすく、重さをためにくい一杯を優先する。
  if (flags.hasHeat && flags.hasDamp && flags.weakDigestion) {
    if (drink.name === "とうもろこし茶") score += 5;
    if (drink.name === "麦茶") score += 2;
    if (tags.includes("neutral") && tags.includes("support_spleen")) score += 2;
    if (String(drink.nature || "").includes("温")) score -= 4;
  }
  if (flags.hasHeat && String(drink.nature || "").includes("温")) score -= 2;
  if (flags.hasHeat && flags.accel && symptomFocus === "headache" && drink.caffeine) score -= 2;

  // 今日の一杯はその場の天気反応へ、明日の一杯は今夜〜朝の準備へ寄せる。
  if (mode === "today") {
    if (flags.hasHeat && tags.includes("clear_heat")) score += 1;
    if (flags.hasDamp && tags.includes("drain_damp")) score += 1;
    if (flags.qiStagnation && tags.includes("aromatic")) score += 0.8;
  } else {
    if (!drink.caffeine && (tags.includes("support_spleen") || tags.includes("fluids") || tags.includes("calm"))) score += 1.2;
  }
  ...
  // コーヒーは眠気対策だけではなく、香りや好みも含めて候補にする。
  // ただしカフェインが睡眠・乾燥・胃腸へ与える負担は別に減点する。
  if (drink.name === "コーヒー") {
  ...
  const sub = getSubLabelFlags(subLabels);
  if (drink.name === "黒豆茶" && (sub.bloodDeficiency || symptomFocus === "fatigue")) score += 2;
  if (drink.servedWarm && flags.hasCold) score += 1;
  return score;
}
...
function getDrinkNoteByContext(drink, context, mark) {
  ...
  if (drink.name === "コーヒー") {
    if (mark === "△") {
      if (symptomFocus === "sleep" || mode === "tomorrow") return "夜はカフェインが残りやすいので、飲むなら朝〜昼に少量";
      if (triggerKey === "dry") return "乾きが気になる日は、水分補給も忘れず、コーヒーは少量に";
      if (triggerKey === "heat") return "暑い日は、量を増やしすぎず水分補給も一緒に";
      if (triggerKey === "pressure_up") return "気圧が変わる日は、いつもの量を増やしすぎない";
      return "空腹で流し込まず、朝〜昼に少量";
    }
    if (triggerKey === "pressure_up") return "香りを楽しみながら、朝〜昼に少量飲む候補";
    return "眠気やだるさが気になる朝〜昼に、食事や水分と一緒に少量飲む候補";
  }
  if (drink.name === "デカフェコーヒー") {
    if (triggerKey === "pressure_up" || symptomFocus === "sleep") return "コーヒーの香りを楽しみながら、カフェインを控えたい時の候補";
    return "コーヒーは飲みたいけれど、カフェインを控えたい時に";
  }
  if (drink.name === "緑茶") {
    if (mark === "△") return "冷え・胃弱・夜は控えめ。暑さや湿気のぼんやりに短く";
    return "カフェインを含むため朝〜昼に1杯。暑さや湿気の日に、甘くない飲み物へ切り替えたい時に";
  }
  if (drink.name === "水") {
...
function buildDrinkFoodNatureReason(drink, context) {
  const flags=drinkContextFlags(context);
  if(drink.servedWarm)return drink.name==="白湯"
    ? "温かい水を少しずつ飲みたいときの候補です。温度は熱すぎない程度にします。"
    : "温かいとろみを少量取り入れる候補です。甘さと量を控えめにします。";
  if(drink.review==="infusion") {
    if(['はとむぎ茶','小豆茶'].includes(drink.name)) {
      const basis=context.symptomFocus==='swelling'?'むくみが気になるとき':flags.hasDamp?'重だるさをためやすいとき':'食事に甘くないお茶を合わせたいとき';
      return `${basis}の候補です。${drink.name==='はとむぎ茶'?'はとむぎの食養生を、一杯のお茶から取り入れます。':'小豆の食養生を、砂糖を加えず取り入れます。'}`;
    }
    if(drink.name==='黒豆茶')return '体を養う黒豆の食養生を、香ばしい一杯から取り入れる候補です。';
    return drink.note;
  }
  if(!drink.nature)return drink.note;
  if(flags.hasHeat&&drink.nature==="涼")return `食養生では${drink.name}を熱を落ち着ける方向で使います。暑さが気になる日の一杯として選びました。`;
  if(flags.hasCold&&drink.nature==="温")return `食養生では${drink.name}を温める方向で使います。冷えへの備えとして選びました。`;
  return drink.note;
}

function buildDrinkComponentReason(drink, context) {
  const flags = drinkContextFlags(context);
  const tags = new Set(drink?.tags || []);
  let component = drink.name === "デカフェコーヒー" ? "カフェインを減らした飲み物です。含有量は製品表示で確認できます。" : "カフェインを含みません。";
  if (drink?.caffeine) {
    if (drink?.caffeineLevel === "high") component = "カフェインが多めなので、飲むなら朝〜昼に少量が目安です。";
    else if (drink?.caffeineLevel === "middle") component = "カフェインを含むため、朝〜昼の一杯に向きます。";
    else component = "カフェインは少なめですが、睡眠が気になる日は夕方以降を控えます。";
  }
  if (!drink?.caffeine && tags.has("fluids")) component += "食事や日中の水分補給へ取り入れやすい候補です。";
  else if (!drink?.caffeine && tags.has("drain_damp")) component += "甘くしない一杯として、少量ずつ飲めます。";

  if (flags.hasCold && (tags.has("cool") || tags.has("clear_heat"))) {
    component += "冷えや胃腸の弱さがある時は、常温か温かめで少量にします。";
  } else if (flags.hasHeat && tags.has("warm_hot")) {
    component += "暑さやほてりがある時は控えめにします。";
  } else if (flags.accel && drink?.caffeine) {
    component += "そわつきや力みが強い時は、続けて何杯も飲みません。";
  }
  return component;
}

function buildDrinkRecommendationRows({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  const context = { mode, triggerKey, secondaryKey, symptomFocus, subLabels, reactionDirection };
  const scored = DRINK_ITEMS
    ...
    .filter((drink) => drink._score > 0 && !(mode === "tomorrow" && drink.caffeine))
  ...
  return selected.slice(0, 3).map((drink) => {
    const flags = drinkContextFlags(context);
    const baseNote = getDrinkNoteByContext(drink, context, drink._mark);
    const note = flags.hasHeat && flags.hasDamp && flags.weakDigestion && String(drink.nature || "") === "平"
      ? `${baseNote}。常温で少量から`
      : baseNote;
    return {
      ...drink,
      note,
      reasons: [
        { label: "体調との相性", text: buildDrinkFoodNatureReason(drink, context) },
        { label: "成分・飲み方", text: buildDrinkComponentReason(drink, context) },
      ],
    };
  });
}

function buildDrinkRecommendations(context) {
  return buildDrinkRecommendationRows(context)
    .map((drink) => `${drink._mark} ${drink.name}：${drink.note}`);
}

function buildDrinkActionCard({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  const rows=buildDrinkRecommendationRows({mode,triggerKey,secondaryKey,symptomFocus,subLabels,reactionDirection}).slice(0,2);
  if(!rows.length)return null;
  const tomorrow=mode==="tomorrow";
  return {key:"drink",label:tomorrow?"明日の朝に合わせる飲み物":"今日、食事と合わせる飲み物",items:rows.map(d=>d.name),
    item_details:rows.map(d=>({label:d.name,nature:d.nature,flavors:d.flavors,public_reason:d.reasons[0].text,reasons:d.reasons,
      ...
      selection_basis:{version:"drink_v69",nature:d.nature,review:d.review,inference_basis:d.inference_basis||null,inferred_tags:d.inferredTags||[],served_warm:!!d.servedWarm,reason:d.reasons[0].text},
    }))};
}

function formatDrinkLeadNames({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  return buildDrinkRecommendationRows({ mode, triggerKey, secondaryKey, symptomFocus, subLabels, reactionDirection })
    .map((item) => item.name)
    .slice(0, 2);
}
...
export function buildIngredientFoodContext({
  ...
  const legacyContext = {
    ...
    drink_model_version: "v7.79.68-drink-audit",
  };
