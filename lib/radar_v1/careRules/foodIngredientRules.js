...
function buildDrinkFoodNatureReason(drink, context) {
  ...
  const direction = nature === "涼"
    ? `食養生では、${drink.name}は体にこもった熱を冷ます方向の飲み物です。`
    : nature === "熱"
      ? `食養生では、${drink.name}は内側をしっかり温める方向の飲み物です。`
      : nature === "温" || nature === "温寄り"
        ? `食養生では、${drink.name}はお腹や体を冷やさず、温める方向の飲み物です。`
        : `食養生では、${drink.name}は温める・冷ますの偏りが少ない飲み物です。`;
...
function buildDrinkRecommendationRows({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  ...
  return selected.slice(0, 3).map((drink) => {
    ...
    return {
      ...
      note,
      reasons: [
        { label: "食養生", text: buildDrinkFoodNatureReason(drink, context) },
