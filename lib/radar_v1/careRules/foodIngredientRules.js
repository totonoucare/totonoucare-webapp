...
function drinkContextFlags({ triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today" }) {
  ...
  return {
    ...
    symptomFocus,
    mode,
    hasCold: keys.includes("cold") || /冷|寒/.test(subLabelText),
    hasHeat: keys.includes("heat") || keys.includes("pressure_up") || /熱|ほて|暑/.test(subLabelText),
    hasDamp: keys.includes("damp") || keys.includes("pressure_down") || /湿|水|痰|むく/.test(subLabelText),
    hasDry: keys.includes("dry") || /乾|燥|陰/.test(subLabelText),
    qiStagnation: /気滞|巡|肝/.test(subLabelText) || symptomFocus === "mood" || symptomFocus === "neck_shoulder",
    weakDigestion: /気虚|脾|胃|虚/.test(subLabelText) || symptomFocus === "digestion",
  };
}
...
function drinkScore(drink, { triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today" }) {
  ...
  if (drink.name === "コーヒー") {
    if (flags.hasDamp || triggerKey === "pressure_down" || flags.qiStagnation) score += 5;
    if (triggerKey === "pressure_up" || symptomFocus === "headache" || symptomFocus === "mood") score += 2;
    if (symptomFocus === "fatigue") score += 2;
    if (symptomFocus === "sleep" || flags.hasDry || flags.weakDigestion) score -= 5;
    if (flags.hasHeat) score -= 2;
  }

  if (drink.name === "デカフェコーヒー") {
    if (triggerKey === "pressure_up" || symptomFocus === "mood" || symptomFocus === "sleep") score += 4;
    if (flags.hasDamp || flags.qiStagnation) score += 3;
...
function buildDrinkRecommendations({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [] }) {
  ...
  const scored = DRINK_ITEMS
    ...
    .filter((drink) => drink._score > 0 || drink.name === "コーヒー")
  ...
  // コーヒー系が条件つきで意味を持つ日は、悪者扱いではなく候補として見せる。
  ["コーヒー", "デカフェコーヒー"].forEach((coffeeName) => {
    const coffee = scored.find((d) => d.name === coffeeName);
    if (coffee && coffee._score >= 2 && !names.has(coffeeName) && selected.length < 3) {
      push(coffee);
    }
  });
...
export function buildIngredientFoodContext({
  ...
  return {
    ...
    drink_model_version: "v7.37-dryness-body-copy",
  };
}
