...
const PAIR_FOOD_PROFILES = {
  ...
  "cold+damp": {
    ...
    reason: "冷えと湿気が重なる日は、冷たいもの・甘いものが胃に残りやすくなります。温かい主食、汁物、きのこ類を使いやすくします。",
  },
  "dry+heat": {
    label: "暑さと乾燥",
...
function formatTomorrowFoodItems({ selectedFoods, triggerKey, secondaryKey, symptomFocus, subLabels }) {
  const scored = getScoredFoodPool({ triggerKey, secondaryKey, symptomFocus, subLabels });
  const usedNames = new Set();

  const pick = (pool, predicate, limit) => {
    const items = [];

    for (const food of pool) {
      if (items.length >= limit) break;
      if (usedNames.has(food.name)) continue;
      if (!predicate(food)) continue;
  ...
  let drinks = pick(scored, (food) => food.role === "drink", 2);

  if (!drinks.length) {
    drinks = getFallbackDrinkNames(triggerKey, secondaryKey)
      .map((name) => FOOD_ITEMS.find((food) => food.name === name))
      .filter(Boolean)
      .filter((food) => !usedNames.has(food.name))
      .slice(0, 2);

    drinks.forEach((food) => usedNames.add(food.name));
  }

  const evening = pick(
    selectedFoods,
    (food) => food.role !== "staple" && food.role !== "drink",
    3
  );

  const morning = pick(
    [...selectedFoods, ...scored],
    (food) => food.role !== "drink" && getMealGroup(food) !== "side",
    3
  );
  ...
  if (drinks.length) {
    items.push(`寝る前に飲むなら：${formatFoodNames(drinks, 2)}`);
  }

  if (evening.length) {
    items.push(`夜に食べるなら：${formatFoodNames(evening, 3)}`);
  }

  if (morning.length) {
    items.push(`朝に食べるなら：${formatFoodNames(morning, 3)}`);
  }
...
export function buildIngredientFoodContext({
  ...
  const symptomReason = symptomProfile
    ? ` ${symptomProfile.label}が気になる時は、食後の重さ・冷えすぎ・刺激の強さも一緒に見ます。`
    : "";
  const reason = `${triggerProfile.reason}${symptomReason}`;
