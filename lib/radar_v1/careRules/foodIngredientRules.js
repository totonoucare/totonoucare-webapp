...
function buildDrinkFoodNatureReason(drink, context) {
  const flags = drinkContextFlags(context);
  const nature = String(drink?.nature || "平");
  const direction = nature === "涼"
    ? `${drink.name}は、体にこもった熱を冷ます方向の飲み物です。`
    : nature === "熱"
      ? `${drink.name}は、内側をしっかり温める方向の飲み物です。`
      : nature === "温" || nature === "温寄り"
        ? `${drink.name}は、お腹や体を冷やさず、温める方向の飲み物です。`
        : `${drink.name}は、温める・冷ますの偏りが少ない飲み物です。`;
  const tags = new Set(drink?.tags || []);
  let fit = "";
  if (flags.hasHeat && tags.has("clear_heat")) fit = "暑さや熱のこもりが気になる日に合わせています。";
  else if (flags.hasDamp && tags.has("drain_damp")) fit = "湿気で重だるさやむくみが出やすい日に合わせています。";
  else if (flags.hasDry && (tags.has("moisten") || tags.has("fluids"))) fit = "口や喉の乾きが気になる日に合わせています。";
  else if (flags.qiStagnation && (tags.has("move_qi") || tags.has("aromatic"))) fit = "香りを、張りつめた感じや気分の滞りを動かす方向へ使います。";
  else if (flags.accel && tags.has("calm")) fit = "高ぶりやそわつきを重ねたくない日に合わせています。";
  else if (flags.weakDigestion && tags.has("support_spleen")) fit = "胃腸を冷やしにくく、食後の重さを増やしにくい候補です。";
  else if (context?.symptomFocus === "sleep") fit = "夜の刺激を増やしにくい候補として選びました。";
  else if (context?.symptomFocus === "headache") fit = "頭まわりの力みや乾きを重ねにくい温度と量で取り入れます。";
  else if (["low_back_pain", "fatigue"].includes(context?.symptomFocus)) fit = "食事と水分補給を切らさない一杯として使います。";
  else if (context?.symptomFocus === "neck_shoulder") fit = "一口飲むたびに手元作業をいったん止め、首肩の力を抜くきっかけにします。";
  else if (context?.symptomFocus === "digestion") fit = "食後の重さを増やしにくい温度と量で取り入れます。";
...
function buildDrinkComponentReason(drink, context) {
  ...
  if (tags.has("polyphenol")) component += "ポリフェノールを含みます。";
  else if (!drink?.caffeine && tags.has("fluids")) component += "食事や日中の水分補給へ取り入れやすい候補です。";
...
function buildDrinkActionCard({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  const rows = buildDrinkRecommendationRows({ mode, triggerKey, secondaryKey, symptomFocus, subLabels, reactionDirection });
  if (!rows.length) return null;
  const body = mode === "tomorrow"
    ? (symptomFocus === "sleep"
      ? "今夜はカフェインを残しにくい一杯を、眠る前の水分補給として選びます。"
      : "明日の朝に取り入れやすい一杯を、食事と体調の組み合わせから選びます。")
    : `食事中から食後に合わせる一杯です。${DRINK_WEATHER_BODY[triggerKey] || DRINK_WEATHER_BODY.default}`;

  const timingReason = mode === "tomorrow"
    ? (symptomFocus === "sleep"
      ? "今夜は夕方以降の刺激を増やさない時間帯で取り入れます。"
      : "明日の朝、食事と一緒か食後に少量から取り入れます。")
    : "今日は食事中から食後に、喉の渇きに合わせて少量ずつ取り入れます。";

  return {
    key: "drink",
    label: mode === "tomorrow" ? "今夜〜明朝の飲み物" : "今日、食事と合わせる飲み物",
    body,
    items: rows.map((drink) => drink.name),
    item_details: rows.map((drink) => ({
      label: drink.name,
      nature: drink.nature,
      flavors: drink.flavors,
      reasons: [...drink.reasons, { label: "取り入れる時間", text: timingReason }],
    })),
  };
}
...
export function buildIngredientFoodContext({
  ...
  const legacyContext = {
    ...
    drink_model_version: "v7.40-food-nature-visible",
  };
