...
function drinkScore(drink, { triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today", reactionDirection = null }) {
  ...
  // 高温が主条件の日は、胃腸を冷やしにくい候補も残しつつ、温性の一杯を
  // 第一候補にはしない。白湯を固定的な正解に見せないための順位調整。
  if (triggerKey === "heat" && ["白湯", "葛湯", "生姜湯"].includes(drink.name)) score -= 4;
...
function buildDrinkActionCard({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  ...
  const body = mode === "tomorrow"
    ? "今日と同じ候補が残る場合は、体質への相性を保ちながら明日の天気にも合うためです。今夜のうちに一つ決め、温度・量・飲む時間まで含めて選びます。"
