...
const MODE_LABELS = {
  ...
  tomorrow: {
    ...
    how_to_label: "今夜〜明朝に食べる・飲むなら",
    avoid_label: "寝る前〜明朝に避けたいもの",
    reason_label: "食材を選んだ理由",
    lifestyle_tip_label: "今夜〜明朝に一緒に",
    titlePrefix: "明日は",
    contextSignalLow: "安定",
    contextSignalMiddle: "いたわり",
    contextSignalHigh: "守り",
...
const SIGNAL_COPY = {
  ...
  tomorrow: {
    ...
    middle: "明日はいたわり寄りの見込み。今夜〜明日の食べものと飲みものを少し選びます。",
...
const TRIGGER_PROFILES = {
  damp: {
    ...
    lead: "湿気が強い日は、冷たい飲み物・甘い飲み物・脂っこいものが続かないようにし、胃に残りにくい食材を優先します。",
    avoid: ["冷たい飲み物と甘いものばかりになる", "脂っこいものを続けて食べる", "パン・麺だけで済ませる"],
    reason: "湿気の日は、冷たいもの・甘いもの・脂っこいものが続くと胃に残りやすくなります。大根、きのこ、海藻のような食材を入れると組み立てやすくなります。",
  },
  cold: {
    label: "冷え込み",
    title: "内側を冷やしすぎない食べ方",
    ...
    lead: "冷え込みが強い日は、温める性質と胃腸を支える食材を優先します。",
  ...
  heat: {
    ...
    reason: "暑さが強い日は、辛味・濃い味・脂っこさでさらに体が熱く感じやすくなります。豆腐、白身魚、トマトのような、食後に重くなりにくい食材を選びます。",
  },
  dry: {
    label: "乾燥",
    title: "乾きを残しにくい食べ方",
    ...
    lead: "乾燥が強い日は、のど・目・肌の乾きを助ける食材を優先します。",
    avoid: ["乾いた菓子だけで済ませる", "コーヒーだけで空腹をつなぐ", "辛いものを続けて食べる"],
  ...
  pressure_down: {
    ...
    lead: "低気圧の日は、気を落としすぎず、重だるさを残しにくい食材を優先します。",
    avoid: ["食事を抜いてカフェインだけで動く", "揚げ物と甘いものばかり食べる", "お酒を飲みながら脂っこいものを食べる"],
  ...
  pressure_up: {
    ...
    lead: "気圧上昇の日は、張りつめや高ぶりを強めにくい食材を優先します。",
...
const PAIR_FOOD_PROFILES = {
  "damp+heat": {
    ...
    reason: "湿気もある日は、水分を足しすぎるより、胃に残りにくい食材を選びます。白身魚、豆腐、大根、きのこ、海藻などを使いやすくします。",
  },
  "cold+damp": {
    label: "冷えと湿気",
    ...
    reason: "冷え込みと湿気の影響がある日は、冷たいものや甘いものが胃に残りやすくなります。温かい主食、汁物、きのこ類を使いやすくします。",
  },
  "dry+heat": {
    label: "暑さと乾燥",
    ...
    lead: "暑さと乾きが重なる日は、辛いものや濃い味のものを食べすぎず、汁気のある食材を優先します。",
    avoid: ["辛いものを続けて食べる", "乾いた菓子だけで済ませる", "コーヒーだけで空腹をつなぐ"],
    reason: "暑さと乾きが重なる日は、熱くなりやすい食べ方を避け、のどや口の乾きにも合う食材を選びます。豆腐、トマト、梨、白ごまなどを使いやすくします。",
  },
  "cold+dry": {
    label: "冷えと乾燥",
    ...
    lead: "冷えと乾きが重なる日は、冷たいものを避けながら、汁気のある食材を優先します。",
  ...
  "damp+pressure_down": {
    ...
    reason: "低気圧と湿気の影響がある日は、甘いもの・脂っこいもの・冷たい飲み物が続くと胃に残りやすくなります。白身魚、大根、きのこ、海藻などを使いやすくします。",
  },
  "heat+pressure_up": {
    label: "暑さと気圧上昇",
    ...
    avoid: ["辛いものを食べたあとにコーヒーを飲む", "濃い味を早食いする", "お酒を飲みながら塩辛いものを食べる"],
    reason: "暑さと気圧上昇が重なる日は、頭や肩に力が入りやすくなることがあります。香味野菜で食べやすくしつつ、刺激を増やしすぎない組み立てにします。",
  },
};
...
function formatTomorrowFoodItems({ triggerKey, secondaryKey }) {
  const options = getTomorrowFoodPrepOptions(triggerKey, secondaryKey);

  return [
    `寝る前に飲むなら：${options.drink.slice(0, 2).join("、")}`,
    `寝る前に小腹が空いたら：${options.night.slice(0, 3).join("、")}`,
    `朝に食べるなら：${options.morning.slice(0, 3).join("、")}`,
  ].filter(Boolean);
}
...
function getActionBody({ mode, triggerProfile, symptomProfile }) {
  if (mode === "tomorrow") {
    ...
    return `${symptomPart}今夜から明日の朝までの飲みものと食べものを選びます。`;
  }

  const symptomPart = symptomProfile ? `${symptomProfile.label}の出方も見ながら、` : "";
  return `${symptomPart}今日は${triggerProfile.label}に合わせて、主食・主菜・副菜を少し選びます。`;
}
...
function buildTomorrowReason({ triggerKey, secondaryKey, symptomFocus }) {
  const pairKey = getFoodTriggerPairKey(triggerKey, secondaryKey);
  const key = pairKey || triggerKey;
  ...
  return `${reasonsByKey[key] || reasonsByKey.default}${symptomReasons[symptomFocus] || ""}`;
}

function buildFoodReason({ mode, triggerKey, secondaryKey, triggerProfile, symptomFocus }) {
  if (mode === "tomorrow") {
    return buildTomorrowReason({ triggerKey, secondaryKey, symptomFocus });
  }

  const symptomReason = symptomFocus
    ? ` ${SYMPTOM_LABELS[symptomFocus] || symptomFocus}が気になる時は、食後の重さ・冷えすぎ・刺激の強さも一緒に見ます。`
    : "";

  return `${triggerProfile.reason}${symptomReason}`;
}

function buildLifestyleTip(mode, triggerKey) {
  if (mode === "tomorrow") {
    if (triggerKey === "cold") return "今夜は寝る前に冷たい飲み物ばかり飲まず、明日の朝に温かいものを食べやすいようにします。";
    if (triggerKey === "damp") return "今夜は食べすぎたあとに座りっぱなしにせず、数分だけ立つか歩きます。";
    if (triggerKey === "heat") return "今夜は辛いものや濃い味のものを食べすぎないようにします。";
    if (triggerKey === "dry") return "今夜は乾いた菓子だけで済ませず、汁物か飲み物もとります。";
    return "今夜は食べすぎず、明日の朝に胃が重くならないようにします。";
  }
