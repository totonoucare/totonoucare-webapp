...
export const DRINK_AUDIT = {
  ...
  "はとむぎ茶": {nature:"涼",tags:["light","cool","fluids"],review:"infusion",note:"湿気が気になる日の甘くない一杯に。冷えが気になるときは温度を調整します。"},
  "とうもろこし茶": {nature:"平",tags:["light","neutral","fluids"],review:"infusion",note:"香ばしい穀物のお茶を、甘い飲み物の代わりに。"},
  "小豆茶": {nature:"平",tags:["light","neutral","fluids"],review:"infusion",note:"小豆の風味を、砂糖を加えない一杯で取り入れます。"},
  "黒豆茶": {nature:"平",tags:["aromatic","neutral","fluids"],review:"infusion",note:"黒豆の香ばしさを、カフェインのない一杯で楽しめます。"},
...
export const DRINK_ITEMS = RAW_DRINK_ITEMS.map((d,index)=>({...d,...DRINK_AUDIT[d.name],id:`drink-${index+1}`}));
...
function getDrinkNoteByContext(drink, context, mark) {
  ...
  if (drink.name === "コーヒー") {
    if (mark === "△") {
      ...
      if (triggerKey === "heat" || triggerKey === "pressure_up") return "暑い日や気持ちが急ぐ日は、量を増やしすぎない";
      return "空腹で流し込まず、朝〜昼に少量";
    }
    if (triggerKey === "pressure_up") return "気持ちが急ぐ日に、香りを楽しみながら少量飲む候補";
...
function buildDrinkFoodNatureReason(drink, context) {
  ...
  if(drink.review==="infusion")return drink.note;
...
function buildDrinkActionCard({ mode, triggerKey, secondaryKey, symptomFocus, subLabels = [], reactionDirection = null }) {
  ...
  return {key:"drink",label:tomorrow?"明日の朝に合わせる飲み物":"今日、食事と合わせる飲み物",items:rows.map(d=>d.name),
    item_details:rows.map(d=>({label:d.name,nature:d.nature,flavors:d.flavors,public_reason:d.reasons[0].text,reasons:d.reasons,
      ...
      selection_basis:{version:"drink_v68_rebuilt",nature:d.nature,review:d.review,served_warm:!!d.servedWarm,reason:d.reasons[0].text},
    }))};
}
