...
function drinkScore(drink, { triggerKey, secondaryKey, symptomFocus, subLabels = [], mode = "today", reactionDirection = null }) {
  ...
  // 高温多湿かつ胃腸が気になる日は、温める飲み物を辞書順で先頭にしない。
  // 胃腸への配慮は「温める一杯」に固定せず、平性の一杯と飲む温度で両立させる。
  if (isHotAndDamp && flags.weakDigestion) {
    ...
    if (tags.includes("warm") || tags.includes("warm_hot")) score -= 4;
  } else if (isActuallyHot && (tags.includes("warm") || tags.includes("warm_hot")) && !tags.includes("clear_heat")) {
    score -= 2;
  }
...
function buildDrinkComponentReason(drink, context) {
  ...
  } else if (flags.keys.includes("heat") && flags.keys.includes("damp") && flags.weakDigestion && tags.has("neutral")) {
