...
function getActionBody({ mode, triggerProfile, symptomProfile }) {
  if (mode === "tomorrow") {
    ...
    return `${symptomPart}今夜から明日の朝まで、体を重くする組み合わせを少し減らします。`;
  }

  const symptomPart = symptomProfile ? `${symptomProfile.label}の出方も見ながら、` : "";
  return `${symptomPart}今日は“何を足すか”より、“重くする組み合わせを減らす”ところから。主食・主菜・副菜を軽く組み替えます。`;
}
...
const FOOD_VOICE_AVOID = {
  ...
  dry: ["乾いたお菓子だけで、内側のカサつきを増やす", "コーヒーだけで空腹をつないで、のどと目をさらに乾かす", "辛いもので乾きに刺激を足す"],
