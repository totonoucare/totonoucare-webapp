...
const MODE_LABELS = {
  ...
  tomorrow: {
    ...
    how_to_label: "今夜から明日の朝に食べる・飲むなら",
...
function getActionBody({ mode, triggerProfile, symptomProfile }) {
  ...
  return `${symptomPart}今日は${triggerProfile.label}に合わせて、主食・主菜・副菜を選びます。`;
}
...
function buildLifestyleTip(mode, triggerKey) {
  ...
  if (triggerKey === "damp") return "食後すぐに座りっぱなしにせず、2〜3分だけ歩きます。";
  if (triggerKey === "heat") return "食後すぐに動き切らず、汗や熱が少し引く時間を作ると楽です。";
  if (triggerKey === "dry") return "乾いた菓子だけで済ませず、汁物か飲み物もとります。";
