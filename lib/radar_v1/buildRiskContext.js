// lib/radar_v1/buildRiskContext.js
import { personalizeForecast } from "@/lib/radar_v1/personalizeForecast";
import { personalizeForecastV2 } from "@/lib/radar_v1/personalizeForecastV2";
import { differentiateForTcm } from "@/lib/radar_v1/differentiation";
import { selectMtestLine } from "@/lib/radar_v1/selectMtestLine";
import { selectMotherChild } from "@/lib/radar_v1/selectMotherChild";
import { getSignalLabel, getTriggerLabel } from "@/lib/radar_v1/copy";
import { exactTriggerToCompat } from "@/lib/radar_v1/weatherStress";
import { readPressureResponseDirection } from "@/lib/radar_v1/pressureResponse";

export function buildRiskContext({ profile, weatherStress, forecastModelVersion = null }) {
  if (!profile) throw new Error("buildRiskContext: profile is required");
  if (!weatherStress) throw new Error("buildRiskContext: weatherStress is required");

  const useV2 = forecastModelVersion === "v2" || String(weatherStress?.model_version || "").includes("v2");
  const personalize = useV2 ? personalizeForecastV2 : personalizeForecast;
  const personalized = personalize({
    weatherStress,
    constitution: {
      core_code: profile.core_code,
      sub_labels: profile.sub_labels,
      env: profile.env,
      axes: profile.axes,
      split_scores: profile.split_scores,
      material_scores: profile.material_scores,
      score_scale: profile.score_scale,
      computed: profile.computed,
      raw: profile.raw,
      review_feedback: profile.review_feedback,
    },
    reviewFeedback: profile.review_feedback,
  });
  const pressureResponseDirection = useV2
    ? readPressureResponseDirection(personalized)
    : personalized?.personal_main_trigger_exact === "pressure_up"
      ? "accel"
      : "brake";

  const compatWeatherForCare = {
    main_trigger: personalized.main_trigger,
    trigger_dir: personalized.trigger_dir,

    personal_main_trigger_exact:
      personalized.personal_main_trigger_exact || "none",
    weather_main_trigger_exact:
      weatherStress.weather_main_trigger_exact || "none",
    personal_secondary_trigger_exact:
      personalized.personal_secondary_trigger_exact || null,

    pressure_down_strength: weatherStress.pressure_down_strength ?? 0,
    pressure_up_strength: weatherStress.pressure_up_strength ?? 0,
    cold_strength: weatherStress.cold_strength ?? 0,
    heat_strength: weatherStress.heat_strength ?? 0,
    damp_strength: weatherStress.damp_strength ?? 0,
    dry_strength: weatherStress.dry_strength ?? 0,
    pressure_shift_strength: weatherStress.pressure_shift_strength ?? 0,
    pressure_direction: weatherStress.pressure_direction || null,
    pressure_presentation_direction: weatherStress.pressure_presentation_direction || null,
    temperature_shift_strength: weatherStress.temperature_shift_strength ?? 0,
    temperature_direction: weatherStress.temperature_direction || null,
    moisture_shift_strength: weatherStress.moisture_shift_strength ?? 0,
    moisture_direction: weatherStress.moisture_direction || null,
    moisture_state: weatherStress.moisture_state || weatherStress.meta?.moisture?.state || null,
    pressure_response_direction: pressureResponseDirection,
    reaction_direction: pressureResponseDirection,
  };

  const tcmContext = differentiateForTcm({
    core_code: profile.core_code,
    sub_labels: profile.sub_labels,
    weatherStress: compatWeatherForCare,
    symptom_focus: profile.symptom_focus || null,
  });

  const selectedLine = selectMtestLine({
    primary_meridian: profile.primary_meridian,
    secondary_meridian: profile.secondary_meridian,
    weatherStress: compatWeatherForCare,
  });

  const motherChild = selectMotherChild({
    weatherStress: compatWeatherForCare,
    core_code: profile.core_code,
    sub_labels: profile.sub_labels,
  });

  const riskFactors = buildRiskFactors({
    personalized,
    profile,
  });

  const careTone = buildCareTone({
    profile,
    personalized,
  });

  const weatherMainCompat = {
    main_trigger: weatherStress.main_trigger,
    trigger_dir: weatherStress.trigger_dir,
  };

  const secondaryCompat = personalized.personal_secondary_trigger_exact
    ? exactTriggerToCompat(personalized.personal_secondary_trigger_exact)
    : null;

  const triggerFactors = buildTriggerFactorsForSummary(
    personalized.trigger_factors,
    personalized.personal_main_trigger_exact,
    personalized.personal_secondary_trigger_exact,
    pressureResponseDirection
  );
  const manifestationSummary = buildManifestationSummary(personalized?.meta?.manifestation);

  return {
    target: {
      score_0_10: personalized.score_0_10,
      score_display_0_10: personalized.score_display_0_10 ?? personalized.score_precise_0_10 ?? personalized.score_0_10,
      score_precise_0_10: personalized.score_precise_0_10 ?? personalized.score_display_0_10 ?? personalized.score_0_10,
      signal: personalized.signal,
      signal_label: getSignalLabel(personalized.signal),
      delta_vs_today: null,
    },

    summary: {
      main_trigger: personalized.main_trigger,
      trigger_dir: personalized.trigger_dir,
      main_trigger_exact: personalized.personal_main_trigger_exact,
      personal_main_trigger_exact: personalized.personal_main_trigger_exact,
      secondary_trigger_exact: personalized.personal_secondary_trigger_exact || null,
      personal_secondary_trigger_exact: personalized.personal_secondary_trigger_exact || null,
      personal_main_event_key: personalized.personal_main_event_key || null,
      secondary_trigger: secondaryCompat?.main_trigger || null,
      secondary_trigger_dir: secondaryCompat?.trigger_dir || null,
      main_trigger_label: getExactPresentationLabel(
        personalized.personal_main_trigger_exact,
        personalized.trigger_dir
      ),
      secondary_trigger_label: personalized.personal_secondary_trigger_exact
        ? getExactPresentationLabel(
            personalized.personal_secondary_trigger_exact,
            secondaryCompat?.trigger_dir
          )
        : null,
      trigger_factors: triggerFactors,
      reaction_direction: personalized?.meta?.manifestation?.reaction_direction || null,
      pressure_response_direction: pressureResponseDirection,
      pressure_direction: weatherStress.pressure_direction || null,
      moisture_state: weatherStress.moisture_state || weatherStress.meta?.moisture?.state || null,
      manifestation_summary: manifestationSummary,
      material_ranking: personalized?.meta?.manifestation?.material_ranking || [],
      weather_load_groups: personalized?.meta?.weather_load_groups || null,
      environmental_cautions: Array.isArray(weatherStress.environmental_cautions)
        ? weatherStress.environmental_cautions
        : [],

      weather_main_trigger: weatherMainCompat.main_trigger,
      weather_trigger_dir: weatherMainCompat.trigger_dir,
      weather_main_trigger_exact: weatherStress.weather_main_trigger_exact || null,
      weather_main_trigger_label: getTriggerLabel(
        weatherMainCompat.main_trigger,
        weatherMainCompat.trigger_dir
      ),

      peak_start: personalized.active_peak_start,
      peak_end: personalized.active_peak_end,
      channel_peaks: personalized?.meta?.channel_peaks || null,

      full_day_peak_start: personalized.full_day_peak_start,
      full_day_peak_end: personalized.full_day_peak_end,
    },

    weather_context: {
      pressure_down_strength: weatherStress.pressure_down_strength ?? 0,
      pressure_up_strength: weatherStress.pressure_up_strength ?? 0,
      cold_strength: weatherStress.cold_strength ?? 0,
      heat_strength: weatherStress.heat_strength ?? 0,
      damp_strength: weatherStress.damp_strength ?? 0,
      dry_strength: weatherStress.dry_strength ?? 0,
      pressure_shift_strength: weatherStress.pressure_shift_strength ?? 0,
      pressure_direction: weatherStress.pressure_direction || null,
      pressure_presentation_direction: weatherStress.pressure_presentation_direction || null,
      pressure_reversal: Boolean(weatherStress.pressure_reversal),
      temperature_shift_strength: weatherStress.temperature_shift_strength ?? 0,
      temperature_direction: weatherStress.temperature_direction || null,
      moisture_shift_strength: weatherStress.moisture_shift_strength ?? 0,
      moisture_direction: weatherStress.moisture_direction || null,
      moisture_state: weatherStress.moisture_state || weatherStress.meta?.moisture?.state || null,
      pressure_response_direction: pressureResponseDirection,
      reaction_direction: pressureResponseDirection,
      environmental_cautions: Array.isArray(weatherStress.environmental_cautions)
        ? weatherStress.environmental_cautions
        : [],

      channel_strengths: weatherStress.channel_strengths || null,
      event_strengths: weatherStress.event_strengths || null,
      channel_peaks: weatherStress.channel_peaks || personalized?.meta?.channel_peaks || null,

      weather_main_trigger_exact: weatherStress.weather_main_trigger_exact || null,
      personal_main_trigger_exact: personalized.personal_main_trigger_exact || null,
      personal_secondary_trigger_exact: personalized.personal_secondary_trigger_exact || null,
      trigger_factors: triggerFactors,
      weather_load_groups: personalized?.meta?.weather_load_groups || null,
      effective_weather_groups: personalized?.meta?.effective_weather_groups || null,

      main_trigger: weatherStress.main_trigger,
      trigger_dir: weatherStress.trigger_dir,

      active_peak_start: weatherStress.active_peak_start,
      active_peak_end: weatherStress.active_peak_end,
      full_day_peak_start: weatherStress.full_day_peak_start,
      full_day_peak_end: weatherStress.full_day_peak_end,
    },

    constitution_context: {
      core_code: profile.core_code || null,
      sub_labels: Array.isArray(profile.sub_labels) ? profile.sub_labels : [],
      symptom_focus: profile.symptom_focus || null,
      env: profile.env || { sensitivity: 0, vectors: [] },
      axes: profile.axes || null,
      split_scores: profile.split_scores || null,
      material_scores: profile.material_scores || null,
      score_scale: profile.score_scale || profile.split_scores?.scale || null,
      manifestation: personalized?.meta?.manifestation || null,
      review_feedback_summary: summarizeReviewFeedback(profile.review_feedback),
      primary_meridian: profile.primary_meridian || null,
      secondary_meridian: profile.secondary_meridian || null,
    },

    risk_factors: riskFactors,

    tcm_context: tcmContext,

    mtest_context: {
      selected_line: selectedLine.selected_line,
      selected_from: selectedLine.selected_from,
      selected_reason: selectedLine.reason,
      selected_scores: selectedLine.scores || null,

      mode: motherChild.mode,
      mode_reason: motherChild.reason,
      mode_meta: motherChild.meta,
    },

    care_tone: careTone,

    meta: {
      forecast_model_version:
        personalized?.meta?.forecast_model_version || personalized?.model_version || (useV2 ? "v2" : "v1"),
      personalized_meta: personalized.meta || null,
      weather_meta: weatherStress.meta || null,
    },
  };
}

function buildTriggerFactorsForSummary(factors, primaryExact, secondaryExact, pressureResponseDirection = "balanced") {
  const source = Array.isArray(factors) ? factors : [];
  const normalized = source
    .filter((item) => item?.exact || item?.key)
    .slice(0, 2)
    .map((item, index) => {
      const exact = item.exact || item.key;
      const compat = exactTriggerToCompat(exact);
      return {
        ...item,
        key: exact,
        exact,
        role: item.role || (index === 0 ? "primary" : "secondary"),
        main_trigger: item.main_trigger || compat.main_trigger,
        trigger_dir: item.trigger_dir || compat.trigger_dir,
        label: getExactPresentationLabel(
          exact,
          item.direction || item.trigger_dir || compat.trigger_dir
        ),
        response_direction: item.response_direction || (
          exact === "pressure_down" || exact === "pressure_up"
            ? readPressureResponseDirection(item)
            : null
        ),
      };
    });

  if (!normalized.length && primaryExact) {
    const compat = exactTriggerToCompat(primaryExact);
    normalized.push({
      key: primaryExact,
      exact: primaryExact,
      role: "primary",
      main_trigger: compat.main_trigger,
      trigger_dir: compat.trigger_dir,
      label: getExactPresentationLabel(primaryExact, compat.trigger_dir),
      response_direction: primaryExact === "pressure_down" || primaryExact === "pressure_up"
        ? pressureResponseDirection
        : null,
      body_response_key: primaryExact === "pressure_down" || primaryExact === "pressure_up"
        ? `pressure_${pressureResponseDirection}`
        : primaryExact,
    });
  }

  if (secondaryExact && !normalized.some((item) => item.exact === secondaryExact)) {
    const compat = exactTriggerToCompat(secondaryExact);
    normalized.push({
      key: secondaryExact,
      exact: secondaryExact,
      role: "secondary",
      main_trigger: compat.main_trigger,
      trigger_dir: compat.trigger_dir,
      label: getExactPresentationLabel(secondaryExact, compat.trigger_dir),
      response_direction: secondaryExact === "pressure_down" || secondaryExact === "pressure_up"
        ? pressureResponseDirection
        : null,
      body_response_key: secondaryExact === "pressure_down" || secondaryExact === "pressure_up"
        ? `pressure_${pressureResponseDirection}`
        : secondaryExact,
    });
  }

  return normalized.slice(0, 2);
}

function getExactPresentationLabel(exact, direction = null) {
  if (exact === "cold") return "低温の日";
  if (exact === "heat") return "高温の日";
  if (exact === "temp_shift" || exact === "temperature_shift") {
    if (direction === "down") return "気温が下がる日";
    if (direction === "up") return "気温が上がる日";
    return "寒暖差が大きい日";
  }
  const compat = exactTriggerToCompat(exact);
  return getTriggerLabel(compat.main_trigger, compat.trigger_dir);
}

function summarizeReviewFeedback(reviewFeedback) {
  if (!reviewFeedback) return null;

  return {
    observed_review_days: Number(reviewFeedback.observed_review_days ?? 0),
    bad_reviewed_days: Number(reviewFeedback.bad_reviewed_days ?? 0),
    baseline_bad_ratio: Number(reviewFeedback.baseline_bad_ratio ?? 0),
    top_channels: Array.isArray(reviewFeedback.top_channels)
      ? reviewFeedback.top_channels.slice(0, 3)
      : [],
    per_channel_bonus: reviewFeedback.per_channel_bonus || null,
  };
}

function buildRiskFactors({ personalized, profile }) {
  const out = [];
  const labels = Array.isArray(profile?.sub_labels) ? profile.sub_labels : [];
  const coreCode = profile?.core_code || "";

  const channels = personalized?.meta?.personal_channel_strengths || {};
  const entries = Object.entries(channels)
    .filter(([, value]) => Number(value) > 0.12)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3);

  for (const [type, weight] of entries) {
    out.push({
      type,
      weight: round3(weight),
      matched_to: matchRiskFactorToProfile(type, labels, coreCode),
    });
  }

  return out;
}

function matchRiskFactorToProfile(type, labels, coreCode) {
  const matched = [];

  const isAccel = coreCode.startsWith("accel");
  const isBrake = coreCode.startsWith("brake");
  const isBattSmall = coreCode.includes("batt_small");

  if (type === "pressure_down" || type === "pressure_up") {
    labels.slice(0, 2).forEach((label) => matched.push(label));
    if (coreCode) matched.push(coreCode);
  }

  if (type === "cold") {
    if (labels.includes("qi_deficiency")) matched.push("qi_deficiency");
    if (labels.includes("blood_deficiency")) matched.push("blood_deficiency");
    if (labels.includes("blood_stasis")) matched.push("blood_stasis");
    if (isBrake) matched.push(coreCode);
    if (isBattSmall) matched.push("batt_small");
  }

  if (type === "heat") {
    if (labels.includes("qi_stagnation")) matched.push("qi_stagnation");
    if (labels.includes("fluid_deficiency")) matched.push("fluid_deficiency");
    if (isAccel) matched.push(coreCode);
  }

  if (type === "damp") {
    if (labels.includes("fluid_damp")) matched.push("fluid_damp");
    if (labels.includes("qi_deficiency")) matched.push("qi_deficiency");
    if (isBrake) matched.push(coreCode);
  }

  if (type === "dry") {
    if (labels.includes("fluid_deficiency")) matched.push("fluid_deficiency");
    if (labels.includes("blood_deficiency")) matched.push("blood_deficiency");
    if (isBattSmall) matched.push("batt_small");
  }

  if (type === "temp_shift") {
    labels.slice(0, 2).forEach((label) => matched.push(label));
    if (coreCode) matched.push(coreCode);
  }

  return uniq(matched);
}

function buildCareTone({ profile, personalized }) {
  const labels = Array.isArray(profile?.sub_labels) ? profile.sub_labels : [];
  const coreCode = profile?.core_code || "";
  const score = personalized?.score_0_10 ?? 0;
  const personalExact = personalized?.personal_main_trigger_exact || "none";
  const reactionDirection = personalized?.meta?.manifestation?.reaction_direction || null;

  const hasDeficiency =
    labels.includes("qi_deficiency") ||
    labels.includes("blood_deficiency") ||
    labels.includes("fluid_deficiency");

  if (coreCode.includes("batt_small") || hasDeficiency) {
    if (score >= 7) return "supportive_strong";
    return "supportive";
  }

  if (personalExact === "pressure_down" || personalExact === "pressure_up") {
    if (reactionDirection === "accel") return "release_and_cool";
    if (reactionDirection === "brake") return "lighten_and_drain";
  }

  if (personalExact === "damp" || personalExact === "pressure_down") {
    return "lighten_and_drain";
  }

  if (
    personalExact === "heat" ||
    personalExact === "dry" ||
    personalExact === "pressure_up"
  ) {
    return "release_and_cool";
  }

  return "balancing";
}

function uniq(arr) {
  return [...new Set((arr || []).filter(Boolean))];
}

function buildManifestationSummary(manifestation) {
  if (!manifestation) return null;
  const direction = manifestation.reaction_direction;
  const directionText = direction === "accel"
    ? "張りや力み、切り替えにくさとして表れやすい見込みです。"
    : direction === "brake"
      ? "重だるさや動き出しにくさとして表れやすい見込みです。"
      : "張りと重だるさの両方に気を配りたい見込みです。";
  const materialLabel = {
    qi_deficiency: "疲れや消耗",
    qi_stagnation: "張りや詰まり感",
    blood_deficiency: "休みに入りにくさ",
    blood_stasis: "固定したこわばり",
    fluid_deficiency: "乾きや消耗",
    fluid_damp: "頭重感やむくみ",
  }[manifestation?.material_ranking?.[0]?.key];
  return materialLabel ? `${directionText.replace(/。$/, "")}。特に${materialLabel}に注意します。` : directionText;
}

function round3(v) {
  return Math.round(v * 1000) / 1000;
}
