...
export function buildRiskContext({ profile, weatherStress, forecastModelVersion = null }) {
  ...
  return {
    ...
    summary: {
      ...
      secondary_trigger_label: secondaryCompat
        ? getTriggerLabel(secondaryCompat.main_trigger, secondaryCompat.trigger_dir)
        : null,
...
function buildTriggerFactorsForSummary(factors, primaryExact, secondaryExact) {
  ...
  if (!normalized.length && primaryExact) {
    ...
    normalized.push({
      ...
      label: getTriggerLabel(compat.main_trigger, compat.trigger_dir),
    });
  }
