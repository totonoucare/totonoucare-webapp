...
export function buildWeatherStress({ points }) {
  ...
  const heatStrength = computeHeatStrength(validTemp, tempTrend);
  ...
  const timeline = points.map((p, idx) => {
    ...
    const heatBurden = pointHeatBurden(p.temp_c, validTemp);
...
// ★ チューニング1：気圧の感受性をアップ（12hPa → 8hPaでMAX）
...
function computeColdStrength(points, tempTrend) {
  ...
  const dayRange = maxT - minT; // 日較差

  const lowTempComponent = clamp01((12 - minT) / 15);
  const trendComponent = tempTrend.dir === "down" ? clamp01(Math.abs(tempTrend.delta) / 6) : 0;
  
  // 寒暖差（日較差）の負担：5℃以上でストレスになり、15℃差でMAX
  const rangeComponent = clamp01((dayRange - 5) / 10);

  // 絶対的な寒さ(50%) + 前日比トレンド(25%) + 日較差(25%)
  return clamp01(lowTempComponent * 0.50 + trendComponent * 0.25 + rangeComponent * 0.25);
}

// ★ チューニング2：寒暖差（日較差）を加味
function computeHeatStrength(points, tempTrend) {
  if (!points.length) return 0;
  ...
  const trendComponent = tempTrend.dir === "up" ? clamp01(Math.abs(tempTrend.delta) / 6) : 0;
  
  // 寒暖差（日較差）の負担：5℃以上でストレスになり、15℃差でMAX
  const rangeComponent = clamp01((dayRange - 5) / 10);

  // 絶対的な暑さ(50%) + 前日比トレンド(25%) + 日較差(25%)
  return clamp01(highTempComponent * 0.50 + trendComponent * 0.25 + rangeComponent * 0.25);
}
...
function pointColdBurden(temp, validTempPoints) {
  ...
  return clamp01(relative * 0.45 + absolute * 0.55);
}

function pointHeatBurden(temp, validTempPoints) {
  if (!isNum(temp) || !validTempPoints.length) return 0;
  const minT = minNum(validTempPoints.map((p) => p.temp_c));
  const maxT = maxNum(validTempPoints.map((p) => p.temp_c));
  const range = Math.max(1, maxT - minT);
