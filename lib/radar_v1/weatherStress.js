 ...
 * - dry
 *
 * v1.4 day-only + tomorrow previous-night bridge policy:
 * - points: targetDate 00:00〜23:59 JST only.
 * - Today's forecast stays day-only / future-only.
 * - Tomorrow forecasts may pass previousNightBridgePoints, extracted from the same
 *   future forecast, to catch today evening→tomorrow morning changes.
 * - The bridge only gives a modest boost to daily channel strengths and early
 *   timeline burdens; it does not rewrite the target day's points.
 */
...
// For timeline/peak detection, do not use 24h windows.
// 24h windows are useful for the daily score, but they can create odd peak times
// by comparing tomorrow afternoon with today afternoon.
const PRESSURE_DOWN_TIMELINE_WINDOWS = [
  { hours: 3, threshold: 3.0, weight: 1.0 },
  { hours: 6, threshold: 5.0, weight: 1.0 },
  { hours: 12, threshold: 7.0, weight: 0.85 },
];

const PRESSURE_UP_TIMELINE_WINDOWS = [
  { hours: 3, threshold: 3.5, weight: 0.95 },
  { hours: 6, threshold: 5.5, weight: 0.95 },
  { hours: 12, threshold: 8.0, weight: 0.8 },
];

const TEMP_TIMELINE_WINDOWS = [
  { hours: 3, threshold: 3.0, weight: 1.0 },
  { hours: 6, threshold: 5.0, weight: 1.0 },
  { hours: 12, threshold: 7.0, weight: 0.75 },
];

const HUMIDITY_TIMELINE_WINDOWS = [
  { hours: 3, threshold: 12.0, weight: 1.0 },
  { hours: 6, threshold: 18.0, weight: 1.0 },
  { hours: 12, threshold: 25.0, weight: 0.75 },
];

/**
 * @param {{ points: WeatherPoint[], contextPoints?: WeatherPoint[], previousNightBridgePoints?: WeatherPoint[] }} args
 */
export function buildWeatherStress({
  points,
  contextPoints = null,
  previousNightBridgePoints = null,
}) {
  ...
  // contextPoints remains in the signature for backward compatibility with older
  // call sites, but today's / default forecast intentionally remains day-only.
  void contextPoints;

  const nightBridgePoints = sortPoints(previousNightBridgePoints);
  const bridgeTimelineContext = nightBridgePoints.length
    ? mergePointsByTs([...nightBridgePoints, ...dayPoints])
    : dayPoints;

  const validPressure = dayPoints.filter((p) => isNum(p.pressure_hpa));
  const validPressureTimelineContext = bridgeTimelineContext.filter((p) => isNum(p.pressure_hpa));
  const validTempDay = dayPoints.filter((p) => isNum(p.temp_c));
  const validTempContext = bridgeTimelineContext.filter((p) => isNum(p.temp_c));
  const validHumidity = dayPoints.filter((p) => isNum(p.humidity_pct));
  const validHumidityTimelineContext = bridgeTimelineContext.filter((p) => isNum(p.humidity_pct));

  const dayRange = computeDayTempRange(validTempDay);
  const tempTrend = computeTempTrendByRolling(validTempDay);
  ...
  const nightBridge = buildNightBridgeStress(nightBridgePoints);

  const timeline = dayPoints.map((p) => {
    const pressureDownBurden = pointRollingDeltaBurden({
      points: validPressureTimelineContext,
      currentPoint: p,
      field: "pressure_hpa",
      mode: "down",
      windows: PRESSURE_DOWN_TIMELINE_WINDOWS,
    });
    ...
    const pressureUpBurden = pointRollingDeltaBurden({
      ...
      field: "pressure_hpa",
      mode: "up",
      windows: PRESSURE_UP_TIMELINE_WINDOWS,
    });
    ...
    const dampBurden = pointDampBurden({
      currentPoint: p,
      humidityPoints: validHumidityTimelineContext,
    });
  ...
  const channelStrengths = mergeNightBridgeChannelStrengths(
    dayOnlyChannelStrengths,
    nightBridge.channel_strengths
  );
  ...
  return {
    ...
    meta: {
      extraction_version: "radar_v1_weather_day_only_plus_tomorrow_night_bridge_2026-05-06",
      point_count: dayPoints.length,
      context_point_count: bridgeTimelineContext.length,
      ...
      context_policy: nightBridge.applied
        ? "day_only_plus_previous_night_bridge"
        : "day_only",
      night_bridge: {
        applied: nightBridge.applied,
        point_count: nightBridge.point_count,
        policy: nightBridge.applied
          ? "previous_day_18_to_target_day_09_future_forecast_only"
          : null,
        channel_strengths: nightBridge.channel_strengths,
        day_only_channel_strengths: dayOnlyChannelStrengths,
        merged_channel_strengths: channelStrengths,
      },
...
function buildNightBridgeStress(points) {
  const bridgePoints = sortPoints(points);
  if (bridgePoints.length < 2) {
    return {
      applied: false,
      point_count: bridgePoints.length,
      channel_strengths: emptyChannelStrengths(),
    };
  }

  const validPressure = bridgePoints.filter((p) => isNum(p.pressure_hpa));
  const validTemp = bridgePoints.filter((p) => isNum(p.temp_c));
  const validHumidity = bridgePoints.filter((p) => isNum(p.humidity_pct));
  ...
  const bridgeRange = computeDayTempRange(validTemp);
  const channelStrengths = {
    pressure_down: round3(pressureDown.strength),
    pressure_up: round3(pressureUp.strength),
    cold: round3(
      computeColdStrength({
        dayTempPoints: validTemp,
        dayRange: bridgeRange,
        tempFallStrength: tempFall.strength,
      })
    ),
    heat: round3(
  ...
  const applied = Object.values(channelStrengths).some((value) => value >= 0.12);
...
function mergeNightBridgeChannelStrengths(dayOnly, bridge) {
  const out = {};
  const bridgeStrengths = bridge || emptyChannelStrengths();

  for (const key of ["pressure_down", "pressure_up", "cold", "heat", "damp", "dry"]) {
    const base = clamp01(Number(dayOnly?.[key] || 0));
    const bridgeValue = clamp01(Number(bridgeStrengths?.[key] || 0));
    ...
    // Modest boost: enough for a strong overnight change to appear as a
    // secondary factor, but not enough to rewrite the whole target day.
    const boosted = Math.max(base, base * 0.82 + bridgeValue * 0.28);
    out[key] = round3(clamp01(boosted));
  }

  return out;
}

function emptyChannelStrengths() {
  return {
    pressure_down: 0,
    pressure_up: 0,
    cold: 0,
...
function pointDampBurden({ currentPoint, humidityPoints }) {
  ...
  const rise = pointRollingDeltaBurden({
    ...
    field: "humidity_pct",
    mode: "up",
    windows: HUMIDITY_TIMELINE_WINDOWS,
  });

  return clamp01(currentIntensity * 0.65 + rise * 0.35);
}
...
function pointDryBurden({ currentPoint, humidityPoints }) {
  ...
  return clamp01(currentIntensity * 0.65 + drop * 0.35);
}

function buildChannelPeaks(timeline, fallback) {
  const out = {};
  for (const key of ["pressure_down", "pressure_up", "cold", "heat", "damp", "dry"]) {
    out[key] = findPeakWindowForChannel(timeline, key, fallback);
  }
  return out;
}

function findPeakWindowForChannel(rows, key, fallback = { start: "09:00", end: "12:00" }) {
  ...
  const targetRows = activeRows.length ? activeRows : rows;
  const channelRows = targetRows.map((row) => ({
    ...row,
    total: Number(row?.[key] || 0),
  }));

  const maxValue = Math.max(...channelRows.map((row) => Number(row.total || 0)));
  if (!Number.isFinite(maxValue) || maxValue <= 0.03) return fallback;

  return findPeakWindow(channelRows, fallback);
}
...
export function exactTriggerToCompat(exact) {
  switch (exact) {
    ...
    case "temp_shift":
      return { main_trigger: "temp", trigger_dir: "change" };
    case "damp":
...
function mergePointsByTs(points) {
  const map = new Map();
  for (const point of Array.isArray(points) ? points : []) {
    if (!point?.ts) continue;
    map.set(point.ts, point);
  }
  return sortPoints([...map.values()]);
}
