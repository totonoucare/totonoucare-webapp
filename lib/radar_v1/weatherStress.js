 ...
 * - dry
 *
 * v1.3 day-only policy:
 * - points: targetDate 00:00〜23:59 JST only.
 * - Do not use previous-day context. This keeps the forecast easy to explain
 *   and avoids day-over-day deltas showing up as odd intraday peak times.
 * - daily channel strengths use rolling windows inside the target day.
 * - peak-time detection also uses target-day points only.
 */
...
export function buildWeatherStress({ points, contextPoints = null }) {
  ...
  // Day-only policy:
  // contextPoints is intentionally ignored for now. It remains in the signature
  // for backward compatibility with older call sites, but all weather burden
  // calculations use only the target day's points.
  void contextPoints;

  const validPressure = dayPoints.filter((p) => isNum(p.pressure_hpa));
  const validTempDay = dayPoints.filter((p) => isNum(p.temp_c));
  const validTempContext = validTempDay;
  ...
  return {
    ...
    meta: {
      extraction_version: "radar_v1_weather_day_only_rolling_2026-04-24",
      point_count: dayPoints.length,
      context_point_count: dayPoints.length,
