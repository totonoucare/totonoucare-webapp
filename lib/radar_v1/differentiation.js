...
export function differentiateForTcm({
  ...
  return {
    ...
    meta: {
      // exact_trigger is the trigger actually used for TCM scoring.
      // Prefer the personalized trigger because care should follow the user's body-weather match,
      // not just the largest raw weather change.
      exact_trigger: ctx.exactTrigger,
      personal_main_trigger_exact: ctx.personalExactTrigger,
      weather_main_trigger_exact: ctx.weatherExactTrigger,
      exact_trigger_source: ctx.personalExactTrigger ? "personal" : ctx.weatherExactTrigger ? "weather" : null,
      ...
      pressure_response_direction: ctx.pressureResponseDirection,
...
function buildContext(core_code, weatherStress, symptom_focus) {
  ...
  const hasPressureResponse = hasExplicitPressureResponseDirection(weatherStress);
  const pressureResponseDirection = readPressureResponseDirection(weatherStress);
  const personalExactTrigger = weatherStress?.personal_main_trigger_exact || null;
  const weatherExactTrigger = weatherStress?.weather_main_trigger_exact || null;
  const exactTrigger = personalExactTrigger || weatherExactTrigger || null;

  return {
    exactTrigger,
    personalExactTrigger,
    weatherExactTrigger,
    ...
    strengths: {
      ...
      pressure_shift: Number(
        weatherStress?.pressure_shift_strength ||
        Math.max(
          Number(weatherStress?.pressure_down_strength || 0),
          Number(weatherStress?.pressure_up_strength || 0)
        )
      ),
      temp_shift: Number(weatherStress?.temperature_shift_strength || 0),
    ...
    isAccel: hasPressureResponse ? pressureResponseDirection === "accel" : code.startsWith("accel_"),
    isBrake: hasPressureResponse ? pressureResponseDirection === "brake" : code.startsWith("brake_"),
  };
}
...
function scoreLabel(label, rankIndex, ctx) {
  ...
  switch (label) {
    case "qi_deficiency": {
      score += s.pressure_shift * 0.12;
      score += s.temp_shift * 0.08;
      score += s.cold * 0.22;
      score += s.damp * 0.10;
      score += s.dry * 0.02;
      ...
      if (ctx.symptomFocus === "digestion") score += 0.12;
...
function getTriggerPreferredSecondaryLabels(ctx) {
  switch (ctx.exactTrigger) {
    ...
    case "temp_shift":
      return ctx.isAccel
        ? ["qi_stagnation", "blood_stasis", "qi_deficiency"]
        : ["qi_deficiency", "fluid_damp", "blood_stasis"];
    default:
      return [];
  }
...
function buildActions(label, ctx, topLabels, slot = "primary") {
  ...
  if (
    (ctx.exactTrigger === "pressure_down" || ctx.exactTrigger === "pressure_up") &&
    ctx.isBattSmall &&
...
function buildOrganFocus(topLabels, ctx) {
  ...
  switch (ctx.exactTrigger) {
    ...
    case "pressure_down":
    case "pressure_up":
      if (ctx.isAccel) scores.liver += 0.42;
      else {
        scores.kidney += 0.30;
        scores.spleen += 0.42;
      }
