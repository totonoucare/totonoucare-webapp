...
function buildForecastTruthPromptSection(promptContext, purpose = "general") {
  ...
  const bodyKeywords = Array.isArray(truth.body_expression_keywords)
    ? truth.body_expression_keywords
    : [];
...
${bodyGuidance ? `- 体質・主訴への出方: ${bodyGuidance}` : ""}
${bodyKeywords.length ? `- 出方の表現候補: ${bodyKeywords.join("、")}` : ""}
...
# 書き方の方針
- 文の骨格は「主因 → 体質・主訴への出方 → 注意時間帯」の順にする
- 1文目では forecast_truth.primary_label を主因として明示する
- 2文目、または同じ文の後半で、forecast_truth.body_expression_guidance を参考に「その人に出やすい形」を1つだけ添える
...
- forecast_truth.body_expression_guidance を参考に、体質・主訴への出方を1つだけ食養生の理由に反映する
- ほかの気象要素を新しい主因として追加しない
...
- forecast_truth.body_expression_guidance を参考に、主因がその人にどう現れやすいかを説明してください
