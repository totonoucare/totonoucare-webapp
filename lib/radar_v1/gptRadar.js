...
const RELATIVE_DATE_EXPRESSION_PATTERN =
  /(今日|本日|明日|あした|あす|今夜|今晩|今朝|明朝|翌朝|翌日|昨日|昨夜|前夜)/u;

function hasRelativeDateExpression(text) {
  return RELATIVE_DATE_EXPRESSION_PATTERN.test(String(text || ""));
}

function buildDateExpressionPromptSection(promptContext) {
  const label = promptContext?.time_context?.target_date_label || "対象日";
  const prohibited = Array.isArray(promptContext?.time_context?.prohibited_relative_date_words)
    ? promptContext.time_context.prohibited_relative_date_words
    : ["今日", "本日", "明日", "あした", "あす", "今夜", "今晩", "今朝", "明朝", "翌朝", "翌日", "昨日", "昨夜", "前夜"];

  return [
    "# 日付表現のルール",
    "- この文章は、前日夜に作られた対象日の予報が、対象日当日にも再利用される前提で保存されます。",
    "- 本文では『今日』『明日』『今夜』など、閲覧タイミングで意味が変わる相対的な日付表現を使わないでください。",
    `- 禁止語: ${prohibited.join("、")}` ,
    `- 対象日を示す場合は「${label}」または「この日」を使ってください。`,
    "- UI側の見出しや通知で閲覧タイミングに応じた表現を補うため、本文内では重ねて表現しないでください。",
    "- 『朝』『昼』『午後』『夕方』『夜』『就寝前』など、対象日内の時間帯を示す表現は使って構いません。",
  ].join("\n");
}
...
function buildForecastTruthPromptSection(promptContext, purpose = "general") {
  ...
  const secondaryExact = truth.secondary_exact || forecast.personal_secondary_trigger_exact || null;
  const secondaryLabel = truth.secondary_label || forecast.personal_secondary_trigger_label || null;
  const secondaryEffect = truth.secondary_effect || null;
  const secondaryGuidance = truth.secondary_body_expression_guidance || null;
  ...
  const allowed = Array.isArray(truth.allowed_sub_effects)
    ...
    : [];
  const bodyGuidance = truth.body_expression_guidance || null;
  const pressureResponseDirection = truth.pressure_response_direction || forecast.pressure_response_direction || null;
  const purposeRule =
    purpose === "food"
      ? "食養生の理由・おすすめ・避けたい方向は主因に合わせる。副因がある場合だけ、味付け・食材選び・避けたい方向に1つ補助ニュアンスとして混ぜてよい。"
      : purpose === "tsubo"
        ? "ツボの overall_reason と各 selection_reason も、主因を軸にする。副因がある場合だけ、2点目や補助説明の理由として軽く使ってよい。選穴理由を東洋医学的に補足してよいが、主因を別の気象要素へ変えない。"
        : "注意点の本文は、この主因を中心に書く。副因がある場合は、1フレーズだけ添えてよい。";
...
${secondaryExact ? `- 今回は副因として「${secondaryLabel}」（${secondaryExact}）も少し拾ってよいです。ただし、主因扱いにしないでください。` : "- 今回は副因が弱いため、別の気象要素を無理に足さないでください。"}
${peak ? `- 天気ストレスが強まる時間帯は ${peak} です。これは天気ストレスのピークであり、症状が出る時刻ではありません。症状発現時刻として書かないでください。` : ""}
- weather_main_trigger_exact${weatherExact ? `（${weatherLabel} / ${weatherExact}）` : ""} は天気単体で目立つ変化です。本人向けの主因とは限らないので、本文の主因にしないでください。
- 気圧が主因の場合、up/down は物理方向だけを表します。体への表れ方は pressure_response_direction=${pressureResponseDirection || "balanced"} に従い、低下=重さ、上昇=張りと決め直さないでください。
- tcm_context、sub_labels、symptom_focus は、主因を説明するための補助情報です。これらから別の気象主因を再推論しないでください。
- ${purposeRule}
${truth.main_effect ? `- 主因の解釈: ${truth.main_effect}` : ""}
${bodyGuidance ? `- 体質・主訴への出方の素材: ${bodyGuidance}` : ""}
${secondaryEffect ? `- 副因の解釈: ${secondaryEffect}` : ""}
${secondaryGuidance ? `- 副因を使う場合の素材: ${secondaryGuidance}` : ""}
${allowed.length ? `- 使ってよい副次テーマ: ${allowed.join("、")}` : ""}
${forbidden.length ? `- 禁止: ${forbidden.join("。")}` : ""}
- 上の「出方の素材」は意味だけ参考にしてください。語句をそのままなぞらず、対象日の主因・時間帯・主訴に合わせて自然な文に組み立ててください。
- 体質・主訴は、主因がその人にどう現れやすいかを説明するために使ってください。副因がある場合も、主因に添える補助要素に留めてください。
...
function hasPrimaryTriggerContradiction(text, promptContext) {
  ...
  if (exact === "heat") {
    return /(冷え込みを主因|冷えが主因|寒さを主因|冷えを中心)/u.test(t);
  }
  if (exact === "cold") {
    return /(気温上昇が主因|暑さが主因|熱こもりが主因|暑さを中心)/u.test(t);
  }
  if (exact === "damp") {
    return /(乾燥が主因|乾きが主因|乾燥を中心)/u.test(t);
  }
  if (exact === "dry") {
    return /(湿っぽさが主因|湿が主因|湿気を中心|湿を中心)/u.test(t);
  }
  if (exact === "pressure_down") {
    return /(気圧上昇が主因|上がる気圧が主因|気圧上昇を中心)/u.test(t);
  }
  if (exact === "pressure_up") {
    return /(気圧低下が主因|下がる気圧が主因|気圧低下を中心)/u.test(t);
  }
...
function buildFallbackSummary(promptContext) {
  ...
  const manifestation = String(promptContext?.forecast?.manifestation_summary || "").trim();

  if (exact === "heat") {
    const tail = has("qi_stagnation")
      ? "張りつめや巡りの詰まりが上に出やすい日です。"
      : "内側に熱っぽさやこもり感が出やすい日です。";
    return `・${label}は${peak}高温の負担があり、${tail}`;
  }
  ...
  if (exact === "cold") {
    const tail = has("qi_deficiency") || has("fluid_damp")
      ...
      : "体がこわばり、巡りが鈍りやすい日です。";
    return `・${label}は${peak}低温の負担があり、${tail}`;
  }

  if (exact === "temp_shift") {
    return `・${label}は${peak}寒暖差が大きく、${manifestation || "こわばりや適応の疲れが出やすい日です。"}`;
  }

  if (exact === "pressure_down") {
    return `・${label}は${peak}気圧が下がる方向の変化で体が揺さぶられ、${manifestation || "体質に沿った張りや重さが出やすい日です。"}`;
  }

  if (exact === "pressure_up") {
    return `・${label}は${peak}気圧が上がる方向の変化で体が揺さぶられ、${manifestation || "体質に沿った張りや重さが出やすい日です。"}`;
  }
...
function buildFallbackFood(promptContext) {
  const label = promptContext?.time_context?.target_date_label || "対象日";
  const truth = promptContext?.forecast_truth || {};
  const exact = truth.primary_exact || promptContext?.forecast?.personal_main_trigger_exact || "none";
  const peak = truth.peak_time ? `${truth.peak_time}ごろ` : "負担が出やすい時間帯";

  const base = {
    title: "食養生",
    timing: "昼",
    recommendation: `${label}は、胃腸に負担をかけにくい温かい食事を軸に整えるのがおすすめです。`,
    how_to: "一度にたくさん食べるより、腹八分目で消化しやすい形にすると取り入れやすいです。",
    avoid: "冷たいものや脂っこいものを重ねすぎないようにしましょう。",
    reason: `${peak}に負担が出やすい流れなので、食事では余分な重さを増やさず、体の立ち上がりを邪魔しないことを優先します。`,
    examples: ["具だくさん味噌汁", "温かい雑炊", "蒸し野菜"],
    lifestyle_tip: "食後は詰め込みすぎず、少し余白を残して過ごすと整えやすくなります。",
  };
  ...
  if (exact === "heat") {
    return {
      ...base,
      timing: "昼",
      recommendation: `${label}は、熱っぽさや張りつめを増やしにくい、軽めでみずみずしい食べ方がおすすめです。`,
      how_to: "油っこさや辛味を控えめにして、野菜や汁物を足すと無理なく整えやすくなります。",
      avoid: "辛いもの、揚げ物、アルコールなど、こもり感を強めやすいものは控えめに。",
      reason: `${peak}に上がりやすさや張りつめが出やすい流れなので、食事では内側に熱をため込みすぎないことを優先します。`,
      examples: ["豆腐と野菜のスープ", "きゅうりの和え物", "緑豆春雨のスープ"],
      lifestyle_tip: "食後は予定を詰めすぎず、少しクールダウンする時間を作ると整えやすいです。",
    };
  }
  ...
  if (exact === "cold") {
    return {
      ...base,
      timing: "朝",
      recommendation: `${label}は、低温で動き出しが鈍りやすいため、温かく消化しやすいものを選ぶのがおすすめです。`,
      how_to: "汁物や煮込み料理のように、体を内側からゆるめる形にすると続けやすいです。",
      avoid: "冷たい飲み物、生野菜だけの食事、甘いもののとりすぎは控えめに。",
      reason: `${peak}にこわばりや重だるさが出やすい流れなので、食事では冷やさず、巡りの立ち上がりを助けることを優先します。`,
      examples: ["生姜入り味噌汁", "卵雑炊", "根菜スープ"],
      lifestyle_tip: "足元やお腹を冷やさないようにして、食事の温かさを逃がさない工夫も合います。",
    };
  }
  ...
  if (exact === "pressure_down" || exact === "pressure_up") {
    return {
      ...base,
      timing: "昼",
      recommendation: `${label}は、気圧変化への適応で疲れを増やさない、軽めで食べやすい食事がおすすめです。`,
      how_to: "温かい汁物と主食、たんぱく質を少量ずつそろえると取り入れやすいです。",
      avoid: "食事抜きや、刺激物・甘い物だけで済ませることは控えめに。",
      reason: `${peak}に体質に沿った張りや重さが出やすいため、食事では余力を減らしすぎないことを優先します。`,
      examples: ["具だくさんスープ", "卵雑炊", "温かいそば"],
      lifestyle_tip: "食後は予定を詰めすぎず、少し休憩を入れると整えやすくなります。",
    };
  }
  ...
  if (exact === "damp") {
    return {
      ...base,
      timing: "昼",
      recommendation: `${label}は、重だるさを増やしにくい、温かく軽めの食事がおすすめです。`,
      how_to: "汁物に薬味や香りのある食材を少し足すと、重さをため込みにくくなります。",
      avoid: "甘いもの、乳製品、脂っこいものを重ねすぎると、重さが抜けにくくなることがあります。",
      reason: `${peak}に巡りの停滞や重さが出やすい流れなので、食事ではさばきやすさを優先します。`,
      examples: ["生姜入りスープ", "はと麦入りごはん", "ねぎ入り味噌汁"],
      lifestyle_tip: "食後に軽く歩くなど、体を少し動かす時間を作ると整えやすいです。",
    };
  }
  ...
  if (exact === "dry") {
    return {
      ...base,
      timing: "昼",
      recommendation: `${label}は、乾きやこわばりを増やしにくい、うるおいを意識した食べ方がおすすめです。`,
      how_to: "汁物や煮物を添えて、ぱさつく食事だけで済ませないようにすると取り入れやすいです。",
      avoid: "焼き菓子や辛いものなど、乾きやすさを強めるものは控えめに。",
      reason: `${peak}に乾きやこわばりが出やすい流れなので、食事ではうるおいを削りすぎないことを優先します。`,
      examples: ["豆腐のスープ", "白きくらげのデザート", "梨やりんご"],
      lifestyle_tip: "こまめな水分と、喉や肌の乾きに気づいた時点で早めに整える意識が合います。",
    };
  }
...
export async function generateRadarSummary({
  ...
  const dateExpressionSection = buildDateExpressionPromptSection(promptContext);

  const modeSection = `
# このカードの役割
- ユーザーは対象日の崩れやすさを確認している
- ここでは「なぜその日が注意日か」だけを短く伝える
- ツボや食養生の具体策までは踏み込まない
- 出力は1行の箇条書き
`;
...
「なるほど、それでこの日はこう整えればいいのか」と腑に落ちる言葉にするのが得意です。
...
${dateExpressionSection}

# 書き方の方針
- forecast_truth.primary_label を今回の主因として扱う
- 主因、体質・主訴への出方、天気ストレスが強まる時間帯が読み取れるようにする。ただし語順は自然さを優先する
- 天気ストレスが強まる時間帯は気象現象のピークであり、症状の発生時刻ではない。症状がその時間に出る、悪化すると断定しない
- forecast_truth.body_expression_guidance は意味の素材としてだけ使い、文言をコピーしない
- 体質・主訴への出方は1つに絞る。説明を盛りすぎない
- 主訴がある場合は、その症状にどうつながりやすいかを自然に添える
- forecast.personal_secondary_trigger_exact がある場合だけ、副因として1フレーズ添えてよい。ない場合は、ほかの気象要素を新しい主因として追加しない
- 決まり文句や例文調ではなく、その日の条件から自然に言い換える
- ケア方法や食事の具体策は書かない
- 動物名、内部コード、ラベル名は出さない
- 医療的な断定や過剰な不安あおりはしない
...
`.trim();
  ...
  if (hasPrimaryTriggerContradiction(summary, promptContext) || hasRelativeDateExpression(summary)) {
...
一般ユーザーに食養生を伝えるときは、
難しい理屈だけで終わらせず、
「この日ならこれならやれそう」「それなら食べてみたい」と思える具体性で話します。
...
- forecast_truth.body_expression_guidance は意味の素材としてだけ使い、体質・主訴への出方を1つだけ食養生の理由に反映する
- 与えられた素材の文言をそのままコピーせず、食材・食べ方の説明に合う自然な言葉へ変える
- forecast.personal_secondary_trigger_exact がある場合だけ、副因として1フレーズ添えてよい。ない場合は、ほかの気象要素を新しい主因として追加しない
...
`.trim();
  ...
  const food = {
    title: String(data?.title || "").trim() || "食養生",
  ...
  const generatedFoodText = joinGeneratedFoodText(food);

  if (
    hasPrimaryTriggerContradiction(generatedFoodText, promptContext) ||
    hasRelativeDateExpression(generatedFoodText)
  ) {
    const fallbackFood = buildFallbackFood(promptContext);
    return {
      food: {
        ...fallbackFood,
...
「なぜそのツボが対象日の自分向けなのか」をユーザーが腑に落ちる文章にしてください。
...
- forecast_truth.body_expression_guidance は意味の素材としてだけ使い、主因がその人にどう現れやすいかを自然に説明してください
- 与えられた素材の文言をそのままコピーせず、選ばれたツボの役割に合う言葉へ変えてください
- forecast.personal_secondary_trigger_exact がある場合だけ、副因として補助的に触れてよいです。ない場合は、ほかの気象要素を新しい主因として追加しないでください
...
- 副因がある日は補助要素として使ってよいですが、主因と矛盾する気象要素を、もっともらしく補ってはいけません
...
`.trim();
  ...
  if (
    hasPrimaryTriggerContradiction(generatedText, promptContext) ||
    hasRelativeDateExpression(generatedText) ||
    reasonMap.size === 0
  ) {
    return {
      tsubo_set: {
        ...
        model: DEFAULT_MODEL,
        generated_at: new Date().toISOString(),
        gpt_generation_status: "rejected",
      },
      overall_reason: "",
      model: DEFAULT_MODEL,
      generated_at: new Date().toISOString(),
...
function buildTsuboReasonPromptContext({
  ...
  return {
    ...
    forecast: {
      ...
      personal_secondary_trigger_exact: baseContext?.forecast?.personal_secondary_trigger_exact || null,
      personal_secondary_trigger_label: baseContext?.forecast?.personal_secondary_trigger_label || null,
    ...
    tsubo_set: {
      ...
      points: safeArray(tsuboSet?.points).map((point) => ({
        ...
        care_trigger_exact: point.care_trigger_exact || null,
        care_trigger_role: point.care_trigger_role || null,
