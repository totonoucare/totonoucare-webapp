...
function buildFallbackSummary(promptContext) {
  ...
  if (exact === "pressure_down") {
    if (has("qi_stagnation")) {
      return `・${label}は${peak}気圧低下で巡りの出口が狭くなり、緊張や詰まり感が気分や頭の重さとして出やすい日です。`;
    }
    if (has("blood_deficiency")) {
      return `・${label}は${peak}気圧低下で揺れを受け止めにくく、気分の波やだるさが出やすい日です。`;
    }
...
function buildFallbackFood(promptContext) {
  ...
  if (exact === "heat" || exact === "pressure_up") {
    return {
      ...base,
      timing: "昼",
  ...
  if (exact === "pressure_down" || exact === "damp") {
    return {
      ...base,
      timing: "昼",
