...
export function getTriggerLabel(mainTrigger, triggerDir) {
  ...
  if (mainTrigger === "temp" && ["change", "mixed", "steady"].includes(triggerDir)) {
    return "寒暖差が大きい日";
  }
...
export function getTonightNote(main, dir, tone) {
  if (main === "pressure") {
    if (tone === "release_and_cool") {
      return {
        title: "今夜の注意",
        body: "張りつめたまま過ごすより、少しゆるめて眠りに入りたい日です。力みを残さない意識が合います。",
      };
    }
    if (tone === "lighten_and_drain") {
      return {
        title: "今夜の注意",
        body: "重だるさを持ち越さないよう、同じ姿勢と重い食事を重ねすぎず、少し余白を残して休みたい日です。",
      };
    }
    return {
      title: "今夜の注意",
      body: "夜更かしを避け、予定を詰め込みすぎず、明日の気圧変化に使う余力を残して休みたい日です。",
    };
  }
