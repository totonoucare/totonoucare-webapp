...
function buildChannelAffinities({ subLabels, coreCode, env }) {
  ...
  // --------------------------------------------------
  // core nuance
  // batt_small: 全面一律ではなく、陰側をやや強め・陽側は控えめ
  // batt_large: 全面一律ではなく、陰側中心に少し耐える
  // --------------------------------------------------
  ...
  for (const ch of CHANNELS) {
    a[ch] = round3(clamp(a[ch], 0.85, 1.65));
  }
...
function computeBaseWeatherScore(personalChannels) {
  return clamp(
    personalChannels.pressure_down * 2.05 +
      ...
      personalChannels.heat * 1.50 +
      personalChannels.damp * 1.80 +
