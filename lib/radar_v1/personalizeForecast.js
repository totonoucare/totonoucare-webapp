...
function computeScoreTrace({ effectiveLoads, weatherStrengths, affinityWeights, batteryScalar }) {
  ...
  const channelContributions = [
    { ...top, rank: 1, base_weight: 4.65, importance: CHANNEL_IMPORTANCE[top.key] || 1 },
    { ...second, rank: 2, base_weight: 1.55, importance: CHANNEL_IMPORTANCE[second.key] || 1 },
    { ...third, rank: 3, base_weight: 0.65, importance: CHANNEL_IMPORTANCE[third.key] || 1 },
  ].map((entry) => {
    const contributionRaw = (entry.load || 0) * entry.base_weight * entry.importance;
    return {
      ...entry,
      contribution: round2(contributionRaw),
      contribution_raw: contributionRaw,
    };
  });

  const baseScore = channelContributions.reduce((sum, entry) => sum + entry.contribution_raw, 0);

  // Battery should shift vulnerability, but not dominate the weather signal.
  const scalar = Number(batteryScalar || 1);
  const appliedBatteryScalar = clamp(scalar, 0.94, 1.08);
  const scoreAfterBattery = baseScore * appliedBatteryScalar;

  // 8〜10は「強い天気負担」と「体質親和性」が重なる日を中心にする。
  let strongWeatherAffinityBonus = 0;
  if ((top.weather || 0) >= 0.88 && (top.affinity || 0) >= 0.72) strongWeatherAffinityBonus = 0.55;
  else if ((top.weather || 0) >= 0.88) strongWeatherAffinityBonus = 0.2;

  const secondaryLoadBonus = (second.load || 0) >= 0.56 ? 0.25 : 0;
  const multiChannelCount = entries.filter((entry) => entry.load >= 0.36).length;
  const multiChannelBonus = multiChannelCount >= 3 ? 0.2 : 0;
  const scoreBeforeClamp =
    scoreAfterBattery + strongWeatherAffinityBonus + secondaryLoadBonus + multiChannelBonus;
  const finalScore = Math.round(clamp(scoreBeforeClamp, 0, 10) * 10) / 10;

  return {
    final_score_0_10: finalScore,
    base_score_before_reserve: round2(baseScore),
    battery_scalar_raw: round2(scalar),
    battery_scalar_applied: round2(appliedBatteryScalar),
    score_after_reserve: round2(scoreAfterBattery),
    top_channel_contributions: channelContributions.map(({ contribution_raw, ...entry }) => entry),
    bonuses: {
      strong_weather_affinity: strongWeatherAffinityBonus,
      strong_secondary_load: secondaryLoadBonus,
      multi_channel_overlap: multiChannelBonus,
      multi_channel_count: multiChannelCount,
    },
  };
}
...
export function personalizeForecast({
  ...
  const scoreTrace = computeScoreTrace({
    effectiveLoads,
    weatherStrengths,
    affinityWeights: affinity.weights,
  ...
  return {
    ...
    meta: {
      ...
      battery_scalar_applied: scoreTrace.battery_scalar_applied,
      forecast_model: {
        universal_weather_share: UNIVERSAL_WEATHER_SHARE,
        personal_affinity_share: PERSONAL_AFFINITY_SHARE,
        effective_load_cap: EFFECTIVE_LOAD_CAP,
        secondary_trigger_min_load: SECONDARY_TRIGGER_MIN_LOAD,
        secondary_trigger_min_ratio: SECONDARY_TRIGGER_MIN_RATIO,
        channel_importance: CHANNEL_IMPORTANCE,
        affinity_core_share: 0.55,
        affinity_primary_sub_share: 0.28,
        affinity_secondary_sub_share: 0.17,
      },
