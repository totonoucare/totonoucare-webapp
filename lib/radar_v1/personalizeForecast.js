...
function buildExactAffinity(constitution, reviewSummary) {
  ...
  const profile = buildPersonalWeatherAffinityProfile({
    ...
    subRiskWeights: constitution?.sub_labels || diagnosis?.sub_risk_weights || [],
    envVectors: diagnosis?.env_vectors || [],
    sensitivity: diagnosis?.sensitivity || "normal",
  ...
  return {
    weights,
    batteryScalar: getBatteryScalar(diagnosis?.core_type),
...
function getWeatherExactStrengths(weatherStress) {
  return {
    ...
    cold: clamp01(weatherStress?.temp_drop_strength ?? 0),
    heat: clamp01(weatherStress?.temp_rise_strength ?? 0),
    damp: clamp01(weatherStress?.humidity_strength ?? 0),
...
export function personalizeForecast({ weatherStress, constitution, reviewFeedback = [] }) {
  ...
  const scoreRaw = computeScore(personalChannels, affinity.batteryScalar);
  const signal = toSignal(scoreRaw);
  const score = clampInt(scoreRaw);
  ...
  return {
    ...
    meta: {
      ...
      score_raw_0_10: round1(scoreRaw),
    },
  };
}
