...
function shapeRadarProfile(data) {
  ...
  return {
    ...
    // Forecast v2 consumes the continuous values already produced by the
    // constitution check. Keep them intact; the legacy fields above remain for
    // old snapshots and rollback.
    axes: computed?.axes && typeof computed.axes === "object"
      ? {
          reaction_score: Number(computed.axes.reaction_score ?? computed.axes.yin_yang_score ?? 0),
          reserve_score: Number(computed.axes.reserve_score ?? computed.axes.drive_score ?? 0),
          yin_yang_score: Number(computed.axes.reaction_score ?? computed.axes.yin_yang_score ?? 0),
          drive_score: Number(computed.axes.reserve_score ?? computed.axes.drive_score ?? 0),
          obstruction_score: Number(computed.axes.obstruction_score ?? 0),
          cold_score: Number(computed.axes.cold_score ?? 0),
          heat_score: Number(computed.axes.heat_score ?? 0),
          thermal_direction: Number(computed.axes.thermal_direction ?? 0),
          thermal_mixed: Boolean(computed.axes.thermal_mixed),
          thermo_answer: computed.axes.thermo_answer || "neutral",
        }
      : null,
    split_scores: computed?.split_scores && typeof computed.split_scores === "object"
      ? computed.split_scores
      : null,
