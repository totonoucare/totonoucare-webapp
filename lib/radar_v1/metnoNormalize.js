 ...
 * - air_temperature (°C)
 * - relative_humidity (%)
 * - dew_point_temperature (°C, forecast v2 moisture calculation)
 *
...
export function normalizeMetnoForTargetDate({
  ...
  const rows = metnoJson.properties.timeseries
    .map((row) => {
      ...
      const dewPoint = numberOrNull(details.dew_point_temperature);
  ...
  const toPublicPoint = ({ ts, pressure_hpa, temp_c, humidity_pct, dew_point_c }) => ({
    ts,
    pressure_hpa,
    temp_c,
    humidity_pct,
