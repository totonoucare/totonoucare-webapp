...
function sensitivityDelta(level) {
  // diagnosis v2 / profileRepo currently passes env.sensitivity as 0..3.
  // Keep string compatibility, but treat numeric sensitivity as the primary source.
  const num = Number(level);
  if (Number.isFinite(num)) {
    if (num >= 3) return 0.09;
    if (num >= 2) return 0.06;
    if (num >= 1) return 0.03;
    return 0;
  }
...
function addDelta(target, source, cap = 1) {
  if (!source) return;
  for (const key of EXACT_WEATHER_KEYS) {
    if (source[key] == null) continue;
    const delta = Math.max(-cap, Math.min(cap, Number(source[key] || 0)));
    target[key] = clamp01((target[key] || 0) + delta);
  }
}

function normalizeReviewChannelBonus(perChannelBonus) {
  const out = {};
  for (const key of EXACT_WEATHER_KEYS) {
    const value = Number(perChannelBonus?.[key] ?? 0);
    out[key] = Number.isFinite(value) ? Math.max(0, Math.min(0.16, value)) : 0;
  }
  return out;
}
...
export function buildPersonalWeatherAffinityProfile({
  ...
  // Current reviewFeedback.js returns per_channel_bonus. Apply it directly.
  addDelta(weights, normalizeReviewChannelBonus(reviewChannelBonus), 0.16);
