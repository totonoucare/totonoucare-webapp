...
function exactTriggerFromSnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== "object") return null;
  const factors = Array.isArray(snapshot.trigger_factors) ? snapshot.trigger_factors : [];
  return (
    snapshot.personal_main_trigger_exact ||
    factors[0]?.exact ||
    factors[0]?.key ||
    compatTriggerToExact(snapshot.main_trigger, snapshot.trigger_dir)
  );
}
...
export async function getReviewFeedback({
  ...
  let reviewResult = await supabase
    .from("radar_reviews")
    .select("target_date, condition_level, forecast_snapshot, created_at, updated_at")
    .eq("user_id", userId)
    .gte("target_date", windowStart)
    .lt("target_date", beforeDate);

  // Migration前の環境でも従来ロジックへ戻せるようにする。
  if (reviewResult.error && (
    reviewResult.error.code === "42703" ||
    reviewResult.error.code === "PGRST204" ||
    String(reviewResult.error.message || "").includes("forecast_snapshot")
  )) {
  ...
  const [{ data: forecasts, error: forecastErr }] = await Promise.all([
      supabase
  ...
  const reviewErr = reviewResult.error;
  ...
  for (const row of reviews || []) {
    ...
    if (!prev || String(row.updated_at || row.created_at) > String(prev.updated_at || prev.created_at || "")) {
      latestReviewByDate.set(date, row);
    }
  }

  const forecastByDate = new Map((forecasts || []).map((row) => [String(row.target_date), row]));
  const rows = [];
  for (const [date, review] of latestReviewByDate.entries()) {
    const forecast = forecastByDate.get(date);
    const exact = exactTriggerFromSnapshot(review.forecast_snapshot) ||
