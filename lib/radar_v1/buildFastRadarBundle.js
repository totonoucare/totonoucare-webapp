...
export async function buildFastRadarBundle({ userId, targetDate, location }) {
  ...
  const weatherStress = buildWeatherStress({
    ...
    contextPoints: normalized.contextPoints,
  });
  ...
  const vendorMeta = {
    ...
    context_point_count: normalized.contextPoints?.length || normalized.points.length,
    ...
    review_feedback: reviewFeedback,
    radar_weather_extraction_version: "radar_v1_weather_context_rolling_peakfix_2026-04-24",
  };
