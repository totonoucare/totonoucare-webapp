import { RADAR_FORECAST_MODEL_VERSION } from "@/lib/radar_v1/personalizeForecastV2";
...
function profileForecastFingerprint(profile) {
  ...
  return JSON.stringify(stableObject({
    ...
    score_scale: profile.score_scale || null,
...
function cachedProfileFingerprint(bundle) {
  ...
  return JSON.stringify(stableObject({
    ...
    material_scores: context.material_scores || null,
    score_scale: context.score_scale || null,
...
function getModelCacheState(bundle) {
  ...
  return {
    ...
    stale_model_version: cachedSetting !== activeSetting || (activeSetting === "v2" && bundle?.forecast?.vendor_meta?.forecast_model_version !== RADAR_FORECAST_MODEL_VERSION),
 ...
 * - 予報本文とケアは構造化ルールから決定し、外部AI生成は使わない
 */
export async function ensureForecastBundle({
  ...
  if (!force) {
    ...
    if (existing?.forecast) {
      ...
      const refreshModel = targetDate >= todayJst && modelCacheState.stale_model_version;
      if (shouldInvalidateCachedBundle(profileCacheState) || staleLocation || refreshModel) {
        cacheInvalidation = {
          ...profileCacheState,
          ...modelCacheState,
          model_refreshed: refreshModel,
