...
function buildTriggerMeaning(exact) {
  switch (exact) {
    ...
    case "temp_shift":
      return "外的な変化としては、絶対的な暑さ・寒さより気温差が目立つ。";
    case "cold":
...
function buildBodyExpressionGuidance({ exact, subCodes = [], symptomFocus = null } = {}) {
  ...
  if (exact === "pressure_down") {
    if (has("qi_stagnation")) add("気滞傾向: 気圧低下時に巡りの停滞・詰まり感・緊張が表に出やすい");
    if (has("blood_deficiency")) add("血虚傾向: 揺れを受け止める余力が薄く、気分・集中・回復感に波が出やすい");
    if (has("fluid_damp")) add("痰湿傾向: 気圧低下時に重さ・頭重感・水分のさばきにくさが出やすい");
    if (has("qi_deficiency")) add("気虚傾向: 気圧低下時に支える力が落ち、だるさ・動き出しにくさが出やすい");
  }
  if (exact === "pressure_up") {
    if (has("qi_stagnation")) add("気滞傾向: 気圧上昇時に張り・力み・上に集まる感じが出やすい");
    if (has("fluid_deficiency") || has("blood_deficiency")) add("うるおい/血の不足傾向: 張りや熱っぽさを逃がしにくい");
    if (has("qi_deficiency") || has("fluid_damp")) add("気虚/痰湿傾向: 気圧変化への適応で余力を使い、疲れや重さとして出やすい");
  }
  if (exact === "temp_shift") {
    if (has("qi_stagnation") || has("blood_stasis")) add("気滞/瘀血傾向: 気温差への適応で張りや固定したこわばりが出やすい");
    if (has("qi_deficiency") || has("blood_deficiency") || has("fluid_deficiency")) add("不足傾向: 気温差への適応で消耗やだるさが出やすい");
    if (has("fluid_damp")) add("痰湿傾向: 気温差への適応で重だるさや動き出しにくさが出やすい");
  }
  if (exact === "heat") {
...
function buildPrimaryTriggerContract(exact, riskContext) {
  ...
  return {
    ...
    rule: "文章の主因は必ず forecast.personal_main_trigger_exact / this_contract.primary_exact に合わせる。副因がある場合だけ、forecast.personal_secondary_trigger_exact / this_contract.secondary_exact を補助要素として添えてよい。weather_main_trigger_exact や tcm_context は補助情報であり、主因を上書きしない。体質・主訴は主因がその人にどう現れやすいかの説明に使う。",
  };
}
function buildPersonalTriggerMeaning(exact, subCodes, coreCode) {
  ...
  switch (exact) {
    ...
    case "pressure_up":
      ...
      return coreCode?.startsWith("brake")
        ? "本人には、気圧上昇への適応が疲れや重さとして響きやすい。"
        : "本人には、気圧上昇が力みやすさとして響きやすい。";

    case "temp_shift":
      if (has("qi_stagnation") || has("blood_stasis")) {
        return "本人には、気温差が張りや固定したこわばりとして響きやすい。";
      }
      return "本人には、気温差への適応がだるさや消耗として響きやすい。";
  ...
  return {
    ...
    forecast: {
      ...
      personal_secondary_trigger_meaning: personalSecondaryExact
        ? buildPersonalTriggerMeaning(personalSecondaryExact, subLabelCodes, coreCode)
        : null,
      trigger_factors: safeArray(riskContext?.summary?.trigger_factors),
      reaction_direction: riskContext?.summary?.reaction_direction || null,
