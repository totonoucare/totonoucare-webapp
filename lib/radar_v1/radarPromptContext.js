...
function buildBodyExpressionGuidance({ exact, subCodes = [], symptomFocus = null } = {}) {
  const has = (code) => subCodes.includes(code);
  const parts = [];
  const keywords = [];

  const add = (text, words = []) => {
    if (text) parts.push(text);
    words.forEach((w) => { if (w && !keywords.includes(w)) keywords.push(w); });
  };

  if (exact === "pressure_down") {
    if (has("qi_stagnation")) add("気圧低下で巡りの出口が狭くなり、もともとの緊張や詰まり感が表に出やすい", ["巡りの出口", "緊張", "詰まり感"]);
    if (has("blood_deficiency")) add("余力やうるおいの薄さがあると、揺れを受け止めにくく気分や集中に波が出やすい", ["余力の薄さ", "受け止めにくさ", "気分の波"]);
    if (has("fluid_damp")) add("湿や重さをさばきにくい傾向があると、頭重感や体の重だるさとして出やすい", ["頭重感", "重だるさ", "さばきにくさ"]);
    if (has("qi_deficiency")) add("支える力が落ちやすい傾向があると、だるさや動き出しにくさとして出やすい", ["支える力", "だるさ", "動き出しにくさ"]);
  }
  if (exact === "pressure_up") {
    if (has("qi_stagnation")) add("気圧上昇で張りが上に集まり、緊張やいら立ちとして出やすい", ["張り", "緊張", "上に集まる"]);
    if (has("fluid_deficiency") || has("blood_deficiency")) add("余力やうるおいが薄いと、力みや熱っぽさを逃がしにくい", ["力み", "熱っぽさ", "逃がしにくさ"]);
  }
  if (exact === "heat") {
    if (has("qi_stagnation")) add("気温上昇で内側の熱っぽさがこもり、張りつめや巡りの詰まりが上に出やすい", ["熱っぽさ", "張りつめ", "巡りの詰まり"]);
    if (has("blood_deficiency") || has("fluid_deficiency")) add("余力やうるおいが薄いと、気温上昇を受け止めにくく消耗感やそわつきにつながりやすい", ["消耗感", "そわつき", "受け止めにくさ"]);
  }
  if (exact === "cold") {
    if (has("qi_deficiency")) add("冷え込みで支える力が落ちやすく、だるさや動き出しにくさとして出やすい", ["支える力", "だるさ", "動き出しにくさ"]);
    if (has("fluid_damp")) add("冷え込みで水っぽい重さがさばけず、むくみ感や重だるさが残りやすい", ["むくみ感", "重だるさ", "さばけなさ"]);
    if (has("blood_deficiency") || has("blood_stasis")) add("冷え込みで巡りが鈍り、こわばりや張りとして出やすい", ["こわばり", "巡りの鈍さ", "張り"]);
  }
  if (exact === "damp") {
    if (has("fluid_damp")) add("湿っぽさで余分な重さをさばきにくく、頭や体の重だるさに出やすい", ["重だるさ", "頭重感", "さばきにくさ"]);
    if (has("qi_deficiency")) add("湿っぽさで消化やエネルギーの立ち上がりが鈍りやすい", ["立ち上がりにくさ", "消化の重さ", "だるさ"]);
  }
  if (exact === "dry") {
    if (has("fluid_deficiency") || has("blood_deficiency")) add("乾燥でうるおいの余白が削られ、こわばりや回復しにくさとして出やすい", ["うるおい不足", "こわばり", "回復しにくさ"]);
    if (has("qi_stagnation")) add("乾燥でこもった張りを逃がしにくく、気分や首肩の詰まりに波及しやすい", ["張り", "逃がしにくさ", "詰まり"]);
  }

  if (symptomFocus === "mood") add("主訴が気分の浮き沈みなので、体の重さ・張り・余力の薄さを気分の波として説明すると自然", ["気分の波", "沈み込み", "張りつめ"]);
  if (symptomFocus === "headache") add("主訴が頭痛なので、上に集まる張り・頭重感・巡りの滞りに着地させると自然", ["頭重感", "上に集まる", "巡りの滞り"]);
  if (symptomFocus === "neck_shoulder") add("主訴が首肩なので、こわばり・張り・力みとして説明すると自然", ["首肩のこわばり", "張り", "力み"]);
  if (symptomFocus === "fatigue") add("主訴が疲れやすさなので、支える力・回復力・動き出しにくさに着地させると自然", ["疲れやすさ", "支える力", "回復しにくさ"]);
  if (symptomFocus === "swelling") add("主訴がむくみ・重だるさなので、さばきにくさや重さとして説明すると自然", ["むくみ感", "重だるさ", "さばきにくさ"]);

  return {
    guidance: parts.length ? parts.slice(0, 3).join("。") : "体質・主訴は、主因がその人にどう現れやすいかの説明にだけ使う。",
...
function buildPrimaryTriggerContract(exact, riskContext) {
  ...
  return {
    ...
    body_expression_keywords: bodyExpression.keywords,
    forbidden_primary_phrases: base.forbidden_primary_phrases,
    rule: "文章の主因は必ず forecast.personal_main_trigger_exact / this_contract.primary_exact に合わせる。weather_main_trigger_exact や tcm_context は補助情報であり、主因を上書きしない。体質・主訴は主因がその人にどう現れやすいかの説明に使う。",
  };
}
