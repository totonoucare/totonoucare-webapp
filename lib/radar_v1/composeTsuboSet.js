...
export function composeTsuboSet({ tcmPoints, mtestPoint, riskContext }) {
  ...
  for (const p of tcmList) {
    ...
    const point = {
      ...
      source: "tcm",
      care_trigger_exact: p.care_trigger_exact || null,
      care_trigger_role: p.care_trigger_role || null,
  ...
  // A point selected by both rules stays a single card, but retains its line
  // provenance for the optional home-tool guidance.
  if (mp?.code && seen.has(mp.code)) {
    const existing = out.find(point => point.code === mp.code);
    if (existing) existing.mtest_selected = true;
  }
  if (mp?.code && !seen.has(mp.code)) {
    const point = {
      code: mp.code,
