...
export const RADAR_WEATHER_MODEL_VERSION = "radar_weather_v2_2026-09-15_load_anchors_smooth_diurnal";
...
const MOISTURE_TIMELINE_WINDOWS = MOISTURE_WINDOWS;
const MOISTURE_COMFORT_TIMELINE_WINDOWS = MOISTURE_COMFORT_WINDOWS;

const TEMPERATURE_NEUTRAL_MIN_C = 18;
const TEMPERATURE_NEUTRAL_MAX_C = 27;
const MOISTURE_NEUTRAL_MIN_GM3 = 8;
const MOISTURE_NEUTRAL_MAX_GM3 = 14;
...
// Product load anchors: threshold = 0.65; stronger events approach 1 smoothly.
// These anchors are not probabilities or clinically validated symptom thresholds.
function changeLoad(ratio) {
  const value = Math.max(0, Number(ratio) || 0);
  return value <= 1 ? value * 0.65 : 0.65 + 0.35 * (1 - Math.exp(-1.6 * (value - 1)));
}
...
export function buildWeatherStressV2({ points, previousNightBridgePoints = null }) {
  ...
  // State describes the target day's water environment. Direction separately
  // describes whether its water content is rising, falling or reversing.
  // Keeping them independent prevents an all-damp day from being presented as
  // dry merely because absolute humidity is temporarily falling.
  const moistureState = day.moisture.state;
  ...
  const environmentalCautions = buildEnvironmentalCautions(day.temperature, dayPoints);
  ...
  return {
    ...
    meta: {
      ...
      night_bridge: {
        ...
        moisture_state: bridge?.moisture?.state || null,
      },
...
function buildEnvironmentalCautions(temperature = {}, points = []) {
  const maxTemp = Number(temperature?.max_c);
  const minTemp = Number(temperature?.min_c);
  const cautions = [];
  if (maxTemp < 35 && points.some((point) => point.temp_c >= 30 && point.humidity_pct >= 65)) {
    cautions.push({
      key: "humid_heat", level: "moderate", label: "蒸し暑さへの備え",
      detail: "気温30℃以上・湿度65％以上が重なる時間があります。冷房や除湿を使い、涼しい場所で休憩しましょう。",
      metric: "simultaneous_temperature_and_relative_humidity", official_alert: false,
      source: "radar_weather_load_anchors_v1",
    });
  }
  ...
  if (Number.isFinite(maxTemp) && maxTemp >= 40) {
    cautions.push({
      ...
      level: "critical",
      label: "酷暑への注意",
      detail: "最高気温40℃以上の予測です。体調警戒度とは別に、暑さを避ける安全対策を優先してください。",
    ...
    cautions.push({
      ...
      level: "high",
      label: "猛暑への注意",
      detail: "最高気温35℃以上の予測です。体調警戒度とは別に、涼しい場所と水分を早めに確保してください。",
      metric: "daily_max_temperature_c",
      value: round3(maxTemp),
      threshold: 35,
      official_alert: false,
  ...
  if (
    ...
    cautions.push({
      ...
      level: "high",
      label: "厳しい寒さへの注意",
      detail: "一日を通した氷点下、または氷点下5℃以下の予測です。体調警戒度とは別に、防寒と移動時の安全を優先してください。",
...
function analyzeDataset(points) {
  ...
  return {
    ...
    moisture: {
      state: classifyMoistureState(moisturePoints),
      ...
      comfort_band_absolute_humidity_gm3: [
        MOISTURE_NEUTRAL_MIN_GM3,
        MOISTURE_NEUTRAL_MAX_GM3,
      ],
    },
    event_strengths: {
...
function buildTimelineRow(point, context) {
  ...
  // Attention uses all score windows (including 24h). Keep the existing
  // general timeline unchanged; only the temperature-shift attention consumes
  // these directional, comfort-weighted values.
  const temperatureAttention = {};
  for (const direction of ["up", "down"]) {
    const departure = pointRollingComfortDeparture(
      context, point, signedTemperatureComfortDistance, TEMPERATURE_COMFORT_WINDOWS,
      { discountExpectedDiurnal: true, direction }
    );
    const transition = pointRollingDirection(
      context, point, "temp_c", direction, TEMPERATURE_WINDOWS,
      { discountExpectedDiurnal: true }
    );
    temperatureAttention[direction] = Math.max(departure, transition * RELIEF_TRANSITION_SHARE);
  }
  const moistureUp = pointRollingDirection(
    context,
    point,
    "absolute_humidity_gm3",
    "up",
    MOISTURE_TIMELINE_WINDOWS
  );
  const moistureDown = pointRollingDirection(
    context,
    point,
    "absolute_humidity_gm3",
    "down",
  ...
  const rawMoistureShift = Math.max(moistureUp, moistureDown);
  const moistureComfortDeparture = pointRollingComfortDeparture(
    context,
    point,
    signedMoistureComfortDistance,
    MOISTURE_COMFORT_TIMELINE_WINDOWS
  );
  const moistureShift = Math.max(
    moistureComfortDeparture,
    rawMoistureShift * MOISTURE_RELIEF_TRANSITION_SHARE
  );
  ...
  const total =
    pressureShift +
    Math.max(tempShift, cold, heat) * 0.9 +
    Math.max(moistureShift, damp, dry) * 0.85;
  ...
  return {
    ...
    temp_up: round3(tempUp),
    temp_down: round3(tempDown),
    temp_attention_up: round3(temperatureAttention.up),
    temp_attention_down: round3(temperatureAttention.down),
    cold: round3(cold),
    heat: round3(heat),
    moisture_shift: round3(moistureShift),
    moisture_up: round3(moistureUp),
    moisture_down: round3(moistureDown),
    damp: round3(damp),
    dry: round3(dry),
    total: round3(total),
  };
...
function signedMoistureComfortDistance(point) {
  const absoluteHumidity = Number(point?.absolute_humidity_gm3);
  if (!Number.isFinite(absoluteHumidity)) return 0;
  if (absoluteHumidity < MOISTURE_NEUTRAL_MIN_GM3) {
    return -clamp01((MOISTURE_NEUTRAL_MIN_GM3 - absoluteHumidity) / 6);
  }
  if (absoluteHumidity > MOISTURE_NEUTRAL_MAX_GM3) {
    return clamp01((absoluteHumidity - MOISTURE_NEUTRAL_MAX_GM3) / 8);
  }
  return 0;
}

function classifyMoistureState(points) {
  const values = points
    .map((point) => Number(point?.absolute_humidity_gm3))
    .filter(Number.isFinite);
  if (!values.length) return "neutral";

  const hasDamp = values.some((value) => value > MOISTURE_NEUTRAL_MAX_GM3);
  const hasDry = values.some((value) => value < MOISTURE_NEUTRAL_MIN_GM3);
  if (hasDamp && hasDry) return "mixed";
    ...
    for (let i = 0; i < points.length; i += 1) {
      for (let j = i + 1; j < points.length; j += 1) {
        ...
        const diurnalFactor = options.discountExpectedDiurnal
          ...
          : 1;
        const strength = changeLoad((transition.worsening / window.threshold) * window.weight * diurnalFactor);
        const reliefStrength = changeLoad((transition.relief / window.threshold) * window.weight * diurnalFactor);
...
function pointRollingComfortDeparture(points, current, distanceFn, windows, options = {}) {
  ...
  for (const window of windows) {
    ...
    for (const previous of points) {
      ...
      if (options.direction === "up" && Number(current.temp_c) <= Number(previous.temp_c)) continue;
      if (options.direction === "down" && Number(current.temp_c) >= Number(previous.temp_c)) continue;
      ...
      best = Math.max(
        best,
        changeLoad((transition.worsening / window.threshold) * window.weight * diurnalFactor)
      );
    }
  }
  return best;
    ...
    for (let i = 0; i < points.length; i += 1) {
      for (let j = i + 1; j < points.length; j += 1) {
        ...
        const diurnalFactor = options.discountExpectedDiurnal
          ...
          : 1;
        const strength = changeLoad((delta / window.threshold) * window.weight * diurnalFactor);
        if (strength > best.strength) {
          best = {
            strength,
            max_delta: delta,
  ...
  for (const window of windows) {
    ...
    for (const previous of points) {
      ...
      const diurnalFactor = options.discountExpectedDiurnal
        ...
        : 1;
      best = Math.max(best, changeLoad((delta / window.threshold) * window.weight * diurnalFactor));
    }
  }
  return best;
}
...
function expectedDiurnalFactor(startTs, endTs, mode, elapsedMs) {
  // Preserve the ordinary warming/cooling discount (0.55..1), but blend across
  // four hours around each former hourly boundary. Fractional JST hours avoid
  // introducing new jumps at :00. This is a model heuristic, not a measured
  // physiological threshold. Scoring and attention windows share this helper.
  void startTs;
  void elapsedMs;
  const ts = new Date(endTs).getTime();
  if (!Number.isFinite(ts)) return 1;
  const hour = ((ts / 3600000 + 9) % 24 + 24) % 24;
  const blend = (start, end) => {
    const x = clamp01((hour - start) / (end - start));
    return x * x * (3 - 2 * x);
  };
  const expected = mode === "up"
    ? blend(4.5, 8.5) * (1 - blend(13.5, 17.5))
    : mode === "down"
      ? 1 - blend(3.5, 7.5) * (1 - blend(13.5, 17.5))
      : 0;
  return 1 - 0.45 * expected;
}
...
function computeColdLevel(points) {
  ...
  const loads = points.map((point) => clamp01((18 - point.temp_c) / 23));
  return clamp01(0.6 * Math.max(...loads) + 0.4 * loads.reduce((a, b) => a + b, 0) / loads.length);
}

function computeHeatLevel(points) {
  if (!points.length) return 0;
  const loads = points.map((point) => clamp01((point.temp_c - 27) / 13));
  return clamp01(0.6 * Math.max(...loads) + 0.4 * loads.reduce((a, b) => a + b, 0) / loads.length);
}
...
function computeDampLevel(points, moistureRiseStrength) {
  ...
  const highRatio = values.reduce((sum, value) => sum + clamp01((value - 0.2) / 0.3), 0) / values.length;
  const severeRatio = values.reduce((sum, value) => sum + clamp01((value - 0.55) / 0.3), 0) / values.length;
...
function computeDryLevel(points, moistureDropStrength) {
  ...
  const average = values.reduce((sum, value) => sum + value, 0) / values.length;
  const lowRatio = values.reduce((sum, value) => sum + clamp01((value - 0.2) / 0.3), 0) / values.length;
  const severeRatio = values.reduce((sum, value) => sum + clamp01((value - 0.55) / 0.3), 0) / values.length;
  const max = Math.max(...values);
  return clamp01(
...
function findChannelPeak(timeline, key, fallback) {
  if (key === "temp_shift") {
    return findPeakWindow(timeline.map((row) => ({
      ...row,
      total: Math.max(row.temp_attention_up, row.temp_attention_down),
    })), fallback, ["temp_attention_up", "temp_attention_down"]);
  }
  return findPeakWindow(timeline.map((row) => ({ ...row, total: Number(row?.[key] || 0) })), fallback);
}

function findPeakWindow(rows, fallback = { start: "09:00", end: "12:00" }, directionKeys = null) {
  if (!Array.isArray(rows) || !rows.length) return fallback;
  let best = { sum: -1, count: 0, startIndex: 0, endIndex: 0 };
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (sum > best.sum) best = { sum, count, startIndex: i, endIndex };
  }
  let direction = null;
  if (directionKeys) {
    const window = rows.slice(best.startIndex, best.endIndex + 1);
    const up = window.reduce((sum, row) => sum + Number(row[directionKeys[0]] || 0), 0);
    const down = window.reduce((sum, row) => sum + Number(row[directionKeys[1]] || 0), 0);
    direction = up === down ? (up > 0 ? "mixed" : "steady") : up > down ? "up" : "down";
  }
  return {
    start: timeLabel(rows[best.startIndex]?.ts) || fallback.start,
    end: timeLabel(rows[best.endIndex]?.ts, 1) || fallback.end,
    strength: round3(best.sum / Math.max(1, best.count)),
