...
function inferPartnerProductRole({ title, primarySurface, categories = [], tags = [] } = {}) {
  ...
  if (surface === "point") {
    if (/首|肩|ネック|ハンド|手元|温熱/.test(text)) return "neck_shoulder_release";
...
function a8({ id, title, clickUrl, imageUrl, impressionUrl, primarySurface, categories, policies, symptoms, weatherFit = [], seasonFit = [], seasonSuppress = [], lifeFit = [], subFit = [], coreFit = [], typeFit = [], priceBands = ["all"], intensity = {}, rewardRank = "medium", riskLevel = "low", buyingBehavior = "official_or_marketplace", reason, tags = [], productRole, productRoleLabel }) {
