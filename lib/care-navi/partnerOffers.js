...
function a8({ id, title, clickUrl, imageUrl, impressionUrl, categories, policies, symptoms, weatherFit = [], seasonFit = [], seasonSuppress = [], lifeFit = [], subFit = [], coreFit = [], typeFit = [], priceBands = ["all"], intensity = {}, rewardRank = "medium", riskLevel = "low", buyingBehavior = "official_or_marketplace", reason, tags = [] }) {
  return {
    id,
    source: "a8",
    sourceType: "partner",
...
export const PARTNER_OFFERS = [
  ...
  a8({
    ...
    reason: "カフェインや冷たい飲み物に寄りすぎず、飲み物から整えたい時の候補です。",
    tags: ["ハーブティー", "ノンカフェイン"],
  }),
  a8({
    id: "cado-air-design",
...
export function scorePartnerOffers({
  ...
  const ranked = PARTNER_OFFERS
    .filter((offer) => safeArray(offer.categories).includes(safeCategory))
    .filter((offer) => priceBand === "all" || safeArray(offer.priceBands).includes(priceBand))
    .map((offer) => {
      let score = 0;
      ...
      if (symptomKey && safeArray(offer.symptoms).includes(symptomKey)) score = add(score, 3.0);
      if (symptomKey === "fatigue" && hasOverlap(offer.symptoms, ["sleep", "digestion"])) score = add(score, 0.65);
      if (symptomKey === "sleep" && hasOverlap(offer.symptoms, ["fatigue", "neck_shoulder"])) score = add(score, 0.5);

      if (environmentMode === "tomorrow") {
      ...
      if (hasOverlap(offer.coreFit, coreHints) || hasOverlap(offer.typeFit, coreHints)) score = add(score, 1.15);
      if (hasOverlap(offer.lifeFit, lifeKeys) || hasOverlap(offer.symptoms, lifeHints) || hasOverlap(offer.policies, lifeHints)) score = add(score, lifeKeys.length ? 1.55 : 0);

      score = add(score, rewardScore(offer.rewardRank));
      ...
      return {
        ...
        tags: unique([...(offer.tags || []), ...(policyKey ? [policyKey] : [])]).slice(0, 4),
