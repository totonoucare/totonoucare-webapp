...
function inferPartnerProductRole({ title, primarySurface, categories = [], tags = [] } = {}) {
  ...
  if (surface === "point") {
    ...
    if (/ハンド|手元/.test(text)) return "hand_release";
    if (/首|肩|ネック/.test(text)) return "neck_shoulder_release";
...
const POINT_PARTNER_ROLES_BY_SYMPTOM = {
  fatigue: ["neck_shoulder_release", "gentle_stretch", "foot_leg_release", "posture_release"],
  sleep: ["neck_shoulder_release", "gentle_stretch", "foot_leg_release"],
  neck_shoulder: ["neck_shoulder_release", "heat_release", "tsubo_support"],
  headache: ["neck_shoulder_release", "heat_release", "tsubo_support"],
  mood: ["neck_shoulder_release", "gentle_stretch"],
  low_back_pain: ["posture_release", "gentle_stretch"],
  digestion: ["foot_leg_release", "gentle_stretch"],
  swelling: ["foot_leg_release", "gentle_stretch"],
  dizziness: ["gentle_stretch"],
};

function partnerPointRoleMatchesSymptom(offer, category, symptomKey) {
  if (category !== "point" || !symptomKey) return true;
  const allowed = POINT_PARTNER_ROLES_BY_SYMPTOM[symptomKey];
  return !allowed || allowed.includes(offer.productRole);
}
...
export function getPartnerWeatherKeys(triggerFactors) {
  return safeArray(triggerFactors)
    // Pressure products follow the predicted body response (careKey), while
    // the physical up/down direction remains available on the factor for UI.
    .map((factor) => factor?.careKey || factor?.key || factor?.exact)
    .filter(Boolean);
}
...
export function scorePartnerOffers({
  ...
  const weatherKeys = getPartnerWeatherKeys(triggerFactors);
  ...
  const desiredFoodRoles = new Set(safeArray(foodProductRoleKeys));
  const desiredFoodNeeds = new Set(safeArray(foodNeedKeys));
  const commerceRoleMatch = (offer) => {
    if (desiredFoodRoles.has("food_therapy") && offer.productRole === "ingredient") return true;
    if (desiredFoodRoles.has("daily_tea") && ["warm_drink", "caffeine_shift"].includes(offer.productRole)) return true;
    if (desiredFoodRoles.has("pantry_food") && offer.productRole === "ingredient") return true;
    if ((desiredFoodRoles.has("prepared_meal") || desiredFoodRoles.has("meal_subscription")) && offer.productRole === "light_meal") return true;
    if (desiredFoodRoles.has("nutrition_support") && offer.productRole === "nutrition_support") return true;
    return false;
  };

  const ranked = PARTNER_OFFERS
    .filter(offer => safeCategory !== "eat" || !desiredFoodRoles.has("food_therapy") || ["warm_drink","caffeine_shift","ingredient"].includes(offer.productRole))
    .filter((offer) => (offer.primarySurface || safeArray(offer.categories)[0]) === safeCategory)
    .filter((offer) => partnerPointRoleMatchesSymptom(offer, safeCategory, symptomKey))
    ...
    .map((offer) => {
      ...
      if (safeCategory === "eat" && desiredFoodRoles.size && commerceRoleMatch(offer)) score = add(score, 2.35);
      if (safeCategory === "eat" && desiredFoodNeeds.has("meal_continuity") && offer.productRole === "light_meal") score = add(score, 1.35);
      if (safeCategory === "eat" && desiredFoodNeeds.has("caffeine_shift") && ["warm_drink", "caffeine_shift"].includes(offer.productRole)) score = add(score, 1.15);
      if (safeCategory === "eat" && desiredFoodNeeds.has("protein_continuity") && ["light_meal", "nutrition_support"].includes(offer.productRole)) score = add(score, 0.9);

      // 報酬は最後の軽い後押しに留め、悩みとの接続を上書きしない。
      score = add(score, Math.min(rewardScore(offer.rewardRank), 0.65));

      // 価格指定なしの初期棚では、寝具・家電・継続サービス等の高関与商品を
      // 最初の候補へ集めすぎない。高めを選んだ時はこの抑制を外す。
      const highCommitmentOffer = safeArray(offer.priceBands).includes("deep")
        && !safeArray(offer.priceBands).includes("standard")
        && !offer.starterEligible;
      if (priceBand === "all" && highCommitmentOffer) score = add(score, -2);
