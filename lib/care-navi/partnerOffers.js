...
function evaluatePartnerFit(offer, { symptomKey, lifeKeys, weatherKeys, coreHints, policyKeys, environmentMode, seasonKey } = {}) {
  ...
  const directWeather = environmentMode === "tomorrow" && hasOverlap(offer.weatherFit, weatherKeys);
  const directSeason = environmentMode === "season" && Boolean(seasonKey && safeArray(offer.seasonFit).includes(seasonKey));
