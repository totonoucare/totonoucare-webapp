import { getPrimaryRadarLocation } from "@/lib/radar_v1/radarRepo";
import { ensureForecastBundle } from "@/lib/radar_v1/ensureForecastBundle";
import { getNotificationTargetDate } from "@/lib/radar_v1/notificationTargetDate";
import { buildRadarPushPayload, stripPushTimeSeconds } from "@/lib/push/notificationText";
import { sendWebPush } from "@/lib/push/webPush";
import { getRecordsAccess } from "@/lib/records/access";
import {
  deactivatePushSubscriptionByEndpoint,
  getActivePushSubscriptions,
  getNotificationLog,
  getNotificationTargetsPage,
  insertNotificationLog,
} from "@/lib/push/pushRepo";

function safeLimit(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 25;
  return Math.max(1, Math.min(100, Math.round(n)));
}

function shouldDeactivatePushSubscription(error) {
  const statusCode = Number(error?.statusCode || error?.status || 0);
  return statusCode === 404 || statusCode === 410;
}

async function runWithConcurrency(items, concurrency, worker) {
  let nextIndex = 0;
  async function run() {
    while (nextIndex < items.length) {
      const index = nextIndex;
      nextIndex += 1;
      await worker(items[index]);
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(Math.max(1, concurrency), items.length) }, run)
  );
}

export async function runRadarNotificationCron({
  kind,
  limit = 25,
  cursor = null,
  dryRun = false,
  userId = null,
}) {
  if (!['night', 'morning'].includes(kind)) {
    throw new Error("kind must be night or morning");
  }

  const targetDate = getNotificationTargetDate({ kind });
  const page = await getNotificationTargetsPage({
    kind,
    limit: safeLimit(limit),
    cursor,
    userId,
  });
  const targets = page.items;

  const summary = {
    ok: true,
    kind,
    target_date: targetDate,
    dry_run: Boolean(dryRun),
    cursor: cursor || null,
    has_more: page.hasMore,
    next_cursor: page.nextCursor,
    target_count: targets.length,
    processed: 0,
    sent: 0,
    skipped: 0,
    failed: 0,
    details: [],
  };

  await runWithConcurrency(targets, 4, async (target) => {
    const uid = target.user_id;
    const minScore = Number.isFinite(Number(target.min_score)) ? Number(target.min_score) : 6;

    try {
      const access = await getRecordsAccess(uid);
      if (!access.notifications_enabled) {
        summary.skipped += 1;
        summary.details.push({ user_id: uid, status: "skipped_access_required" });
        return;
      }

      const existingLog = await getNotificationLog({
        userId: uid,
        targetDate,
        kind,
      });

      if (existingLog) {
        summary.skipped += 1;
        summary.details.push({ user_id: uid, status: "skipped_already_logged" });
        return;
      }

      const subscriptions = await getActivePushSubscriptions({ userId: uid });
      if (!subscriptions.length) {
        await insertNotificationLog({
          userId: uid,
          targetDate,
          kind,
          status: "skipped_no_subscription",
        });
        summary.skipped += 1;
        summary.details.push({ user_id: uid, status: "skipped_no_subscription" });
        return;
      }

      const location = await getPrimaryRadarLocation({ userId: uid });
      if (!location) {
        await insertNotificationLog({
          userId: uid,
          targetDate,
          kind,
          status: "skipped_no_location",
        });
        summary.skipped += 1;
        summary.details.push({ user_id: uid, status: "skipped_no_location" });
        return;
      }

      let bundle = await ensureForecastBundle({
        userId: uid,
        targetDate,
        location,
      });

      // 01:05の「今日」は一日固定する。夜通知だけは、16:30の明日分
      // スナップショットが失敗・未生成だった場合に限り、明日分を補完する。
      if (kind === "night" && !hasEveningTomorrowSnapshot(bundle?.forecast)) {
        bundle = await ensureForecastBundle({
          userId: uid,
          targetDate,
          location,
          force: true,
          generationSlot: "notification_fallback",
        });
      }

      // AI補完は一時停止中。通知もルールベースの予報スナップショットだけで送る。

      const forecast = bundle.forecast;
      const score = Number(
        forecast?.score_display_0_10 ??
        forecast?.score_precise_0_10 ??
        forecast?.computed?.forecast_snapshot?.score_display_0_10 ??
        forecast?.score_0_10 ??
        0
      );

      if (score < minScore) {
        await insertNotificationLog({
          userId: uid,
          forecastId: forecast?.id || null,
          targetDate,
          kind,
          score,
          status: "skipped_low_score",
          payload: { min_score: minScore },
        });
        summary.skipped += 1;
        summary.details.push({
          user_id: uid,
          status: "skipped_low_score",
          score,
          min_score: minScore,
        });
        return;
      }

      const rawPayload = buildRadarPushPayload({ kind, forecast, targetDate });
      const payload = {
        ...rawPayload,
        title: stripPushTimeSeconds(rawPayload.title),
        body: stripPushTimeSeconds(rawPayload.body),
      };

      if (dryRun) {
        summary.sent += 1;
        summary.details.push({
          user_id: uid,
          status: "dry_run_would_send",
          score,
          payload,
        });
        return;
      }

      let sentTo = 0;
      const errors = [];

      for (const subscription of subscriptions) {
        try {
          await sendWebPush({ subscription, payload });
          sentTo += 1;
        } catch (error) {
          const message = String(error?.message || error);
          errors.push(message);

          if (shouldDeactivatePushSubscription(error)) {
            await deactivatePushSubscriptionByEndpoint({
              userId: uid,
              endpoint: subscription.endpoint,
              reason: message,
            });
          }
        }
      }

      if (sentTo > 0) {
        await insertNotificationLog({
          userId: uid,
          forecastId: forecast?.id || null,
          targetDate,
          kind,
          score,
          status: "sent",
          payload: { ...payload, sent_to: sentTo },
          errorMessage: errors.length ? errors.join(" / ").slice(0, 1000) : null,
        });
        summary.sent += 1;
        summary.details.push({ user_id: uid, status: "sent", score, sent_to: sentTo });
      } else {
        await insertNotificationLog({
          userId: uid,
          forecastId: forecast?.id || null,
          targetDate,
          kind,
          score,
          status: "failed",
          payload,
          errorMessage: errors.join(" / ").slice(0, 1000),
        });
        summary.failed += 1;
        summary.details.push({ user_id: uid, status: "failed", score, errors });
      }
    } catch (error) {
      const message = String(error?.message || error);
      summary.failed += 1;
      summary.details.push({ user_id: uid, status: "failed", error: message });
    } finally {
      summary.processed += 1;
    }
  });

  return summary;
}

function hasEveningTomorrowSnapshot(forecast) {
  const slot = String(forecast?.vendor_meta?.snapshot_generation_slot || "");
  return slot === "evening_tomorrow" || slot === "notification_fallback";
}
