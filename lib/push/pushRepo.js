...
export async function getNotificationTargetsPage({
  kind,
  limit = 25,
  cursor = null,
  userId = null,
  ...
  if (error) throw new Error(`getNotificationTargetsPage failed: ${error.message}`);

  const rows = data || [];
  const items = rows.slice(0, safeLimit);
  const hasMore = !userId && rows.length > safeLimit;
  return {
    items,
    hasMore,
...
export async function getNotificationTargets(options) {
  const page = await getNotificationTargetsPage(options);
  return page.items;
}
