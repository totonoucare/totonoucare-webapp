...
const PROMPT_VERSION = "personal-karte-v4.2-meridian-relationship-editor";
...
function buildAiKarteFacts(ctx) {
  ...
  return cleanObject({
    ...
    movementChecks: {
      ...
      methodNote: "M-testの要領で、動作時の張り・重さ・動きづらさから、不調の背景にある経絡ラインの負担を探る材料。経絡ラインは、気血水の過不足・偏り、姿勢、冷え、生活負担などが表に出る通り道として扱う。movementChecks.first/second/concernArea には経絡名と読み仮名つきの meridianNamesWithReadings が入る。first/second は mainConcern の原因を断定するものではないが、今お困りの不調と別々の部品として並べず、身体の前後・左右・上下のつながりとして説明する。たとえば concernArea と first が違う場合は、first のラインの張りや縮こまりが concernArea 側へ負担を逃がす可能性まで自然に読み解く。動作負担チェックの動きは軽いセルフチェックにもなり、予報ページのツボ提案の一部には体質・天気に加えて、この経絡ラインの情報が反映される。",
    },
    seasonSignals: {
...
function buildPrompt(source) {
  return [
    ...
    "- 動作負担チェックは、動作時の張り・重さ・動きづらさから、不調の背景にある経絡（けいらく）ラインの負担を探る材料です。経絡ラインは、気血水の過不足・偏り、姿勢、冷え、生活負担などが表に出る通り道として扱います。経絡名が入力データにある場合は、体の場所だけで終わらせず、経絡名（読み仮名つき）も自然に添えて説明します。",
    "- このカルテは、今お困りの不調だけの説明書ではなく、今後出やすい軽い不調や前触れも含めて読み返せる整理です。",
    "- 詳しいツボケアや食養生は、その日の気象データで変わるため、予報ページで今日・明日の内容を確認する使い方につなげます。ツボ提案の一部には、動作負担チェックで見えた経絡ラインの情報も反映されます。",
    "",
    "# 書き方",
    "- その人の一日を想像し、前触れ、崩れやすい場面、戻し方を自然につなげます。",
    "- 説明は、体感、時間帯、天気、食事、予定量、動きの変化に落とし込みます。",
    "- コア体質 → 気血津液の偏り → 注意したい天気 → 表に出るサイン → 警戒日の判断、というつながりを大切にします。",
    "- 経絡の章では、movementChecks.concernArea（今お困りの不調から見た観察ライン）と movementChecks.first/second（動作負担チェックで見えたライン）を使います。これらを同格に並べず、first/second の負担が mainConcern にどう関わり得るかを、体のつながりとして説明します。",
    "- 予報ページのツボ提案は、体質・天気だけでなく、動作負担チェックの経絡ラインも一部反映しているものとして説明します。",
    ...
    "meridian-care：動作負担チェックで探る、不調の背景にある経絡（けいらく）ラインとケア。movementChecks.first/second の meridianNamesWithReadings を使い、経絡ラインを今お困りの不調と同格に並べず、背景にある負担の通り道として説明する。前面・後面・側面などのラインが今お困りの不調にどう関わり得るか、身体のつながりとして書く。動作チェックは軽いセルフチェックにもなり、予報ページのツボ提案の一部にはこの経絡ラインが反映されることまで書く。",
