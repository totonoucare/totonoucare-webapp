...
const PROMPT_VERSION = "personal-karte-v4.3-meridian-prompt-polish";
...
function buildPrompt(source) {
  return [
    ...
    "- 動作負担チェックは、動作時の張り・重さ・動きづらさから、不調の背景にある経絡（けいらく）ラインの負担を探る材料です。経絡ラインは、気血水の過不足・偏り、姿勢、冷え、生活負担などが表に出る通り道として扱います。経絡名が入力データにある場合は、体の場所だけで終わらせず、経絡名（読み仮名つき）も自然に添えて説明します。ただし経絡名の臓腑説明だけで終わらせず、今お困りの不調と身体のつながりとして読める文章にします。",
    "- このカルテは、今お困りの不調だけの説明書ではなく、今後出やすい軽い不調や前触れも含めて読み返せる整理です。",
    "- 詳しいツボケアや食養生は、その日の気象データで変わるため、予報ページで今日・明日の内容を確認する使い方につなげます。予報ページのラインケアのツボは、体質・注意天気に加えて、動作負担チェックで見えた経絡ラインの情報も反映して提案されます。",
    "",
    "# 書き方",
    "- その人の一日を想像し、前触れ、崩れやすい場面、戻し方を自然につなげます。",
    "- 説明は、体感、時間帯、天気、食事、予定量、動きの変化に落とし込みます。",
    "- コア体質 → 気血津液の偏り → 注意したい天気 → 表に出るサイン → 警戒日の判断、というつながりを大切にします。",
    "- 経絡の章では、movementChecks.concernArea（今お困りの不調から見た観察ライン）と movementChecks.first/second（動作負担チェックで見えたライン）を使います。最初から動作チェックの説明に入らず、まず mainConcern.label と concernArea を起点にします。そのうえで、first/second が同じラインなら表に出ているラインの反応として、違うラインなら別の通り道の張り・縮こまり・重さが今お困りの不調へどう影響し得るかとして説明します。",
    "- 予報ページのツボ提案については、ユーザーが自分で天気と経絡を組み合わせて考えるように書かず、アプリ側が体質・注意天気・動作負担チェックの経絡ラインを反映して提案している、と説明します。",
    ...
    "- 経絡章では、『不調名を決めるものではなく』『診断ではなく』のような防御的な前置きを本文に入れません。必要な安全配慮は、相談メモや全体の文脈で自然に扱います。",
    ...
    "meridian-care：動作負担チェックで探る、不調の背景にある経絡（けいらく）ラインとケア。最初に mainConcern.label と concernArea を起点にし、movementChecks.first/second の meridianNamesWithReadings を使って、不調の背景にある負担の通り道を説明する。経絡名の臓腑説明だけに寄らず、前面・後面・側面などのラインが今お困りの不調にどう関わり得るかを、身体のつながりとして書く。特に concernArea と first が違う場合は、別のラインの張り・縮こまり・重さが concernArea 側へ負担を逃がす可能性まで自然に説明する。予報ページのラインケアのツボは、この動作負担チェックで見えた経絡ラインも反映して提案されることを書く。",
