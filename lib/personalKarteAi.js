...
const PROMPT_VERSION = "personal-karte-v3.9-weather-care-column-editor";
...
function buildAiKarteFacts(ctx) {
  ...
  return cleanObject({
    ...
    movementChecks: {
      ...
      methodNote: "M-testの要領で、動作時の張り・重さ・動きづらさから負担がかかる経絡ラインを見る材料。動作負担チェックの動きは軽いセルフチェックとして使え、予報ページのツボ提案にもつながる。",
    },
    seasonSignals: {
...
function buildPrompt(source) {
  return [
    ...
    "- 動作負担チェックは、その人のお困りの不調とは別に、動作時の張り・重さ・動きづらさから、負担が表れやすい経絡（けいらく）ラインを見る材料です。経絡ラインは、日々のセルフチェックと予報ページのツボ提案につながります。",
    ...
    "meridian-care：動作負担チェックでわかる、不調に関わる経絡（けいらく）ラインとケア。動作チェックの意味、軽いセルフチェック、予報ページのツボ提案との関係まで書く。",
