...
const PROMPT_VERSION = "personal-karte-v3.7-integrated-weather-editor";
...
function buildAiKarteFacts(ctx) {
  ...
  return cleanObject({
    ...
    movementChecks: {
      ...
      methodNote: "M-testの要領で、動作時の張り・重さ・動きづらさから負担がかかる経絡ラインを見る材料。今お困りの不調とは分けて扱う。動作チェックの動きは軽いセルフチェックとして使え、予報ページのツボ提案にもつながる。",
    },
    seasonSignals: {
...
function normalizeSection(generated, fallback) {
  ...
  return {
    id: fallback.id,
    badge: clampText(polishUserText(section.badge || fallback.badge), 20),
    title: clampText(polishUserText(section.title || fallback.title), 90),
...
function buildPrompt(source) {
  return [
    ...
    "- 天気シグナルは、体質から見た注意天気と本人の自覚を合わせて読む材料です。湿気・冷え込み・気圧低下・乾燥・暑さなど、現代の天気の言葉で説明します。天気だけの一般論ではなく、気血津液（きけつしんえき）の偏りや動物タイプと重ねて読みます。",
    "- 動作チェックは、その人のお困りの不調とは別に、負担が表れやすい動作から推察できる経絡（けいらく）ラインを見る材料です。経絡ラインは、日々のセルフチェックと予報ページのツボ提案につながります。",
    ...
    "- 章ごとに役割を分け、同じ助言が続く場合は観察点か判断基準を変えます。とくに forecast-guide・advance-care・consult は、同じ内容を重ねず、それぞれ『使い方』『先回り』『相談時の伝え方』に分けます。",
    ...
    "- 『主訴』ではなく『今お困りの不調』または具体的な不調名で書きます。",
    "",
    "# 章の狙い",
    "weather-map：体質・天気・不調をひとつながりで見せる章。今お困りの不調だけで終わらせない。",
    "inner-pattern：気血津液（きけつしんえき）の偏りを扱う章。東洋医学用語と生活語の両方で理解させる。",
    "weather-rokuin：注意したい天気を扱う章。注意天気を単独で説明せず、inner-pattern で扱った気血津液の偏りと結びつけて読む。",
    "symptom-signs：今お困りの不調と、次に出やすい軽いサインを扱う章。今後も見返せる価値を作る。",
    "meridian-line：負担がかかる経絡（けいらく）ラインを扱う章。今お困りの不調の名前と、動作チェックで見えたラインを混同せず、予報ページのツボ提案や動作チェックの意味までつなげる。",
    "forecast-guide：カルテと毎日の予報ページの使い分けを扱う章。カルテは体質傾向の整理、予報ページは今日のツボ・食養生・過ごし方を見る場所として説明する。",
    "advance-care：注意天気の日に先回りする章。forecast-guide と重複させず、予定量・食事・冷え対策など、当日の負担を減らす判断に絞る。",
    "consult：鍼灸・整体・漢方などで相談するときに伝えやすいメモの章。症状名だけでなく、天気・前触れ・動作チェック・気血津液の偏りを短く伝える形にする。",
    "",
    "# 出力",
