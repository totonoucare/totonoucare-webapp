...
const PROMPT_VERSION = "personal-karte-v3.4-weather-map-editor";
...
const WEATHER_ROKUIN = {
  pressure_down: { term: "風邪", reading: "ふうじゃ", nuance: "急な揺れ・変化" },
  pressure_up: { term: "風邪", reading: "ふうじゃ", nuance: "急な締まり・変化" },
  cold: { term: "寒邪", reading: "かんじゃ", nuance: "冷え・縮こまり" },
  heat: { term: "暑邪", reading: "しょじゃ", nuance: "暑さ・消耗" },
  damp: { term: "湿邪", reading: "しつじゃ", nuance: "湿気・重だるさ" },
  dry: { term: "燥邪", reading: "そうじゃ", nuance: "乾燥・潤い不足" },
};
...
function rokuinForWeather(key) {
  return WEATHER_ROKUIN[key] || WEATHER_ROKUIN.pressure_down;
}
...
function compactWeatherItem(item) {
  ...
  return cleanObject({
    ...
    label: item.label,
    dayLabel: item.day,
    rokuin: rokuinForWeather(item.key),
...
function buildAiKarteFacts(ctx) {
  ...
  return cleanObject({
    ...
    movementChecks: {
      ...
      methodNote: "M-testの要領で、動作時の張り・重さ・動きづらさから負担がかかる経絡ラインを見る材料。主訴そのものとは別レイヤー。",
    },
    seasonSignals: {
...
function buildPrompt(source) {
  return [
    ...
    "",
    "# 未病レーダーの前提",
    "- 未病レーダーは、中医学的体質傾向、天気変化感受性、今気になる不調、動作のチェック（M-testの要領）を合わせて、天気で体調が揺れやすい日の見立てとセルフケアの手がかりを整理するアプリです。",
    "- 動物タイプ（アクセル優位・ブレーキ優位×余力の大・中・小）は、陰陽の傾きやすさ、気血の総量、回復力をまとめたメタファーです。",
    "- 気血津液（きけつしんえき）の偏りは、体に出やすいサインを読む材料です。東洋医学用語は読み仮名つきで自然に出し、説明は生活語で行います。",
    "- 天気シグナルは、六淫（ろくいん）の考え方と、体質から見た注意天気、本人の自覚を合わせて読む材料です。",
    "- 動作チェックは、その人のお困りの不調とは別に、負担が表れやすい動作から推察できる経絡（けいらく）ラインを見る材料です。",
    "- このカルテは、今の不調だけの説明書ではなく、今後出やすい軽い不調や前触れも含めた“天気で揺れる体調の地図”です。",
    "- 詳しいツボケアや食養生は、その日の気象データで変わるため予報ページで確認する、という使い方につなげます。",
    "",
    "# 書き方",
    "- その人の一日を想像し、前触れ、崩れやすい場面、戻し方を自然につなげます。",
    "- 説明は、体感、時間帯、天気、食事、予定量、動きの変化に落とし込みます。",
    "- コア体質 → 気血津液の偏り → 六淫の負担 → 表に出るサイン → 今日の予報で見ること、という流れを意識します。",
    "- 章ごとに役割を分け、同じ助言が続く場合は観察点か判断基準を変えます。",
    "- 語り口は、専門家が隣で整理してくれるような、落ち着いた自然な日本語にします。",
    "",
    "# 章の狙い",
    "weather-map：体質・天気・不調を一枚の地図として見せる章。今の不調だけで終わらせない。",
    "inner-pattern：気血津液（きけつしんえき）の偏りを扱う章。東洋医学用語と生活語の両方で理解させる。",
    "weather-rokuin：六淫（ろくいん）と注意天気を扱う章。天気病アプリとしての独自性を出す。",
    "symptom-signs：今の不調と、次に出やすい軽いサインを扱う章。今後も見返せる価値を作る。",
    "meridian-line：負担がかかる経絡（けいらく）ラインを扱う章。主訴と動作チェックをつなげる。",
    "forecast-guide：カルテと毎日の予報ページの使い分けを扱う章。ツボ・食養生は予報ページで確認する流れにする。",
