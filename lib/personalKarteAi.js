...
function get詳細版IntakeFromEvent(event) {
  return (
...
function build詳細版LoopForSource({ event, computed, ctx }) {
  const intake = get詳細版IntakeFromEvent(event);
  const diagnosis = {
  ...
  return {
    ...
    intake,
    profile: buildKarte詳細版LoopProfile({ diagnosis, intake }),
  };
}
...
export function buildKarteSourcePayload({ event, computed, baseKarte }) {
  ...
  const plusLoop = build詳細版LoopForSource({ event, computed: computedPayload, ctx: karteContext });
  ...
  return {
    ...
    plusLoop,
    outputContract: {
      productName: baseKarte?.productName || "体質トリセツ",
      ...
      toolCards: {
        ngCombinations: baseKarte?.karte詳細版Loop?.ngCombinationCards || [],
...
export function normalizeAiKarte(generated, baseKarte, { model = DEFAULT_MODEL } = {}) {
  ...
  return {
    ...baseKarte,
    productName: clampText(polishUserText(generated?.productName || baseKarte?.productName || "体質トリセツ"), 40),
...
function buildPrompt(source) {
  return [
    "WEBアプリ『未病レーダー』内の詳細版コンテンツ『体質トリセツ』を書きます。",
    "今回の完成物は、体質トリセツの延長ではなく『不調ループ見える化レポート＋同梱ツール』です。",
    "あなたの役割は、体質チェック結果・天気感受性・動作負担チェック・追加チェックを統合し、ユーザーが自分の不調のくり返し方を理解し、避けたい組み合わせ・身体ライン・相談メモまで見返せる読み物へ編集することです。",
    "",
    "# 商品の位置づけ",
    "- 商品名は『体質トリセツ』。中身は『不調ループ見える化レポート』。同梱ツールとして重ねすぎ注意カードと身体ラインのツボ候補カードを含む。",
    "- 体質トリセツは、体質・気血水・天気相性・経絡ライン・今日明日のケアの入口を示す。",
    "- 詳細版は、そこに時間帯・悪化条件・楽になる条件・生活負荷・睡眠/食事・続けやすさを重ねて、なぜ不調がくり返されるのか、どの組み合わせを避けると崩れにくいのかを整理する。",
    "- 予報ページは『今日・明日の作戦表』。詳細版は『なぜその作戦が必要になりやすいかを理解する保存版』。",
    "",
    "# 未病レーダーの前提",
    ...
    "- plusLoop.intakeProvided が false の場合は、追加チェック未回答の暫定トリセツとして、体質チェック結果から見える仮説にとどめ、断定を弱めて書きます。",
    "",
    "# 絶対に守る表現ルール",
    ...
    "- 詳しいツボケアや食養生はその日の気象データで変わるため、予報ページで今日・明日の内容を確認する使い方につなげる。詳細版側のツボ候補カードは「保存版の候補」であり、当日使う1点は予報ページを優先する。",
    ...
    "ng-combo-checker：重ねすぎ注意カード。追加チェックの悪化条件・睡眠/食事/作業負荷を使い、避けたい習慣・飲食の組み合わせを示す。",
    ...
    "forecast-bridge：今日・明日の予報ページの使いどころ。詳細版と予報ページの役割を分け、混乱しない使い方にする。",
