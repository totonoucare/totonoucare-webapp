...
function buildPrompt(source) {
  return [
    ...
    "- 【必須要件】専門用語は使わず、必ず情景が浮かぶ「比喩（メタファー）」を用いて解説してください（例：水を吸った重いスポンジ、オーバーヒート、バッテリー切れなど）。",
    "- 【必須要件】主訴（お困りの不調）と、動作チェックや体質のサインとの間に「なぜ繋がりがあるのか」のプロならではの謎解きを必ず組み込んでください。",
    "- 章ごとに役割を分け、同じ助言が続く場合は観察点か判断基準を変えます。",
    "- 語り口は、専門家が隣で伴走してくれるような落ち着いたトーンにします。「〜すべき」というタスクの押し付けではなく、「〜というサインを見逃さないでください」「まずは〜を減らしてみましょう」と、ユーザーの肩の荷を下ろすニュアンスを含めてください。",
    "",
    "# 章の狙い",
    "overview：最初に全体像をつかむ章。その人らしい崩れ方の輪郭を描く。",
    "route：不調が強くなる前の前触れを扱う章。早めに止める場所を見つける。",
    "weather：注意したい天気と、その日に先に出やすい身体のサインを扱う章。",
    "body-line：主訴（痛い場所）と動作チェック（負担が出る場所）が異なる場合、「なぜそこが繋がっているのか」という謎解きを行い、ハッとさせる気づきを与える章。",
