...
const PROMPT_VERSION = "personal-karte-v2.1-user-facing";
...
function buildPrompt(source) {
  return [
    ...
    "# 絶対にユーザー向け本文へ出さない内部語" ,
    "- 以下の語を本文・見出し・箇条書きにそのまま出さない：ランキング、強さ順、病機、時系列、M-test、自己申告、計算ロジック、結果ページと同じロジック、サブラベル、上位レイヤー、気血水の地図、経絡ライン、ハルシネーション防止、内部データ。",
    "- 内部語は必ず生活語に翻訳する。例：『ランキング』→『上位に出ている傾向』または本文では単に『注意したい天気』、『M-test的なライン』→『動きで見えた負担ポイント』、『サブラベル』→『目立ちやすいサイン』。",
    "- ユーザーは東洋医学や身体理論を知らない前提で書く。専門語を出す場合は、1文で生活のサインに言い換える。",
    "- ただし、入力データの解釈ルールは内部では厳密に守る。内部の説明をユーザーに読ませない。",
    "",
    "# 文体と商品価値",
    ...
    "- deterministicKarte に内部語が含まれていても、そのまま本文に出さず、ユーザーが読んで自然な言葉に置き換える。",
