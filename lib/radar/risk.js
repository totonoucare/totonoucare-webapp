...
//   ※「受けやすい/通常/受けにくい」の表示用。変化ストレスの警戒計算とは独立（ここ重要）。
...
function coreTitleFromProfile(profile) {
  // labels.js を import しない（risk.jsは純ロジック寄りで保つ）
  // route.js 側で labels.js を使って表示文を最終合成する想定でもOK。
  // ただし、ここでは core_code を返しておく。
  const code = profile?.core_code ?? profile?.computed?.core_code ?? null;
  return code;
}

function tempBurdenDirection(yinYangLabel) {
  // cold: 低温側が負担 / heat: 高温側が負担 / mixed: 両方あり得るがここでは中立寄り / neutral: 中立
...
function humidityBurdenDirection(profile) {
  // fluid_damp: 高湿が負担 / fluid_deficiency: 低湿が負担
...
function signedComponentPenalty({ anomaly, burdenDir, a, b }) {
  // returns signed contribution: -2..+2 (benefit negative)
  ...
  if (burdenDir === "high") {
    // anomaly positive => burden, negative => benefit
    return x >= 0 ? +mag : -mag;
  }
  if (burdenDir === "low") {
    // anomaly negative => burden, positive => benefit
    return x <= 0 ? +mag : -mag;
  }
  return 0;
}
...
export function computeInfluenceFromBaselines({ current, baseline, profile }) {
  ...
  const yy = profile?.computed?.axes?.yin_yang_label_internal || null;
  ...
  // 表示レベルへ（-側は「受けにくい」）
  let level = 1; // 通常
  if (rawSigned <= -0.8) level = 0;         // 受けにくい
  else if (rawSigned >= 1.0) level = 2;     // 受けやすい
  else level = 1;
  ...
  return {
    ...
    constitution: {
      core_code: coreTitleFromProfile(profile),
...
export function buildTimeWindowsFromHourly(hourly, nowIdx, hoursForward, profile) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (ext.change_score <= 0) {
      ...
      // 2) 小変化：内因の影響を抑制（vuln最大+1）
      finalScore = ext.change_score + Math.min(vulnerability, 1);
    } else {
      // 3) 中〜大変化：内因フル加算
      finalScore = ext.change_score + vulnerability;
    }
...
export function buildWindowHintJa({ window, influenceLevel }) {
  ...
  if (influenceLevel === 2) {
    return `影響を受けやすい日。ここは「${trig}」が主因なので、詰めすぎ注意。`;
  }
  if (influenceLevel === 0) {
