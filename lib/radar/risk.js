// lib/radar/risk.js
// Phase1b: 「人（体質/主訴）が主語」→ 気象変化は入力
// - 個人感受性ベクトル S_user = {Wp, Wt, Wh} を先に確定
// - 各時刻 t で寄与 E_p/E_t/E_h を計算し、risk(t) を作る
// - ピーク帯は max同値ではなく「p90以上の連続帯」
// - 主因はピーク帯の寄与合計（E_*）から決定（＝個人化される）
...
function constitutionAdjust(profile) {
  ...
  // 3-2) core_code補正（寒熱/混合は気温・気圧に薄く）
...
export function computeRiskTimelineToday({ hourly, dateStrJST, profile, confidence }) {
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    // その時間の“主因”（個人寄与ベース。interactionも主因に寄せる）
    // interaction寄与の割り当て（シンプル）
...
export function summarizeHeroFromTimeline({ items, profile, confidence }) {
  ...
  // 強度は “最大＋持続” を混ぜる（体感に近づける）
  const raw = 0.7 * rMax + 0.3 * rP90;

  // 係数は Phase1 なので保守的（運用で調整）
  ...
  if (band.startIdx >= 0) {
    ...
    // ピーク帯が取れない時は max時刻で決める
    const idx = items.reduce((best, it, i) => {
      const r = safeNum(it?.risk) ?? 0;
      const br = best ? (safeNum(items[best]?.risk) ?? 0) : -1;
      return r > br ? i : best;
    }, null);
    if (idx != null) {
