...
function pressureBurdenDirection(defExScore) {
  ...
  if (dx >= 0.22) return "high"; // 実寄り：高圧が負担
  if (dx <= -0.22) return "low"; // 虚寄り：低圧が負担
  return "none";
}
...
export function computeInfluenceFromBaselines({ current, baseline, profile }) {
  ...
  // ✅ 日常で差が出る閾値へ（方針2）
  const pSigned = signedComponentPenalty({ anomaly: aP, burdenDir: pDir, a: 1.5, b: 3.5 });
  const tSigned = signedComponentPenalty({ anomaly: aT, burdenDir: tDir, a: 1.5, b: 3.5 });
  const hSigned = signedComponentPenalty({ anomaly: aH, burdenDir: hDir, a: 6.0, b: 12.0 });
