...
function buildMatchReason(candidate, input, explicitTerm, symptomMatches, clueMatches, baselineMatches) {
  ...
  const baselineEvidence = baselineMatches
    ...
    .filter(Boolean)
    .slice(0, 2);
  const lead = evidence.length && baselineEvidence.length
    ? `今回選んだ「${evidence.join("・")}」に加え、体質チェックの「${baselineEvidence.join("・")}」も候補条件に合います。`
    : evidence.length
      ? `今回選んだ「${evidence.join("・")}」が候補条件に合います。`
      : baselineEvidence.length
        ? `体質チェックの「${baselineEvidence.join("・")}」が候補条件に合います。`
        : "今回の回答と体質傾向を合わせて確認しました。";
  return `${lead}${candidate.why}`;
}
