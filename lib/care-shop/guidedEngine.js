...
const ANSWER_LABELS = {
  ...
  reserve: { low: "少し動くだけで疲れそう", standard: "体力は普段とあまり変わらない", high: "普段より動けそう" },
...
export function buildBaselineFromProfile(profile) {
  ...
  const axes = computed?.axes || profile?.axes || {};
  const reserveValue = Number(axes?.reserve_score ?? axes?.drive_score);
  const reserve = Number.isFinite(reserveValue)
    ? reserveValue <= -0.25 ? "low" : reserveValue >= 0.25 ? "high" : "standard"
    : core.includes("small") ? "low" : core.includes("large") ? "high" : "standard";
  const coldScore = Math.max(0, Math.min(1, Number(axes?.cold_score || 0)));
  const heatScore = Math.max(0, Math.min(1, Number(axes?.heat_score || 0)));
  const thermal = coldScore >= 0.5 && heatScore >= 0.5
    ? "mixed"
    : coldScore > heatScore ? "cold" : heatScore > coldScore ? "heat" : "neutral";
  const scores = {};
  if (reserve === "low") bump(scores, "energy_low", 2);
  if (coldScore > 0) bump(scores, "cold", coldScore * 3);
  if (heatScore > 0) bump(scores, "heat", heatScore * 3);

  const split = computed?.split_scores || profile?.split_scores || {};
  const materials = computed?.material_scores || profile?.material_scores || {
  ...
  const normalizedMaterial = Object.fromEntries(Object.entries(materials).map(([key, raw]) => {
  ...
  const hasMaterials = Object.values(normalizedMaterial).some((value) => value > 0);
  if (hasMaterials) {
    bump(scores, "energy_low", 3 * Number(normalizedMaterial.qi_deficiency || 0));
    bump(scores, "energy_low", 2.4 * Number(normalizedMaterial.blood_deficiency || 0));
    bump(scores, "tension", 3 * Number(normalizedMaterial.qi_stagnation || 0));
    bump(scores, "stagnation", 3 * Number(normalizedMaterial.blood_stasis || 0));
    bump(scores, "damp", 3 * Number(normalizedMaterial.fluid_damp || 0));
    bump(scores, "dry", 3 * Number(normalizedMaterial.fluid_deficiency || 0));
  } else {
  ...
  // A 0–100 material score around the middle range should be available as
  // supporting evidence, while avoid rules use only stronger tendencies.
  const stateKeys = rankedKeys(scores, 1.35);
  const strongStateKeys = rankedKeys(scores, 2);
  const labels = stateKeys.slice(0, 3).map((key) => BASELINE_LABELS[key]).filter(Boolean);
  return {
    reserve,
    thermal,
    sub_labels: subLabels,
    material_scores: normalizedMaterial,
...
export function buildSelectedAnswerLabels(input = {}, concerns = []) {
  const concernLabels = unique(concerns.map((key) => SYMPTOM_LABELS[key]).filter(Boolean));
  const intensity = input.intensity && input.intensity !== "moderate"
    ? ANSWER_LABELS.intensity[input.intensity]
    : null;
  const clues = selectedClues(input).map((key) => CLUE_LABELS[key]);
  return unique([
    ...concernLabels,
    ANSWER_LABELS.duration[input.duration],
    intensity,
    ANSWER_LABELS.thermal[input.thermal],
    ANSWER_LABELS.moisture[input.moisture],
    ANSWER_LABELS.reserve[input.reserve],
    ANSWER_LABELS.digestion[input.digestion],
    ANSWER_LABELS.response[input.response],
    ...clues,
  ]).slice(0, 7);
}

export function buildSelectedAnswerSummary(input = {}, concerns = [], baseline = null) {
  const concernLabels = unique(concerns.map((key) => SYMPTOM_LABELS[key]).filter(Boolean)).slice(0, 1);
  const clueLabels = selectedClues(input).map((key) => CLUE_LABELS[key]).filter(Boolean).slice(0, 2);
  const supportingLabels = [
    ["weeks", "months"].includes(input.duration) ? ANSWER_LABELS.duration[input.duration] : null,
    ANSWER_LABELS.response[input.response],
    ANSWER_LABELS.thermal[input.thermal],
    ANSWER_LABELS.moisture[input.moisture],
    input.reserve === "low" ? ANSWER_LABELS.reserve.low : null,
    ANSWER_LABELS.digestion[input.digestion],
    input.intensity && input.intensity !== "moderate" ? ANSWER_LABELS.intensity[input.intensity] : null,
  ];
  const answerLabels = unique([...concernLabels, ...clueLabels, ...supportingLabels]).slice(0, 5);
  const answerText = answerLabels.length ? `今回の「${answerLabels.join("・")}」` : "今回の回答";
  const baselineLabels = unique(baseline?.labels || []).slice(0, 2);
  if (baselineLabels.length) {
    return `体質チェックの「${baselineLabels.join("・")}」と、${answerText}を合わせて確認しています。`;
  }
  return `${answerText}を確認しています。`;
}
...
export function deriveCurrentState(input = {}, profile = null) {
  ...
  selectedClues(input).forEach((key) => {
    ...
    if (key === "leg_pain_numbness") bump(scores, "stagnation", 3);
    if (key === "cold_damp_pain") { bump(scores, "cold", 2); bump(scores, "damp", 2); bump(scores, "stagnation", 2); }
    if (key === "fixed_night_pain") bump(scores, "stagnation", 4);
    if (key === "urination_change_cold") { bump(scores, "cold", 2); bump(scores, "damp", 2); bump(scores, "energy_low", 2); }
  ...
  const answerLabels = buildSelectedAnswerLabels(input, concerns);
  const summary = buildSelectedAnswerSummary(input, concerns, baseline);

  return { concerns, scores, stateKeys, stateLabels: STATE_LABELS, labels, answerLabels, summary, baseline };
}
...
function buildMatchReason(candidate) {
  return candidate.why;
}

function buildMatchSummary(candidate, input, explicitTerm, symptomMatches, clueMatches, baselineMatches) {
  if (explicitTerm) return `入力：${explicitTerm}`;
  const evidence = directAnswerEvidence(candidate, input, symptomMatches, clueMatches);
  const baselineEvidence = baselineMatches
    .map((key) => BASELINE_LABELS[key])
    .filter(Boolean)
    .slice(0, 1);
  const currentText = evidence.slice(0, 2).join("・");
  const baselineText = baselineEvidence.length ? `体質：${baselineEvidence.join("・")}` : "";
  return [currentText, baselineText].filter(Boolean).join(" ／ ") || "今回の回答と体質傾向";
}

function hasAvoidConflict(candidate, input, currentState) {
  return candidate.avoidStates.some((key) => {
    if (currentState.baseline.strongStateKeys.includes(key)) return true;
    if (key === "heat") return input.thermal === "heat";
    if (key === "cold") return input.thermal === "cold";
    if (key === "damp") return input.moisture === "damp";
    if (key === "dry") return input.moisture === "dry";
    if (key === "energy_low") return input.reserve === "low";
    if (key === "digestive_weak") return ["appetite_low", "postmeal_heavy", "bowel_change"].includes(input.digestion);
    return currentState.stateKeys.includes(key);
  });
}
...
function evaluateCandidate(candidate, input, currentState, activeIngredientIds) {
  ...
  if (!explicitTerm && hasAvoidConflict(candidate, input, currentState)) return null;
  const baselineSupport = candidate.type === "selfcare"
    ? 0
    : Math.min(2, baselineMatches.filter((key) => !currentMatches.includes(key)).length);
  if (!explicitTerm && currentMatches.length + baselineSupport < candidate.minCurrentStateMatches) return null;

  const duplicateIngredients = candidate.ingredientIds.filter((id) => activeIngredientIds.includes(id));
  const score = (explicitTerm ? 50 : 0) + symptomMatches.length * 8 + clueMatches.length * 6 + currentMatches.length * 4 + baselineMatches.length * 3 - duplicateIngredients.length * 2 + (candidate.type === "selfcare" ? 1 : 0) + Number(candidate.scoreAdjustment || 0);
  return {
    ...candidate,
    explicitTerm,
    symptomMatches,
    ...
    matchSummary: buildMatchSummary(candidate, input, explicitTerm, symptomMatches, clueMatches, baselineMatches),
...
export function buildGuidedSearchResult(input = {}, profile = null, activeEntries = []) {
  ...
  const currentState = deriveCurrentState(input, input.subject === "other" ? null : profile);
  ...
  const groups = types.map((type) => {
    const candidates = GUIDED_CANDIDATES
      ...
      .filter(Boolean)
      .sort((a, b) => Number(!!b.explicitTerm) - Number(!!a.explicitTerm) || b.score - a.score || a.title.localeCompare(b.title, "ja"))
