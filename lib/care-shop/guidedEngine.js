...
function evaluateCandidate(candidate, input, currentState, activeIngredientIds) {
  ...
  const score = (explicitTerm ? 50 : 0) + symptomMatches.length * 8 + clueMatches.length * 6 + currentMatches.length * 4 + baselineMatches.length * 3 - duplicateIngredients.length * 2 + (candidate.type === "selfcare" ? 1 : 0);
  return {
    ...candidate,
    explicitTerm,
    symptomMatches,
