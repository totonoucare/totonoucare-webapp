...
export const RECORDS_FORECAST_MODEL_CONTEXT = {
  model_version: "radar_forecast_v2_2026-09-11_load_anchors",
  hierarchy: [
    ...
    "第5層は主因・副因と体調警戒度。急な変化による揺さぶりと、温湿度を一体化した絶対環境負担の大きい方を土台にし、両方が重なる時だけ小さく加点する。",
  ...
  personalization: {
    formula: "気象強度 ×（全員共通分0.50＋体質親和分0.50×本人親和性）",
    universal_weather_share: 0.50,
    personal_affinity_share: 0.50,
  ...
  score_construction: {
    weighted_groups: "気圧・気温・水分環境の変化負担を、強い順に10.20・2.40・1.20で加重する。気象強度は閾値で0.65、より強い変化は滑らかに1へ近づく。",
    ...
    overlap: "変化スコアと絶対環境スコアの大きい方を土台とし、独立した気圧負担と絶対環境負担の小さい方の22％を加える。7を超える部分は滑らかに10へ近づけ、極端環境の共通下限も適用する。",
    modes: [
  ...
  important_boundaries: [
    "体調警戒度は発症確率や症状の重さではなく、先回りケアの強さを決める0〜10の物差し。",
    ...
    "environmental_cautionsは体調警戒度とは別の絶対気温注意であり、公的警戒アラートやWBGTとして説明しない。",
...
export function buildForecastReasoningContext(forecast = {}) {
  ...
  const isLoadAnchoredV2 = modelVersion.includes("load_anchors");
  const isComfortCalibratedV2 = isLoadAnchoredV2 || modelVersion.includes("moisture_state_direction") || modelVersion.includes("comfort_calibrated") ||
  ...
  return {
    ...
    personalization: {
      formula: isV2
        ? isComfortCalibratedV2
          ? isLoadAnchoredV2 ? RECORDS_FORECAST_MODEL_CONTEXT.personalization.formula : "保存当時のV2式：気象強度 ×（全員共通分0.38＋体質親和分0.62×本人親和性）"
