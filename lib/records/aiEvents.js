...
export const RECORDS_AI_CONSENT_VERSION = "records_ai_v5_consultation_context_2026-07-14";
...
export const RECORDS_AI_MONTHLY_CHAT_LIMIT = positiveInt(
  process.env.RECORDS_AI_MONTHLY_CHAT_LIMIT,
  100
);
export const RECORDS_AI_DAILY_ANALYSIS_LIMIT = positiveInt(
  process.env.RECORDS_AI_DAILY_ANALYSIS_LIMIT,
  3
);
export const RECORDS_AI_PER_MINUTE_LIMIT = positiveInt(
  process.env.RECORDS_AI_PER_MINUTE_LIMIT,
  6
);
...
function estimatedCostUsd(usage = {}) {
  const inputRate = Number(process.env.OPENAI_RECORDS_INPUT_USD_PER_MTOK || 1);
  const outputRate = Number(process.env.OPENAI_RECORDS_OUTPUT_USD_PER_MTOK || 6);
