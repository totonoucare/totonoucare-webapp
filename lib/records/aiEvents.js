...
export async function getFreeConsultTrialUsage(userId, limit = 2) {
  const { count, error } = await supabaseServer
    .from("records_ai_events")
    .select("id", { count: "exact", head: true })
    .eq("user_id", userId)
    .eq("event_type", "free_chat_response");
  if (error) {
    const wrapped = new Error(`records AI trial infrastructure is not ready: ${error.message}`);
    wrapped.code = "records_ai_schema_required";
    throw wrapped;
  }
...
export function assertQuota(usage, kind) {
  ...
  if (kind === "analysis" && Number(usage?.analysis?.used || 0) >= Number(usage?.analysis?.limit || 0)) {
    const error = new Error("本日の振り返り更新上限に達しました。保存済みの内容は引き続き確認できます。");
