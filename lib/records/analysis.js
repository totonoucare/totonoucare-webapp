// Symptom timing is confirmed per action. Calendar timing and legacy bulk answers
// remain in timing_relation; neither is evidence of an individual answer.
export const ACTION_SYMPTOM_TIMINGS = ["before_peak", "after_symptom", "mixed", "no_symptoms", "unknown"];
export function actionSymptomTiming(action) {
  const meta = action?.item_snapshot?.meta || {};
  return meta.timing_source === "individual" && ACTION_SYMPTOM_TIMINGS.includes(meta.symptom_timing)
    ? meta.symptom_timing : "unknown";
}
export function aggregateActionTiming(actions) {
  if (!Array.isArray(actions) || !actions.length) return "";
  const values = new Set(actions.map(actionSymptomTiming));
  if (values.has("unknown") || (values.has("no_symptoms") && values.size > 1)) return "unknown";
  if (values.has("mixed") || values.size > 1) return "mixed";
  return [...values][0];
}
export function recordCareTiming(row) {
  const actions = Array.isArray(row?.care_actions) ? row.care_actions : [];
  if (!actions.length) return reviewCareTiming(row?.review);
  const actionTiming = aggregateActionTiming(actions);
  const manual = Number(row?.review?.manual_prevent_level || 0) > 0 ? row.review.manual_care_timing : "";
  if (!ACTION_SYMPTOM_TIMINGS.includes(manual)) return actionTiming;
  if (manual !== actionTiming && (manual === "no_symptoms" || actionTiming === "no_symptoms")) return "unknown";
  if (manual === "unknown" || actionTiming === "unknown") return "unknown";
  return manual === actionTiming ? manual : "mixed";
}
export function actionSymptomTimingLabel(action) {
  const meta = action?.item_snapshot?.meta || {};
  if (meta.timing_source !== "individual") return meta.timing_source === "unanswered"
    ? "ケアのタイミング：未回答" : "ケアのタイミング：個別の確認なし";
  return { before_peak: "つらくなる前", after_symptom: "つらくなってから", mixed: "前後どちらも", no_symptoms: "つらさは出なかった", unknown: "覚えていない" }[actionSymptomTiming(action)];
}
...
export const RECORD_TIMING_OPTIONS = [
  { value: "before_peak", label: "天気ストレスのピーク前" },
...
const TIMING_VALUES = new Set([...RECORD_TIMING_OPTIONS.map((item) => item.value), "no_symptoms"]);
...
export function careTimingLabel(value) {
  if (value === "no_symptoms") return "つらさは出なかった";
  if (value === "before_peak") return "前もって";
...
function actionTimingToReviewTiming(actionSummary) {
  ...
  return aggregateActionTiming(actionSummary.actions);
}

function careTimingFlags(row) {
  const actions = safeArray(row?.care_actions);
  const relations = new Set(actions.map(actionSymptomTiming));
  ...
  const hasBefore = relations.has("before_peak")
    || relations.has("mixed")
    || aggregate === "before_peak"
    || aggregate === "mixed";
  const hasAfter = relations.has("after_symptom")
    || relations.has("mixed")
    || aggregate === "after_symptom"
    || aggregate === "mixed";
  const hasNoSymptoms = relations.has("no_symptoms") || aggregate === "no_symptoms";
  const hasUnknown = relations.has("unknown") || aggregate === "unknown" || (!hasBefore && !hasAfter && !hasNoSymptoms);
  return {
    before_peak: hasBefore,
    after_symptom: hasAfter,
    mixed: (hasBefore && hasAfter) || relations.has("mixed") || aggregate === "mixed",
    unknown: hasUnknown,
    no_symptoms: hasNoSymptoms,
...
function normalizeRow(row) {
  ...
  return {
    ...
    care_timing: recordCareTiming({ ...row, care_actions: actionSummary.actions }),
    care_actions: actionSummary.actions,
    care_action_summary: actionSummary,
    timing_flags: careTimingFlags({ ...row, care_actions: actionSummary.actions, care_timing: recordCareTiming({ ...row, care_actions: actionSummary.actions }) }),
...
function createTimingOutcomes() {
  return {
    ...
    no_symptoms: { days: 0, actual_counts: createActualCounts() },
  };
}

function addTimingOutcome(outcomes, timing, severity) {
  const key = ["before_peak", "after_symptom", "mixed", "no_symptoms", "unknown"].includes(timing) ? timing : "unknown";
  outcomes[key].days += 1;
...
function addTimingEvidence(outcomes, flags, severity) {
  ...
  if (flags?.no_symptoms) addTimingOutcome(outcomes, "no_symptoms", severity);
...
function comparisonModelVersion(forecast) {
  return forecast?.forecast_model_version || forecast?.reason_trace?.forecast_model_version ||
    forecast?.computed?.forecast_snapshot?.forecast_model_version ||
    forecast?.logic_version?.forecast_context || "legacy_unspecified";
}
...
function buildMatchedForecastComparisons(rows) {
  ...
  rows.filter((row) => row.classification.comparable).forEach((row) => {
    ...
    const key = `${trigger}:${signal}:${band.min}:${comparisonModelVersion(row.forecast)}`;
    if (!groups.has(key)) {
      groups.set(key, {
        key,
        forecast_model_version: comparisonModelVersion(row.forecast),
        exact_trigger: trigger,
        trigger_label: triggerLabelExact(trigger),
        signal,
        signal_label: signalLabel(signal),
...
function buildSpecificCarePatterns(rows) {
  ...
  rows.filter((row) => row.classification.comparable && row.care_actions.some((action) => action.entry_origin !== "record_page")).forEach((row) => {
    ...
    dailyCare.forEach((daily) => {
      const key = `${trigger}:${signal}:${band.min}:${daily.identity}:${comparisonModelVersion(row.forecast)}`;
      if (!groups.has(key)) {
        groups.set(key, {
          key,
      ...
      daily.actions.forEach((action) => timings.add(actionSymptomTiming(action)));
...
export function buildRecordsSummary(rows) {
  ...
  return {
    ...
    no_symptoms_care_days: careRows.filter((row) => row.timing_flags.no_symptoms).length,
...
function formatActualCountSummary(counts = {}) {
  return [
    ["穏やか", counts.good],
    ["軽い不調", counts.mild],
    ["つらい", counts.hard],
    ...
    .map(([label, days]) => `${label} ${days}日`)
    .join("／");
}

/**
 * Turns matched comparison facts into a readable UI model.
 * AI evidence strings remain useful for individual dates, but comparison
 * counts are rendered from the deterministic summary so the condition,
 * group size, and outcomes cannot be compressed into an ambiguous sentence.
 */
export function buildReflectionEvidenceDetails(summary, aiEvidence = []) {
  const evidenceItems = safeArray(aiEvidence)
    .map((item) => String(item || "").trim())
    .filter(Boolean);
  const comparisonCandidates = safeArray(summary?.matched_forecast_comparisons)
    .filter((comparison) => (
      Number(comparison?.total_days) > 0
      && Number(comparison?.care_days) > 0
      && Number(comparison?.no_care_days) > 0
    ));
  const selectedComparison = comparisonCandidates
    .map((comparison, index) => ({
      comparison,
      index,
      matchScore: evidenceItems.reduce((score, item) => (
        score
        + (comparison?.trigger_label && item.includes(comparison.trigger_label) ? 2 : 0)
        + (comparison?.signal_label && item.includes(comparison.signal_label) ? 1 : 0)
      ), 0),
    }))
    .sort((a, b) => b.matchScore - a.matchScore || a.index - b.index)[0]?.comparison;
  const comparisons = (selectedComparison ? [selectedComparison] : [])
    .map((comparison) => {
      const scoreBand = comparison?.score_band?.label ? `体調警戒度${comparison.score_band.label}` : "近い体調警戒度";
      const signal = comparison?.signal_label ? `（${comparison.signal_label}）` : "";
      const trigger = comparison?.trigger_label || "同じ天気ストレス";
      const groups = [
        {
          key: "care",
          label: "ケアあり",
          days: Number(comparison?.care_days) || 0,
          outcome: formatActualCountSummary(comparison?.care_actual_counts),
        },
        {
          key: "no_care",
          label: "ケアなし",
          days: Number(comparison?.no_care_days) || 0,
          outcome: formatActualCountSummary(comparison?.no_care_actual_counts),
        },
      ].filter((group) => group.days > 0);

      return {
        key: comparison.key,
        condition: `${trigger}が主・${scoreBand}${signal}`,
        total_days: Number(comparison.total_days) || groups.reduce((sum, group) => sum + group.days, 0),
        groups,
      };
    });

  const notes = evidenceItems
    .filter((item) => !(
      comparisons.length
      && item.includes("ケアあり")
      && item.includes("ケアなし")
...
/**
 * Builds the single, outcome-first message shown at the top of the reflection page.
 * "Proactive care" is limited to care recorded before the weather-stress peak,
 * including care completed from the previous day's tomorrow card.
 */
export function buildReflectionBenefit(summary) {
  const rows = safeArray(summary?.rows);
  const recordedRows = rows.filter((row) => actualSeverity(row?.review) != null);
  const comparableRows = recordedRows.filter((row) => forecastSeverity(row?.forecast) != null);
  const attentionRows = comparableRows.filter((row) => Number(forecastSeverity(row?.forecast)) >= 1);
  const proactiveRows = attentionRows.filter((row) => (
    Boolean(row?.classification?.careDone) && Boolean(row?.timing_flags?.before_peak)
  ));
  const proactiveGoodRows = proactiveRows.filter((row) => actualSeverity(row?.review) === 0);
  const proactiveDifficultRows = proactiveRows.filter((row) => actualSeverity(row?.review) >= 1);
  const attentionGoodRows = attentionRows.filter((row) => actualSeverity(row?.review) === 0);
  const attentionDifficultRows = attentionRows.filter((row) => actualSeverity(row?.review) >= 1);
  const stableDifficultRows = comparableRows.filter((row) => (
    forecastSeverity(row?.forecast) === 0 && actualSeverity(row?.review) >= 1
  ));

  const flow = proactiveRows.length
    ? [
        { key: "attention", value: attentionRows.length, label: "いたわり・守り" },
        { key: "proactive", value: proactiveRows.length, label: "先回りケア" },
        { key: "good", value: proactiveGoodRows.length, label: "そのうち体調○" },
      ]
    : [];

  if (proactiveGoodRows.length) {
    return {
      state: "proactive_good",
      headline: `先回りして穏やかに過ごせた日が${proactiveGoodRows.length}日ありました`,
      detail: "同じ日の体調予報・先回りケア・実感をつないだ記録です。",
      flow,
    };
  }

  if (proactiveDifficultRows.length) {
    return {
      state: "proactive_difficult",
      headline: `先回りケアを試した記録が${proactiveRows.length}日あります`,
      detail: "ケア後の実感も、次の種類やタイミングを考える手がかりになります。",
      flow,
    };
  }

  if (attentionDifficultRows.length) {
    return {
      state: "attention_difficult",
      headline: `いたわり・守り予報でつらさがあった日が${attentionDifficultRows.length}日あります`,
      detail: "次の似た予報で軽いケアを一つ試すと、比べる記録が増えていきます。",
      flow: [],
    };
  }

  if (attentionGoodRows.length) {
    return {
      state: "attention_good",
      headline: `いたわり・守り予報でも穏やかに過ごせた日が${attentionGoodRows.length}日ありました`,
      detail: "穏やかだった日の生活やケアも、次に再現したい条件の一つです。",
      flow: [],
    };
  }

  if (stableDifficultRows.length) {
    return {
      state: "stable_difficult",
      headline: `安定予報でもつらさがあった日が${stableDifficultRows.length}日あります`,
      detail: "睡眠や忙しさなど、その日に重なった生活条件も一緒に振り返れます。",
      flow: [],
    };
  }
  ...
  if (recordedRows.length) {
    return {
      state: "progress",
      headline: `この期間に${recordedRows.length}日分の記録が集まりました`,
      detail: "体調予報・ケア・実感が同じ日にそろうと、整え方を比べやすくなります。",
      flow: [],
    };
  }
  ...
  return {
    state: "empty",
    headline: "記録が増えると、整え方の手がかりが見えてきます",
    detail: "まずは今日の実感を○・△・×から残してみましょう。",
    flow: [],
  };
}
...
function compactGroupKey(point, index, periodDays) {
  if (periodDays <= 7) return point.date || String(index);
  if (periodDays <= 30) return `week-${Math.floor(index / 7)}`;
  const [year, month] = String(point.date || "").split("-").map(Number);
  if (!year || !month) return `unknown-${index}`;
  if (periodDays <= 180) return `${year}-${String(month).padStart(2, "0")}`;
  const serialMonth = year * 12 + (month - 1);
  return `two-month-${Math.floor(serialMonth / 2)}`;
}

function compactDateLabel(start, end, periodDays) {
  const [, startMonth, startDay] = String(start || "").split("-").map(Number);
  const [, endMonth, endDay] = String(end || "").split("-").map(Number);
  if (!startMonth) return "";
  if (periodDays <= 7) return `${startMonth}/${startDay}`;
  if (periodDays <= 30) return start === end
    ? `${startMonth}/${startDay}`
    : `${startMonth}/${startDay}〜`;
  if (periodDays <= 180 || startMonth === endMonth) return `${startMonth}月`;
  return `${startMonth}–${endMonth}月`;
}

function dominantCountKey(counts = {}) {
  return Object.entries(counts)
    .filter(([, count]) => Number(count) > 0)
    .sort((a, b) => Number(b[1]) - Number(a[1]) || String(a[0]).localeCompare(String(b[0])))[0]?.[0] || null;
}

export function buildCompactChartPoints(rows, periodDays = 30) {
  const daily = buildChartPoints(rows, 30);
  const groups = new Map();
  daily.forEach((point, index) => {
    const key = compactGroupKey(point, index, periodDays);
  ...
  return Array.from(groups.values()).map((items) => {
    ...
    const end = items[items.length - 1]?.end_date || items[items.length - 1]?.date || start;
    const forecastValues = items.map((item) => Number(item.forecast)).filter(Number.isFinite);
    const forecast = forecastValues.length
      ? Math.round((forecastValues.reduce((sum, value) => sum + value, 0) / forecastValues.length) * 10) / 10
      : null;
    const actualCounts = items.reduce((result, item) => ({
      good: result.good + Number(item.actual_counts?.good || 0),
      mild: result.mild + Number(item.actual_counts?.mild || 0),
      hard: result.hard + Number(item.actual_counts?.hard || 0),
    }), { good: 0, mild: 0, hard: 0 });
    const timingCounts = items.reduce((result, item) => {
      Object.entries(item.care_timing_counts || {}).forEach(([key, count]) => {
        result[key] = Number(result[key] || 0) + Number(count || 0);
      });
      return result;
    }, {});
    const triggerCounts = items.reduce((result, item) => {
      Object.entries(item.trigger_counts || {}).forEach(([key, count]) => {
        result[key] = Number(result[key] || 0) + Number(count || 0);
      });
      return result;
    }, {});
    const exactTrigger = dominantCountKey(triggerCounts);
    const single = items.length === 1;
    const actualSeverity = single
      ? items[0].actual_severity
      : ({ good: 0, mild: 1, hard: 2 }[dominantCountKey(actualCounts)] ?? null);
    const forecastSeverityValue = forecast == null ? null : forecast >= 70 ? 2 : forecast >= 40 ? 1 : 0;

    return {
      date: start,
      end_date: end,
      label: compactDateLabel(start, end, periodDays),
      is_aggregate: !single,
      forecast,
      forecast_severity: forecastSeverityValue,
      actual_severity: actualSeverity,
      actual_counts: actualCounts,
      exact_trigger: exactTrigger,
      trigger_label: triggerLabelExact(exactTrigger),
      care_timing: single ? items[0].care_timing : dominantCountKey(timingCounts) || "",
      care_timing_counts: timingCounts,
      domains: Array.from(new Set(items.flatMap((item) => item.domains || []))),
      recorded_count: items.reduce((sum, item) => sum + Number(item.recorded_count || 0), 0),
      forecast_count: items.reduce((sum, item) => sum + Number(item.forecast_count || 0), 0),
      care_count: items.reduce((sum, item) => sum + Number(item.care_count || 0), 0),
      rows: items.flatMap((item) => item.rows || []),
    };
  });
}
...
export function deterministicAnalysis(summary) {
  ...
  if (count < 3) {
    return {
      ...
      hypotheses: "同じ天気ストレスと近い体調警戒度の日がもう少し増えると、ケアの有無やタイミングと実感を比べやすくなります。",
  ...
  if (careDifficult >= 2) {
    return {
      ...
      question: "前もってケアできた日と、つらくなってからの日で違いはありましたか？",
  ...
  if (noCareDifficult >= 1) {
    return {
      ...
      hypotheses: "似た天気ストレスの日には、天気ストレスのピーク前に軽いケアを入れると比較材料になります。",
      next_step: "次は「暮らす・食べる・ほぐす」から一つだけ選び、先回りして記録してみましょう。",
      question: "次に試しやすいのは、暮らす・食べる・ほぐすのどれですか？",
      suggested_questions: ["次に試すケアを選びたい", "天気ストレスのピーク前に何をすればいい？"],
  ...
  return {
    ...
    evidence: [`いたわり・守り予報・つらさあり ${attentionDifficult}日`],
  };
}
...
export function trimRecordForAi(row) {
  ...
  const reasonTrace = row?.forecast?.reason_trace && typeof row.forecast.reason_trace === "object"
    ? row.forecast.reason_trace
    : {};
  const pressureResponseDirection =
    row?.forecast?.pressure_response_direction ||
    reasonTrace?.pressure_response_direction ||
    reasonTrace?.manifestation?.reaction_direction ||
    null;
  const pressureDirection = row?.forecast?.pressure_direction || reasonTrace?.pressure_direction || null;
  ...
  const serializeCareItem = (item) => ({
    ...
    ...(item.domain === "eat" ? {
      record_semantics:item.item_snapshot?.meta?.record_semantics||null,
      consumed_name:["ingredient_consumed","drink_consumed"].includes(item.item_snapshot?.meta?.record_semantics)?item.item_snapshot?.meta?.consumed_name||null:null,
      consumed_id:item.item_snapshot?.meta?.consumed_id||null,
      selection_reason:item.item_snapshot?.meta?.selection_reason||null,
      selection_basis:item.item_snapshot?.meta?.selection_basis||null,
      meal_example:item.item_snapshot?.meta?.meal_example||null,
    }:{}),
    source_label: item.source_mode === "tomorrow" ? "前日の明日カード" : "当日の今日カード",
    timing_relation: item.timing_relation,
    symptom_timing: actionSymptomTiming(item),
    timing_source: item.item_snapshot?.meta?.timing_source || "legacy_bulk_or_calendar",
    ...(item.kind === "tsubo_line_care" ? {
      meridian_code: item.item_snapshot?.meta?.meridian_code || null,
      selection_source: item.item_snapshot?.meta?.selection_source || null,
      selection_reason: item.item_snapshot?.meta?.selection_reason || null,
    } : {}),
  });
  return {
    ...
    forecast: row?.forecast
      ? {
          ...
          personal_main_event_key:
            row.forecast.personal_main_event_key || reasonTrace?.personal_main_event_key || null,
          ...
          trigger_factors: safeArray(row.forecast.trigger_factors).slice(0, 2).map((item) => {
            ...
            const isPressure = ["pressure_down", "pressure_up", "pressure_shift"].includes(exact);
            const responseDirection = item?.response_direction || (isPressure ? pressureResponseDirection : null);
            const physicalDirection =
              item?.physical_direction || item?.direction ||
              (exact === "pressure_down" ? "down" : exact === "pressure_up" ? "up" : isPressure ? pressureDirection : null);
            return {
              exact,
              event_key: item?.event_key || (isPressure ? "pressure_shift" : null),
              role: item?.role || null,
              physical_direction: physicalDirection,
              response_direction: responseDirection,
              body_response_key:
                item?.body_response_key || (isPressure && responseDirection ? `pressure_${responseDirection}` : null),
          ...
          environmental_cautions: safeArray(
            row.forecast.environmental_cautions || reasonTrace?.environmental_cautions
          ).slice(0, 3),
...
function compactEnvironmentalCautions(value) {
  return safeArray(value).slice(0, 3).map((item) => ({
    key: item?.key || null,
    ...
    official_alert: Boolean(item?.official_alert),
  }));
}
...
function reasonTraceFromForecast(forecast) {
  ...
  return {
    ...
    pressure_response_direction:
      riskContext?.summary?.pressure_response_direction ||
      riskContext?.weather_context?.pressure_response_direction ||
      personalized?.manifestation?.reaction_direction ||
      null,
    personal_main_event_key: riskContext?.summary?.personal_main_event_key || null,
    ...
    environmental_cautions: compactEnvironmentalCautions(
      riskContext?.summary?.environmental_cautions || riskContext?.weather_context?.environmental_cautions
    ),
...
export function snapshotFromForecast(forecast, source = "record_save") {
  ...
  return {
    ...
    personal_secondary_trigger_exact:
      ...
      null,
    personal_main_event_key:
      forecast.personal_main_event_key || computedSnapshot.personal_main_event_key || null,
    pressure_direction:
      forecast.pressure_direction || computedSnapshot.pressure_direction || null,
    pressure_response_direction:
      forecast.pressure_response_direction || computedSnapshot.pressure_response_direction || null,
    trigger_factors: triggerFactors,
    environmental_cautions: compactEnvironmentalCautions(
      forecast.environmental_cautions || computedSnapshot.environmental_cautions
    ),
    ...
    displayed_care: forecast.displayed_care || computedSnapshot.displayed_care || null,
