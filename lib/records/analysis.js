...
export const ACTION_SYMPTOM_TIMINGS = ["before_peak", "after_symptom", "mixed", "unknown"];
...
export function actionSymptomTimingLabel(action) {
  ...
  if (meta.timing_source !== "individual") return meta.timing_source === "unanswered"
    ? "つらさとの前後：未回答" : "つらさとの前後：個別の確認なし";
  return { before_peak: "つらくなる前", after_symptom: "つらくなってから", mixed: "前後どちらも", unknown: "覚えていない" }[actionSymptomTiming(action)];
}
...
function careTimingFlags(row) {
  ...
  const hasAfter = relations.has("after_symptom")
    ...
    || aggregate === "mixed";
  const hasUnknown = relations.has("unknown") || aggregate === "unknown" || (!hasBefore && !hasAfter);
  return {
