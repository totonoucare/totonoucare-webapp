...
function summarizeCareActions(actions) {
  const normalized = (Array.isArray(actions) ? actions : []).map(normalizeAnalysisCareAction).filter(Boolean);
...
function buildSpecificCarePatterns(rows) {
  const groups = new Map();
  rows.filter((row) => row.classification.comparable && row.care_actions.length).forEach((row) => {
    const score = forecastPercent(row.forecast);
    if (score == null) return;
    const trigger = row.exact_trigger || "unknown";
    const signal = forecastSeverity(row.forecast);
    ...
    const severity = actualSeverity(row.review);
    const seen = new Set();
    row.care_actions.forEach((action) => {
      const identity = action.item_key || `${action.domain}:${action.label}`;
      if (!identity || seen.has(identity)) return;
      seen.add(identity);
      const key = `${trigger}:${signal}:${band.min}:${identity}`;
      if (!groups.has(key)) {
        groups.set(key, {
          key,
          item_key: identity,
          ...
          domain_label: domainLabel(action.domain),
          exact_trigger: trigger,
          trigger_label: triggerLabelExact(trigger),
          signal,
          signal_label: signalLabel(signal),
      ...
      if (action.source_mode === "tomorrow") group.previous_night_days += 1;
      if (action.source_mode === "today") group.same_day_days += 1;
      const timing = action.source_mode === "tomorrow"
        ? "before_peak"
        : action.timing_relation === "same_day_before"
          ? "before_peak"
          : action.timing_relation === "same_day_after"
            ...
            : action.timing_relation === "same_day_mixed"
              ? "mixed"
              : "unknown";
      addTimingOutcome(group.timing_outcomes, timing, severity);
      group.dates.push(row.date);
    });
  });
  ...
  return {
    ...
    review: row?.review
      ? {
          ...
          care_label: careLabel(normalized.classification.careDone ? Math.max(1, Number(row.review.prevent_level ?? 0)) : 0),
      ...
      : null,
    performed_care_items: normalized.care_actions.slice(0, 12).map((item) => ({
