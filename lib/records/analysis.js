...
export function buildForecastActualMapPoints(rows) {
  return safeArray(rows)
    .map(normalizeRow)
    .filter((row) => row.classification.comparable)
    .map((row) => ({
      date: row.date,
      forecast: forecastPercent(row.forecast),
      actual: actualPercent(row.review),
      forecast_severity: forecastSeverity(row.forecast),
      actual_severity: actualSeverity(row.review),
      comparison: row.classification.comparison,
      care_done: row.classification.careDone,
...
export function deterministicAnalysis(summary) {
  ...
  if (worse >= 2 && worse > better) {
    return {
      mood: "thinking",
      headline: "予報よりゆらいだ日を一緒に整理できます",
      empathy: "予報よりつらかった日は、うまくできなかった日ではありません。天気以外の負担も含めて見直せます。",
      observed: `予報よりゆらいだ日が${worse}日ありました。${topFactor ? `${factorLabel(topFactor[0])}の記録は${topFactor[1]}日です。` : ""}`.trim(),
      ...
      suggested_questions: ["予報よりつらかった日を整理して", "生活状況で共通することは？", "次の安定日に何を試す？"],
      evidence: [`予報よりゆらいだ ${worse}日`],
    };
  }

  const repeatableCare = safeArray(summary?.care_patterns).find((item) => item.days >= 2 && item.good_days >= 2);
  if (better >= 2 || repeatableCare) {
    return {
      mood: "insight",
      headline: "予報より穏やかに過ごせた日があります",
      empathy: "負担が出やすい条件でも穏やかに過ごせたのは、大切な記録です。",
      observed: `予報より穏やかだった日が${better}日あります。${repeatableCare ? `${repeatableCare.trigger_label}の日に「${repeatableCare.domain_label}」を行った記録は${repeatableCare.days}日です。` : ""}`.trim(),
  ...
  return {
    ...
    observed: `比較できる${summary?.comparable_days || 0}日のうち、予報と実感が近かった日は${summary?.aligned_days || 0}日でした。`,
    ...
    evidence: [`予報と実感が近い ${summary?.aligned_days || 0}日`],
  };
}
