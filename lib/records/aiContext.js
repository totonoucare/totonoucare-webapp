...
import { normalizeDisplayedCareForAi } from "@/lib/records/displayedCareProvenance";
...
import { safeArray, trimRecordForAi, triggerLabelExact } from "@/lib/records/analysis";
...
export const RECORDS_AI_PRODUCT_CONTEXT = {
  knowledge_version: "records_product_context_v15_care_provenance_complete_2026-09-05",
  ...
  communication_translation: {
    ...
    sensitivity_examples: [
      { concept: "アクセル優位×余力小", example: "動き始めは早い一方で、頑張りを長く続ける余力は少なめ" },
  ...
  constitution_model: {
    ...
    body_lines: "結果後に任意登録された主・副の体ラインは、ケア選定の材料。未登録でも異常ではなく、コアタイプや臓腑疾患の診断と混同しない。",
  },
  weather_model: {
    ...RECORDS_FORECAST_MODEL_CONTEXT,
    event_groups: ["気圧変化", "快適域から遠ざかる気温変化・高温低温", "水分環境の変化・湿気乾燥"],
    pipeline: "気圧変化と、快適域から遠ざかる気温・水分環境の変化で揺さぶりを求め、絶対的な温湿度負担と重複させず統合する。連続値のアクセル／ブレーキで反応方向、余力で崩れやすさ、気血津液の全6スコアで表れ方を決め、主因・副因、体調警戒度、ケアへ進む。",
    personalization: "現行版の有効負担は気象強度×（全員共通分0.50＋体質親和分0.50×本人親和性）。過去の記録は保存された版の計算済み値を使う。気圧の上昇・低下は表示と最大6％の体質一致補正に残すが、症状の表れ方は気圧方向で固定しない。気温と湿度の絶対負担は一つの温熱環境へまとめ、変化スコアと単純加算しない。40℃級や厳寒だけは全員共通の下限を残す。",
    record_policy: "現行の一般公開運用では記録を予報計算へ戻さない。予報はベース体質×天気ストレスで固定し、記録はケアと実感の分析にだけ使う。",
    explanation_policy: "ミモルは計算済み値を再計算せず、forecast_reasoningの階層に沿って説明する。気圧ではpressure_directionを物理変化、pressure_response_directionを身体反応として分け、各trigger_factorsのresponse_directionとbody_response_keyを優先する。peak_startとpeak_endは天気ストレスのピークであり、症状発現時刻として扱わない。environmental_cautionsは体質別予報と別の絶対気温注意として扱う。",
  },
  forecast_modes: [
  ...
  interpretation_rules: [
    ...
    "いたわり・守り予報で穏やかだった日は、ケア・生活の安定・ストレスの短さなどを仮説として比べる。",
  ...
  care_model: {
    ...
    selection: "主・副天気ストレス、予報モード、不調フォーカス、体質傾向、経絡・東洋医学的整理から、暮らす・食べる・ほぐすへ落とし込む。displayed_careは計算・保存された基礎ケア案であり、sourceごとの出自を守って土台として使う。ただし提案可能範囲の上限ではない。",
    live_support_expansion: "ミモルは、アプリのロジックで計算・保存された基礎ケア案と同じ方向性を保ちながら、東洋医学の原則から低リスクな置き換え・追加案を作ってよい。その場合は基礎ケア案とミモルの応用案を言葉で区別し、sourceが閲覧事実を保証しない基礎ケア案を『画面で表示された』とは呼ばない。",
    ...
    action_recording: "対策ケアカードの『やってみた』は、本人が実際に取り入れた具体的ケアの記録。前日の明日カードは対象日への先回りケア。当日の今日カードは、記録画面で症状との前後関係を本人が補足する。",
...
function reconstructDisplayedCarePlan(row, profile, mode) {
  const forecast = row?.forecast || null;
  const riskContext = row?.care_reconstruction_context || null;
  ...
  return resolveDisplayedCarePlan({
    completedCare: row?.completed_care || [],
    forecast,
    storedCarePlan: row?.care_plan || null,
    riskContext,
    mode,
    targetDate: row?.date || forecast?.target_date || null,
    symptomFocus,
  });
}

export function buildDisplayedCareContext(row, profile, { mode = "today" } = {}) {
  const saved = row?.forecast?.displayed_care;
  const displayMode = mode === "tomorrow" ? "tomorrow" : "today";
  // A saved preparation plan must not be reused as the day's care (or vice versa).
  // Legacy snapshots without a mode need reconstruction rather than a guessed mode.
  if (saved && typeof saved === "object" && saved.display_mode === displayMode) {
    return normalizeDisplayedCareForAi(saved);
  }

  const finalPlan = reconstructDisplayedCarePlan(row, profile, displayMode);
  if (!finalPlan) return null;
  return normalizeDisplayedCareForAi(buildDisplayedCareSnapshot({
    carePlan: finalPlan,
    mode: displayMode,
    targetDate: row?.date || row?.forecast?.target_date || null,
    source: "reconstructed_from_complete_risk_context",
  }));
}

export function buildAiRecordContext(row, profile, options = {}) {
  ...
  return {
    ...
    forecast: compact.forecast
      ...
      : null,
    displayed_care: buildDisplayedCareContext(row, profile, options),
  };
}
