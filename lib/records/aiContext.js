...
export const RECORDS_AI_PRODUCT_CONTEXT = {
  knowledge_version: "records_product_context_v9_forecast_v2_2026-07-21",
  ...
  weather_model: {
    ...RECORDS_FORECAST_MODEL_CONTEXT,
    event_groups: ["気圧変化", "気温変化・厳しい暑さ寒さ", "湿気・乾燥"],
    pipeline: "気象の変化量で揺さぶりの強さを求め、連続値のアクセル／ブレーキで反応方向、余力で崩れやすさ、気血津液の全6スコアで表れ方を決め、主因・副因、体調ゆらぎ度、ケアへ進む。",
    personalization: "有効負担は気象強度×（全員共通分0.62＋体質親和分0.38×本人親和性）。気圧の上昇・低下は表示と最大6％の体質一致補正に残すが、症状の表れ方は気圧方向で固定しない。気圧・気温・水分環境の各群は一つだけ点数へ採用し、相関した変化を重複加点しない。",
    record_policy: "現行の一般公開運用では記録を予報計算へ戻さない。予報はベース体質×天気ストレスで固定し、記録はケアと実感の分析にだけ使う。",
    explanation_policy: "Ekkenは計算済み値を再計算せず、forecast_reasoningの階層に沿って、なぜこの人に今日この負担が主因になったかを説明する。",
  },
  forecast_modes: [
