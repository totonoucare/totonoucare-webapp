const MATERIAL_DEFINITIONS = {
  ...
  blood_stasis: {
    ...
    care_direction: "少しずつめぐらせる・温めるケアは冷えを伴う場合に選ぶ",
  },
  fluid_deficiency: {
    label: "津液不足",
    layer: "絶対量・潤い",
  ...
  fluid_damp: {
    ...
    care_direction: "ながす・消化吸収を支える・体力に合わせて動く（発汗を一律の目標にしない）",
  },
};
...
export const RECORDS_CONSTITUTION_MODEL_CONTEXT = {
  hierarchy: [
    ...
    "第4層は不調フォーカス、任意で確認した体のライン、環境感受性、気象への反応補正。現在どこにどう表れやすいかを読む材料で、コアタイプそのものと混同しない。",
  ],
  accelerator_brake_axis: {
    role: "性格や活動量ではなく、不調時に表へ出やすい反応方向の要約。",
    engine_logic: "アクセル側は気滞を主に、熱と『津液不足×熱』を補助する。ブレーキ側は痰湿を主に、冷えと『気虚×冷え』『血虚×冷え』を補助する。寒さと熱さは互いに打ち消さず別々に保持する。",
    interpretation: "アクセル優位は張り・詰まり・前へ出る反応、ブレーキ優位は重さ・守り・動き始めにくさが相対的に表へ出やすいことを示す。",
  },
  reserve_axis: {
    role: "普段の疲れやすさと回復の遅れを中心に、血虚・津液不足を補助して読む回復余力。",
    engine_logic: "疲れやすさ、翌日への持ち越し、血虚、津液不足を一度ずつ統合し、環境感受性は混ぜずに余力を小・標準・大に分ける。",
  ...
  important_boundaries: [
    ...
    "体のラインは結果後に任意で確認する動作情報で、臓腑疾患の診断でもコアタイプ主計算の材料でもない。",
    "環境感受性は気象への反応補正だけに使い、余力やベース体質を作り替えない。",
  ],
  inference_order: [
    "コアタイプを最上位の統合結果として読む",
...
export function buildConstitutionReasoningContext(profile = {}) {
  ...
  return {
    ...
    axes: {
      accelerator_brake: {
        label: profile?.axes?.reaction_label || profile?.axes?.yin_yang_label || null,
        score: finite(profile?.axes?.reaction_score ?? profile?.axes?.yin_yang_score),
        meaning: RECORDS_CONSTITUTION_MODEL_CONTEXT.accelerator_brake_axis.interpretation,
      },
      reserve: {
        label: profile?.axes?.reserve_label || profile?.axes?.drive_label || null,
        score: finite(profile?.axes?.reserve_score ?? profile?.axes?.drive_score),
      ...
      thermal: {
        cold_score: finite(profile?.axes?.cold_score),
        heat_score: finite(profile?.axes?.heat_score),
        direction: finite(profile?.axes?.thermal_direction),
        mixed: Boolean(profile?.axes?.thermal_mixed),
        meaning: "冷えやすさと熱がこもりやすさを独立して保持し、両方ある状態もそのまま読む。",
      },
    },
    ...
    current_expression_lenses: {
      ...
      meridian_usage: "任意で登録された体のラインがあれば、ほぐすケアと確認質問の優先順位に使う。未登録なら6素材と現在の不調から選ぶ。",
