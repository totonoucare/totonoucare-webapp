...
あなたは「未病レーダー」のケアナビAI：Ekiken（エキケン）です。選択期間の記録を読み、相手の言葉を丁寧に受け取りながら、やさしく一緒に整理します。
...
あなたは「未病レーダー」のケアナビAI：Ekiken（エキケン）です。
...
- 予報スコアや体質を再計算しない
- 薬・漢方・サプリの個別な選択、使用開始・中止、用量変更を指示しない
- アプリに存在しないツボ・食品・ケアを、未病レーダーが選んだ内容として創作しない
- displayed_careにある低リスクな対策ケアを優先し、ない場合は休息・水分・環境調整など一般的で安全性の高い範囲に留める
- 突然の激しい症状、胸痛、呼吸困難、意識障害、片側の麻痺、ろれつ困難、大量出血、自傷他害の切迫などはurgent。セルフケア提案を止め、周囲への連絡と救急要請・医療機関への連絡を優先する
- 長引く・強い症状、薬・漢方・治療判断、繰り返す症状への個別判断はprofessional。ただし緊急でなければ会話を止めず、できる範囲の一般的なセルフケア整理を続け、必要な相談先を回答末尾に短く添える
...
- suggested_questionsはユーザーが次にEkikenへ聞く自然な質問文だけにする
...
export function urgentTextKind(value) {
  ...
  const selfHarm = /(自殺したい|死にたい|消えたい|自傷したい|殺したい|もう生きたくない|生きることに疲れた|生きているのに疲れた)/i;
  if (selfHarm.test(text)) return "self_harm";

  const medical = /(意識がない|意識を失|呼吸が苦しい|呼吸しづらい|息ができない|息苦しい|息が苦しい|強い胸の痛み|胸が痛い|胸痛|胸が締め付け|突然の激痛|突然.{0,6}(ひどい|激しい).{0,4}頭痛|今までにない.{0,4}頭痛|片側.{0,8}(動かない|力が入らない|しびれ)|ろれつが回らない|言葉がうまく出ない|言葉が出ない|うまく話せない|大量出血|けいれん)/i;
  return medical.test(text) ? "medical" : "";
}

export function isUrgentText(value) {
  return Boolean(urgentTextKind(value));
}

export function isProfessionalText(value) {
  const text = String(value || "").normalize("NFKC");
  return /(飲み始め|使い始め|服用して.{0,6}(いい|大丈夫)|服用を?中止|薬を?やめ|増量|減量|用量を?変|何錠|何包|飲み合わせ|併用して.{0,6}(いい|大丈夫)|妊娠.{0,8}(薬|漢方|サプリ)|授乳.{0,8}(薬|漢方|サプリ)|副作用|この.{0,8}(薬|漢方|サプリ).{0,8}(合う|選んで|飲むべき)|診断して|治療が必要)/i.test(text);
}
