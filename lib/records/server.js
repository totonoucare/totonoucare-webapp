...
import { selectCareReconstructionContext } from "@/lib/records/careReconstruction";
...
function normalizeForecast(row, source = "current_forecast") {
  ...
  const riskContext = row?.computed?.radar_plan_meta?.risk_context || {};
  const riskSummary = riskContext?.summary || {};
  const reasonTrace = forecastReasonTrace(row, computedSnapshot);
  return {
    ...
    personal_main_event_key:
      row.personal_main_event_key || computedSnapshot.personal_main_event_key || riskSummary.personal_main_event_key ||
      reasonTrace?.personal_main_event_key || null,
    pressure_direction:
      row.pressure_direction || computedSnapshot.pressure_direction || riskSummary.pressure_direction ||
      riskContext?.weather_context?.pressure_direction || null,
    pressure_response_direction:
      row.pressure_response_direction || computedSnapshot.pressure_response_direction ||
      riskSummary.pressure_response_direction || reasonTrace?.pressure_response_direction ||
      reasonTrace?.manifestation?.reaction_direction || null,
    trigger_factors: safeArray(row.trigger_factors || computedSnapshot.trigger_factors),
    environmental_cautions: compactEnvironmentalCautions(
      row.environmental_cautions || computedSnapshot.environmental_cautions || riskSummary.environmental_cautions ||
      reasonTrace?.environmental_cautions
    ),
...
export async function loadRecentCompletedCare(userId, targetDate) {
  if (!isValidYmd(targetDate)) return [];
  const result = await loadCareActionRows(userId, addDaysYmd(targetDate, -7), addDaysYmd(targetDate, -1));
  return result.rows || [];
}
...
export async function loadRecordsRange(userId, start, end, { includeCarePlans = false } = {}) {
  ...
  const [forecastResult, reviewResult, careActionResult] = await Promise.all([
    ...
    loadCareActionRows(userId, includeCarePlans ? addDaysYmd(start, -7) : start, end),
  ]);
  ...
  for (let index = 0; index < span; index += 1) {
    ...
    const careReconstructionContext = includeCarePlans
      ? selectCareReconstructionContext({
          savedSnapshot,
          currentForecast,
          hasSavedReview: Boolean(rawReview),
        })
      : null;
    ...
    rows.push({
      ...
      ...(includeCarePlans ? {
        care_reconstruction_context: careReconstructionContext,
        completed_care: careActionResult.rows.filter((action) => action.target_date < date && action.target_date >= addDaysYmd(date, -7)),
      } : {}),
  ...
  return {
    ...
    axes: computed?.axes && typeof computed.axes === "object"
      ? {
          ...
          reaction_label: computed.axes.reaction_label || computed.axes.yin_yang_label || null,
          reserve_label: computed.axes.reserve_label || computed.axes.drive_label || null,
          reaction_score: finiteOrNull(computed.axes.reaction_score ?? computed.axes.yin_yang_score),
          reserve_score: finiteOrNull(computed.axes.reserve_score ?? computed.axes.drive_score),
          yin_yang_score: finiteOrNull(computed.axes.reaction_score ?? computed.axes.yin_yang_score),
          drive_score: finiteOrNull(computed.axes.reserve_score ?? computed.axes.drive_score),
          obstruction_score: finiteOrNull(computed.axes.obstruction_score),
          cold_score: finiteOrNull(computed.axes.cold_score),
          heat_score: finiteOrNull(computed.axes.heat_score),
          thermal_direction: finiteOrNull(computed.axes.thermal_direction),
          thermal_mixed: Boolean(computed.axes.thermal_mixed),
          thermo_answer: computed.axes.thermo_answer || null,
        }
      : null,
    split_scores: compactSplitScores(computed?.split_scores),
    material_scores: computed?.material_scores && typeof computed.material_scores === "object"
      ? computed.material_scores
      : null,
