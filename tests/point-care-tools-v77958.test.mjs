...
test('ラインケアの対象ツボだけにシール鍼を案内する',()=>{
  assert.deepEqual(tools.pointToolKinds(context),['stick','seal','moxa']);
  assert.deepEqual(tools.pointToolKinds(tools.pointToolContext([{code:'ST36',source:'tcm'}],true)),['stick','moxa']);
...
test('温める方針がある時にお灸を案内する',()=>{
  assert.ok(!tools.pointToolGuides({code:'ST36',source:'tcm'},false).some(x=>x.kind==='moxa'));
...
test('API検索3枠はツボに対応した道具を優先し、余計な検索を増やさない',()=>{
  ...
  assert.deepEqual(rows.map(x=>x.pointToolKind),['stick','seal','moxa']);
  assert.ok(rows.every(x=>x.reason.includes('曲池')));
  assert.ok(!plan(tools.pointToolContext([{code:'ST36',source:'tcm'}],false)).some(x=>['seal','moxa'].includes(x.pointToolKind)));
...
test('セットの部位制約でも汎用ツボ道具を受け入れ、機械より優先する',()=>{
  ...
  assert.equal(shop.itemMatchesSlot({category:'point',title:'お灸 ソフト 管理医療機器'},{...slot,pointToolContext:{codes:['ST36'],warming:false}}),false);
});
