...
test("具体策は個別家事名へ固定せず、広い場面の中で使える文章にする", () => {
  ...
  assert.doesNotMatch(publicBlock, /まな板|モップ|掃除機|洗濯かご|買い物袋|スマホ|重い扉|引き出し|キーボード|マウス/);
  assert.match(publicBlock, /手元で使う物を、こぶし一つぶん手前へ寄せる/);
  assert.match(publicBlock, /押す・引く物へ近づいて軽く持つ/);
});

test("明日タブは身体操作と環境調整で実行タイミングを分ける", () => {
  const today = build({ date: "2026-08-02", mode: "today" }).lifestyle_plan;
  const tomorrowBody = build({ date: "2026-08-03", mode: "tomorrow" }).lifestyle_plan;
  assert.equal(today.timing_label, "今日の一手");
  assert.equal(tomorrowBody.primary_action?.care_kind, "body");
  assert.equal(tomorrowBody.timing_label, "明日の一手");

  const tomorrowEnvironment = Array.from({ length: 21 }, (_, index) => build({
    date: `2026-08-${String(index + 1).padStart(2, "0")}`,
    mode: "tomorrow",
    symptomFocus: "sleep",
    coreCode: "brake_batt_large",
  }).lifestyle_plan).find((plan) => plan.primary_action?.care_kind === "environment");
  assert.ok(tomorrowEnvironment);
  assert.equal(tomorrowEnvironment.timing_label, "今夜の準備");
...
test("今日と明日は適合度を落とさず、近い候補だけを巡回する", () => {
  ...
  assert.equal(today.primary_action?.care_kind, "body");
  assert.equal(tomorrow.primary_action?.care_kind, "body");
  assert.ok(today.selection_basis.primary_candidate_score >= 2.5);
  assert.ok(tomorrow.selection_basis.primary_candidate_score >= 2.5);
});
