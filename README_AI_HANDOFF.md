## v7.79.20 ケア文言・可読性・暮らす導入文の現行契約

- 現行ロジック版は`daily_care_v2_18_2026-08-07_care_copy_readability`
- 食事・飲み物の第一理由は、内部では食性を正本にするが、画面の見出しは`体調との相性`とする。`食養生`を画面の項目名へ戻さない
- 第一理由の本文を`食養生では、`で始めない。食材名または飲み物名から直接、温める・冷ます・うるおす・重さをためにくくする等の相性を説明する
- 食べるタブ上部の種別名は`食事ケア`。食性の内部採点、`tcm_function_keys`、ショップの継続軸は変更しない
- 食べるタブの主要本文は15px以上、取り入れたい食材と暮らす一手は16pxを基準にする。補足理由も13〜14pxを基準にし、11pxは主要本文へ使わない
- `lifestyle_plan.recommendation`は`buildLifestyleForecastInsight`で生成し、主・副天気、反応方向、選択中の不調、選ばれた主提案のレーンを一文脈で示す
- todayは`今日は…暮らしでは、`、tomorrowは`明日は…今夜からの暮らしでは、`とする
- 主提案が身体操作なら`身体の使い方を少し変え`、環境調整なら`道具や配置を整え`とする。選定結果と無関係な定型文へ固定しない
- 暮らすの候補選定、身体操作OS、環境調整の商品境界、食事カタログ、ショップ接続、予報スコア、DBは変更しない

詳細: `docs/RADAR_CARE_COPY_READABILITY_V77920.md`

> v7.79.19の画面見出し`食養生`と、暮らすタブに予報導入文がない表示契約は廃止済み。食性中心の選定ロジック自体は継続する。
