最新版：v7.79.80。記録保存のDB制約更新SQLが必要です。SETUP-v7.79.80.txt参照。

最新版：v7.79.79。体質チェック結果画面の保存案内・タブ移動を調整。SETUP-v7.79.79.txt参照。

最新版：v7.79.78。本番ホスト限定のMeta計測。SETUP-v7.79.78.txt参照。

# 最新: v7.79.77
Meta計測を共通の初回PageViewに統一。SETUP-v7.79.77.txtを参照。

# 最新: v7.79.76
Meta初回ロード修正。SETUP-v7.79.76.txtを参照。

# 最新: v7.79.75
Meta計測修正。RELEASE-v7.79.75.mdを参照。

# 最新: v7.79.74

Meta広告のファネル計測（体質チェック開始・完了・新規登録）を追加。健康・体質・症状等の内容はMetaイベントのパラメータとして送信しません。詳細は `RELEASE-v7.79.74.md`。

使い方の案内・選択式フィードバックを追加。反映前に `SETUP-v7.79.73.md` のSQL手順を実行してください。詳細は `RELEASE-v7.79.73.md`。

# v7.79.72 更新

飲み物の選定根拠保存・近接候補の日替わり・説明文を改善。詳細は RELEASE-v7.79.72.md と UPDATED-FILES-v7.79.72.txt。

## v7.79.71 現行の飲み物・食べるタブ契約

- 現行リリースは `RELEASE-v7.79.71.md`、飲み物の設計は `docs/FOOD_DRINK_DESIGN_V77971.md`。
- `drinkRules.js` を飲み物14種の属性・重み・選定・説明の正本とする。強=1、中=0.75、弱=0.5。推論タグは由来を示し、一律半減には使わない。
- ほうじ茶の温方向、はとむぎの健脾・利水、とうもろこし・小豆・黒豆の主属性と補助属性を保持。
- 葛湯は任意の補食としてカタログに保持。メイン2候補から除外し、内部の第3候補には残せる。
- 保存済みの提案も現在の主訴と対象日の予報で再選定。食材81種・記録の契約を継承。
- 旧 `foodScore()` の未定義 `drink` 参照を修正。実行テストは `tests/drink-personality-v77971.test.mjs`。
- 全テストは `npm test`、28,500条件の監査は `node scripts/audit-food-context-v77971.mjs`。

## v7.79.64 現行の契約

- 正本は `RELEASE-v7.79.64.md`。v63のミモル表示で不足したimport・件数定義を補完。
- `tests/radar-client-bindings-v77964.test.mjs`はJS/JSXの未定義参照を検出。通常のNextビルドのみで表示成功と判定しない。
- ほぐす18件の現行表示は `getCareMovementImage(id)` のWebP。SVGは旧版資料用。SVG生成スクリプトではWebPを生成できない。
- 線画は性別を強調せず顔なし、淡い暖色。緑の肌色・リアルな人物画へ戻さない。制作メモは `docs/CARE-IMAGE-PROMPTS-v7.79.64.md`。
- v63のケア別タイミング記録と暮らすの場面別選定は維持。DB migrationなし。

## v7.79.63 引き継ぐ契約

- 正本は `RELEASE-v7.79.63.md`。v62の冒頭ミモル削除は取り消し、短い記録案内を復元。
- 症状との前後関係はケアIDごとの任意回答。API日次一括上書きと、前夜＝症状前の推定を復活させない。
- `item_snapshot.meta.symptom_timing` は `timing_source=individual` の時だけ個別回答として集計する。旧記録は保持し、前後関係は不明とする。
- 暮らす候補は17件を維持。`LIFESTYLE_USAGE`で道具・場面・狭い条件を管理し、既定の日常枠に机作業等の限定候補を混ぜない。
- 道具なし1件＋道具あり1件が基本。適合しない場合は空枠を埋めない。狭い条件の案は折りたたむ。
- 明日の選定に今日の表示候補IDを渡す。近い適合度なら別案を優先し、共有する場合は明示する。
- ほぐし図は18件のIDと1対1。`scripts/generate-care-movements.py`で再生成可。紫の矢印は動作の方向。
- DB migration、依存パッケージ追加、予報点数・体質判定・ツボ選定の変更なし。

## v7.79.62 ケア内容と実行記録

- 変更の正本は `RELEASE-v7.79.62.md`、暮らす全候補は `docs/LIFESTYLE-CATALOG-v7.79.62.md`。
- 画面は食べる・ほぐす・暮らすの順。カテゴリ指定リンクを優先し、通常は食べるを表示する。
- 共通方針は動詞アイコン＋選定理由一文。暮らす・食べるの要因ピルと前置きを再追加しない。
- 食べるの新規献立カードの実行記録は `ingredients_or_eating_pattern`。料理例は参考情報。旧記録のラベル・意味を変更しない。
- 暮らすの一般13件＋条件付き4件は日常語の動作と確認ポイントを持つ。数合わせや毎日の入れ替えを目的に、削除した準備案を復活させない。
- 予報・体質・ツボ選定・DB schemaの変更なし。画面とAPIを同時に反映する。

## v7.79.54 振り返り比較記録の表示契約

- `buildReflectionEvidenceDetails(summary, aiEvidence)`は、`matched_forecast_comparisons`からケアあり・なしの両方が存在する比較を1件選び、表示用の構造へ変換する
- AIの`evidence`に天気ストレス名・モード名が含まれる場合は対応する比較を優先し、該当がなければ集計上の最上位比較を使う
- 比較条件は、主な天気ストレス、体調警戒度の10点幅、モード、合計日数を表示する
- ケアあり・なしは別行とし、各群の母数と`穏やか / 軽い不調 / つらい`の内訳を表示する
- AIが生成した「ケアあり…ケアなし…」の圧縮比較文は、構造化比較が表示できる時だけ重複表示を抑止する。個別日の`evidence`は残す
- 保存済みAI分析にも現在の集計から表示変換を適用する。AI再生成、DB migration、prompt/schema更新は不要
...
- 要素の強さはカード中央の`高 / 中 / 低`が示す。低負荷を理由に現象名を`○○穏やか`へ置き換えない
- 旧予報でグループ自体の保存値がない場合は、推測した現象名を出さず`気温ストレス / 湿度ストレス / 気圧ストレス`と`—`を表示する
- 幅430px未満はアイコンと要素名を縦積みにし、3列表示を維持する
- 予報点数、天気ストレス計算、ケア選定、DB、AI prompt/context、料金・14日体験の変更はない
...
- compare時の`safety.reasons`は空配列。`選択した安全確認では大きな該当なし / 内服しない候補を表示`をユーザー向け文言へ戻さない
- 候補カードの初期表示は、商品名、`productClass`、`matchSummary`、成分重複警告、保存／楽天操作を正本とする
- `matchSummary`は今回の候補固有の照合点を最大2件、体質一致を最大1件表示する。候補が温めた反応を使う場合は、混在寒熱より`温めると少し楽`を優先して示す
- `matchReason`は候補マスターの`why`と同じ候補固有説明。現在回答や体質ラベルを長文で再生成しない
- `候補に入った理由 / 選ぶときの確認 / 同じ素材を含む区分 / 購入前に確認`は、単一の`理由と選び方を見る`details内へ置く。detailsを入れ子にしない
- `duplicateIngredients`警告はdetails外に置き、閉じた状態でも見えるようにする
- 保存時の`item_snapshot.reason`相当には`合っている条件：matchSummary`と`matchReason`を連結して残す
- 候補評価、表示上限、安全判定、明示AI、楽天リンク、DB/RLSはv7.79.41から変更しない
- 全349件の回帰テストとNext.js本番ビルドを通過

## v7.79.41 体質統合ケアショップの現行契約

- `/care-navi`の`悩みから探す`は、現在状態と体質を別オブジェクトで保持するが、候補評価では両方の一致を合算する。結果UIで別々の説明カードへ戻さない
- 体質土台は`material_scores`の気虚・気滞・血虚・血瘀・痰湿・津液不足、`axes.reserve_score`、`cold_score / heat_score`を使う。`core_code / sub_labels`だけへ縮退しない
- 0〜100の素材スコアは連続値として正規化し、中程度以上を候補支持へ使う。内服候補では現在状態にない体質一致を最大2要素まで必要一致数へ加え、強い傾向だけを矛盾除外へ使う
- `avoidStates`の寒熱・燥湿は、今回の単独回答が明確に逆の場合に除外する。`thermal === mixed`は冷え・熱の両方を保持し、双方を一律除外しない。ただし体質側の強い矛盾は引き続き除外する
- `GUIDED_CANDIDATES`は`selfcare 16 / health 21 / kampo 29`。追加方剤は疎経活血湯、桂枝加朮附湯、八味地黄丸、牛車腎気丸
- 桂枝加朮附湯は腰・首肩のつらさに冷えまたは温めた反応があり、冷え・湿気・体力低下の状態一致が2つ以上ある時に補助質問なしでも比較できる。ほかの追加方剤は痛み方・しびれ・排尿変化等の補助回答を要求する
- 腰・首肩向け補助質問は`leg_pain_numbness / cold_damp_pain / fixed_night_pain / urination_change_cold / muscle_cramp`。`muscle_cramp`の表示は`足などの筋肉がつりやすい`とし、`けいれん`を含めない
- `currentState.summary`は体質ラベル最大2件と今回の高信号回答最大5件を一文で結ぶ。全回答の読み上げ、内部推定状態の回答化、`もともとの傾向 / 今回の回答`の二重カードを戻さない
- 候補の`matchReason`は、今回選んだ根拠と該当する体質根拠を明示し、その後に候補固有の`why`を続ける
- 通常の`compare`安全表示は一行。`consult / stop`は理由と行動が必要なため強いカード表示を維持する
- 体力の高い回答は`普段より動けそう`。これだけを`tension`へ加点したり、候補理由を`力が抜けにくい`に変換しない
- `subject === other`ではログイン中ユーザーのprofileを`deriveCurrentState`へ渡さない
- AIは自由入力整理の明示ボタンだけで使用し、候補評価・安全判定へ関与しない。DB migrationなし
- 全346件の回帰テストとNext.js本番ビルドを通過
...
- `GUIDED_CANDIDATES`は`selfcare 16 / health 21 / kampo 25`。ケア用品の表示単位はカテゴリではなく楽天で比較できる具体的な用品形状とする
- 一般的なノンカフェイン健康茶はショップ候補にしない。健康食品は生薬系素材、機能性関与成分、菌株等を表示単位にし、商品ごとの表示確認事項を`compare`へ保持する
- 追加した生薬・機能性候補には、高麗人参、エゾウコギ、ハトムギ、杜仲葉、田七人参、桑葉、ブラックジンジャー、ウコン、イチョウ葉、マカ、クコ、GABA、L-テアニン、グリシン、ビフィズス菌等を含む
- 漢方は全25方剤。体力、寒熱、胃腸、経過に加え、`GUIDED_CLUE_OPTIONS`の補助回答または自由入力語が必要な方剤を設ける。補助回答なしに方剤数だけを増やさない
- 補助質問は内服候補を含む範囲でのみ表示し、選択中の悩みと関連する項目を優先して最大8件、回答は最大3件とする
- `currentState.scores/stateKeys`は候補順位用の内部状態。`currentState.summary`は実際に選択した回答だけから作り、内部状態名を`今回の回答`として表示しない
- `matchReason`は選択した悩み、補助回答、該当する寒熱・重さ／乾き・体力・胃腸・楽になる条件を根拠にする。体質チェック由来の根拠は`体質チェックの…も補助的に参照`と分離する
- 温熱／冷却候補は`answerAnyConditions`で実回答の寒熱または温めた反応を確認する。状態スコアだけから温冷用品を決めない
- 単一範囲では最大5候補、まとめて見る場合はケア用品3・健康食品3・漢方4を上限とし、情報量を増やしすぎない
- ステップ移行、戻る、結果、条件見直しは`scrollIntoView`で探索カード先頭へ戻す。`prefers-reduced-motion`時は滑らかなスクロールを使わない
- 既存のAI明示実行、レッドフラッグ、安全確認、使用中成分重複、楽天リンク、保存互換、DB/RLSは変更しない
- 全331件の回帰テストとNext.js本番ビルドを通過

## v7.79.38 未病ケアショップの現行契約

- `/care-navi`の上位導線は引き続き`おすすめ / 悩みから探す`。保存一覧はヘッダーのハートから開く
- `悩みから探す`の表示対象は`selfcare / health / kampo`の3分類。新規の一般用西洋薬候補は扱わない
- `health`には健康茶、健康飲料、栄養補助飲料、生薬系健康食品、サプリメントを含む。一般的な料理・食材・スープは対象外
- カードは製品区分、`候補に入った理由`、`選ぶときの確認`、折りたたみの`購入前に確認`を分離する
- `trust / 用途確認済み / 成分量を確認 / 添付文書確認`をステータスタグとして表示しない。カード右上は事実としての`productClass`だけを表示する
- 検索語は楽天で探す対象名だけにし、`成分量 / 原材料 / 含有量 / 規格`等の比較語を混ぜない。比較事項は`candidate.compare`へ保持する
- `deriveCurrentState`はフォーム回答だけから`currentState.scores/stateKeys`を作る。体質チェックは`currentState.baseline.scores/stateKeys`へ別保持し、相互に上書きしない
- フォームの寒熱・体力は体質チェックから自動入力しない。体質は結果画面で`もともとの傾向`、フォーム回答は`今日の回答`として表示する
- ケア用品は現在状態の一致で表示できる。継続健康食品は`weeks/months`等の経過と複数条件、漢方は候補ごとの症状・現在状態・入力条件を満たす場合だけ表示する
- マグネシウム、ヘム鉄、ビタミンB群、イチョウ葉、ウコン、マカ、杜仲は`explicitOnly`。自由入力に対象名がある場合だけ比較候補にする
- 五苓散、防已黄耆湯、半夏厚朴湯、葛根湯、芍薬甘草湯は、処方名の明示または候補ごとの補助症状語がなければ自動表示しない
- `matchReason`は一致した悩み・今日の状態・補助的な体質傾向から生成し、保存スナップショットの`reason`にも使う
- `health / kampo`は購入済みだけでは体調予報へ出さず、`activeUse === true`の時だけ成分重複と使用記録へ反映する。旧`supplement / otc`も保存互換のため同じ扱いを維持する
- レッドフラッグ、安全確認、自由入力AIの明示実行、楽天検索、既存おすすめタブ、DB/RLSは変更しない
- 全324件の回帰テストとNext.js本番ビルドを通過
...
- `次に一つだけ`、AI見立て、仮説、根拠は`analysisMeta.source === "ai"`の場合だけ表示する
- 記録件数と`RecordsSimpleTrendChart`は`AIを使わない基本集計`として常時確認できる
- 基本集計の詳細では仮説文を出さず、AI結果がある場合だけ`この見立ての理由 / 記録で確認したこと`を表示する
- 振り返り生成は明示的な`generate: true`だけで行う。画面表示時の`generate: false`は保存済み結果の検索と基本集計だけを行う
- `records_ai_analyses`の保存済み結果は、完全一致範囲を優先し、なければ同じ`period_key`の最新結果を表示する
- 日付範囲が進んだ保存済み結果には元の`analysis_range`を表示し、現在範囲での再生成は手動ボタンだけで行う
- 期間振り返りチャットは完全一致範囲を優先し、なければ同じ`period_key`の最新activeスレッドと履歴を引き継ぐ
- 期間枠そのものが変わったスレッドIDは`thread_period_mismatch`で拒否し、日付範囲の1日移動だけでは拒否しない
- Ekkenは実行前、AI結果、期間相談の各入口で`ケアナビAI`と明示する
- AI生成回数、相談回数、予報計算、対策ケア選定、体質判定、DB、RLSは変更しない
- 全312件の回帰テストとNext.js本番ビルドを通過

## v7.79.35 記録・振り返り・相談の旧契約

- `/records`の主タブは`記録 / 振り返り / 相談`。ユーザー向け機能名として`AI分析`を戻さない
- 記録カレンダーと日次記録は無料。振り返りの結果・期間チャットはプレミアム対象
- 無料会員のEkken相談は、体調記録3日以上で初めて開き、アカウント生涯で合計2 assistant回答まで
- 安全ルールだけで返した緊急案内は無料体験回数を消費しない
- 無料2回答を使い切った後は新規送信を閉じるが、既存スレッドと過去回答の閲覧を残す
- 先行体験中と有効なプレミアム契約では無料体験制限を適用しない。プレミアムの月100回答・1分6回答を維持
- 無料体験消費は`records_ai_events.event_type = free_chat_response`を正本にし、月次`chat_response`と混ぜない
- 振り返りの表示順は`期間 → 結論 → 次に一つ → 同じ日付列の直感グラフ → 詳細根拠 → 期間チャット`
- 初期グラフは体調警戒度の安定・いたわり・守り帯、実感`○△×`、ケア時期を同じ列へ置く
- 初期グラフは7日=日別、30日=7日単位、31〜180日=月別、181日以上=2か月別。横スクロールを前提にしない
- 旧`RecordsTrendChart`は`詳しい根拠を見る`内の検証用詳細として残す
- Ekken相談の価値は、解釈済み体質トリセツ、今日・明日の予報とケア、直近3日の詳細、直近14日の要約、任意の相談状況、会話履歴を引き継ぐこと
- 日次記録の生活要因は全条件で保存可能。`天気以外に気になったこと（任意）`を通常は閉じ、`stable_difficult`では自動で開く
- 生活要因は`寝不足 / 忙しかった / 食事や飲酒 / 気分の負担 / 生理・体調の変化 / その他 / 特にない`。`特にない`は排他
- ユーザー向け用語は`体調予報 / 体調警戒度 / 振り返り`。内部DB識別子やAPIの`analysis`は互換性のため維持可能
- 記録は予報アルゴリズムを学習・再計算しない。予報計算、対策ケア選定、体質判定、既存DB、RLSは変更しない
- 全311件の回帰テストとNext.js本番ビルドを通過

## v7.79.34 ほぐす一手の現行契約

- 経絡ラインケアは`lung_li`、`heart_si`、`kidney_bl`、`liver_gb`、`spleen_st`、`pc_sj`の6系統を正本とする
- 各系統は呼吸、なでる、手を当てる、軽く動かす等の3候補を持ち、計18候補とする
- 一つの候補へ異なる部位の複数動作を連結せず、姿勢、部位、方向、回数が一読で分かる一動作にする
- `getDailyCareRotationIndex`へ対象日・経絡・主天気トリガーを渡し、同日再読込では固定、同条件の隣接日では3候補を巡回する
- today/tomorrowは選定キーへ混ぜず、同じ対象日なら翌日にtodayへ変わっても同じ候補IDを維持する
- 天気トリガーは候補の動作種別順と`guidance_note`へ反映し、末尾に別の身体操作を追加しない
- 乾燥時の服の上からの実施、気圧・寒暖差時の呼吸速度、余力・信号に応じた刺激量は一つの案内文へ統合する
- ほぐすカードの時制バッジはtoday=`今日の一手`、tomorrow=`今夜〜明朝の一手`
- 実行記録の`rule_id`は候補固有ID、`line_group_id`は経絡ライン共通ID、`meridian_code`は従来どおり保持する
- 現行ロジック版は`daily_care_v2_26_2026-08-26_loosen_care_rotation`
- 予報点数、ケア方針、ツボ選定、体質判定、DB、RLS、環境変数は変更しない
- 全306件の回帰テストとNext.js本番ビルドを通過

## v7.79.33 体調予報カードの現行契約

- 指数のユーザー表示名は`体調警戒度`。`%`ではなく`0 / 100〜100 / 100`として表示する
- ゲージでは警戒度の整数を主役にし、`/ 100`は小さい補助表記にする
- 予報カードのタイトルは`今日の体調予報`／`明日の体調予報`とし、カード内へ上部と同じ日付を重ねない
- モード解釈は安定=`安定しやすい日`、いたわり=`いたわりたい日`、守り=`無理せず守りたい日`
- 予報カードはモード表示の直後にゲージを置き、長いリード文をゲージより前へ戻さない
- リード文と出やすいサインを別々に並べず、`今日の見立て`／`明日の見立て`の一文へ統合する
- 明日の見立ては同じ利用者・地域の今日の予報指数と3つの天気負荷を比較し、数値差・続く負担・強まる／弱まる負担・今夜の行動から全文を組み直す
- 今日と同じ体感文へ比較ラベルだけを足す実装には戻さない
- 5ポイント以上の指数差を`高まる`／`下がる`、5ポイント未満を`ほぼ同じ`と表現する
- 比較用の今日データが取得できない場合は、比較表現を付けず従来の明日文を表示する
- 今日の見立ては比較表現を付けず、その日の予報データから表示時に組み立てる
- 天気ストレスのミニカードはカテゴリ名ではなく`高温`、`低温`、`寒暖差`、`湿気`、`乾燥`、`気圧低下`、`気圧上昇`、`気圧変動`を直接表示する
- 並び順は内部の連続負荷が高い順。同値では主因、既定カテゴリ順の順に安定化する
- 負荷が中以上で有効な時刻がある場合だけ、各カード内に`⚡ 開始–終了時`を表示する
- 予報計算、信号閾値、天気負荷、体質親和性、余力、ケア選定、DB、RLSは変更しない
- 全303件の回帰テストとNext.js本番ビルドを通過

## v7.79.30 リリース前UI・サイン・商品検索の契約

- 体質チェック入口の説明は`lib/diagnosis/v2/uiCopy.js`を正本とし、所要時間は`約5分`とする
- 出やすいサインは必ず3件へ埋めず、天気×不調、体質×不調、広い天気反応の順に、意味が異なるものだけ1〜3件表示する
- 同じ部位・同じ体感の言い換えは`selectDistinctBodySigns`でまとめ、湿気の固定比喩`湿気を含んだ服を着たような重さ`を戻さない
- 商品用Action IDは`lib/care-navi/lifestyleShopContext.js`の`tool-*`許可リストを画面とAPIで共用する。`tension-*`は商品へ直結しない
- 楽天検索は限定フィールドと取得後の複数語一致を併用し、1語一致だけの商品を表示候補へ通さない
- 単品表示は選択カテゴリだけを検索し、セット表示もカテゴリ並列数2、自動検索600ms待機とする
- 429は短時間で自動再試行しない。端末キャッシュは最大8条件を15分保持する
- 予報点数、ケア選定、体質判定、DB、RLS、環境変数は変更しない

詳細: `docs/RELEASE_POLISH_V77930.md`
...
- v7.79.27時点のロジック版は`daily_care_v2_25_2026-08-13_natural_copy_polish`。現行版はv7.79.34契約を参照する
...
- 暮らすの`forecast_insight`は、選択中の不調に起こりそうな崩れ方だけを一度示す。個別カードの`reason`を連結して重複させない
- 胃腸の環境調整は`tool-work-height`だけへ固定せず、`tool-back-support`と`tool-foot-support`も不調コピー・商品役割を保ったまま天気適合で選ぶ
- 夏候補は`avoid_triggers`で低温・乾燥時に強く減点する。秋冬の通常表示へトマト・きゅうりの柑橘だれ、ズッキーニのバジルトマト、冬瓜などを出さない
- 料理候補へ飲み物名を埋め込まない。飲み物カードは今日=`食事中〜食後`、明日=`今夜〜明朝`として役割と時間帯を明示する
- 食性の説明ラベルは`体調との相性`。ユーザー文へ`食養生では`を出さず、栄養理由は`栄養面`として補足する
- バランス型ペルソナは作らない。アクセル／ブレーキ×余力の実データ構造で、5人×四季の20条件を回帰確認する

## v7.79.19 食性中心・食材優先表示の旧契約
