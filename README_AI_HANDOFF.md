## v7.79.11 暮らすケアの有用性ゲート・表示文契約

- v7.79.10の適合度優先と近似候補ローテーションは維持する
- `ENVIRONMENT_EXPERIMENT_CANDIDATES`の現行候補は`env-stop-line / env-breathing-room`だけ
- 実際の部屋、持ち物、作業、食後行動をアプリ側で推測して候補化しない
- `FOUNDATION_LIFESTYLE_CANDIDATES`は廃止。一般的な水分・着替え・休憩を件数合わせに使わない
- 暮らすの表示数は最大2件。主提案1件と別案1件を超えて補充しない
- 身体操作は選択中の不調と直接つながる候補だけを有効にする
- 環境候補は主または副の天気ストレスに一致する場合だけ有効にする
- 両レーンとも無効なら`primary_action=null / steps=[] / no_suggestion=true`を返す
- `PUBLIC_ACTION_COPY_BY_ID`のユーザー文へ`ではなく / ためです / できています`を使わない
- ユーザー文は具体場面、一手、短い理由、ラクさの目安、しっくりこない時の順にする
- 姿勢の左右対称、耳・肩の位置などを成功条件として採点しない
- 商品名は対策ケアへ出さず、登録済みaction idからショップ検索だけを解決する
- 予報点数、天気ストレス、体質親和性、余力補正、DBは変更しない

詳細: `docs/RADAR_HUMAN_LIFESTYLE_CARE_V77911.md`
