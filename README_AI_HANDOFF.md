## v7.79.21 全画面UX仕上げの現行契約

- 現行ロジック版は`daily_care_v2_19_2026-08-08_ux_polish`
- ほぐすラインは体質ラインを維持しつつ、`today`と`tomorrow`で行う場面・動作文を分ける。明日表示を`今日の一手`へ戻さない
- 下部ナビの`記録・相談`は`/records`へ着地させる。相談は記録画面内でユーザーが選ぶ
- 主要画面の意味を持つ本文・ラベル・CTAは12px未満へ戻さない
- ショップは初期状態で`sets`表示、条件調整は閉じる、おすすめセット1件を主役、別案は`details`内という情報階層を維持する
- 体質チェックは`約4〜6分 / 基本14問＋最大5問`を正本とする。実測なしに短い所要時間へ戻さない
- 診断質問・結果コピーへ機械比喩や断定的な水分停滞説明を戻さず、観察できる傾向と生活行動で説明する
- 7方針は体質・余力・気血水・主訴を中心にするため、天気だけで日替わりにしない。画面では`体質の土台 × 今日の条件`と暮らす導入文で日々の現れ方を分ける
- 食事・飲み物の第一理由ラベルは`体調との相性`。ユーザー向け本文を`食養生では`で始めない
- 高温が主条件なら、胃腸弱めでも白湯・葛湯・生姜湯を第一候補へ固定しない。冷ます／中立な候補を優先し、胃腸への配慮は温度・量で補足する
- tomorrowの飲み物カードは、今日と同じ候補が残る場合に`体質への相性と明日の天気の両方に合う`ことを説明する
- 身体操作5件、商品境界、料理ローテーション、マクロなショップ軸はv7.79.15〜19の契約を維持する

詳細: `docs/UX_POLISH_V77921.md`

> v7.79.19の理由ラベル`食養生`と、全画面の8〜11px表示、ほぐすのtoday/tomorrow同文は廃止済み。
