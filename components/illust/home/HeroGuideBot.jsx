...
export default function HeroGuideBot({
  ...
  return (
    <div className={["relative", widthClass].join(" ")}>
      {showBubble ? (
        <div className={`absolute rounded-2xl border border-[color:var(--ring)] bg-white px-4 py-2.5 text-left text-[12px] font-bold leading-6 text-[#5d6773] shadow-[0_12px_24px_-12px_rgba(40,55,48,0.2)] z-20 transition-all ${bubbleStyles}`}>
          {tailStyles}
        ...
        <svg viewBox="0 0 120 120" className="absolute inset-0 h-full w-full" aria-hidden="true">
          <defs>
            ...
            <linearGradient id="bodyGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#e2efe7" />
              <stop offset="100%" stopColor="#cddfd4" />
            </linearGradient>
            <linearGradient id="radarGlow" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#e6ae40" stopOpacity="0.88" />
              <stop offset="100%" stopColor="#f0c15c" stopOpacity="0.24" />
            </linearGradient>
            <filter id="softShadow" x="-10%" y="-10%" width="120%" height="120%">
              <feDropShadow dx="0" dy="5" stdDeviation="4" floodColor="#446e61" floodOpacity="0.12" />
            </filter>
          </defs>
          ...
          <circle cx="60" cy="85" r="12" fill="none" stroke="#ffffff" strokeWidth="1.5" opacity="0.96" />
          <circle cx="60" cy="85" r="6" fill="none" stroke="#ffffff" strokeWidth="1.5" opacity="0.7" />
          <circle cx="60" cy="85" r="3" fill="url(#radarGlow)" />
          <path d="M60 28 L60 18" stroke="#7ea892" strokeWidth="2.5" strokeLinecap="round" />
          <path d="M60 18 C52 10, 57 4, 65 4 C72 4, 70 14, 60 18 Z" fill="#a2cab4" />
          <rect x="24" y="28" width="72" height="52" rx="22" fill="url(#headGrad)" filter="url(#softShadow)" />
          <circle cx="44" cy="50" r="4.5" fill="#5d856f" />
          <circle cx="76" cy="50" r="4.5" fill="#5d856f" />
          <ellipse cx="36" cy="56" rx="5" ry="3" fill="#e2b4b4" opacity="0.42" />
          <ellipse cx="84" cy="56" rx="5" ry="3" fill="#e2b4b4" opacity="0.42" />
          <path d="M54 58 C 58 61, 62 61, 66 58" fill="none" stroke="#5d856f" strokeWidth="2.2" strokeLinecap="round" />
        </svg>
      </div>
    </div>
  );
