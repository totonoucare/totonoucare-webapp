...
export default function HeroGuideBot({
  ...
  if (compact) {
    if (bubbleSide === "left-belly") {
      // ▼ ログイン後ダッシュボード用（形・位置は元コードのまま）
      bubbleStyles = "right-[90px] bottom-[25px] w-[210px]";
      tailStyles = (
        <div className="absolute top-[50%] right-[-6px] -translate-y-1/2 h-3 w-3 rotate-45 border-t border-r border-[var(--ring)] bg-[var(--surface)]"></div>
      );
    } else if (bubbleSide === "right") {
      // ▼ 未ログインLP用（形・位置は元コードのまま）
      bubbleStyles = "left-[110px] top-[20px] w-max whitespace-nowrap";
      tailStyles = (
        <div className="absolute top-[50%] left-[-6px] -translate-y-1/2 h-3 w-3 rotate-45 border-b border-l border-[var(--ring)] bg-[var(--surface)]"></div>
      );
    } else {
