...
function StatusChip({ label, tone }) {
  const toneClass =
    tone === "warn"
      ? "bg-[var(--gold-soft)] text-[#a57522] ring-1 ring-inset ring-[#ecd79f] shadow-[0_4px_10px_-8px_rgba(128,93,27,0.25)]"
      : tone === "danger"
...
export default function HeroMiniCards({ compact = false }) {
      ...
      <div className="rounded-[32px] border border-[color:var(--ring)] bg-white p-2 shadow-[0_26px_56px_-26px_rgba(40,55,48,0.22)]">
        <div className="relative h-[320px] overflow-hidden rounded-[26px] bg-[#fcfcf8] ring-1 ring-inset ring-[color:var(--ring)]">
          <svg viewBox="0 0 320 320" className="absolute inset-0 h-full w-full" aria-hidden="true">
            <circle cx="30" cy="250" r="180" fill="#edf3ee" fillOpacity="0.96" />
            <circle cx="30" cy="250" r="102" fill="#edf3ee" fillOpacity="0.74" />

            <circle cx="30" cy="250" r="72" fill="none" stroke="#c7d8cf" strokeWidth="1.6" strokeOpacity="0.8" />
            <circle cx="30" cy="250" r="112" fill="none" stroke="#c7d8cf" strokeWidth="1.5" strokeOpacity="0.56" />
            <circle cx="30" cy="250" r="152" fill="none" stroke="#c7d8cf" strokeWidth="1.5" strokeOpacity="0.34" />
          ...
          <div className="absolute left-4 top-6 z-20 w-[190px] rounded-[22px] bg-white p-4 shadow-[0_16px_30px_-12px_rgba(40,55,48,0.16)] ring-1 ring-[color:var(--ring)] transition-all hover:shadow-[0_20px_36px_-12px_rgba(40,55,48,0.2)]">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#a8adb7]">あなたの体質</span>
              <StatusChip label="チーター型" tone="warn" />
            </div>
            <div className="mt-2 text-sm font-black leading-tight text-slate-900">アクセル優位 × 余力小</div>
            <div className="mt-3 flex gap-1.5">
              <span className="rounded-md bg-[#f8f8f5] px-2 py-1 text-[10px] font-bold text-[#7a8390] ring-1 ring-inset ring-[color:var(--ring)]">血虚</span>
              <span className="rounded-md bg-[#f8f8f5] px-2 py-1 text-[10px] font-bold text-[#7a8390] ring-1 ring-inset ring-[color:var(--ring)]">気滞</span>
            </div>
          </div>
          ...
          <div className="absolute right-4 top-[120px] z-10 w-[170px] rounded-[22px] bg-white p-4 shadow-[0_16px_30px_-12px_rgba(40,55,48,0.16)] ring-1 ring-[color:var(--ring)] transition-all hover:shadow-[0_20px_36px_-12px_rgba(40,55,48,0.2)]">
            <div className="text-[10px] font-black uppercase tracking-widest text-[#a8adb7]">体調予報</div>
            <div className="mt-1 text-sm font-black text-slate-900">明日：気圧上昇</div>
            <div className="mt-3 flex items-end justify-between">
              <div className="text-[11px] font-bold leading-tight text-[#7a8390]">
                崩れやすさ
                <br />
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="group relative overflow-hidden rounded-[30px] border border-[color:var(--ring)] bg-[color-mix(in_srgb,var(--mint),white_30%)] p-5 shadow-[0_18px_34px_-14px_rgba(40,55,48,0.12)]">
        <div className="relative z-10">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {["ツボ", "食養生", "記録"].map((tag) => (
              <span key={tag} className="rounded-full bg-white px-3.5 py-1.5 text-[10px] font-black text-[#5a6472] shadow-[0_8px_16px_-12px_rgba(40,55,48,0.18)] ring-1 ring-[color:var(--ring)]">
                {tag}
              </span>
            ))}
          </div>

          <div className="mt-8 h-12 w-full overflow-hidden rounded-xl bg-white/86 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] ring-1 ring-[color:var(--ring)]">
            <svg viewBox="0 0 200 40" className="h-full w-full">
              <path d="M0 20 C 30 10, 60 30, 90 20 S 150 10, 200 20" fill="none" stroke="#6ba08d" strokeOpacity="0.48" strokeWidth="2.5" strokeLinecap="round" />
              <circle cx="90" cy="20" r="3" fill="#e6ae40" />
            </svg>
          </div>
        </div>
      </div>
