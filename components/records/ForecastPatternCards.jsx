    ...
    <section className="overflow-hidden rounded-[26px] bg-white ring-1 ring-[#DCE8DD] shadow-[0_16px_34px_-30px_rgba(15,23,42,0.34)]">
      ...
      <div className="grid grid-cols-2 gap-2 p-3">
        {groups.map((group) => {
          ...
          return (
            ...
            >
              <div className={["text-[12px] font-black tracking-[0.08em]", meta.ink].join(" ")}>{meta.eyebrow}</div>
              ...
              <div className="mt-1 text-[12px] font-bold leading-4 text-slate-500">ケアあり {group.care_days}日・なし {group.no_care_days}日</div>
            </button>
          );
        })}
      </div>
      ...
      {activeGroup?.days ? (
        <div className="m-3 mt-0 rounded-[20px] bg-[#F7FAF8] px-4 py-3 ring-1 ring-[#E8F0EB]">
          <div className="text-[12px] font-black text-slate-900">{PATTERN_META[activeGroup.key].title}</div>
          <div className="mt-1 text-[12px] font-bold leading-4 text-slate-500">{PATTERN_META[activeGroup.key].description}</div>
            ...
            {visiblePoints.map((point) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-0.5 text-[12px] font-bold text-slate-400">{care}{timing ? `・${timing}` : ""}</div>
                </button>
              );
            })}
          </div>
          {activeGroup.points.length > visiblePoints.length ? (
            <div className="mt-2 text-[12px] font-bold text-slate-400">直近12日を表示・ほか{activeGroup.points.length - visiblePoints.length}日</div>
          ) : null}
        </div>
      ) : null}
    </section>
