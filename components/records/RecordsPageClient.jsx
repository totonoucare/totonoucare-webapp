...
export default function RecordsPageClient({ initialTab = "calendar" }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setReportError("");
      try {
        const res = await authedFetch("/api/insights/14days");
        setReport(res.data || null);
      } catch (e) {
        setReportError(e?.message || "締めレポートの読み込みに失敗しました。");
      } finally {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setWeeklyAiError(e?.message || "AI締めレポートの読み込みに失敗しました。");
      } finally {
  ...
  useEffect(() => {
    ...
    if (reportLoading || weeklyAiLoading) return;
    if (weeklyAiGenerating) return;
    if (autoGeneratedRef.current) return;

    const recordedDays = report?.summary?.recorded_days ?? 0;
    const shouldAutoGenerate = recordedDays > 0 && weeklyAi?.status === "empty";

    if (!shouldAutoGenerate) return;

    autoGeneratedRef.current = true;
    generateWeeklyAi({ silent: true });
  }, [tab, reportLoading, weeklyAiLoading, weeklyAiGenerating, report, weeklyAi]);
  ...
  async function saveReview(payload) {
    try {
      ...
      const [monthRes, reportRes, weeklyAiRes] = await Promise.all([
        ...
        authedFetch("/api/insights/14days"),
  ...
  async function generateWeeklyAi({ silent = false } = {}) {
    try {
      setWeeklyAiGenerating(true);
      if (!silent) setWeeklyAiError("");
      ...
      setWeeklyAiError(e?.message || "AI締めレポートの生成に失敗しました。");
    } finally {
      ...
      <AppShell title="記録と振り返り" subtitle="ログインが必要です">
        <Module className="p-6">
          ...
          <div className="mt-2 text-[13px] font-bold leading-6 text-slate-600">
            記録カレンダーや締めレポートは、ログイン後にご利用いただけます。
          </div>
            ...
            {selectedRow?.review ? (
              ...
              <div className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-center">
                ...
                <div className="mt-2 text-[12px] font-bold leading-5 text-slate-500">
                  しんどかった日だけでも残しておくと、締めレポートの精度が上がります。
                </div>
              </div>
            )}
          </Module>
              ...
              <>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <div className="rounded-[18px] bg-slate-50 px-4 py-3 ring-1 ring-inset ring-[var(--ring)]">
                    ...
                    <div className="mt-1 text-[18px] font-black tracking-tight text-slate-900">
                      {report?.summary?.attention_forecast_days ?? 0}日
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-5">
                  <Button
                    ...
                    disabled={!canGenerateWeeklyAi || weeklyAiGenerating}
                  ...
                  <div className="mt-2 text-[11px] font-bold leading-5 text-slate-500">
                    同じ内容なら前回のレポートを再表示します。この週の再生成は最大2回です。
                  </div>
                </div>
                ...
                {weeklyAiLoading ? (
                  ...
                  <div className="mt-6 rounded-[28px] bg-white p-6 ring-1 ring-[var(--ring)] shadow-sm">
                    ...
                    <div className="mt-5 border-t border-slate-100 pt-4 text-[11px] font-bold leading-5 text-slate-500">
                      ...
                      <div className="mt-1">
                        この週の再生成残り: {weeklyAi.remaining_regenerations ?? 0}回
                      </div>
                    </div>
                  </div>
                ) : (
