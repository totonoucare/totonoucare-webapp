...
function ChoiceButton({ active, label, sub, onClick, tone = "mint" }) {
  ...
  return (
    ...
    >
      ...
      {sub ? <div className={["mt-1 text-[12px] font-bold", active ? "text-white/80" : "text-slate-400"].join(" ")}>{sub}</div> : null}
    </button>
  );
}
    ...
    <div className="rounded-[22px] bg-[#F7FAF8] p-3.5 ring-1 ring-[#DCE8DD]">
      <div className="flex items-center justify-between gap-3">
        ...
        {editable && onOpenRadar ? (
          <button type="button" onClick={onOpenRadar} className="shrink-0 rounded-full bg-white px-3 py-2 text-[12px] font-black text-[#2F816E] ring-1 ring-[#CFE7DE]">
            予報ページ
          </button>
        ) : null}
      </div>
      <div className="mt-3 space-y-3">
        {groups.map((group) => (
          <div key={group.key}>
            <div className="text-[12px] font-black text-slate-500">{group.label}</div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {editable && onRemoveAction && item.id ? (
                      <button
                        ...
                        className="shrink-0 rounded-full bg-white px-2.5 py-1.5 text-[12px] font-black text-slate-400 ring-1 ring-slate-200 disabled:opacity-50"
                      >
    ...
    <section className="overflow-hidden rounded-[30px] bg-white ring-1 ring-[#DCE8DD] shadow-[0_18px_42px_-34px_rgba(15,23,42,0.38)]">
      <div className="bg-[#F4FAF7] px-4 pb-4 pt-5">
        <div className="flex items-end gap-3">
          ...
          <div className="relative mb-2 min-w-0 flex-1 rounded-[20px] bg-white px-4 py-3 text-[14px] font-bold leading-6 text-slate-600 ring-1 ring-[#CFE7DE] shadow-sm">
            ...
            <div className="text-[12px] font-black tracking-[0.14em] text-[#2F816E]/65">記録サポート</div>
        ...
        <div className="mt-3 flex items-center justify-between gap-3 rounded-[18px] bg-white/85 px-3.5 py-3 ring-1 ring-white shadow-sm">
          <div>
            <div className="text-[12px] font-black tracking-widest text-slate-400">RECORD DATE</div>
            <div className="mt-1 text-[14px] font-black text-slate-900">{formatDate(date)}</div>
          </div>
          {forecast ? (
            <span className={["rounded-full px-3 py-1.5 text-[12px] font-black ring-1", forecastTone.surface, forecastTone.text, forecastTone.ring].join(" ")}>
              予報：{signalLabel(forecast.signal)}
            </span>
          ) : (
            <span className="rounded-full bg-slate-50 px-3 py-1.5 text-[12px] font-black text-slate-400 ring-1 ring-slate-200">予報なし</span>
          )}
        </div>
      </div>
      ...
      {review && !editing ? (
        <div className="p-4">
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-[20px] bg-[#F7FAF8] p-3.5 ring-1 ring-[#DCE8DD]">
              <div className="text-[12px] font-black text-slate-400">実際の体調</div>
              <div className="mt-1 text-[14px] font-black text-slate-900">{conditionLabel(review.condition_level)}</div>
            </div>
            <div className="rounded-[20px] bg-[#F7FAF8] p-3.5 ring-1 ring-[#DCE8DD]">
              <div className="text-[12px] font-black text-slate-400">{careActionSummary.count > 0 ? "行った具体的ケア" : "ケアはした？"}</div>
          ...
          {displayedCareDomains.length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {displayedCareDomains.map((domain) => {
                ...
                return (
                  <span key={domain} className="rounded-full bg-white px-3 py-1.5 text-[12px] font-black text-slate-600 ring-1 ring-[#DCE8DD]" style={{ borderLeft: `4px solid ${meta?.color || "#DCE8DD"}` }}>
                    {meta?.label || domain}
                  </span>
                );
              })}
          ...
          {careActionSummary.count > 0 && manualCareLevel > 0 ? (
            <div className="mt-3 rounded-[18px] bg-white px-4 py-3 ring-1 ring-[#DCE8DD]">
              <div className="text-[12px] font-black text-slate-400">対策ケア以外にまとめて記録したケア</div>
          ...
          {reviewFactors(review).length ? (
            <div className="mt-3 rounded-[18px] bg-[#FFF8EC] px-4 py-3 ring-1 ring-[#EED8B4]">
              <div className="text-[12px] font-black text-[#A56C18]">天気以外で思い当たること</div>
          ...
          <div className="mt-4 rounded-[20px] bg-[#EFF8F4] p-4 ring-1 ring-[#CFE7DE]">
            <div className="text-[12px] font-black tracking-widest text-[#2F816E]/70">この日の見比べ</div>
        ...
        <div className="p-4">
          ...
          {careActionSummary.count === 0 && care > 0 && !timing ? <div className="mt-2 text-center text-[12px] font-bold text-slate-400">ケアした時間も選ぶと保存できます</div> : null}
          {review ? (
