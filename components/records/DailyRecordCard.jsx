...
function ChoiceButton({ active, label, sub, onClick, tone = "mint" }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "rounded-[18px] px-3 py-3 text-center transition-all ring-1",
        active
          ? `${activeClass} shadow-[0_14px_26px_-20px_rgba(15,23,42,0.38)]`
          : "bg-white text-slate-600 ring-[#DCE8DD] hover:bg-[#F7FAF8]",
      ].join(" ")}
    >
      <div className="text-[12px] font-black leading-4">{label}</div>
...
export default function DailyRecordCard({
  ...
  useEffect(() => {
    ...
    setDomains(inferCareDomains(review?.action_tags));
    setTiming(inferCareTiming(review?.action_tags));
    setNote(review?.note || "");
    setEditing(!review);
  }, [date, review?.id, review?.created_at]);
  ...
  const previewRow = useMemo(
    ...
    [date, forecast, review, condition, care, domains, timing, note]
  );

  const classification = classifyRecord(previewRow);
  const forecastTone = signalTone(forecast?.signal ?? 0);
    ...
    <section className="overflow-hidden rounded-[30px] bg-white ring-1 ring-[#DCE8DD] shadow-[0_18px_42px_-34px_rgba(15,23,42,0.38)]">
      <div className="bg-[#F4FAF7] px-4 pb-4 pt-5">
        <div className="flex items-end gap-3">
          <div className="shrink-0">
            <GuideBotAvatar mood={botMood} className="h-[82px] w-[82px]" />
          </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-3 rounded-[18px] bg-white/85 px-3.5 py-3 ring-1 ring-white shadow-sm">
          ...
          {forecast ? (
            ...
            <span className="rounded-full bg-slate-50 px-3 py-1.5 text-[10px] font-black text-slate-400 ring-1 ring-slate-200">
              予報なし
            </span>
          )}
        </div>
      ...
      {review && !editing ? (
        <div className="p-4">
          ...
          {inferCareDomains(review.action_tags).length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {inferCareDomains(review.action_tags).map((domain) => {
                const meta = RECORD_DOMAIN_OPTIONS.find((item) => item.value === domain);
                return (
                  <span
                    key={domain}
                    className="rounded-full bg-white px-3 py-1.5 text-[10px] font-black text-slate-600 ring-1 ring-[#DCE8DD]"
                    style={{ borderLeft: `4px solid ${meta?.color || "#DCE8DD"}` }}
                  >
                    {meta?.label || domain}
                  </span>
                );
          ...
          {review.note ? (
            <div className="mt-3 rounded-[18px] bg-[#F7FAF8] px-4 py-3 text-[12px] font-bold leading-6 text-slate-600 ring-1 ring-[#E8F0EB]">
              {review.note}
            </div>
          ) : null}

          <div className="mt-4 rounded-[20px] bg-[#EFF8F4] p-4 ring-1 ring-[#CFE7DE]">
            <div className="text-[10px] font-black tracking-widest text-[#2F816E]/70">今日の見比べ</div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            <Button variant="secondary" onClick={() => setEditing(true)} className="w-full bg-white">
              記録を編集
            </Button>
            <Button onClick={() => onGoAnalysis?.({ date, classification })} className="w-full">
              AIと振り返る
            </Button>
          </div>
        </div>
        ...
        <div className="p-4">
              ...
              <div className="mt-5 rounded-[22px] bg-[#F7FAF8] p-3.5 ring-1 ring-[#DCE8DD]">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {RECORD_DOMAIN_OPTIONS.map((item) => (
                    <ChoiceButton
                      key={item.value}
                      active={domains.includes(item.value)}
                      label={item.label}
                      onClick={() => toggleDomain(item.value)}
                      tone={item.value === "eat" ? "amber" : item.value === "loosen" ? "violet" : "mint"}
                    />
                  ))}
                </div>
              </div>
          ...
          <Button
            disabled={saving || condition == null || care == null || (care > 0 && domains.length === 0)}
            onClick={submit}
          ...
          {review ? (
            <button
              ...
              className="mt-3 w-full text-center text-[11px] font-black text-slate-400"
            >
              編集をやめる
            </button>
          ) : null}
